...
LAYER 4: Execution Backends
  ├─ LocalBackend (CPU simulation, memory-derived width ≤30, default)
...
- LocalBackend provides free, fast simulation (width derived from available memory)
...
- LocalBackend simulation (free, fast, memory-derived width)
- Future-proof as quantum hardware improves
