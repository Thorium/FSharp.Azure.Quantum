...
| Ok state1 ->
    ...
    | Error err -> printfn "✗ Controlled addition (control=0) failed: %A" err
| Error err -> printfn "✗ Failed to initialize state: %A" err
...
| Ok state2 ->
    ...
    | Ok state2_controlled ->
        ...
        | Ok res ->
            ...
            printfn "  Operations: %d" res.OperationCount
        | Error err -> printfn "✗ Controlled addition (control=1) failed: %A" err
    | Error err -> printfn "✗ Failed to apply X gate: %A" err
| Error err -> printfn "✗ Failed to initialize state: %A" err
    ...
    | Ok state3_c1 ->
        ...
        | Ok state3_c2 ->
            ...
            | Ok res ->
                ...
                printfn "  Operations: %d" res.OperationCount
            | Error err -> printfn "✗ Doubly-controlled addition failed: %A" err
        | Error err -> printfn "✗ Failed to apply second X gate: %A" err
    | Error err -> printfn "✗ Failed to apply first X gate: %A" err
| Error err -> printfn "✗ Failed to initialize state: %A" err
...
| Ok state4 ->
    ...
    | Ok state4_controlled ->
        ...
        | Ok res ->
            ...
            printfn "  Operations: %d" res.OperationCount
        | Error err -> printfn "✗ Controlled subtraction failed: %A" err
    | Error err -> printfn "✗ Failed to apply X gate: %A" err
| Error err -> printfn "✗ Failed to initialize state: %A" err
