...
module BinaryClassificationBuilderTests =
    ...
    let ``train with Quantum architecture should succeed`` () =
        ...
        | Ok classifier ->
            ...
            Assert.True(
                classifier.Metadata.TrainingAccuracy >= 0.0
                && classifier.Metadata.TrainingAccuracy <= 1.0
            )
    ...
    let ``binaryClassification CE with architecture should work`` () =
        ...
        result
        |> Result.map (fun classifier -> Assert.Equal(Hybrid, classifier.Metadata.Architecture))
