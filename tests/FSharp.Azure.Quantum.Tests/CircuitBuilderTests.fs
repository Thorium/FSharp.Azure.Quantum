...
module CircuitBuilderTests =
    ...
    let ``toOpenQASM should output X gate correctly`` () =
        let circuit =
            CircuitBuilder.empty 2 |> CircuitBuilder.addGate (CircuitBuilder.Gate.X 0)
    ...
    let ``toOpenQASM should output Y gate correctly`` () =
        let circuit =
            CircuitBuilder.empty 2 |> CircuitBuilder.addGate (CircuitBuilder.Gate.Y 1)
    ...
    let ``toOpenQASM should output Z gate correctly`` () =
        let circuit =
            CircuitBuilder.empty 2 |> CircuitBuilder.addGate (CircuitBuilder.Gate.Z 0)
    ...
    let ``toOpenQASM should output H gate correctly`` () =
        let circuit =
            CircuitBuilder.empty 2 |> CircuitBuilder.addGate (CircuitBuilder.Gate.H 1)
    ...
    let ``validate should fail for negative qubit index`` () =
        let circuit =
            CircuitBuilder.empty 3 |> CircuitBuilder.addGate (CircuitBuilder.Gate.H -1) // Negative index
