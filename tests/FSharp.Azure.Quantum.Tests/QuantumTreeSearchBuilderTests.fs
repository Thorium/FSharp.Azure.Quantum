...
module QuantumTreeSearchBuilderTests =
    ...
    let simpleMoveGen (state: SimpleGameState) : SimpleGameState list = [ state + 1; state + 2; state * 2 ]
    ...
    let ``QuantumTreeSearch.forGameAI should create game AI problem`` () =
        ...
        // Act
        let problem =
            QuantumTreeSearch.forGameAI board depth branching simpleEval simpleMoveGen
    ...
    let ``QuantumTreeSearch.forDecisionProblem should create decision problem`` () =
        ...
        // Act
        let problem =
            QuantumTreeSearch.forDecisionProblem initialDecision steps optionsPerStep simpleEval simpleMoveGen
    ...
    let ``QuantumTreeSearch.solve should use LocalBackend by default`` () =
        ...
        match result with
        | Ok solution -> Assert.Contains("LocalBackend", solution.BackendName)
        | Error err ->
    ...
    let ``QuantumTreeSearch.solve should return error for invalid problem`` () =
        ...
        // Assert
        result
        |> Result.map (fun _ -> Assert.Fail("Expected solve to return error"))
        |> Result.defaultWith (fun err -> Assert.Contains("qubits", err.Message))
