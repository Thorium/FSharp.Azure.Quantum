...
module VariationalFormTests =
    ...
    let ``RealAmplitudes - generates correct circuit structure for depth 1`` () =
        ...
        | Error msg -> Assert.True(false, $"Should succeed but got error: {msg}")
    ...
    let ``RealAmplitudes - rejects incorrect parameter count`` () =
        ...
        | Ok _ -> Assert.True(false, "Should fail with wrong parameter count")
    ...
    let ``TwoLocal - handles case-insensitive gate names`` () =
        ...
        | Error msg -> Assert.True(false, $"Should handle lowercase: {msg}")
        ...
        | Ok circuit ->
            Assert.Equal(2, countRyGates circuit)
            Assert.Equal(1, countCZGates circuit)
        | Error msg -> Assert.True(false, $"Should handle uppercase: {msg}")
    ...
    let ``TwoLocal - defaults to Ry and CZ for unknown gates`` () =
        ...
        | Error msg -> Assert.True(false, $"Should use defaults: {msg}")
    ...
    let ``buildVariationalForm - rejects zero qubits`` () =
        ...
        (buildVariationalForm (RealAmplitudes 1) parameters 0)
        |> Result.map (fun _ -> Assert.True(false, "Should reject zero qubits"))
        |> Result.defaultWith (fun msg -> Assert.Contains("at least 1", msg.Message))

    [<Fact>]
    let ``buildVariationalForm - rejects negative qubits`` () =
        let parameters = [||]

        (buildVariationalForm (RealAmplitudes 1) parameters -1)
        |> Result.map (fun _ -> Assert.True(false, "Should reject negative qubits"))
        |> Result.defaultWith (fun msg -> Assert.Contains("at least 1", msg.Message))
    ...
    let ``buildVariationalForm - rejects zero depth`` () =
        ...
        (buildVariationalForm (RealAmplitudes 0) parameters 3)
        |> Result.map (fun _ -> Assert.True(false, "Should reject zero depth"))
        |> Result.defaultWith (fun msg -> Assert.Contains("Depth must be at least 1", msg.Message))

    [<Fact>]
    let ``buildVariationalForm - rejects negative depth`` () =
        let parameters = [||]

        (buildVariationalForm (EfficientSU2 -1) parameters 3)
        |> Result.map (fun _ -> Assert.True(false, "Should reject negative depth"))
        |> Result.defaultWith (fun msg -> Assert.Contains("Depth must be at least 1", msg.Message))
    ...
    let ``composeWithFeatureMap - successfully composes matching circuits`` () =
        ...
        | Ok varCircuit ->
            ...
            | Error msg -> Assert.True(false, $"Composition should succeed: {msg}")

        | Error msg -> Assert.True(false, $"Circuit generation failed: {msg}")
    ...
    let ``composeWithFeatureMap - rejects mismatched qubit counts`` () =
        ...
        match composeWithFeatureMap featureMap varCircuit with
        | Ok _ -> Assert.True(false, "Should reject qubit count mismatch")
    ...
    let ``End-to-end - RealAmplitudes depth 3 on 5 qubits`` () =
        ...
        | Error msg -> Assert.True(false, $"Integration test failed: {msg}")
