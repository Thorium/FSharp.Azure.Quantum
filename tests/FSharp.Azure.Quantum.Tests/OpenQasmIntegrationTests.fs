...
module OpenQasmIntegrationTests =
    ...
    let ``export, import, then transpile for IonQ`` () =
        ...
        | Ok circuit ->
            ...
            Assert.True(
                transpiled.Gates.Length > original.Gates.Length,
    ...
    let ``export and import GHZ state circuit`` () =
        ...
        let circuit =
            {
                QubitCount = 4
                Gates = [ H 0; CNOT(0, 1); CNOT(0, 2); CNOT(0, 3) ]
            }
...
"""
    ...
    let ``export, import, modify, re-export workflow`` () =
        ...
        | Ok imported ->
            ...
            (OpenQasmImport.parse qasm2)
            |> Result.map (fun final -> Assert.Equal(2, final.Gates.Length))
            |> Result.defaultWith (fun msg -> Assert.True(false, $"Re-import failed: {msg}"))
        | Error msg -> Assert.True(false, $"Import failed: {msg}")
...
"""
    ...
    let ``exported then imported circuit maintains optimization opportunities`` () =
        ...
        | Ok imported ->
            ...
            Assert.True(
                optimized.Gates.Length < circuit.Gates.Length,
    ...
    let ``multiple round-trips maintain circuit integrity`` () =
        let original =
            {
                QubitCount = 3
                Gates = [ H 0; RX(1, Math.PI / 3.0); CNOT(0, 1); S 2; CZ(1, 2); T 0 ]
            }
    ...
    let ``Ising gates survive QASM export-import roundtrip`` () =
        let original =
            {
                QubitCount = 2
                Gates = [ RXX(0, 1, 0.5); RYY(0, 1, -1.25); RZZ(0, 1, 3.14) ]
            }
    ...
    let ``Ising gates transpile to CNOT basis for IonQ`` () =
        let circuit =
            {
                QubitCount = 2
                Gates = [ RXX(0, 1, 0.5); RYY(0, 1, 0.5); RZZ(0, 1, 0.5) ]
            }
    ...
    let ``export emits builder-built circuits in program order`` () =
        ...
        let circuit =
            empty 2 |> addGate (H 0) |> addGate (CNOT(0, 1)) |> addGate (Measure 0)
