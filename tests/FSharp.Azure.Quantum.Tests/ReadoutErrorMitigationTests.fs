...
module ReadoutErrorMitigationTests =
    ...
    let ``withCalibrationShots should override calibration shots`` () =
        ...
        let customConfig = config |> ReadoutErrorMitigation.withCalibrationShots 5000
    ...
    let ``withConfidenceLevel should override confidence level`` () =
        ...
        let customConfig = config |> ReadoutErrorMitigation.withConfidenceLevel 0.99
    ...
    let ``withClipNegative should toggle negative clipping`` () =
        ...
        let customConfig = config |> ReadoutErrorMitigation.withClipNegative false
    ...
    let ``validateCalibrationMatrix should accept valid 1-qubit matrix`` () =
        ...
        let matrix = Array2D.init 2 2 (fun i j -> if i = j then 0.98 else 0.02) // Diagonal: correct measurement
    ...
    let ``validateCalibrationMatrix should reject matrix with invalid dimensions`` () =
        ...
        match result with
        | Error msg -> Assert.Contains("Expected 2x2 matrix", msg)
        | Ok() -> Assert.True(false, "Should reject invalid dimensions")
    ...
    let ``validateCalibrationMatrix should reject probabilities outside [0,1]`` () =
        ...
        // Assert: Should fail
        match result with
        | Error msg -> Assert.Contains("not in [0,1]", msg)
        | Ok() -> Assert.True(false, "Should reject invalid probabilities")
    ...
    let ``validateCalibrationMatrix should reject columns not summing to 1.0`` () =
        ...
        // Assert: Should fail
        match result with
        | Error msg -> Assert.Contains("sums to", msg)
        | Ok() -> Assert.True(false, "Should reject invalid probability distribution")
    ...
    let ``invertCalibrationMatrix should invert well-conditioned matrix`` () =
        ...
        | Error msg -> Assert.True(false, $"Inversion should succeed: %s{msg}")
    ...
    let ``invertCalibrationMatrix should reject nearly singular matrix`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Should reject singular matrix"))
        |> Result.defaultWith (fun msg -> Assert.Contains("singular", msg.ToLower()))
    ...
    let ``invertCalibrationMatrix should warn on poorly-conditioned matrix`` () =
        ...
        let matrix = Array2D.init 2 2 (fun i j -> if i = j then 0.99 else 0.01)
        ...
        result
        |> Result.map (fun _ -> Assert.True(true, "Should invert but warn"))
        |> Result.defaultWith (fun _ -> Assert.True(false, "Should not fail, just warn"))
    ...
    let ``correctReadoutErrors should correct perfect measurement (no errors)`` () =
        ...
        | Ok corrected ->
            ...
            Assert.True(abs (count0 - 1000.0) < 10.0, $"Expected ~1000 counts for |0⟩, got %.1f{count0}")
        | Error msg -> Assert.True(false, $"Correction should succeed: %s{msg}")

    [<Fact>]
    let ``correctReadoutErrors should improve biased measurement`` () =
        // Arrange: Realistic 98% accuracy calibration
        let matrix = Array2D.init 2 2 (fun i j -> if i = j then 0.98 else 0.02) // 2% readout error
        ...
        | Ok corrected ->
            ...
            Assert.True(count0 > 990.0, $"Expected corrected |0⟩ > 990, got %.1f{count0}")
            Assert.True(count1 < 10.0, $"Expected corrected |1⟩ < 10, got %.1f{count1}")
        | Error msg -> Assert.True(false, $"Correction should succeed: %s{msg}")

    [<Fact>]
    let ``correctReadoutErrors should clip negative counts when configured`` () =
        // Arrange: Calibration matrix
        let matrix = Array2D.init 2 2 (fun i j -> if i = j then 0.95 else 0.05)
        ...
        | Ok corrected ->
            for (bitstring, count) in Map.toList corrected.Histogram do
                Assert.True(count >= 0.0, $"Count for %s{bitstring} should be non-negative, got %.1f{count}")
        | Error msg -> Assert.True(false, $"Correction should succeed: %s{msg}")

    [<Fact>]
    let ``correctReadoutErrors should normalize probabilities to sum to 1.0`` () =
        // Arrange: Standard calibration
        let matrix = Array2D.init 2 2 (fun i j -> if i = j then 0.97 else 0.03)
        ...
        | Ok corrected ->
            ...
            Assert.True(abs (totalCounts - 1000.0) < 5.0, $"Total counts should be ~1000, got %.1f{totalCounts}")

            // Check goodness of fit is high
            Assert.True(
                corrected.GoodnessOfFit > 0.95,
    ...
    let ``correctReadoutErrors should filter noise below threshold`` () =
        ...
        | Ok corrected ->
            ...
            let count0 = corrected.Histogram.["0"]
            Assert.True(count0 > 9900.0, $"Dominant state should have > 9900 counts, got %.1f{count0}")
        | Error msg -> Assert.True(false, $"Correction should succeed: %s{msg}")
    ...
    let ``correctReadoutErrors should provide confidence intervals for all states`` () =
        ...
        | Ok corrected ->
            for (bitstring, count) in Map.toList corrected.Histogram do
                Assert.True(
                    corrected.ConfidenceIntervals.ContainsKey(bitstring),
                ...
                Assert.True(upper > lower, $"Upper %.1f{upper} should exceed lower %.1f{lower}")
        | Error msg -> Assert.True(false, $"Correction should succeed: %s{msg}")
    ...
    let ``Confidence intervals should widen with smaller sample sizes`` () =
        ...
        // Act
        let resultHigh =
            ReadoutErrorMitigation.correctReadoutErrors measuredHigh calibration configHigh

        let resultLow =
            ReadoutErrorMitigation.correctReadoutErrors measuredLow calibration configLow
        ...
        | Ok high, Ok low ->
            ...
            Assert.True(
                ...
                sprintf
                    "Low shots CI (%.3f) should be relatively wider than high shots (%.3f)"
                    relativeWidthLow
                    relativeWidthHigh
            )
        | _ -> Assert.True(false, "Both corrections should succeed")
    ...
    let ``Confidence intervals should be non-negative`` () =
        ...
        | Ok corrected ->
            for (bitstring, (lower, upper)) in Map.toList corrected.ConfidenceIntervals do
                Assert.True(lower >= 0.0, $"Lower CI bound for %s{bitstring} should be non-negative, got %.1f{lower}")
                Assert.True(upper >= 0.0, $"Upper CI bound for %s{bitstring} should be non-negative, got %.1f{upper}")
        | Error msg -> Assert.True(false, $"Correction should succeed: %s{msg}")
    ...
    let private createNoisyExecutor
        ...
        fun (circuit: CircuitBuilder.Circuit) (shots: int) ->
            async {
                    ...
                    for gate in CircuitBuilder.getGates circuit do
                        state <-
                            ...
                            | CircuitBuilder.MCZ(controls, t) ->
                                LocalSimulator.Gates.applyMultiControlledZ controls t state
                            ...
                            | CircuitBuilder.Reset q ->
                                ...
                                if outcome = 1 then
                                    LocalSimulator.Gates.applyX q collapsed
                                else
                                    collapsed
                ...
                with ex ->
                    return Error $"Simulation error: %s{ex.Message}"
            }
    ...
    let ``Integration: 1-qubit calibration should measure correct confusion matrix`` () =
        ...
        task {
            ...
            | Ok calibration ->
                ...
                Assert.True(abs (matrix.[0, 0] - 0.98) < 0.05, sprintf "M[0,0] should be ~0.98, got %.3f" matrix.[0, 0])
                Assert.True(abs (matrix.[1, 1] - 0.98) < 0.05, sprintf "M[1,1] should be ~0.98, got %.3f" matrix.[1, 1])

                // Off-diagonal (errors) should be ~0.02
                Assert.True(matrix.[0, 1] < 0.08, sprintf "M[0,1] should be ~0.02, got %.3f" matrix.[0, 1])
                Assert.True(matrix.[1, 0] < 0.08, sprintf "M[1,0] should be ~0.02, got %.3f" matrix.[1, 0])
            | Error msg -> Assert.True(false, $"Calibration should succeed: %s{msg}")
        }
        :> Task
    ...
    let ``Integration: 1-qubit REM should reduce readout errors`` () =
        ...
        task {
            ...
            | Ok corrected ->
                ...
                Assert.True(fidelity > 0.99, $"REM should improve fidelity to > 99%%, got %.3f{fidelity}")
            ...
            | Error msg -> Assert.True(false, $"REM should succeed: %s{msg}")
        }
        :> Task
    ...
    let ``Integration: 2-qubit calibration should work`` () =
        ...
        task {
            ...
            | Error msg -> Assert.True(false, $"2-qubit calibration should succeed: %s{msg}")
        }
        :> Task
    ...
    let ``Integration: 2-qubit REM should reduce errors`` () =
        ...
        task {
            ...
            | Ok corrected ->
                ...
                Assert.True(fidelity > 0.97, $"2-qubit REM should achieve > 97%% fidelity, got %.3f{fidelity}")
            | Error msg -> Assert.True(false, $"2-qubit REM should succeed: %s{msg}")
        }
        :> Task
    ...
    let ``Integration: 3-qubit calibration should work`` () =
        ...
        task {
            ...
            | Ok calibration ->
                ...
                | Ok() -> Assert.True(true)
                | Error msg -> Assert.True(false, $"Calibration should be valid: %s{msg}")
            | Error msg -> Assert.True(false, $"3-qubit calibration should succeed: %s{msg}")
        }
        :> Task
    ...
    let ``Integration: 3-qubit REM should demonstrate error reduction`` () =
        ...
        task {
            ...
            | Ok corrected ->
                ...
                Assert.True(fidelity > 0.94, $"3-qubit REM should achieve > 94%% fidelity, got %.3f{fidelity}")
