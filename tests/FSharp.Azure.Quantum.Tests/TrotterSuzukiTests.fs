...
module TrotterSuzukiTests =
    ...
    let ``decomposeMatrixToPauli with identity matrix returns I term`` () =
        let identity =
            Array2D.init 2 2 (fun i j -> if i = j then Complex.One else Complex.Zero)
    ...
    let ``decomposeMatrixToPauli rejects non-square matrix`` () =
        ...
        (TrotterSuzuki.decomposeMatrixToPauli matrix)
        |> Result.iter (fun _ -> failwith "Expected Error for non-square matrix")

    [<Fact>]
    let ``decomposeMatrixToPauli rejects non-power-of-2 dimension`` () =
        let matrix =
            Array2D.init 3 3 (fun i j -> if i = j then Complex.One else Complex.Zero)

        (TrotterSuzuki.decomposeMatrixToPauli matrix)
        |> Result.iter (fun _ -> failwith "Expected Error for non-power-of-2 dimension")

    [<Fact>]
    let ``decomposeMatrixToPauli handles 4x4 matrix`` () =
        // 2-qubit identity
        let identity =
            Array2D.init 4 4 (fun i j -> if i = j then Complex.One else Complex.Zero)

        (TrotterSuzuki.decomposeMatrixToPauli identity)
        |> Result.map (fun hamiltonian -> Assert.Equal(2, hamiltonian.NumQubits))
    ...
    let ``synthesizePauliEvolution with all-identity returns unchanged circuit`` () =
        let pauliString: TrotterSuzuki.PauliString =
            {
                Operators = [| 'I' |]
    ...
    let ``synthesizePauliEvolution with multi-qubit string adds CNOT ladder`` () =
        ...
        let result =
            TrotterSuzuki.synthesizePauliEvolution pauliString 1.0 [| 0; 1 |] circuit
    ...
    let ``synthesizeHamiltonianEvolution with order 1 produces valid circuit`` () =
        ...
        let result =
            TrotterSuzuki.synthesizeHamiltonianEvolution hamiltonian config [| 0 |] circuit
    ...
    let ``synthesizeControlledPauliEvolution with single Z adds CRZ gate`` () =
        ...
        let result =
            TrotterSuzuki.synthesizeControlledPauliEvolution 0 pauliString 1.0 [| 1 |] circuit
    ...
    let ``estimateTrotterSteps with smaller tolerance returns more steps`` () =
        ...
        Assert.True(
            stepsFine >= stepsCoarse,
            $"Finer tolerance ({stepsFine}) should need >= steps than coarse ({stepsCoarse})"
        )
