...
module QuboEncodingTests =
    ...
    let ``toQubo produces correct matrix size for 3-SAT`` () =
        ...
        let problem: Problem =
            {
                NumVariables = 3
                Clauses = [ clause [ pos 0; pos 1; pos 2 ] ]
            }
    ...
    let ``toQubo QUBO is symmetric`` () =
        let problem: Problem =
            {
                NumVariables = 3
                Clauses = [ clause [ pos 0; neg 1 ]; clause [ pos 1; pos 2 ]; clause [ neg 0; neg 2 ] ]
            }
        ...
        | Ok qubo ->
            ...
            for i in 0..1 do
                for j in 0..1 do
                    eUnsatisfied <-
                        eUnsatisfied
                        + qubo.[i, j] * float unsatisfiedBits.[i] * float unsatisfiedBits.[j]

            Assert.True(
                eSatisfied < eUnsatisfied,
...
module DecodingTests =
    ...
    let ``isValid for 2-SAT needs exactly NumVariables qubits`` () =
        let problem: Problem =
            {
                NumVariables = 3
                Clauses = [ clause [ pos 0; pos 1 ]; clause [ neg 1; pos 2 ] ]
            }
...
module QubitEstimationTests =
    ...
    let ``estimateQubits for pure 2-SAT`` () =
        let problem: Problem =
            {
                NumVariables = 5
                Clauses = [ clause [ pos 0; neg 1 ]; clause [ pos 2; pos 3 ]; clause [ neg 3; pos 4 ] ]
            }
    ...
    let ``estimateQubits adds one per 3-literal clause`` () =
        let problem: Problem =
            {
                NumVariables = 3
                Clauses = [ clause [ pos 0; pos 1; pos 2 ]; clause [ neg 0; neg 1; neg 2 ] ]
            }
