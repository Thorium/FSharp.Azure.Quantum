...
module CircuitCompositionTests =
    ...
    let ``filterGates removes measurements`` () =
        let circuit =
            empty 2 |> addGate (Gate.H 0) |> addGate (Gate.Measure 0) |> addGate (Gate.X 1)

        let notMeasurement g =
            match g with
    ...
    let ``twoQubitGateCount counts CNOTs`` () =
        let circuit = bellState 0 1 // H + CNOT
    ...
    let ``depth calculates correct circuit depth`` () =
        ...
        let circuit1 = empty 1 |> addGate (Gate.H 0) |> addGate (Gate.X 0)
        Assert.Equal(2, depth circuit1)

        // Parallel gates on different qubits: depth = 1
        let circuit2 = empty 2 |> addGate (Gate.H 0) |> addGate (Gate.X 1)
        Assert.Equal(1, depth circuit2)

    [<Fact>]
    let ``removeIdentities removes RZ with zero angle`` () =
        let circuit = empty 1 |> addGate (Gate.RZ(0, 0.0)) |> addGate (Gate.H 0)
    ...
    let ``optimize merges consecutive rotations`` () =
        ...
        let circuit =
            empty 1 |> addGate (Gate.RZ(0, angle1)) |> addGate (Gate.RZ(0, angle2))
