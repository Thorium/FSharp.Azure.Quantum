...
module QuboEncodingTests =
    ...
    let ``Binary variable encoding - two variables should create 2x2 QUBO`` () =
        ...
        let variables =
            [ { Name = "x"; VarType = BinaryVar }; { Name = "y"; VarType = BinaryVar } ]
    ...
    let ``Constraint penalty - equality constraint`` () =
        ...
        let qubo =
            QuboEncoding.encodeVariablesWithCustomConstraints variables [ constr ] 10.0
    ...
    let ``Complex problem - TSP with 3 cities`` () =
            ...
            [
                {
                    Name = "pos0"
                    VarType = IntegerVar(0, 2)
                } // Which city at position 0
                {
                    Name = "pos1"
                    VarType = IntegerVar(0, 2)
                } // Which city at position 1
                {
                    Name = "pos2"
                    VarType = IntegerVar(0, 2)
                } // Which city at position 2
            ]
        ...
        let expectedNames =
            [ "pos0_bit0"; "pos0_bit1"; "pos1_bit0"; "pos1_bit1"; "pos2_bit0"; "pos2_bit1" ]
    ...
    let ``Portfolio optimization - 5 assets with priority levels`` () =
        ...
        let reduction =
            float (oneHotTotalQubits - domainWallTotalQubits) / float oneHotTotalQubits
    ...
    let ``ConstraintPenalty Hard constraint should use Lucas Rule`` () =
        ...
        let penalty =
            ConstraintPenalty.calculatePenalty ConstraintType.Hard objectiveMax problemSize
    ...
    let ``ConstraintPenalty Soft constraint should use preference weight`` () =
        ...
        let softPenalty =
            ConstraintPenalty.calculatePenalty (ConstraintType.Soft preferenceWeight) objectiveMax problemSize
    ...
    let ``ConstraintPenalty should scale with problem size`` () =
        ...
        let smallPenalty =
            ConstraintPenalty.calculatePenalty ConstraintType.Hard objectiveMax 5

        let largePenalty =
            ConstraintPenalty.calculatePenalty ConstraintType.Hard objectiveMax 20
    ...
    let ``ConstraintPenalty TSP example - 20 cities with max distance 500km`` () =
        ...
        let visitOncePenalty =
            ConstraintPenalty.calculatePenalty ConstraintType.Hard objectiveMax problemSize

        // Expected: (500 + 1) * sqrt(20) ≈ 501 * 4.47 ≈ 2240
        Assert.True(visitOncePenalty > 2200.0 && visitOncePenalty < 2250.0, $"Expected ~2240, got %f{visitOncePenalty}")
    ...
    let ``ConstraintPenalty Portfolio example - soft constraint at 30 percent`` () =
        ...
        let softPenalty =
            ConstraintPenalty.calculatePenalty (ConstraintType.Soft 0.3) objectiveMax problemSize

        // Expected: (500 + 1) * sqrt(20) * 0.3 ≈ 2240 * 0.3 ≈ 672
        Assert.True(softPenalty > 660.0 && softPenalty < 680.0, $"Expected ~672, got %f{softPenalty}")
    ...
    let ``ConstraintPenalty Hard vs Soft comparison`` () =
        ...
        let softPenalty =
            ConstraintPenalty.calculatePenalty (ConstraintType.Soft 0.5) objectiveMax problemSize
    ...
    let ``ConstraintPenalty multiple soft weights`` () =
        ...
        let veryWeak =
            ConstraintPenalty.calculatePenalty (ConstraintType.Soft 0.1) objectiveMax problemSize
        ...
        let strong =
            ConstraintPenalty.calculatePenalty (ConstraintType.Soft 0.8) objectiveMax problemSize
    ...
    let ``tuneAdaptive should start with initial penalty`` () =
        ...
        let finalPenalty =
            ConstraintPenalty.tuneAdaptive encoding initialPenalty maxIterations solver
    ...
    let ``tuneAdaptive should stop when valid solution found`` () =
        ...
        Assert.True(finalPenalty > 200.0 && finalPenalty < 250.0, $"Expected ~225, got %f{finalPenalty}")
    ...
    let ``Property: Hard constraint penalty always exceeds objective maximum`` () =
        ...
        let testCases = [ (10.0, 5); (100.0, 10); (500.0, 20); (1000.0, 50); (2500.0, 100) ]

        testCases
        |> List.iter (fun (objectiveMax, problemSize) ->
            let penalty =
                ConstraintPenalty.calculatePenalty ConstraintType.Hard objectiveMax problemSize
            ...
            Assert.True(
                penalty >= lucasMin,
                sprintf
                    "Penalty %f must be >= Lucas minimum %f for objectiveMax=%f, n=%d"
                    penalty
                    lucasMin
                    objectiveMax
                    problemSize
    ...
    let ``Property: Soft constraint penalty is always less than hard penalty`` () =
        ...
        let testCases = [ (100.0, 10, 0.3); (500.0, 20, 0.5); (1000.0, 50, 0.8) ]

        testCases
        |> List.iter (fun (objectiveMax, problemSize, weight) ->
            let hardPenalty =
                ConstraintPenalty.calculatePenalty ConstraintType.Hard objectiveMax problemSize

            let softPenalty =
                ConstraintPenalty.calculatePenalty (ConstraintType.Soft weight) objectiveMax problemSize
    ...
    let ``Property: Penalty scales monotonically with problem size`` () =
        ...
        let penalties =
            sizes
            |> List.map (fun n -> ConstraintPenalty.calculatePenalty ConstraintType.Hard objectiveMax n)

        // Verify penalties are in ascending order
        penalties
        |> List.pairwise
        |> List.iter (fun (prev, next) ->
            Assert.True(next > prev, $"Penalty should increase with problem size: %f{next} !> %f{prev}"))
    ...
    let ``EncodingStrategy NodeBased should create n-squared variables for TSP`` () =
        ...
        let quboSize =
            ProblemTransformer.calculateQuboSize EncodingStrategy.NodeBased numCities
    ...
    let ``EncodingStrategy EdgeBased should create n-squared variables for TSP`` () =
        ...
        let quboSize =
            ProblemTransformer.calculateQuboSize EncodingStrategy.EdgeBased numCities
    ...
    let ``Portfolio CorrelationBased encoding should create QUBO from covariance`` () =
        ...
        let covariance =
            array2D [ [ 0.04; 0.01; 0.02 ]; [ 0.01; 0.09; 0.03 ]; [ 0.02; 0.03; 0.05 ] ]

        let riskWeight = 0.5

        let qubo =
            ProblemTransformer.encodePortfolioCorrelation returns covariance riskWeight
    ...
    let ``validateTransformation should verify QUBO matrix symmetry`` () =
        ...
        let validQubo =
            {
                Size = 3
                Coefficients = array2D [ [ 1.0; 2.0; 3.0 ]; [ 2.0; 4.0; 5.0 ]; [ 3.0; 5.0; 6.0 ] ]
    ...
    let ``validateTransformation should detect asymmetric QUBO`` () =
        ...
        let asymmetricQubo =
            {
                Size = 3
                Coefficients = array2D [ [ 1.0; 2.0; 3.0 ]; [ 2.0; 4.0; 5.0 ]; [ 999.0; 5.0; 6.0 ] ] // Q[2,0] != Q[0,2]
                VariableNames = [ "x0"; "x1"; "x2" ]
            }
    ...
    let ``validateTransformation should check coefficient bounds`` () =
        ...
        let invalidQubo =
            {
                Size = 2
                Coefficients = array2D [ [ infinity; 1.0 ]; [ 1.0; 2.0 ] ]
                VariableNames = [ "x0"; "x1" ]
            }
    ...
    let ``validateTransformation should check for NaN values`` () =
        ...
        let invalidQubo =
            {
                Size = 2
                Coefficients = array2D [ [ nan; 1.0 ]; [ 1.0; 2.0 ] ]
                VariableNames = [ "x0"; "x1" ]
            }
    ...
    let ``Benchmark EdgeBased vs NodeBased TSP encoding efficiency`` () =
        ...
        let objectiveTermCount =
            [ 0..24 ]
            |> List.sumBy (fun i -> if edgeQubo.GetCoefficient(i, i) < 0.0 then 1 else 0)
