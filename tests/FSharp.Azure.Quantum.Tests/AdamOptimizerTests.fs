...
let ``createConfig should reject negative learning rate`` () =
    ...
    result
    |> Result.map (fun _ -> failwith "Should have failed")
    |> Result.defaultWith (fun msg -> Assert.Contains("Learning rate", msg.Message))
...
let ``createConfig should reject Beta1 outside range`` () =
    ...
    result1
    |> Result.map (fun _ -> failwith "Should have failed for negative Beta1")
    |> Result.defaultWith (fun msg -> Assert.Contains("Beta1", msg.Message))

    let result2 = createConfig 0.001 1.0 0.999 1e-8

    result2
    |> Result.map (fun _ -> failwith "Should have failed for Beta1 = 1.0")
    |> Result.defaultWith (fun msg -> Assert.Contains("Beta1", msg.Message))
...
let ``createConfig should reject Beta2 outside range`` () =
    ...
    result1
    |> Result.map (fun _ -> failwith "Should have failed for negative Beta2")
    |> Result.defaultWith (fun msg -> Assert.Contains("Beta2", msg.Message))

    let result2 = createConfig 0.001 0.9 1.0 1e-8

    result2
    |> Result.map (fun _ -> failwith "Should have failed for Beta2 = 1.0")
    |> Result.defaultWith (fun msg -> Assert.Contains("Beta2", msg.Message))
...
let ``createConfig should reject non-positive epsilon`` () =
    ...
    result
    |> Result.map (fun _ -> failwith "Should have failed")
    |> Result.defaultWith (fun msg -> Assert.Contains("Epsilon", msg.Message))
...
let ``update should increment time step`` () =
    ...
    match result with
    | Ok(_, newState) -> Assert.Equal(1, newState.T)
    | Error err -> failwithf "Should not fail: %s" err.Message
...
let ``update should reject mismatched parameter dimensions`` () =
    ...
    result
    |> Result.map (fun _ -> failwith "Should have failed")
    |> Result.defaultWith (fun msg -> Assert.Contains("Parameters length", msg.Message))
...
let ``update should reject mismatched gradient dimensions`` () =
    ...
    result
    |> Result.map (fun _ -> failwith "Should have failed")
    |> Result.defaultWith (fun msg -> Assert.Contains("Gradients length", msg.Message))
...
let ``update should converge on simple quadratic function`` () =
    ...
    Assert.True(
        abs parameters.[0] < abs 10.0,
        sprintf "Expected convergence closer to 0 than 10.0, got %f" parameters.[0]
    )
...
let ``update should handle very small gradients without underflow`` () =
    ...
    match result with
    | Ok(newParams, _) -> Assert.False(System.Double.IsNaN(newParams.[0]), "Should not be NaN")
    | Error err -> failwithf "Should not fail: %s" err.Message
