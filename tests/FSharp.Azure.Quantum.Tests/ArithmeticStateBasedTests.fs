...
module ArithmeticStateBasedTests =
    ...
    let private runCircuitAddConstant
        (numQubits: int)
        ...
        | Ok state0 ->
            ...
            | Ok stateWithX ->
                ...
                | Ok result ->
                    ...
                    | otherStateType ->
                        Assert.Fail(
                            $"Expected StateVector simulation state, got: {QuantumState.stateType otherStateType}"
                        )
                ...
                | Ok result ->
                    ...
                    | QuantumState.StateVector sv ->
                        ...
                        let (circuitIdx, circuitProb) =
                            runCircuitAddConstant 2 registerQubits constantToAdd (fun c -> c |> addGate (X 1))
    ...
    let ``controlledAddConstantModN: applies when control=|1⟩`` () =
        ...
        | Ok state0 ->
            ...
            let preparedState =
                ...
                |> List.fold
                    (fun st op ->
                        (bknd.ApplyOperation op st)
                        |> Result.defaultWith (fun err -> failwith $"Prep failed: {err}"))
                    state0
    ...
    let ``controlledAddConstantModN: no-op when control=|0⟩`` () =
        ...
        | Error err -> Assert.Fail($"Init failed: {err}")
        | Ok state0 ->
            let ops = [ QuantumOperation.Gate(X 0); QuantumOperation.Gate(X 1) ]
            ...
            | Ok result ->
                ...
                | QuantumState.StateVector sv ->
                    ...
                    for i in 0 .. List.length tempQubits - 1 do
                        ...
                        Assert.True(prob0 > 0.95, $"Temp qubit {i} (qubit #{tq}) should be |0>, but P(0) = {prob0:F6}")
                | _ -> Assert.Fail("Expected StateVector")
            ...
            | Ok stateWithX ->
                ...
                | Ok result ->
                    ...
                    | QuantumState.StateVector sv ->
                        ...
                        let (circuitIdx, circuitProb) =
                            runCircuitAddConstant 3 registerQubits constantToAdd (fun c -> c |> addGate (X 2))
