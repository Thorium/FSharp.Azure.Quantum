...
module QuantumPatternMatcherBuilderTests =
    ...
    let costEffectivePattern (config: Config) : bool = config.Cost < 20.0

    /// Pattern for integer search space
    let evenNumberPattern (n: int) : bool = n % 2 = 0
    ...
    let ``QuantumPatternMatcher.solve should return error for invalid problem`` () =
        ...
        |> Result.defaultWith (fun err -> Assert.Contains("exceeds maximum", err.Message))
