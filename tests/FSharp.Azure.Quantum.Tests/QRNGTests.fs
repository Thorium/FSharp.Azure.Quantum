...
module QRNGTests =
    ...
    let ``entropy calculation is correct`` () =
        ...
        let perfectBits = Array.init 1000 (fun i -> i % 2 = 0) // [true, false, true, false, ...]
    ...
    let ``integer conversion is correct for simple pattern`` () =
        ...
        match result.AsInteger with
        | Some intValue -> Assert.Equal(uint64 result.AsBytes.[0], intValue)
        | None -> Assert.True(false, "AsInteger should be Some for 8 bits")
...
module QuantumDistributionsTests =
    ...
    let ``Normal distribution requires positive stddev`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Should fail with negative stddev"))
        |> Result.defaultWith (fun msg -> Assert.Contains("stddev must be positive", msg))
    ...
    let ``Normal distribution requires finite parameters`` () =
        ...
        result1
        |> Result.map (fun _ -> Assert.True(false, "Should fail with NaN mean"))
        |> Result.defaultWith (fun msg -> Assert.Contains("mean must be finite", msg))

        let dist2 = Normal(0.0, Double.PositiveInfinity)
        let result2 = sample dist2

        result2
        |> Result.map (fun _ -> Assert.True(false, "Should fail with infinite stddev"))
        |> Result.defaultWith (fun msg -> Assert.Contains("stddev must be finite", msg))
    ...
    let ``LogNormal distribution requires positive sigma`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Should fail with negative sigma"))
        |> Result.defaultWith (fun msg -> Assert.Contains("sigma must be positive", msg))
    ...
    let ``Exponential distribution requires positive lambda`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Should fail with negative lambda"))
        |> Result.defaultWith (fun msg -> Assert.Contains("lambda must be positive", msg))
    ...
    let ``Uniform distribution requires min less than max`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Should fail with min >= max"))
        |> Result.defaultWith (fun msg -> Assert.Contains("min must be < max", msg))
    ...
    let ``Custom distribution requires non-empty name`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Should fail with empty name"))
        |> Result.defaultWith (fun msg -> Assert.Contains("name cannot be empty", msg))
    ...
    let ``sample Normal with custom mean and stddev`` () =
        ...
        let results =
            [ 1..1000 ]
            |> List.choose (fun _ -> (sample dist) |> Result.map (fun r -> Some r.Value) |> Result.defaultValue None)
    ...
    let ``sampleMany generates correct number of samples`` () =
        ...
        | Ok samples ->
            ...
            samples |> Array.iter (fun s -> Assert.Equal(53, s.QuantumBitsUsed))
        | Error msg -> Assert.True(false, $"Should succeed: {msg}")
    ...
    let ``sampleMany fails with zero count`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Should fail with zero count"))
        |> Result.defaultWith (fun msg -> Assert.Contains("must be positive", msg))
    ...
    let ``sampleMany fails with excessive count`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Should fail with excessive count"))
        |> Result.defaultWith (fun msg -> Assert.Contains("too large", msg))
    ...
    let ``sampleMany propagates validation errors`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Should fail with invalid distribution"))
        |> Result.defaultWith (fun msg -> Assert.Contains("stddev must be positive", msg))
    ...
    let ``sample Custom validates result`` () =
        ...
        resultNaN
        |> Result.map (fun _ -> Assert.True(false, "Should fail with NaN transform"))
        |> Result.defaultWith (fun msg -> Assert.Contains("NaN", msg))
        ...
        resultInf
        |> Result.map (fun _ -> Assert.True(false, "Should fail with Infinity transform"))
        |> Result.defaultWith (fun msg -> Assert.Contains("Infinity", msg))
