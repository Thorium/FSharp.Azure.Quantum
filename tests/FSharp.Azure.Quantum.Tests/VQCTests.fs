...
module VQCTests =
    ...
    let ``train - rejects empty training set`` () =
        ...
        (train backend featureMap variationalForm parameters emptyFeatures emptyLabels config)
    ...
    let ``train - rejects mismatched features and labels length`` () =
        ...
        (train backend featureMap variationalForm parameters features labels config)
    ...
    let ``evaluate - rejects empty dataset`` () =
        ...
        (evaluate backend featureMap variationalForm parameters emptyFeatures emptyLabels 100)
    ...
    let ``evaluate - rejects mismatched features and labels length`` () =
        ...
        (evaluate backend featureMap variationalForm parameters features labels 100)
        |> Result.map (fun _ -> Assert.True(false, "Should have rejected mismatched lengths"))
        |> Result.defaultWith (fun msg -> Assert.Contains("same length", msg.Message))
    ...
    let ``train - returns trained parameters with correct length`` () =
        ...
        (train backend featureMap variationalForm initialParams features labels config)
        |> Result.map (fun result -> Assert.Equal(initialParams.Length, result.Parameters.Length))
        |> Result.defaultWith (fun msg -> Assert.True(false, $"Training failed: {msg}"))
    ...
    let ``train - respects max epochs limit`` () =
        ...
        |> Result.map (fun result ->
            Assert.True(
                result.Epochs <= config.MaxEpochs,
                $"Epochs {result.Epochs} should not exceed {config.MaxEpochs}"
            ))
        |> Result.defaultWith (fun msg -> Assert.True(false, $"Training failed: {msg}"))
    ...
    let ``End-to-end - train and evaluate workflow`` () =
        ...
        | Ok trainResult ->
            ...
            | Ok accuracy ->
                ...
                Assert.True(
                    trainResult.TrainAccuracy >= 0.0 && trainResult.TrainAccuracy <= 1.0,
                    "Train accuracy valid"
                )
    ...
    let ``train with Adam optimizer - should complete successfully`` () =
        ...
        let parameters =
            VariationalForms.randomParameters variationalForm (features.[0].Length) (Some 42)
        ...
        let result =
            train backend featureMap variationalForm parameters features labels adamConfig
    ...
    let ``train with SGD vs Adam - both should converge`` () =
        ...
        let sgdResult =
            train backend featureMap variationalForm parameters features labels sgdConfig
        ...
        | Error sgdErr, _ -> Assert.True(false, $"SGD training failed: {sgdErr}")
        | _, Error adamErr -> Assert.True(false, $"Adam training failed: {adamErr}")
    ...
    let ``trainMultiClass - rejects single class`` () =
        ...
        (trainMultiClass backend featureMap variationalForm parameters features labels config)
        |> Result.map (fun _ -> Assert.True(false, "Should have rejected single class"))
        |> Result.defaultWith (fun msg -> Assert.Contains("at least 2 classes", msg.Message))
    ...
    let ``trainMultiClass - rejects empty training set`` () =
        ...
        (trainMultiClass backend featureMap variationalForm parameters emptyFeatures emptyLabels config)
        |> Result.map (fun _ -> Assert.True(false, "Should have rejected empty training set"))
        |> Result.defaultWith (fun msg -> Assert.Contains("cannot be empty", msg.Message))
        ...
        | Ok trainResult ->
            ...
            | Ok prediction ->
                ...
                let predictedClassIdx =
                    Array.findIndex ((=) prediction.Label) trainResult.ClassLabels

                let expectedConfidence = prediction.Probabilities.[predictedClassIdx]

                Assert.True(
                    abs (prediction.Confidence - expectedConfidence) < 0.001,
    ...
    let ``predictRegressionAsync - returns valid prediction for single sample`` () : Task =
        task {
            ...
            with
            | Error msg -> Assert.True(false, $"predictRegressionAsync failed: {msg}")
            | Ok prediction ->
                Assert.True(
                    prediction.Value >= 0.0 && prediction.Value <= 1.0,
                    $"Value {prediction.Value} must be in [0,1]"
                )
        }
    ...
    let ``predictRegressionAsync - produces equivalent results to sync version`` () : Task =
        task {
            ...
            let syncResult =
                predictRegression backend featureMap variationalForm parameters features 1000 valueRange
    ...
    let ``predictRegressionAsync - handles wide value range`` () : Task =
        task {
            ...
            | Ok prediction ->
                Assert.True(
                    prediction.Value >= 0.0 && prediction.Value <= 10.0,
                    $"Value {prediction.Value} should be in [0,10]"
                )
        }
