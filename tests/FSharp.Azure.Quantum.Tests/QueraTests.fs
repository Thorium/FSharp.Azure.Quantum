...
module QueraTests =
    ...
    let ``toAhsProgram writes coordinates in metres`` () =
        ...
        let sites =
            doc.RootElement.GetProperty("setup").GetProperty("ahs_register").GetProperty "sites"
    ...
    let ``the amplitude time-series matches the number of pulse-segment boundaries`` () =
        ...
        let series =
            doc.RootElement
                .GetProperty("hamiltonian")
                .GetProperty("drivingFields")
                .[0].GetProperty("amplitude")
                .GetProperty
                "time_series"
    ...
    let ``toAhsProgram preserves discontinuous segment boundaries as steep ramps`` () =
        ...
        let values =
            [ for v in series.GetProperty("values").EnumerateArray() -> v.GetDouble() ]

        let times =
            [ for t in series.GetProperty("times").EnumerateArray() -> t.GetDouble() ]
        ...
        times
        |> List.pairwise
        |> List.iter (fun (a, b) -> Assert.True(a < b, $"times not strictly increasing: {a} !< {b}"))
