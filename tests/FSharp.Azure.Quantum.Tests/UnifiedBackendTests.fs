...
module UnifiedBackendTests =
    ...
    let wrapCircuit (circuit: CircuitBuilder.Circuit) : ICircuit = CircuitWrapper(circuit) :> ICircuit
    ...
    let ``LocalBackend ExecuteToState returns StateVector`` () =
        ...
        | Ok(QuantumState.StateVector sv) -> Assert.Equal(2, StateVector.numQubits sv)
        | Ok _ -> Assert.True(false, "Expected StateVector, got different state type")
        | Error err -> Assert.True(false, $"Execution failed: {err}")
    ...
    let ``LocalBackend InitializeState creates |0⟩^⊗n`` () =
        ...
        | Error err -> Assert.True(false, $"InitializeState failed: {err}")
    ...
    let ``LocalBackend ApplyOperation applies gate correctly`` () =
        ...
        | Ok initialState ->
            ...
            | Ok _ -> Assert.True(false, "Expected StateVector")
            | Error err -> Assert.True(false, $"ApplyOperation failed: {err}")
        | Error err -> Assert.True(false, $"InitializeState failed: {err}")
    ...
    let ``LocalBackend unified state-based execution with IQuantumBackend`` () =
        ...
        | Error err -> Assert.True(false, $"Execution failed: {err.Message}")
    ...
    let ``TopologicalBackend implements IQuantumBackend`` () =
        let backend =
            TopologicalUnifiedBackend.TopologicalUnifiedBackend(AnyonSpecies.AnyonType.Ising, 20)
    ...
    let ``TopologicalBackend InitializeState creates topological |0⟩^⊗n`` () =
        ...
        | Ok _ -> Assert.True(false, "Expected FusionSuperposition")
        | Error err -> Assert.True(false, $"InitializeState failed: {err}")
    ...
    let ``Convert StateVector to StateVector is identity`` () =
        ...
        | Ok(QuantumState.StateVector sv2) -> Assert.Equal(StateVector.numQubits sv, StateVector.numQubits sv2)
        | Ok _ -> Assert.True(false, "Conversion returned wrong type")
        | Error err -> Assert.True(false, $"Conversion failed: {err}")
    ...
    let ``Convert StateVector to FusionSuperposition`` () =
        ...
        | Ok(QuantumState.StateVector _) -> Assert.True(false, "Should not silently return unchanged state")
        | Ok(QuantumState.FusionSuperposition fs) ->
            // If topological package implements conversion, this would work
            let fusion = TopologicalOperations.fromInterface fs |> Option.get
            Assert.NotEmpty(fusion.Terms)
        | Ok _ -> Assert.True(false, "Unexpected state type")
    ...
    let ``Round-trip conversion StateVector → Fusion → StateVector is lossless`` () =
        ...
        | Ok fusionState ->
            ...
            | Ok _ -> Assert.True(false, "Expected StateVector after round-trip")
            | Error err -> Assert.True(false, $"Back-conversion failed: {err}")
        | Error err -> Assert.True(false, $"Unexpected error: {err}")

    [<Fact>]
    let ``Round-trip conversion on superposition state is lossless`` () =
        // Create |+⟩ state: (|0⟩ + |1⟩)/√2
        let amplitudes = [| Complex(1.0 / sqrt 2.0, 0.0); Complex(1.0 / sqrt 2.0, 0.0) |]
    ...
    let ``Execute circuit on TopologicalBackend compiles gates to braids`` () =
        ...
        | Ok state ->
            ...
            | _ -> Assert.True(false, $"Expected FusionSuperposition state, got {state.GetType().Name}")
        | Error err ->
            ...
            | Ok state2 ->
                ...
                | Ok topState ->
                    ...
                    | Ok(QuantumState.FusionSuperposition fs) ->
                        ...
                        Assert.NotEmpty(fusion.Terms)
                    | Ok _ -> Assert.True(false, "Expected FusionSuperposition")
                    | Error err -> Assert.True(false, $"Braiding operation failed: {err}")
    ...
    let ``UnifiedBackend applyWithConversion avoids unnecessary conversion`` () =
        ...
        | Ok state ->
            ...
            | Ok _ -> Assert.True(false, "Expected StateVector")
            | Error err -> Assert.True(false, $"Operation failed: {err}")
        | Error err -> Assert.True(false, $"InitializeState failed: {err}")
    ...
    let ``UnifiedBackend applySequence optimizes batch execution`` () =
        ...
        | Ok initialState ->
            ...
            | Ok(QuantumState.StateVector sv) -> Assert.Equal(2, StateVector.numQubits sv)
            | Ok _ -> Assert.True(false, "Expected StateVector")
            | Error err -> Assert.True(false, $"Sequence execution failed: {err}")
        | Error err -> Assert.True(false, $"InitializeState failed: {err}")
