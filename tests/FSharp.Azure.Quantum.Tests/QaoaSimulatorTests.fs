...
module QaoaSimulatorTests =
    ...
    let ``Initialize uniform superposition - should reject invalid qubit counts`` () =
        Assert.Throws<Exception>(fun () -> QaoaSimulator.initializeUniformSuperposition 0 |> ignore)
        |> ignore

        Assert.Throws<Exception>(fun () -> QaoaSimulator.initializeUniformSuperposition 17 |> ignore)
        |> ignore
    ...
    let ``Apply cost layer - should reject mismatched array length`` () =
        ...
        Assert.Throws<Exception>(fun () -> QaoaSimulator.applyCostLayer 1.0 costCoeffs state |> ignore)
    ...
    let ``Apply cost interaction - should reject invalid indices`` () =
        ...
        Assert.Throws<Exception>(fun () -> QaoaSimulator.applyCostInteraction 1.0 0 0 1.0 state |> ignore)
        |> ignore

        // Out of range
        Assert.Throws<Exception>(fun () -> QaoaSimulator.applyCostInteraction 1.0 0 2 1.0 state |> ignore)
        |> ignore
    ...
    let ``Run QAOA circuit - should reject mismatched array lengths`` () =
        ...
        Assert.Throws<Exception>(fun () -> QaoaSimulator.runQaoaCircuit 2 gammas betas costCoeffs |> ignore)
    ...
    let ``Compute cost expectation - should reject mismatched array length`` () =
        ...
        Assert.Throws<Exception>(fun () -> QaoaSimulator.computeCostExpectation costCoeffs state |> ignore)
    ...
    let ``simulate - should handle multiple QAOA layers`` () =
        ...
        | Ok qaoaResult ->
            ...
            qaoaResult.Counts
            |> Map.iter (fun bitstring _ -> Assert.Equal(3, bitstring.Length))
        | Error msg -> Assert.True(false, $"Expected Ok, got Error: {msg}")
    ...
    let ``simulate - should return Error for invalid NumQubits`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Expected Error, got Ok"))
        |> Result.defaultWith (fun msg -> Assert.Contains("qubits", msg.ToLower()))
    ...
    let ``simulate - should return Error for invalid number of shots`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Expected Error, got Ok"))
        |> Result.defaultWith (fun msg -> Assert.Contains("shots", msg.ToLower()))
    ...
    let ``simulate - should return Error for mismatched Parameters length`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.True(false, "Expected Error, got Ok"))
        |> Result.defaultWith (fun msg -> Assert.Contains("parameters", msg.ToLower()))
    ...
    let ``simulate - MaxCut example from documentation`` () =
        ...
        | Ok qaoaResult ->
            ...
            qaoaResult.Counts
            |> Map.iter (fun bitstring _ -> Assert.Equal(4, bitstring.Length))

            // Find most common outcome
            let mostCommon = qaoaResult.Counts |> Map.toSeq |> Seq.maxBy snd |> fst
