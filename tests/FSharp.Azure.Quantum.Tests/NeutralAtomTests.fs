...
module NeutralAtomTests =
    ...
    let private countOf (key: string) (h: Map<string, int>) =
        h |> Map.tryFind key |> Option.defaultValue 0
    ...
    let ``single-atom quench reproduces Rabi dynamics <n>(t) = sin^2(t/2)`` () =
        ...
        let densityAt (t: float) =
            (evolve backend (quench [ { X = 0.0; Y = 0.0 } ] 1.0 1.0 0.0 t) 200
             |> Result.bind (rydbergDensities 1))
            |> Result.map (fun d -> d.[0])
    ...
    let ``solveMaximumIndependentSet returns the MIS atom indices of a 3-atom path`` () =
        ...
        (solveMaximumIndependentSet (backend ()) register 30.0 1.0 3.0 12.0 120 4000)
        |> Result.map (fun mis -> Assert.Equal<int list>([ 0; 2 ], mis))
        |> Result.defaultWith (fun e -> failwith e.Message) // {A, C}
    ...
    let ``optimizeAnalog tunes a pulse to minimise a cost Hamiltonian`` () =
        ...
        let paramsToProgram (p: float[]) =
            quench [ { X = 0.0; Y = 0.0 } ] 1.0 1.0 0.0 p.[0]
    ...
    let ``blockade caps the collective excitation of a nearby pair`` () =
        ...
        let program =
            quench [ { X = 0.0; Y = 0.0 }; { X = 1.0; Y = 0.0 } ] 100.0 1.0 0.0 Math.PI
