...
module QuantumDistributionsComprehensiveTests =
    ...
    let ``sample Normal rejects non-positive stddev`` () =
        (sample (Normal(0.0, 0.0)))
        |> Result.map (fun _ -> failwith "Expected Error for zero stddev")
        |> Result.defaultWith (fun msg -> Assert.Contains("positive", msg.ToLower()))

    [<Fact>]
    let ``sample Normal rejects negative stddev`` () =
        (sample (Normal(0.0, -1.0)))
        |> Result.map (fun _ -> failwith "Expected Error for negative stddev")
        |> Result.defaultWith (fun msg -> Assert.Contains("positive", msg.ToLower()))

    [<Fact>]
    let ``sample Normal rejects NaN mean`` () =
        (sample (Normal(Double.NaN, 1.0)))
        |> Result.map (fun _ -> failwith "Expected Error for NaN mean")
        |> Result.defaultWith (fun msg -> Assert.Contains("finite", msg.ToLower()))

    [<Fact>]
    let ``sample Exponential rejects non-positive lambda`` () =
        (sample (Exponential(0.0)))
        |> Result.map (fun _ -> failwith "Expected Error for zero lambda")
        |> Result.defaultWith (fun msg -> Assert.Contains("positive", msg.ToLower()))

    [<Fact>]
    let ``sample Uniform rejects min >= max`` () =
        (sample (Uniform(5.0, 3.0)))
        |> Result.map (fun _ -> failwith "Expected Error for min >= max")
        |> Result.defaultWith (fun msg -> Assert.Contains("min", msg.ToLower()))

    [<Fact>]
    let ``sample LogNormal rejects non-positive sigma`` () =
        (sample (LogNormal(0.0, -1.0)))
        |> Result.map (fun _ -> failwith "Expected Error for negative sigma")
        |> Result.defaultWith (fun msg -> Assert.Contains("positive", msg.ToLower()))

    [<Fact>]
    let ``sample Custom rejects empty name`` () =
        (sample (Custom("", id)))
        |> Result.map (fun _ -> failwith "Expected Error for empty name")
        |> Result.defaultWith (fun msg -> Assert.Contains("name", msg.ToLower()))
    ...
    let ``sample Normal returns finite value`` () =
        (sample (Normal(10.0, 2.0)))
        |> Result.map (fun result -> Assert.True(Double.IsFinite(result.Value)))
        |> Result.defaultWith (fun msg -> failwith $"Expected Ok, got Error: {msg}")

    [<Fact>]
    let ``sample Exponential returns positive value`` () =
        (sample (Exponential(1.0)))
    ...
    let ``sample Uniform returns value in range`` () =
        ...
        | Ok result ->
            Assert.True(
                result.Value >= 2.0 && result.Value <= 5.0,
    ...
    let ``sampleMany returns correct count`` () =
        (sampleMany StandardNormal 10)
        |> Result.map (fun samples -> Assert.Equal(10, samples.Length))
        |> Result.defaultWith (fun msg -> failwith $"Expected Ok, got Error: {msg}")

    [<Fact>]
    let ``sampleMany rejects non-positive count`` () =
        (sampleMany StandardNormal 0)
        |> Result.map (fun _ -> failwith "Expected Error for zero count")
        |> Result.defaultWith (fun msg -> Assert.Contains("positive", msg.ToLower()))

    [<Fact>]
    let ``sampleMany rejects count > 1000000`` () =
        (sampleMany StandardNormal 1000001)
        |> Result.map (fun _ -> failwith "Expected Error for too-large count")
        |> Result.defaultWith (fun msg -> Assert.Contains("large", msg.ToLower()))
    ...
    let ``computeStatistics throws on empty array`` () =
        Assert.Throws<Exception>(fun () -> computeStatistics [||] |> ignore) |> ignore
