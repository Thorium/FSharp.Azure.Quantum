...
module ResetBarrierTests =
    ...
    let ``import parses multiple reset instructions`` () =
        ...
        result
        |> Result.map (fun circuit -> Assert.Equal<Gate list>([ Reset 0; Reset 1; Reset 2 ], getGates circuit))
        |> Result.defaultWith (fun msg -> Assert.Fail(msg))
    ...
    let ``import parses barrier with single qubit`` () =
        ...
        result
        |> Result.map (fun circuit -> Assert.Equal<Gate list>([ Barrier [ 1 ] ], circuit.Gates))
        |> Result.defaultWith (fun msg -> Assert.Fail(msg))
    ...
    let ``import parses barrier V3_0`` () =
        ...
        result
        |> Result.map (fun circuit -> Assert.Equal<Gate list>([ Barrier [ 0; 2 ] ], circuit.Gates))
        |> Result.defaultWith (fun msg -> Assert.Fail(msg))
    ...
    let ``import parses barrier with spaces around brackets`` () =
        ...
        result
        |> Result.map (fun circuit -> Assert.Equal<Gate list>([ Barrier [ 0; 1 ] ], circuit.Gates))
        |> Result.defaultWith (fun msg -> Assert.Fail(msg))
    ...
    let ``import mixed circuit with gates reset and barrier`` () =
        ...
        | Ok circuit ->
            Assert.Equal(3, circuit.QubitCount)
            Assert.Equal<Gate list>([ H 0; Barrier [ 0; 1; 2 ]; CNOT(0, 1); Reset 2; X 1 ], getGates circuit)
        | Error msg -> Assert.Fail(msg)
    ...
    let ``LocalBackend executes circuit with Reset without crashing`` () =
        ...
        | Ok _ -> Assert.Fail("Expected StateVector, got different state type")
