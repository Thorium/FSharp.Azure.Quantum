    ...
    let ``solve should factor N=15 (classic example)`` () =
        ...
        | Ok prob ->
            ...
            | Ok result ->
                ...
                Assert.True(
                    result.Success || result.Factors.IsSome || result.Attempts > 0,
    ...
    let ``solve should handle prime numbers gracefully`` () =
        ...
        | Ok prob ->
            ...
            | Ok result ->
                ...
                Assert.True(
                    result.Factors.IsNone || not result.Success || result.Attempts > 0,
    ...
    let ``solve should populate result metadata`` () =
        ...
        | Ok prob ->
            ...
            | Ok result ->
                ...
                Assert.True(
                    result.Attempts >= 1 && result.Attempts <= 3,
