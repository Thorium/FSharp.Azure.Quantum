...
module RigettiBackendTests =
    ...
    let ``QuilProgram - contains declarations and instructions`` () =
        // Arrange
        let declarations = [ QuilGate.DeclareMemory("ro", "BIT", 2) ]
    ...
    let ``serializeProgram - program without declarations`` () =
        ...
        let program =
            {
                Declarations = []
                Instructions = [ QuilGate.SingleQubit("H", 0); QuilGate.SingleQubit("X", 1) ]
            }
    ...
    let ``createJobSubmission - serializes program to Quil text in InputData`` () =
        ...
        let program =
            {
                Declarations = [ QuilGate.DeclareMemory("ro", "BIT", 1) ]
                Instructions = [ QuilGate.SingleQubit("H", 0); QuilGate.Measure(0, "ro[0]") ]
            }
    ...
    let ``parseRigettiResults - handles empty histogram`` () =
        ...
        // Assert
        result
        |> Result.map (fun histogram -> Assert.Empty(histogram))
        |> Result.defaultWith (fun e -> Assert.True(false, $"Expected Ok, got Error: %A{e}"))
    ...
    let ``mapRigettiError - maps InvalidProgram to InvalidCircuit`` () =
        ...
        | QuantumError.ValidationError(field, reason) -> Assert.Contains("Malformed Quil", reason)
    ...
    let ``mapRigettiError - maps TopologyError to InvalidCircuit`` () =
        ...
        // Assert
        match quantumError with
        | QuantumError.ValidationError(field, reason) -> Assert.Contains("connectivity", reason)
        | _ -> Assert.True(false, "Expected ValidationError")
    ...
    let ``mapRigettiError - maps TooManyQubits to InvalidCircuit`` () =
        ...
        // Assert
        match quantumError with
        | QuantumError.ValidationError(field, reason) -> Assert.Contains("40", reason)
        | _ -> Assert.True(false, "Expected ValidationError")
