...
module OpenQasmExportTests =
    ...
    let ``export gates in correct order`` () =
        let circuit = empty 3 |> addGates [ H 0; X 1; CNOT(0, 1); Y 2; RZ(2, 0.5) ]
        let qasm = OpenQasm.export circuit
    ...
    [<Fact>]
    let ``version returns 2.0`` () = Assert.Equal("2.0", OpenQasm.version)
    ...
    let ``export QAOA-style circuit with rotations`` () =
        let circuit =
            {
                QubitCount = 2
                Gates = [ H 0; H 1; RZ(0, 0.5); RZ(1, 0.5); CNOT(0, 1); RX(0, 1.0); RX(1, 1.0) ]
            }
