...
let ``estimateCost should increase with shot count`` () =
    ...
    | Ok lowEstimate, Ok highEstimate ->
        Assert.True(highEstimate.ExpectedCost > lowEstimate.ExpectedCost,
            sprintf "Higher shot count should cost more: %M vs %M" 
...
let ``estimateCost should return error for invalid shot count`` () =
    ...
    match result with
    | Ok _ ->
        Assert.True(false, "Expected error for zero shots")
    | Error msg ->
...
let ``estimateCost should return error for empty target`` () =
    ...
    match result with
    | Ok _ ->
        Assert.True(false, "Expected error for empty target")
    | Error msg ->
        Assert.Contains("Target backend cannot be empty", msg)
...
let ``parseCostFromMetadata should parse estimated cost from JSON`` () =
    ...
    | None ->
        Assert.True(false, "Expected cost info but got None")

[<Fact>]
let ``parseCostFromMetadata should parse actual cost from JSON`` () =
    // Arrange
    let costData = Some """{"actual": 142.75, "currency": "USD", "billingStatus": "Charged"}"""
