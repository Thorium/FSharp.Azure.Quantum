...
module QFTTests =
    ...
    let ``QFT then inverse QFT returns original state`` () =
        ...
        | Ok forwardResult ->
            ...
            | Ok inverseResult ->
                ...
                let allZeros =
                    measurements |> Array.forall (fun bits -> bits |> Array.forall ((=) 0))
    ...
    let ``QFT on computational basis state produces uniform superposition`` () =
        ...
        | Ok result ->
            ...
            for bits in measurements do
                let stateIndex =
                    bits |> Array.indexed |> Array.fold (fun acc (i, bit) -> acc + (bit <<< i)) 0
    ...
    let ``QFT verifyUnitarity confirms QFT is unitary`` () =
        ...
        (QFT.verifyUnitarity 3 backend config)
        |> Result.map (fun isUnitary -> Assert.True(isUnitary, "QFT should be unitary transformation"))
        |> Result.defaultWith (fun err -> Assert.Fail($"verifyUnitarity failed: {err}"))
    ...
    let ``QFT transformBasisState rejects invalid basis index`` () =
        ...
        | Error(QuantumError.ValidationError _) -> Assert.True(true, "Correctly rejected invalid index")
    ...
    let ``QFT transformBasisState with all basis states`` () =
        ...
        for basisIndex in 0 .. (1 <<< numQubits) - 1 do
            (QFT.transformBasisState numQubits basisIndex backend config)
            |> Result.map (fun result ->
                Assert.True(result.GateCount > 0, $"Should have applied gates for |{basisIndex}⟩"))
            |> Result.defaultWith (fun err -> Assert.Fail($"transformBasisState failed for |{basisIndex}⟩: {err}"))
    ...
    let ``QFT verifyUnitarity works with inverse config`` () =
        ...
        (QFT.verifyUnitarity 3 backend config)
        |> Result.map (fun isUnitary -> Assert.True(isUnitary, "Inverse QFT should also be unitary"))
        |> Result.defaultWith (fun err -> Assert.Fail($"verifyUnitarity with inverse failed: {err}"))
    ...
    let ``QFT verifyUnitarity works without swaps`` () =
        ...
        (QFT.verifyUnitarity 3 backend config)
        |> Result.map (fun isUnitary -> Assert.True(isUnitary, "QFT without swaps should still be unitary"))
        |> Result.defaultWith (fun err -> Assert.Fail($"verifyUnitarity without swaps failed: {err}"))
