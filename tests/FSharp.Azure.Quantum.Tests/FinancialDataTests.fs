...
module FinancialDataTests =
    ...
    let ``calculateReturns with 2 prices produces 1 return`` () =
        let bars =
            [| makeBar (DateTime(2024, 1, 1)) 100.0; makeBar (DateTime(2024, 1, 2)) 110.0 |]
        ...
        let returns =
            {
                ...
                Dates = Array.init 5 (fun i -> DateTime(2024, 1, i + 2))
            }
    ...
    let ``tryGetLatestPrice prefers AdjustedClose`` () =
        let bars = [| makeBarAdj (DateTime(2024, 1, 1)) 100.0 95.0 |]
        let series = makePriceSeries "TEST" bars
    ...
    let ``calculateCovarianceMatrix annualized has larger values`` () =
        ...
        Assert.True(
            abs (covAnn.Values.[0].[0]) > abs (covDaily.Values.[0].[0]),
    ...
    let ``createPortfolio calculates total value`` () =
        let positions =
            [ makePosition "AAPL" 10.0 150.0 Equity; makePosition "MSFT" 5.0 300.0 Equity ]
    ...
    let ``portfolioToFeatures returns weights summing to 1`` () =
        let positions =
            [ makePosition "AAPL" 10.0 100.0 Equity; makePosition "MSFT" 10.0 100.0 Equity ]
    ...
    let ``calculateParametricVaR VaRPercent is VaR / portfolio value`` () =
        ...
        (calculateParametricVaR portfolio covMatrix riskParams)
        |> Result.map (fun result -> Assert.True(abs (result.VaRPercent - result.VaR / result.PortfolioValue) < 1e-10))
        ...
        let riskParams =
            {
                ConfidenceLevel = 0.95
                TimeHorizon = 1
                Distribution = ReturnDistribution.Historical
                LookbackPeriod = 252
            }
    ...
    let ``calculateHistoricalVaR returns positive VaR with sufficient data`` () =
        ...
        | Ok result ->
            ...
            Assert.True(
                result.VaR < result.PortfolioValue,
    ...
    let ``applyStressScenario reduces equity portfolio`` () =
        ...
        Assert.True(
            stressedValue < portfolio.TotalValue,
    ...
    let ``applyStressScenario cash is unaffected`` () =
        ...
        let stressedValue = applyStressScenario portfolio financialCrisis2008
        Assert.True(abs (stressedValue - portfolio.TotalValue) < 1e-10, "Cash should be unaffected by equity stress")
    ...
    let ``applyStressScenario applies symbol-specific shock`` () =
        ...
        Assert.True(abs (stressedValue - 8000.0) < 1e-10, $"Expected 8000, got {stressedValue}")
