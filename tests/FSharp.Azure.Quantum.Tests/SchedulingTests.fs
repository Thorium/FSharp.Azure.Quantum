...
module SchedulingTests =
    ...
    let ``Dependency types represent task relationships`` () =
        ...
        | Scheduling.StartToStart(t1, t2, lag) -> Assert.Equal(1.0, lag)
    ...
    let ``SchedulingConstraint types define problem constraints`` () =
        // Arrange & Act
        let deps =
            Scheduling.TaskDependencies([ Scheduling.FinishToStart("T1", "T2", 0.0) ])
        ...
        match deps with
        | Scheduling.TaskDependencies(depList) -> Assert.Equal(1, List.length depList)
    ...
    let ``Classical List Scheduling solves simple problem with dependencies`` () =
        ...
        | Error msg -> Assert.Fail($"Solver failed: {msg}")
