...
module BatchAccumulatorTests =
    ...
    let ``BatchConfig with valid parameters should succeed`` () =
        ...
        | Error err -> Assert.True(false, $"Expected success but got error: {err.Message}")
    ...
    let ``BatchConfig with zero batch size should fail`` () =
        ...
        // Assert
        config
        |> Result.map (fun _ -> Assert.True(false, "Expected validation error for zero batch size"))
        |> Result.defaultWith (fun err -> Assert.Contains("must be positive", err.Message))
    ...
    let ``BatchConfig with negative batch size should fail`` () =
        ...
        // Assert
        config
        |> Result.map (fun _ -> Assert.True(false, "Expected validation error for negative batch size"))
        |> Result.defaultWith (fun err -> Assert.Contains("must be positive", err.Message))
    ...
    let ``BatchConfig with zero timeout should fail`` () =
        ...
        // Assert
        config
        |> Result.map (fun _ -> Assert.True(false, "Expected validation error for zero timeout"))
        |> Result.defaultWith (fun err -> Assert.Contains("must be positive", err.Message))
    ...
    let ``BatchConfig with negative timeout should fail`` () =
        ...
        // Assert
        config
        |> Result.map (fun _ -> Assert.True(false, "Expected validation error for negative timeout"))
        |> Result.defaultWith (fun err -> Assert.Contains("must be positive", err.Message))
    ...
    let ``BatchAccumulator should trigger on max batch size`` () =
        // Arrange
        let config =
            { BatchConfig.defaultConfig with
                MaxBatchSize = 3
            }
    ...
    let ``batchCircuitsAsync should batch multiple circuits based on size limit`` () =
        ...
        task {
            ...
            Assert.Equal<string seq>([ "c1_result"; "c2_result"; "c3_result"; "c4_result"; "c5_result" ], results)
        }
        :> Task
