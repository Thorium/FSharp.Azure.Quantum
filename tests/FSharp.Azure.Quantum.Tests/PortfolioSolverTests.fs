...
let ``Greedy-by-ratio should allocate assets by return-risk ratio`` () =
    ...
    let lowAllocExists =
        solution.Allocations |> List.exists (fun a -> a.Asset.Symbol = "LOW")
...
let ``Mean-variance optimization should balance return and risk`` () =
    ...
    let hasBalanced =
        solution.Allocations |> List.exists (fun a -> a.Asset.Symbol = "BALANCED")
...
let ``Mean-variance optimization with high risk tolerance should favor high returns`` () =
    ...
    let riskyAlloc =
        solution.Allocations |> List.tryFind (fun a -> a.Asset.Symbol = "RISKY")
...
let ``Greedy-by-ratio should handle 50 assets efficiently`` () =
    ...
    |> List.iter (fun alloc ->
        Assert.True(
            alloc.Value <= constraints.MaxHolding,
...
let ``Portfolio solver should handle assets with zero risk`` () =
    ...
    let riskFreeGreedy =
        greedySolution.Allocations
        |> List.tryFind (fun a -> a.Asset.Symbol = "RISKFREE")
    ...
    let highBuyCosts =
        {
            BuyCostRate = 0.05
            SellCostRate = 0.0
            FixedCostPerTrade = 0.0
        }

    let problemWithCosts =
        createProblemWithCosts assets constraints 0.5 currentHoldings highBuyCosts
...
let ``Transaction cost QUBO includes sell incentive for held positions`` () =
    ...
    let problemWithCosts =
        createProblemWithCosts assets constraints 0.5 currentHoldings highSellCosts
...
let ``Transaction cost QUBO with fixed costs adds turnover penalty`` () =
    ...
    let problemWithCosts =
        createProblemWithCosts assets constraints 0.5 currentHoldings fixedCosts
    ...
    | Ok quboMatrix ->
        ...
        Assert.True(hasPair01 || hasPair02, "Fixed costs should create off-diagonal turnover penalty terms")
...
let ``Transaction cost QUBO validation rejects negative cost rates`` () =
    ...
    let problemWithCosts =
        createProblemWithCosts assets constraints 0.5 Map.empty invalidCosts
    ...
    // Assert
    result
    |> Result.map (fun _ -> Assert.Fail("Should fail with negative cost rate"))
    |> Result.defaultWith (fun err -> Assert.Contains("non-negative", err.Message.ToLower()))
...
let ``Transaction cost QUBO handles empty portfolio`` () =
    ...
    let problemWithCosts =
        createProblemWithCosts assets constraints 0.5 Map.empty defaultTransactionCosts
    ...
    // Assert
    result
    |> Result.map (fun _ -> Assert.Fail("Should fail with empty assets"))
    |> Result.defaultWith (fun err -> Assert.Contains("no assets", err.Message.ToLower()))
