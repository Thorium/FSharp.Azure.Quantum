...
let ``normalizeMinMax with constant feature should return 0.5`` () =
    let data = [| [| 5.0; 10.0 |]; [| 5.0; 20.0 |]; [| 5.0; 30.0 |] |]
...
let ``standardize should produce zero mean and unit variance`` () =
    ...
    let feature0Var = standardized |> Array.averageBy (fun s -> s.[0] * s.[0])
...
let ``trainTestSplit should fail with mismatched data and labels`` () =
    ...
    Assert.Throws<System.Exception>(fun () -> DataPreprocessing.trainTestSplit data labels 0.5 None |> ignore)
...
let ``trainTestSplit should fail with invalid testSize`` () =
    ...
    Assert.Throws<System.Exception>(fun () -> DataPreprocessing.trainTestSplit data labels 1.5 None |> ignore)
...
let ``kFoldSplit should cover all samples exactly once`` () =
    ...
    let allTestSamples = folds |> Array.collect (fun (_, (testData, _)) -> testData)
...
let ``kFoldSplit should fail with k > n`` () =
    ...
    Assert.Throws<System.Exception>(fun () -> DataPreprocessing.kFoldSplit data labels 5 None |> ignore)
...
let ``kFoldSplit should fail with k < 2`` () =
    ...
    Assert.Throws<System.Exception>(fun () -> DataPreprocessing.kFoldSplit data labels 1 None |> ignore)
