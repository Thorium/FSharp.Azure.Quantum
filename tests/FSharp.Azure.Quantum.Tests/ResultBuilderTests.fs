...
module ResultBuilderTests =
    ...
    let ``result CE ReturnFrom passes Result through`` () =
        let r = result { return! Ok 99 }
        Assert.Equal(Ok 99, r)

    [<Fact>]
    let ``result CE ReturnFrom passes Error through`` () =
        let r: Result<int, string> = result { return! Error "oops" }
    ...
    [<Fact>]
    let ``Result.isOk returns true for Ok`` () = Assert.True(Result.isOk (Ok 1))

    [<Fact>]
    let ``Result.isOk returns false for Error`` () = Assert.False(Result.isOk (Error "x"))

    [<Fact>]
    let ``Result.isError returns true for Error`` () = Assert.True(Result.isError (Error "x"))

    [<Fact>]
    let ``Result.isError returns false for Ok`` () = Assert.False(Result.isError (Ok 1))
    ...
    let ``Result.traverse maps and sequences`` () =
        let r =
            Result.traverse (fun x -> if x > 0 then Ok(x * 2) else Error "neg") [ 1; 2; 3 ]
    ...
    let ``Result.traverse returns first Error`` () =
        let r =
            Result.traverse (fun x -> if x > 0 then Ok(x * 2) else Error "neg") [ 1; -1; 3 ]
    ...
    let ``Result.sequenceArray with all Ok returns Ok array`` () =
        ...
        r
        |> Result.map (fun arr -> Assert.Equal<int array>([| 10; 20; 30 |], arr))
    ...
    let ``Result.sequenceArray with empty array returns Ok empty`` () =
        ...
        r
        |> Result.map (fun arr -> Assert.Empty(arr))
        |> Result.defaultWith (fun e -> failwith $"Expected Ok, got Error: {e}")

    [<Fact>]
    let ``Result.sequenceArray with Error returns first Error`` () =
        let r: Result<int array, string> =
            Result.sequenceArray [| Ok 1; Error "e1"; Error "e2" |]
    ...
    let ``Result.traverseArray maps and sequences array`` () =
        let r =
            Result.traverseArray (fun x -> if x > 0 then Ok(x * 10) else Error "neg") [| 1; 2; 3 |]
    ...
    let ``Result.traverseArray returns first Error`` () =
        let r =
            Result.traverseArray (fun x -> if x > 0 then Ok x else Error "neg") [| 1; -2; 3 |]
    ...
    let ``Result.unsafeGet throws for Error`` () =
        Assert.Throws<InvalidOperationException>(fun () -> Result.unsafeGet (Error "boom") |> ignore)
        |> ignore
