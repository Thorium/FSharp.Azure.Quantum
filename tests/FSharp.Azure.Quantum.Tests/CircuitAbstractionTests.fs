...
module CircuitAbstractionTests =
    ...
    let ``circuitToQaoaCircuit should preserve qubit count`` () =
        ...
        result
        |> Result.map (fun qaoaCircuit -> Assert.Equal(5, qaoaCircuit.NumQubits))
        |> Result.defaultWith (fun err -> Assert.True(false, $"Conversion failed: %s{err.Message}"))
    ...
    let ``circuitToQaoaCircuit should handle single qubit circuits`` () =
        ...
        result
        |> Result.map (fun qaoaCircuit -> Assert.Equal(1, qaoaCircuit.NumQubits))
        |> Result.defaultWith (fun err -> Assert.True(false, $"Conversion failed: %s{err.Message}"))
