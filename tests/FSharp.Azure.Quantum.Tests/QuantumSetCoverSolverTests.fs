...
module QuboEncodingTests =
    ...
    let ``toQubo optimal bitstring minimises energy`` () =
        ...
        | Ok qubo ->
            ...
            Assert.True(energyS1 < energyS2S3, $"S1 ({energyS1}) should beat S2+S3 ({energyS2S3})")
            Assert.True(energyS1 < energyAll, $"S1 ({energyS1}) should beat All ({energyAll})")
    ...
    let ``toQubo penalises uncovering solutions`` () =
        ...
        | Ok qubo ->
            ...
            Assert.True(energyValid < energyNone, $"Valid ({energyValid}) should beat empty ({energyNone})")
    ...
    let ``toQubo returns error for zero universe`` () =
        ...
        (toQubo problem)
        |> Result.map (fun _ -> Assert.Fail("Should fail with zero universe size"))
