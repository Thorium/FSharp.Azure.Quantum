...
module GroverSearchTests =
    ...
    let unwrapOracle (result: Result<Oracle.CompiledOracle, QuantumError>) : Oracle.CompiledOracle =
        result
        |> Result.defaultWith (fun err -> failwith $"Oracle creation failed: {err.Message}")

    /// Helper: Unwrap Result<int, QuantumError> for testing
    let unwrapInt (result: Result<int, QuantumError>) : int =
        result
        |> Result.defaultWith (fun err -> failwith $"Operation failed: {err.Message}")
    ...
    let ``SearchSingle finds different targets`` () =
        ...
        let reproducibleConfig =
            { Grover.defaultConfig with
                RandomSeed = Some 42
            }

        for target in [ 0; 3; 5; 7 ] do
            (Grover.searchSingle target numQubits (createBackend ()) reproducibleConfig)
            |> Result.map (fun result -> Assert.Contains(target, result.Solutions))
            |> Result.defaultWith (fun err -> Assert.True(false, $"Search for {target} failed: {err}"))
    ...
    let ``SearchSingle with reproducible seed gives consistent results`` () =
        ...
        | Ok result1 ->
            ...
            | Error err -> Assert.True(false, $"Second search failed: {err}")
        | Error err -> Assert.True(false, $"First search failed: {err}")
    ...
    let ``HighPrecisionConfig achieves higher success rate`` () =
        ...
        | Ok result ->
            ...
            Assert.Contains(target, result.Solutions)
        | Error err -> Assert.True(false, $"High precision search failed: {err}")
    ...
    type MockCloudBackend() =
        ...
        let incrementalError: Result<QuantumState, QuantumError> =
            Error(
                QuantumError.OperationError(
                    "ApplyOperation",
                    "MockCloud does not support incremental ApplyOperation. Use ExecuteToState with a complete circuit instead."
                )
            )
        ...
        interface IQuantumBackend with
            ...
            member _.ApplyOperationAsync _op _state _ct = task { return incrementalError }
    ...
    let ``Grover runs on a cloud-style backend via whole-circuit submission`` () =
        ...
        | Ok result ->
            ...
            Assert.True(
                mockCloud.ExecuteToStateCalls > 0,
                "Grover should submit a complete circuit via ExecuteToState on a cloud-style backend"
            )
        | Error err -> Assert.True(false, $"Cloud-style Grover search failed: {err}")
