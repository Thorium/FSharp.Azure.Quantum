...
module QuantumArithmeticTests =
    ...
    let ``Doubly controlled addition restores ancilla to |0⟩`` () =
        ...
        Assert.True(ancillaProb0 > 0.999, $"Ancilla should be restored to |0⟩, but P(0) = {ancillaProb0}")
    ...
    let ``Doubly controlled subtraction restores ancilla to |0⟩`` () =
        ...
        let ancillaProb0 = measureQubitProbability ancillaQubit finalState
        Assert.True(ancillaProb0 > 0.999, $"Ancilla should be restored to |0⟩ after subtraction, P(0) = {ancillaProb0}")
    ...
    let ``Doubly controlled operations only apply when both controls are |1⟩`` () =
        ...
        Assert.True(hasUnchangedRegister, "Register should be unchanged when only one control is |1⟩")
    ...
    let ``In-place modular multiplication restores temp qubits to |0⟩`` () =
        ...
        for i, tempQubit in List.indexed tempQubits do
            ...
            Assert.True(prob0 > 0.95, $"Temp qubit {i} (qubit #{tempQubit}) should be |0⟩, but P(0) = {prob0}")

        // ✅ Ancilla IS properly restored
        let ancillaProb0 = measureQubitProbability ancillaQubit finalState
        Assert.True(ancillaProb0 > 0.95, $"Ancilla should be |0⟩, but P(0) = {ancillaProb0}")
    ...
    let ``In-place modular multiplication works for basic example`` () =
        ...
        Assert.True(hasCorrectResult, $"Expected {inputValue} × {constant} mod {modulus} = {expectedResult}")
    ...
    let ``U3 gate applies correct universal rotation`` () =
        ...
        let circuit = empty numQubits |> addGate (U3(qubit, theta, phi, lambda))

        let finalState = executeCircuit circuit

        // Should be in |1⟩ state with high probability
        let prob0 = measureQubitProbability qubit finalState
        Assert.True(prob0 < 0.01, $"U3(π, 0, π) should flip |0⟩ to |1⟩, but P(0) = {prob0}")
    ...
    let ``U3 gate decomposes correctly via RZ-RY-RZ`` () =
        ...
        let circuit1 = empty numQubits |> addGate (U3(qubit, theta, phi, lambda))
        ...
        Assert.True(difference < 0.001, $"U3 and RZ-RY-RZ decomposition should match. Difference = {difference}")
    ...
    let ``U3 gate with zero parameters is identity`` () =
        ...
        let circuit = empty numQubits |> addGate (U3(qubit, 0.0, 0.0, 0.0))

        let finalState = executeCircuit circuit

        // Should remain in |0⟩ state
        let prob0 = measureQubitProbability qubit finalState
        Assert.True(prob0 > 0.999, $"U3(0,0,0) should be identity, but P(0) = {prob0}")
    ...
    let ``U3 gate can create superposition`` () =
        ...
        let circuit = empty numQubits |> addGate (U3(qubit, theta, 0.0, 0.0))
        ...
        Assert.True(difference < 0.01, $"U3(π/2, 0, 0) should create 50/50 superposition, but P(0) = {prob0}")
