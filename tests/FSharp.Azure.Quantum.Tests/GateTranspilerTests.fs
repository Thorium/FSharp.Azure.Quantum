...
module GateTranspilerTests =
    ...
    let ``CZ decomposition is equivalent to CZ gate`` () =
        ...
        let state11 =
            initialState |> LocalSimulator.Gates.applyX 0 |> LocalSimulator.Gates.applyX 1
    ...
    let ``CRX decomposition is equivalent to CRX gate`` () =
        ...
        let initialState =
            LocalSimulator.StateVector.init 2 |> LocalSimulator.Gates.applyH 0
        ...
        let resultDecomposed =
            ...
            |> List.fold
                (fun s g ->
                    ...
                    | g -> failwith $"Unexpected gate in CRX decomposition: {g}")
                initialState
