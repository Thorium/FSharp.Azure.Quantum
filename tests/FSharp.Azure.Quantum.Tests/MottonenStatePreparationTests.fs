...
module MottonenStatePreparationTests =
    ...
    let ``normalizeState with zero vector throws`` () =
        ...
        Assert.Throws<Exception>(fun () -> MottonenStatePreparation.normalizeState amplitudes |> ignore)
        |> ignore
    ...
    let ``prepareStateFromAmplitudes creates circuit for 1-qubit state`` () =
        ...
        let circuit =
            MottonenStatePreparation.prepareStateFromAmplitudes amplitudes [| 0 |] emptyCircuit
    ...
    let ``prepareStateFromAmplitudes creates circuit for 2-qubit state`` () =
        ...
        let circuit =
            MottonenStatePreparation.prepareStateFromAmplitudes amplitudes [| 0; 1 |] emptyCircuit
    ...
    let ``prepareStateFromRealAmplitudes creates valid circuit`` () =
        ...
        let circuit =
            MottonenStatePreparation.prepareStateFromRealAmplitudes amplitudes [| 0; 1 |] emptyCircuit
    ...
    let ``prepareStateFromRealAmplitudes with basis state`` () =
        ...
        let circuit =
            MottonenStatePreparation.prepareStateFromRealAmplitudes amplitudes [| 0 |] emptyCircuit
    ...
    let private assertPreparesState (amplitudes: Complex[]) =
        ...
        | Error err -> failwith $"ExecuteToState failed: %A{err}"
        | Ok(QuantumState.StateVector sv) ->
            let actual =
                Array.init (StateVector.dimension sv) (fun i -> StateVector.getAmplitude i sv)

            Assert.Equal(target.Length, actual.Length)

            // Align on the largest target amplitude to factor out global phase
            let refIdx =
                target |> Array.mapi (fun i a -> i, a.Magnitude) |> Array.maxBy snd |> fst
        ...
        | Ok _ -> failwith $"Expected StateVector result, calling assertPreparesState with amplitudes: {amplitudes}"
