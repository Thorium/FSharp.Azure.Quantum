...
let altSyntaxFciDump =
    """$FCI NORB=8, NELEC=6, MS2=0,
...
let withNirrepFciDump =
    """&FCI NORB=10, NELEC=8, MS2=0, NIRREP=4,
...
let ``parseHeader extracts NORB and NELEC from H2`` () =
    ...
    | Error e -> Assert.True(false, $"Parse failed: {e}")
...
let ``parseHeader extracts MS2 correctly`` () =
    ...
    result
    |> Result.map (fun header -> Assert.Equal(Some 0, header.MS2))
    |> Result.defaultWith (fun e -> Assert.True(false, $"Parse failed: {e}"))
...
let ``parseHeader extracts NIRREP when present`` () =
    ...
    result
    |> Result.map (fun header -> Assert.Equal(Some 4, header.NumIrrep))
    |> Result.defaultWith (fun e -> Assert.True(false, $"Parse failed: {e}"))
...
"""
    ...
    result
    |> Result.map (fun _ -> Assert.True(false, "Should have failed"))
    |> Result.defaultWith (fun msg -> Assert.Contains("header", msg.ToLower()))
...
"""
    ...
    result
    |> Result.map (fun _ -> Assert.True(false, "Should have failed"))
    |> Result.defaultWith (fun msg -> Assert.Contains("NORB", msg))
...
"""
    ...
    result
    |> Result.map (fun _ -> Assert.True(false, "Should have failed"))
    |> Result.defaultWith (fun msg -> Assert.Contains("NELEC", msg))
