...
let ``SmilesListDatasetProvider returns error for unknown SMILES`` () =
    ...
    (provider.Load(ByName "NOT_IN_LIST"))
    |> Result.iter (fun _ -> Assert.Fail("Expected Error for unknown SMILES")) // Expected
...
let ``CompositeDatasetProvider tries providers in order`` () =
    ...
    let composite =
        CompositeDatasetProvider([ empty; withData ]) :> IMoleculeDatasetProvider

    (composite.Load All)
    |> Result.map (fun dataset -> Assert.True(dataset.Molecules.Length >= 1))
    |> Result.defaultWith (fun e -> Assert.Fail($"Expected Ok but got Error: {e}"))
...
let ``CompositeDatasetProvider combines with built-in provider`` () =
    ...
    let composite =
        CompositeDatasetProvider([ builtIn; custom ]) :> IMoleculeDatasetProvider

    // Should find H2 from built-in
    (composite.Load(ByName "H2"))
    |> Result.map (fun dataset -> Assert.Equal(1, dataset.Molecules.Length))
    |> Result.defaultWith (fun e -> Assert.Fail($"Expected Ok but got Error: {e}"))
...
let ``CompositeDatasetProvider ListNames combines all providers`` () =
    ...
    let provider2 =
        SmilesListDatasetProvider([ "c1ccccc1" ]) :> IMoleculeDatasetProvider

    let composite =
        CompositeDatasetProvider([ provider1; provider2 ]) :> IMoleculeDatasetProvider
...
let ``SMILES provider molecules have valid topology`` () =
    let provider =
        SmilesListDatasetProvider([ "CCO"; "c1ccccc1" ]) :> IMoleculeDatasetProvider
