...
module QuantumStateTests =
    ...
    let ``StateVector - measure should return valid bitstrings`` () =
        ...
        Assert.All(measurements, fun bits -> Assert.All(bits, fun bit -> Assert.True(bit = 0 || bit = 1)))

        // Bell state should only give |00⟩ or |11⟩
        Assert.All(
            measurements,
            fun bits -> Assert.True((bits.[0] = 0 && bits.[1] = 0) || (bits.[0] = 1 && bits.[1] = 1))
        )
    ...
    let ``IsingSamples - measure should respect NumOccurrences weighting`` () =
        ...
        let counts = measurements |> Array.countBy id |> Map.ofArray
    ...
    let ``IsingSamples - measure should handle empty solutions`` () =
        ...
        Assert.All(measurements, fun bits -> Assert.All(bits, fun bit -> Assert.Equal(0, bit)))
    ...
    let ``IsingSamples - probability validation should reject mismatched bitstring length`` () =
        ...
        Assert.Throws<ArgumentException>(fun () -> QuantumState.probability [| 0; 0 |] state |> ignore // Too short
        )
        |> ignore

        Assert.Throws<ArgumentException>(fun () -> QuantumState.probability [| 0; 0; 0; 0 |] state |> ignore // Too long
        )
        |> ignore
    ...
    let ``All probabilities should sum to 1.0 for IsingSamples`` () =
        ...
        let allBitstrings =
            [| for i in 0..7 -> [| (i >>> 2) &&& 1; (i >>> 1) &&& 1; i &&& 1 |] |]

        let totalProb =
            allBitstrings |> Array.sumBy (fun bits -> QuantumState.probability bits state)
