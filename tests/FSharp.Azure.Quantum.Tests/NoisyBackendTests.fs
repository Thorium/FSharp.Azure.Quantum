...
module NoisyBackendTests =
    ...
    let private sampleWith (config: NoiseConfig) (shots: int) : Map<string, int> =
        ...
        (Primitives.sample backend (bell ()) shots)
        |> Result.defaultWith (fun e -> failwith $"sample failed: {e.Message}")
    ...
    let ``more noise leaks more probability`` () =
        let leakOf (p: float) =
            ...
            (h |> Map.tryFind "01" |> Option.defaultValue 0)
            + (h |> Map.tryFind "10" |> Option.defaultValue 0)
    ...
    let ``observe works on the density-matrix backend and decays under noise`` () =
        ...
        | Ok clean, Ok degraded ->
            ...
            Assert.True(
                degraded < 0.95 && degraded > 0.0,
                $"noisy ⟨Z₀Z₁⟩ should be reduced but positive, got {degraded}"
            )
    ...
    let ``a gate on an out-of-range qubit returns Error, not an exception`` () =
        ...
        (simulate noiseless bad)
        |> Result.iter (fun _ -> failwith "expected an Error for a gate referencing a qubit outside the register")
