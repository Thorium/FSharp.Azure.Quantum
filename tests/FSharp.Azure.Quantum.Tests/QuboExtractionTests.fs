...
module QuboExtractionTests =
                    ...
                    [|
                        {
                            Coefficient = 1.0
                            QubitsIndices = [| 0; 1; 2 |]
                            PauliOperators = [| PauliZ; PauliZ; PauliZ |]
                        }
                    |]
            }

        // Should throw exception for 3-qubit term
        Assert.Throws<System.Exception>(fun () -> fromProblemHamiltonian hamiltonian |> ignore)

    [<Fact>]
    let ``fromProblemHamiltonian handles empty Hamiltonian`` () =
        let hamiltonian: ProblemHamiltonian = { NumQubits = 2; Terms = [||] }
    ...
    let ``validateSymmetric rejects asymmetric QUBO`` () =
        ...
        |> Result.defaultWith (fun msg -> Assert.Contains("not symmetric", msg.Message))

    [<Fact>]
    let ``validateSymmetric handles diagonal-only QUBO`` () =
        let qubo = Map.ofList [ ((0, 0), 2.0); ((1, 1), 3.0); ((2, 2), 1.0) ]
    ...
    let ``getNumVariables returns correct count`` () =
        let qubo = Map.ofList [ ((0, 0), 2.0); ((0, 2), -5.0); ((1, 1), 3.0) ]
    ...
    let ``toDenseArray creates correct matrix`` () =
        let qubo = Map.ofList [ ((0, 0), 2.0); ((0, 1), -5.0); ((1, 1), 3.0) ]
    ...
    let ``fromDenseArray skips near-zero terms`` () =
        let dense =
            Array2D.init 2 2 (fun i j -> if i = 0 && j = 0 then 2.0 else 1e-12 // Very small, should be skipped
            )
    ...
    let ``toDenseArray and fromDenseArray are inverses`` () =
        let originalQubo = Map.ofList [ ((0, 0), 2.0); ((0, 1), -5.0); ((1, 1), 3.0) ]
    ...
    let ``fromDenseArray rejects non-square matrix`` () =
        ...
        Assert.Throws<System.Exception>(fun () -> fromDenseArray nonSquare |> ignore)
