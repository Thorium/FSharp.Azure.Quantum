...
let createSimpleCities n =
    Array.init n (fun i -> (float i, 0.0))
...
let ``twoOptImprove should improve tour quality`` () =
    ...
    Assert.True(improvedLength <= badLength, 
        $"Not improved: bad={badLength}, improved={improvedLength}")
...
let ``solve with small TSP should find optimal solution`` () =
    ...
    Assert.True(abs(solution.TourLength - optimalLength) < 0.01,
...
let ``solve with 10 cities should complete in under 10ms`` () =
    ...
    Assert.True(solution.ElapsedMs < 10.0, 
        $"Too slow for 10 cities: {solution.ElapsedMs}ms")
...
let ``solve with 20 cities should complete in under 50ms`` () =
    ...
    Assert.True(solution.ElapsedMs < 50.0, 
        $"Too slow for 20 cities: {solution.ElapsedMs}ms")
...
let ``solve with 50 cities should complete in under 1 second`` () =
    ...
    Assert.True(solution.ElapsedMs < 1000.0, 
        $"Too slow for 50 cities: {solution.ElapsedMs}ms")

[<Fact>]
let ``solveWithDistances should work with custom distance matrix`` () =
    // Asymmetric TSP with custom distances
    let n = 4
    let distances = Array2D.init n n (fun i j ->
        if i = j then 0.0
        else float ((i + 1) * (j + 1))  // Custom distance formula
    )
...
let ``solve with circle cities should find near-optimal tour`` () =
    ...
    Assert.True(solution.TourLength <= maxAcceptable,
...
let ``solve without nearest neighbor should still work`` () =
    let cities = createCircleCities 10
    let config = { defaultConfig with UseNearestNeighbor = false }
    let solution = solve cities config
