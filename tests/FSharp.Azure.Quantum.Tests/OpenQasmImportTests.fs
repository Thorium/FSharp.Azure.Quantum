...
"""
        ...
        | Ok circuit ->
            match (getGates circuit).[0] with
            | RX(0, angle) -> Assert.InRange(angle, 1.5707, 1.5708) // π/2
...
"""
        ...
        | Ok circuit ->
            match (getGates circuit).[0] with
            | RY(0, angle) -> Assert.InRange(angle, 3.1415, 3.1416) // π
...
"""
        ...
        | Ok circuit ->
            match (getGates circuit).[0] with
            | RZ(0, angle) -> Assert.InRange(angle, 0.7853, 0.7854) // π/4
...
"""
        ...
        result
        |> Result.map (fun circuit -> Assert.Equal(2, circuit.Gates.Length))
        |> Result.defaultWith (fun msg -> Assert.True(false, $"Parse failed: {msg}"))
...
"""
    ...
    [<Fact>]
    let ``parse index too large for an int should fail, not throw`` () =
        // Found by the property suite: Int32.Parse on the digit run threw an
        // OverflowException out of parse instead of returning an Error.
        for line in
            [
                "qreg q[99999999999];"
                "qreg q[2];\nx q[99999999999];"
                "qreg q[2];\ncreg c[2];\nif (c[99999999999] == 1) { x q[0]; }"
            ] do
            ...
            | Ok _ -> Assert.True(false, $"Should fail on {line}")
