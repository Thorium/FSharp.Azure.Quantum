...
module HHLUnifiedTests =
        ...
        let config: HHLConfig =
            {
                ...
                InputVector =
                    {
                        Dimension = 2
                        Components = [| Complex.One; Complex.Zero |]
                    }
                EigenvalueQubits = 3
                SolutionQubits = 1
    ...
    type private RigettiGateOnlyBackend(inner: IQuantumBackend) =
        interface IQuantumBackend with
            ...
            member _.SupportsOperation operation =
                ...
                | QuantumOperation.Sequence _ -> false
    ...
    let ``HHL planner produces explicit lowered ops when intent op unsupported`` () =
        let backend =
            (LocalBackend() :> IQuantumBackend) |> NoHhlIntentBackend :> IQuantumBackend
    ...
    let ``HHL planner lowers full general-matrix path to gates only`` () =
        ...
        | Ok matrix, Ok vector ->
            ...
            | Ok baseConfig ->
                ...
                | Ok(HHL.HhlPlan.ExecuteNatively _, _, _, _) -> Assert.Fail("Expected general-matrix to lower to ops")
                | Error err -> Assert.Fail($"Planning failed: {err}")

    [<Fact>]
    let ``HHL planner transpiles general-matrix lowering to Rigetti gate set`` () =
        let backend =
            (LocalBackend() :> IQuantumBackend) |> RigettiGateOnlyBackend :> IQuantumBackend
        ...
        | Ok matrix, Ok vector ->
            ...
            | Ok baseConfig ->
                ...
                | Ok(HHL.HhlPlan.ExecuteViaOps(ops, _), _, _, _) ->
                    ...
                    Assert.True(ops |> List.forall backend.SupportsOperation)
                | Ok(HHL.HhlPlan.ExecuteNatively _, _, _, _) -> Assert.Fail("Expected Rigetti to use explicit lowering")
                | Error err -> Assert.Fail($"Planning failed: {err}")
    ...
    let ``createDiagonalMatrix validates power of 2 dimensions`` () =
        // Valid: power of 2
        (createDiagonalMatrix [| 1.0; 2.0 |])
        |> Result.map (fun _ -> ())
        |> Result.defaultWith (fun err -> Assert.Fail($"Should accept 2 eigenvalues: {err}"))

        (createDiagonalMatrix [| 1.0; 2.0; 3.0; 4.0 |])
        |> Result.map (fun _ -> ())
        |> Result.defaultWith (fun err -> Assert.Fail($"Should accept 4 eigenvalues: {err}"))
    ...
    let ``createQuantumVector validates power of 2 dimensions`` () =
        // Valid: power of 2
        (createQuantumVector [| Complex(1.0, 0.0); Complex(0.0, 0.0) |])
        |> Result.map (fun _ -> ())
        |> Result.defaultWith (fun err -> Assert.Fail($"Should accept 2 components: {err}"))
    ...
    let ``solve4x4Diagonal validates array lengths`` () =
        ...
        let goodInputVector =
            [| Complex(1.0, 0.0); Complex.Zero; Complex.Zero; Complex.Zero |]
    ...
    let ``solveIdentity handles 4x4 identity`` () =
        ...
        let inputVector =
            [| Complex(1.0, 0.0); Complex(2.0, 0.0); Complex.Zero; Complex.Zero |]
        ...
        | Error(QuantumError.NotImplemented _) -> Assert.Fail("Unexpected error: Not implemented?")
        | Error err -> Assert.Fail($"Unexpected error: {err}")
    ...
    let ``createQuantumVector produces normalized vector`` () =
        ...
        | Ok vector ->
            ...
            let norm =
                vector.Components |> Array.sumBy (fun c -> c.Magnitude * c.Magnitude) |> sqrt
    ...
    let ``HHL result contains solution amplitudes when available`` () =
        ...
        | Ok result ->
            ...
            | Some amplitudes ->
                ...
                amplitudes
                |> Map.iter (fun idx amp -> Assert.True(amp.Magnitude > 0.0, $"Amplitude at {idx} should be non-zero"))
            | None ->
    ...
    let ``HHL result contains estimated eigenvalues`` () =
        ...
        | Error(QuantumError.OperationError _) -> () // Acceptable
        | Error err -> Assert.Fail($"Unexpected error: {err}")
    ...
    let ``HHL calculates success probability`` () =
        ...
        | Ok result ->
            ...
            Assert.True(
                result.SuccessProbability >= 0.0 && result.SuccessProbability <= 1.0,
    ...
    let ``HHL reports gate count`` () =
        ...
        | Ok result ->
            ...
            let expectedMinGates =
                result.Config.EigenvalueQubits * result.Config.EigenvalueQubits

            Assert.True(
                result.GateCount >= expectedMinGates,
