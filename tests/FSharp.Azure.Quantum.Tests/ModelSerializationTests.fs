...
module ModelSerializationTests =
    ...
    let ``Save VQC model creates JSON file`` () =
        task {
            ...
            try
                ...
                match result with
                | Ok() -> Assert.True(File.Exists testFile, "JSON file should be created")
    ...
    let ``Load nonexistent file returns error`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.Fail("Should return error for nonexistent file"))
        |> Result.defaultWith (fun msg -> Assert.Contains("not found", msg.Message.ToLower()))
    ...
    let ``Save VQC training result extracts final loss from history`` () =
        ...
        try
            ...
            match
                ModelSerialization.saveVQCTrainingResult testFile result 2 "ZZFeatureMap" 2 "RealAmplitudes" 1 None
            with
            | Error e -> Assert.Fail($"Save failed: {e}")
            | Ok() ->
    ...
    let ``Save VQC training result handles empty loss history`` () =
        ...
        try
            ...
            | Ok() ->
                ...
                (ModelSerialization.loadVQCModel testFile)
                |> Result.map (fun model -> Assert.Equal(0.0, model.FinalLoss))
                |> Result.defaultWith (fun e -> Assert.Fail($"Load failed: {e}"))
        finally
            cleanupTestFile testFile
        ...
        task {
            ...
            try
                ...
                | Ok() ->
                    ...
                    | Error e -> Assert.Fail($"Load failed: {e}")
                    | Ok loadedParams -> Assert.Equal<float seq>(parameters, loadedParams)
            finally
                cleanupTestFile testFile
        }
        :> Task
        ...
        task {
            ...
            try
                ...
                | Ok() ->
                    ...
                    | Ok() -> Assert.True(true, "Print should succeed")
            finally
                cleanupTestFile testFile
        }
        :> Task
    ...
    let ``Save VQC model batch creates multiple files`` () =
        ...
        let testFiles =
            [| $"{testBase}_1.json"; $"{testBase}_2.json"; $"{testBase}_3.json" |]
    ...
    let ``Load VQC model batch loads all matching files`` () =
        ...
        try
            ...
            let models = [| ([| 0.1 |], 0.9, None); ([| 0.2 |], 0.8, None) |]

            (ModelSerialization.saveVQCModelBatch testBase models 1 "ZZFeatureMap" 1 "RealAmplitudes" 1)
            |> Result.map (fun _ ->
                (ModelSerialization.loadVQCModelBatch testDir "model_*.json")
                |> Result.map (fun loadedModels -> Assert.Equal(2, loadedModels.Length))
                |> Result.defaultWith (fun e -> Assert.Fail($"Batch load failed: {e}")))
            |> Result.defaultWith (fun e -> Assert.Fail($"Batch save failed: {e}"))
        finally
            if Directory.Exists testDir then
                Directory.Delete(testDir, true)
    ...
    let ``Load VQC model batch from nonexistent directory returns error`` () =
        ...
        result
        |> Result.map (fun _ -> Assert.Fail("Should return error for nonexistent directory"))
        |> Result.defaultWith (fun msg -> Assert.Contains("not found", msg.Message.ToLower()))
    ...
    let ``Load VQC model batch with no matching files returns error`` () =
        ...
        try
            ...
            result
            |> Result.map (fun _ -> Assert.Fail("Should return error when no files match"))
            |> Result.defaultWith (fun msg -> Assert.Contains("no files", msg.Message.ToLower()))
        finally
            if Directory.Exists testDir then
                Directory.Delete(testDir, true)
        ...
        task {
            ...
            try
                ...
                | Ok() ->
                    ...
                    match ModelSerialization.loadVQCModel testFile with
                    | Error e -> Assert.Fail($"Load failed: {e}")
                    | Ok model -> Assert.True(model.Note.IsNone)
            finally
                cleanupTestFile testFile
        }
        :> Task
    ...
    let ``Load multi-class from nonexistent file returns error`` () =
        let result =
            ModelSerialization.loadVQCMultiClassModel "nonexistent_multiclass_12345.json"
    ...
    let ``Multi-class model preserves metadata timestamp`` () =
        ...
        try
            ...
            | Ok() ->
                ...
                (ModelSerialization.loadVQCMultiClassModel testFile)
    ...
    let ``Portfolio solution with QUBO matrix roundtrip`` () =
        task {
            ...
            try
                ...
                let quboMatrix = Map.ofList [ ((0, 0), -0.5); ((1, 1), -0.3); ((0, 1), 0.2) ]
    ...
    let ``Load nonexistent portfolio file returns error`` () =
        let result =
            ModelSerialization.loadPortfolioSolution "nonexistent_portfolio_12345.json"
