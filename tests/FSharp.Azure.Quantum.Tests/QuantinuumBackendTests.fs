...
module QuantinuumBackendTests =
    ...
    let ``parseQuantinuumResult - parses histogram from Quantinuum result JSON`` () =
        ...
        // Act
        let histogram =
            (parseQuantinuumResult resultJson)
            |> Result.defaultWith (fun msg -> failwith $"Expected successful parse, got error: {msg}")
    ...
    let ``mapQuantinuumError - InvalidCircuit maps to QuantumError InvalidCircuit`` () =
        ...
        | QuantumError.ValidationError(field, reason) -> Assert.Contains(errorMessage, reason)
    ...
    let ``mapQuantinuumError - TooManyQubits maps to InvalidCircuit`` () =
        ...
        // Assert
        match quantumError with
        | QuantumError.ValidationError(field, reason) -> Assert.Contains("too large", reason)
        | _ -> Assert.True(false, "Expected ValidationError")
    ...
    let ``mapQuantinuumError - QuotaExceeded maps to QuantumError QuotaExceeded`` () =
        ...
        // Assert
        match quantumError with
        | QuantumError.AzureError(AzureQuantumError.QuotaExceeded msg) -> Assert.Equal("Insufficient HQC credits", msg)
