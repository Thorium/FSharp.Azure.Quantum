...
module GraphOptimizationTests =
    ...
    let ``edge with properties stores metadata`` () =
        let e =
            { edge "A" "B" 10.0 with
                Properties = Map.ofList [ "road", box "highway" ]
            }
    ...
    let ``Builder with nodes sets graph nodes`` () =
        let nodes = [ node "A" 1; node "B" 2 ]
        let problem = GraphOptimizationBuilder<int, unit>().Nodes(nodes).Build()
    ...
    let ``Builder with edges sets graph edges`` () =
        ...
        let problem =
            GraphOptimizationBuilder<int, unit>().Nodes(nodes).Edges(edges).Build()
    ...
    let ``NoAdjacentEqual constraint adds penalty terms`` () =
        ...
        let hasEdgePenalty =
            qubo.Q |> Map.exists (fun (i, j) coeff -> i <> j && coeff > 0.0) // Quadratic penalty terms should be positive

        Assert.True(hasEdgePenalty, "QUBO should contain quadratic penalty terms for adjacent node pairs")
    ...
    let ``TSP encodes to QUBO with one-hot time variables`` () =
        ...
        let edges = [ edge "A" "B" 10.0; edge "B" "C" 15.0; edge "A" "C" 20.0 ]
    ...
    let ``MaxCut encodes to QUBO with binary partition variables`` () =
        ...
        let problem =
            GraphOptimizationBuilder<int, unit>().Nodes(nodes).Edges(edges).Objective(MaximizeCut).Build()
    ...
    let ``solution contains objective value`` () =
        let problem =
            GraphOptimizationBuilder<int, unit>().Nodes([ node "A" 1 ]).Objective(MinimizeColors).Build()
    ...
    let ``calculateObjectiveValue - graph coloring counts colors used`` () =
        let nodes = [ node "A" "Node A"; node "B" "Node B"; node "C" "Node C" ]
        let edges = [ edge "A" "B" 1.0; edge "B" "C" 1.0 ]
    ...
    let ``calculateObjectiveValue - TSP sums edge weights in tour`` () =
        ...
        let problem =
            GraphOptimizationBuilder<string, float>().Nodes(cities).Edges(routes).Objective(MinimizeTotalWeight).Build()

        // Solution: Tour A->B->C->A
        let edgeAB =
            problem.Graph.Edges |> List.find (fun e -> e.Source = "A" && e.Target = "B")

        let edgeBC =
            problem.Graph.Edges |> List.find (fun e -> e.Source = "B" && e.Target = "C")

        let edgeCA =
            problem.Graph.Edges |> List.find (fun e -> e.Source = "C" && e.Target = "A")
    ...
    let ``validateConstraints - DegreeLimit enforces maximum degree`` () =
        ...
        let edges =
            [ edge "A" "B" 1.0; edge "A" "C" 1.0; edge "A" "D" 1.0; edge "B" "C" 1.0 ]
    ...
    let ``decodeSolution calculates objective value for graph coloring`` () =
        ...
        let quboSolution =
            [
                ...
                0
                0
                0 // Node A (index 0): color 0
                0
                1
                0
                0 // Node B (index 1): color 1
                1
                0
                0
                0 // Node C (index 2): color 0
            ]
    ...
    let ``Graph with single node should handle gracefully`` () =
        ...
        let problem =
            GraphOptimizationBuilder<int, unit>().Nodes(nodes).Objective(MinimizeColors).Build()
    ...
    let ``Large graph coloring (10 nodes) should encode efficiently`` () =
        ...
        let edges = List.init 9 (fun i -> edge $"N{i}" $"N{i + 1}" 1.0) // Linear chain
