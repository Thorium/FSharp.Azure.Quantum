...
module OpenQasmVersionTests =
    ...
    let ``detectVersion returns error for unsupported version`` () =
        ...
        |> Result.defaultWith (fun msg -> Assert.Contains("4.0", msg))
...
"""
    ...
    let ``V3_0 round-trip preserves measurement`` () =
        let original =
            {
                QubitCount = 2
                Gates = [ H 0; CNOT(0, 1); Measure 0; Measure 1 ]
            }
    ...
    let ``V3_0 round-trip preserves multi-qubit circuit`` () =
        let original =
            {
                QubitCount = 3
                Gates = [ H 0; CNOT(0, 1); SWAP(1, 2); CZ(0, 2); CCX(0, 1, 2) ]
            }
...
"""
        ...
        result
        |> Result.map (fun _ -> failwith "Expected error")
        |> Result.defaultWith (fun msg -> Assert.Contains("mismatch", msg.ToLowerInvariant()))
...
"""
        ...
        result
        |> Result.map (fun _ -> failwith "Expected error")
        |> Result.defaultWith (fun msg -> Assert.Contains("Duplicate", msg))
