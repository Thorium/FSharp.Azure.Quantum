...
module VisualizationTests =
    ...
    let ``Mermaid sequence renderer should generate valid mermaid`` () =
        // Arrange
        let gates =
            [ CircuitGate(CircuitBuilder.H 0); CircuitGate(CircuitBuilder.CNOT(0, 1)) ]
    ...
    let ``Mermaid flowchart renderer should generate valid mermaid`` () =
        // Arrange
        let gates =
            [ CircuitGate(CircuitBuilder.H 0); CircuitGate(CircuitBuilder.Measure 0) ]
    ...
    let ``Mermaid graph renderer should generate colored graph`` () =
        ...
        let edges = [ MermaidRenderer.Graph.edge "A" "B" ]
