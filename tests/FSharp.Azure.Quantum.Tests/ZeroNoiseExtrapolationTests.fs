...
module ZeroNoiseExtrapolationTests =
    ...
    let ``applyNoiseScaling IdentityInsertion should increase circuit depth`` () =
        ...
        Assert.True(
            ...
            sprintf
                "Expected noisy circuit (%d gates) to have more gates than original (%d gates)"
                noisyDepth
                originalDepth
        )
    ...
    let ``applyNoiseScaling PulseStretching amplifies depth without changing qubits or the ideal result`` () =
        ...
        Assert.True(
            CircuitBuilder.gateCount noisyCircuit > CircuitBuilder.gateCount circuit,
    ...
    let ``fitPolynomial should handle linear fit`` () =
        ...
        Assert.True(abs (coefficients.[0] - 1.0) < 0.1, sprintf "Expected a₀ ≈ 1.0, got %f" coefficients.[0])
        ...
        async {
            ...
            let mockExecutor (noisyCircuit: CircuitBuilder.Circuit) =
                async {
                    ...
                    let expectation =
                        0.95 - 0.15 * noiseLevelEstimate + 0.05 * (noiseLevelEstimate ** 2.0)
        ...
        async {
            ...
            | Ok zneResult ->
                ...
                Assert.True(
                    ...
                    sprintf
                        "Expected error reduction: zero-noise (%f) > baseline (%f)"
                        zneResult.ZeroNoiseValue
                        baseline
                )
        ...
        Assert.All(
            ...
            fun scaling ->
                ...
                | ZeroNoiseExtrapolation.NoiseScaling.PulseStretching _ ->
                    Assert.Fail("Expected IdentityInsertion for IonQ")
        )
    ...
    let ``defaultConfig should provide sensible Rigetti defaults`` () =
        ...
        Assert.All(
            ...
            fun scaling ->
                ...
                | ZeroNoiseExtrapolation.NoiseScaling.PulseStretching _ -> ()
                | ZeroNoiseExtrapolation.NoiseScaling.IdentityInsertion _ ->
                    Assert.Fail("Expected PulseStretching for Rigetti")
        )
    ...
    let ``fitPolynomial should fail with insufficient data points`` () =
        ...
        Assert.Throws<exn>(fun () -> ZeroNoiseExtrapolation.fitPolynomial degree insufficientData |> ignore)

    [<Fact>]
    let ``applyNoiseScaling with zero rate should return original circuit`` () =
        // Arrange: Circuit with zero insertion rate
        let circuit = CircuitBuilder.empty 2 |> CircuitBuilder.addGate (CircuitBuilder.H 0)
    ...
    let ``Benchmark ZNE should demonstrate 30-50 percent error reduction`` () =
        async {
            ...
            | Ok zneResult ->
                ...
                Assert.True(
                    errorReduction >= 30.0 && errorReduction <= 55.0,
                    sprintf
                        "Expected 30-55%% error reduction, got %.1f%% (baseline error: %.3f → ZNE error: %.3f)"
                        errorReduction
                        baselineError
                        zneError
                )
                ...
                printfn
                    "✓ Benchmark: %.1f%% error reduction (baseline: %.3f → ZNE: %.3f)"
                    errorReduction
                    baselineError
                    zneError
