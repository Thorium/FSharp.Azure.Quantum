...
module ProbabilisticErrorCancellationTests =
    ...
    let ``QuasiProbDecomposition should hold terms with negative probabilities`` () =
        ...
        let hasNegativeProbability =
            decomposition.Terms |> List.exists (fun (_, prob) -> prob < 0.0)
    ...
    let ``PECResult should track corrected expectation and overhead`` () =
        ...
        Assert.True(result.CorrectedExpectation > result.UncorrectedExpectation, "PEC should improve expectation value")
    ...
    let ``decomposeSingleQubitGate should produce 4-term decomposition`` () =
        ...
        let decomposition =
            ProbabilisticErrorCancellation.decomposeSingleQubitGate gate noiseModel
    ...
    let ``decomposeSingleQubitGate should have correct probability formula`` () =
        ...
        correctionProbs |> List.iter (fun prob -> Assert.Equal(expectedQP, prob, 10))
    ...
    let ``decomposeSingleQubitGate should have correct normalization`` () =
        ...
        let manualNorm = decomposition.Terms |> List.sumBy (fun (_, prob) -> abs prob)
    ...
    let ``decomposeSingleQubitGate should include Pauli corrections`` () =
        ...
        let correctionGates = decomposition.Terms |> List.skip 1 |> List.map fst
        ...
        correctionProbs
        |> List.iter (fun prob -> Assert.True(prob < 0.0, "Correction terms must have negative probabilities"))
    ...
    let ``decomposeSingleQubitGate should work with zero noise`` () =
        ...
        let correctionProbs = decomposition.Terms |> List.skip 1 |> List.map snd
        correctionProbs |> List.iter (fun prob -> Assert.Equal(0.0, prob, 10))
    ...
    let ``decomposeTwoQubitGate should produce 16-term decomposition`` () =
        ...
        let decomposition =
            ProbabilisticErrorCancellation.decomposeTwoQubitGate gate noiseModel
    ...
    let ``sampleQuasiProb should return gate and weight`` () =
        ...
        let (sampledGate, weight) =
            ProbabilisticErrorCancellation.sampleQuasiProb decomposition rng
    ...
    let ``sampleQuasiProb should sample according to absolute probabilities`` () =
        ...
        Assert.True(hGateRatio > 0.85 && hGateRatio < 0.95, $"Expected H gate ratio ~0.90, got %.3f{hGateRatio}")
    ...
    let ``sampleQuasiProb should handle all positive probabilities`` () =
        ...
        let decomposition: ProbabilisticErrorCancellation.QuasiProbDecomposition =
            {
                Terms = [ ([ CircuitBuilder.H 0 ], 0.6); ([ CircuitBuilder.X 0 ], 0.4) ]
                Normalization = 1.0
            }
    ...
    let ``sampleQuasiProb should be deterministic with same seed`` () =
        ...
        let decomposition: ProbabilisticErrorCancellation.QuasiProbDecomposition =
            {
                Terms = [ ([ CircuitBuilder.H 0 ], 1.0); ([ CircuitBuilder.X 0 ], -0.5) ]
                Normalization = 1.5
            }
    ...
    let ``mitigate should demonstrate error reduction`` () =
        async {
            ...
            | Ok pecResult ->
                ...
                Assert.True(pecResult.ErrorReduction >= 0.0, "Error reduction should be non-negative")

                printfn
                    "Baseline error: %.3f, PEC error: %.3f, Reduction: %.1f%%"
                    baselineError
                    pecError
                    ...
                    |> List.fold
                        (fun state gate ->
                            ...
                            let afterGate =
                                ...
                                | CircuitBuilder.Gate.U3(q, theta, phi, lambda) ->
                                    Gates.applyU3 q theta phi lambda state
                                ...
                                | CircuitBuilder.Gate.MCZ(controls, tgt) ->
                                    Gates.applyMultiControlledZ controls tgt state
                            ...
                            if rng.NextDouble() < noise then
                                ...
                                let qubit =
                                    ...
                                    | CircuitBuilder.Gate.Barrier qubits ->
                                        if List.isEmpty qubits then 0 else List.head qubits
    ...
    let ``Integration: PEC should mitigate single-qubit gate errors`` () =
        async {
            ...
            | Ok pecResult ->
                ...
                Assert.True(
                    abs pecResult.CorrectedExpectation > 0.01
                    || abs pecResult.UncorrectedExpectation > 0.01,
    ...
    let ``Integration: PEC should handle Pauli rotation gates`` () =
        async {
            ...
            | Ok pecResult ->
                Assert.True(
                    pecResult.CorrectedExpectation <> pecResult.UncorrectedExpectation,
    ...
    let ``Integration: PEC should work with multi-gate QAOA-like circuit`` () =
        async {
            ...
            | Ok pecResult ->
                ...
                Assert.True(
                    pecResult.CorrectedExpectation <> 0.0 || pecResult.UncorrectedExpectation <> 0.0,
