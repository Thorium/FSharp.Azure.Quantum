...
module AtomComputingBackendTests =
    ...
    let ``OpenQasmExport - CZ gate (native Atom Computing)`` () =
        // Arrange
        let circuit =
            CircuitBuilder.empty 2 |> CircuitBuilder.addGate (CircuitBuilder.CZ(0, 1))
    ...
    let ``mapAtomComputingError - maps TooManyQubits error`` () =
        ...
        // Assert
        match quantumError with
        | QuantumError.ValidationError(field, reason) -> Assert.Contains("Circuit too large", reason)
        | _ -> Assert.True(false, $"Expected ValidationError, got {quantumError}")
    ...
    let ``mapAtomComputingError - maps QuotaExceeded error`` () =
        ...
        // Assert
        match quantumError with
        | QuantumError.AzureError(AzureQuantumError.QuotaExceeded msg) -> Assert.Equal(errorMessage, msg)
