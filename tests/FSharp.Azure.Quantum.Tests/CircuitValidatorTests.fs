...
let ``IonQ backends should support standard gate set`` () =
    ...
    let expectedGates =
        Set.ofList [ "X"; "Y"; "Z"; "H"; "Rx"; "Ry"; "Rz"; "CNOT"; "SWAP" ]
    ...
    let circuit =
        {
            ...
            GateCount = 10
            Depth = None
            UsedGates = Set.ofList [ "H"; "CZ"; "Toffoli" ]
            TwoQubitGates = []
        }
...
let ``Full validation should pass for valid circuit`` () =
    ...
    match result with
    | Ok() -> Assert.True(true)
    | Error errors -> Assert.True(false, $"Expected validation to pass but got errors: %A{errors}")
...
let ``Format validation error should provide clear actionable message`` () =
    ...
    let gateError =
        UnsupportedGate("CZ", "IonQ Simulator", Set.ofList [ "H"; "CNOT"; "X" ])
