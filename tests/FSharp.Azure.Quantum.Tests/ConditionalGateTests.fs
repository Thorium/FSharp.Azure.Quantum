...
module ConditionalGateTests =
    ...
    let ``Conditional X fires when measurement yields 1`` () =
        ...
        // Result must be |11⟩.
        let circuit =
            empty 2 |> addGate (X 0) |> addGate (Measure 0) |> addGate (Conditional(0, X 1))

        let sv = execute circuit

        Assert.True(
            (StateVector.getAmplitude 3 sv).Magnitude > 0.999,
    ...
    let ``Conditional X stays idle when measurement yields 0`` () =
        ...
        let circuit = empty 2 |> addGate (Measure 0) |> addGate (Conditional(0, X 1))

        let sv = execute circuit
        Assert.True((StateVector.getAmplitude 0 sv).Magnitude > 0.999, "Expected |00⟩ when conditional X does not fire")
    ...
    let ``Conditional correction always yields |1> on target (deferred X)`` () =
        ...
        for _ in 1..20 do
            let circuit =
                empty 2 |> addGate (H 0) |> addGate (Measure 0) |> addGate (Conditional(0, X 1))

            let sv = execute circuit
            let p00 = (StateVector.getAmplitude 0 sv).Magnitude
            let p11 = (StateVector.getAmplitude 3 sv).Magnitude
            Assert.True(p00 > 0.999 || p11 > 0.999, $"Expected |00⟩ or |11⟩ (correlated), got p00={p00}, p11={p11}")

    [<Fact>]
    let ``Conditional before measurement is an execution error`` () =
        let circuit = empty 2 |> addGate (Conditional(0, X 1))

        let backend = LocalBackend.LocalBackend() :> IQuantumBackend

        (backend.ExecuteToState(CircuitWrapper(circuit) :> ICircuit))
        |> Result.iter (fun _ -> Assert.Fail("Expected error for conditional referencing unmeasured qubit"))
    ...
    let ``Conditional export to OpenQASM 2 fails with clear message`` () =
        ...
        let ex =
            Record.Exception(fun () -> OpenQasmExport.exportWithConfig (configFor V2_0) circuit |> ignore)
