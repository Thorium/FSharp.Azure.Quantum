...
module AnyonSpeciesTests =
    ...
    let ``SU2Level 2 is equivalent to Ising`` () =
        match
            AnyonSpecies.particles AnyonSpecies.AnyonType.Ising,
            AnyonSpecies.particles (AnyonSpecies.AnyonType.SU2Level 2)
        with
    ...
    let ``Ising total quantum dimension is 2`` () =
        ...
        // D = √4 = 2
        (AnyonSpecies.totalQuantumDimension AnyonSpecies.AnyonType.Ising)
        |> Result.map (fun d -> Assert.Equal(2.0, d, 10))
    ...
    let ``Total quantum dimension is always positive`` () =
        ...
        types
        |> List.iter (fun t ->
            (AnyonSpecies.totalQuantumDimension t)
            |> Result.map (fun d -> Assert.True(d > 0.0))
            |> Result.defaultWith (fun err -> Assert.Fail($"Expected Ok but got Error: {err.Message}")))
    ...
    let ``Vacuum is valid in all theories`` () =
        match
            AnyonSpecies.isValid AnyonSpecies.AnyonType.Ising AnyonSpecies.Particle.Vacuum,
            AnyonSpecies.isValid AnyonSpecies.AnyonType.Fibonacci AnyonSpecies.Particle.Vacuum,
            AnyonSpecies.isValid (AnyonSpecies.AnyonType.SU2Level 2) AnyonSpecies.Particle.Vacuum
        with
    ...
    let ``Sigma is valid only in Ising`` () =
        match
            AnyonSpecies.isValid AnyonSpecies.AnyonType.Ising AnyonSpecies.Particle.Sigma,
            AnyonSpecies.isValid AnyonSpecies.AnyonType.Fibonacci AnyonSpecies.Particle.Sigma
        with
        | Ok true, Ok false -> ()
    ...
    let ``Psi is valid only in Ising`` () =
        match
            AnyonSpecies.isValid AnyonSpecies.AnyonType.Ising AnyonSpecies.Particle.Psi,
            AnyonSpecies.isValid AnyonSpecies.AnyonType.Fibonacci AnyonSpecies.Particle.Psi
        with
        | Ok true, Ok false -> ()
    ...
    let ``Tau is valid only in Fibonacci`` () =
        match
            AnyonSpecies.isValid AnyonSpecies.AnyonType.Fibonacci AnyonSpecies.Particle.Tau,
            AnyonSpecies.isValid AnyonSpecies.AnyonType.Ising AnyonSpecies.Particle.Tau
        with
        | Ok true, Ok false -> ()
