...
module ModularDataTests =
    ...
    let ``Ising S-matrix is computed correctly`` () =
        ...
        | Error e -> Assert.Fail($"Failed to compute S-matrix: {e}")
    ...
    let ``Ising T-matrix is diagonal`` () =
        ...
        | Error e -> Assert.Fail($"Failed to compute T-matrix: {e}")
    ...
    let ``Ising S-matrix is unitary`` () =
        ...
        // Assert
        result
        |> Result.map (fun s -> Assert.True(ModularData.verifySMatrixUnitary s))
        |> Result.defaultWith (fun e -> Assert.Fail($"Failed: {e}"))
    ...
    let ``Ising modular data satisfies (ST)^3 = S^2`` () =
        ...
        | Ok s, Ok t -> Assert.True(ModularData.verifyModularSTRelation s t)
        | _ -> Assert.Fail("Failed to compute S or T matrix")
    ...
    let ``S-matrix first row gives normalized quantum dimensions`` () =
        ...
        | _ -> Assert.Fail("Failed to compute S-matrix or total dimension")
    ...
    let ``SU(2)_2 is same as Ising`` () =
        ...
        | _ -> Assert.Fail("Failed to compute S-matrices")
    ...
    let ``SU(2)_k S-matrix is computed for arbitrary k`` () =
        ...
        | Error err -> Assert.Fail($"Should compute S-matrix for SU(2)_4: {err.Message}")
    ...
    let ``SU(2)_k T-matrix is computed for arbitrary k`` () =
        ...
        | Error err -> Assert.Fail($"Should compute T-matrix for SU(2)_5: {err.Message}")
    ...
    let ``SU(2)_3 S-matrix matches known properties`` () =
        ...
        | Ok s ->
            ...
            [ 0..3 ]
            |> List.iter (fun i -> [ 0..3 ] |> List.iter (fun j -> Assert.Equal(s.[i, j].Real, s.[j, i].Real, 10)))

            // Verify unitarity
            Assert.True(ModularData.verifySMatrixUnitary s)
        | Error err -> Assert.Fail($"Failed to compute SU(2)_3 S-matrix: {err.Message}")
    ...
    let ``SU(2)_3 T-matrix has correct structure`` () =
        ...
        | Ok t ->
            ...
            |> List.iter (fun i ->
                let magnitude = Complex.Abs(t.[i, i])
                Assert.InRange(magnitude, 0.9999, 1.0001))
        | Error err -> Assert.Fail($"Failed to compute SU(2)_3 T-matrix: {err.Message}")
    ...
    let ``SU(2)_k S-matrix is symmetric and unitary`` () =
        ...
        | Ok s ->
            ...
            // Verify unitarity
            Assert.True(ModularData.verifySMatrixUnitary s)
        | Error err -> Assert.Fail($"Failed: {err.Message}")
