...
module BraidGroupTests =
    ...
    let ``Identity braid requires at least 2 strands`` () =
        ...
        (BraidGroup.identity 1)
        |> Result.map (fun _ -> failwith "Should have rejected 1-strand identity")
        |> Result.defaultWith (fun err -> Assert.Contains("2 strands", err.Message))
    ...
    let ``Composing two braids concatenates their generators`` () =
        ...
        let braid1 =
            braidFromGensOrFail 3 [ BraidGroup.sigma 0; BraidGroup.sigma 1 ] "First braid"
    ...
    let ``Composing braids with different strand counts fails`` () =
        ...
        (BraidGroup.compose braid3 braid4)
        |> Result.map (fun _ -> failwith "Should have rejected different strand counts")
        |> Result.defaultWith (fun err -> Assert.Contains("different strand counts", err.Message))
    ...
    let ``Composing with identity braid is a no-op`` () =
        ...
        let braid =
            braidFromGensOrFail 3 [ BraidGroup.sigma 0; BraidGroup.sigma 1 ] "Test braid"
    ...
    let ``Inverting a braid reverses generators and flips directions`` () =
        ...
        let braid =
            braidFromGensOrFail 3 [ BraidGroup.sigma 0; BraidGroup.sigma 1; BraidGroup.sigmaInv 0 ] "Original braid"
    ...
    let ``Double inverse returns original braid`` () =
        ...
        let braid =
            braidFromGensOrFail 3 [ BraidGroup.sigma 0; BraidGroup.sigma 1 ] "Original"
    ...
    let ``Simplify does not change already-simple braids`` () =
        ...
        let braid =
            braidFromGensOrFail 3 [ BraidGroup.sigma 0; BraidGroup.sigma 1 ] "Simple braid"
    ...
    let ``Applying identity braid gives phase 1`` () =
        ...
        let result =
            applyBraidOrFail idBraid anyons channel AnyonSpecies.AnyonType.Ising "Identity"
    ...
    let ``Braiding Ising σ×σ→1 accumulates R-matrix phase`` () =
        ...
        let result =
            applyBraidOrFail braid anyons channel AnyonSpecies.AnyonType.Ising "σ×σ→1"
    ...
    let ``Braiding with inverse gives conjugate phase`` () =
        ...
        let counterClock =
            braidFromGensOrFail 2 [ BraidGroup.sigmaInv 0 ] "Counter-clockwise"

        let anyons = [ AnyonSpecies.Particle.Sigma; AnyonSpecies.Particle.Sigma ]
        let channel = AnyonSpecies.Particle.Vacuum

        let cwResult =
            applyBraidOrFail clockwise anyons channel AnyonSpecies.AnyonType.Ising "CW"

        let ccwResult =
            applyBraidOrFail counterClock anyons channel AnyonSpecies.AnyonType.Ising "CCW"
    ...
    let ``Braid and inverse composition gives identity phase`` () =
        ...
        let result =
            applyBraidOrFail composed anyons channel AnyonSpecies.AnyonType.Ising "Verification"
    ...
    let ``Applying braid with wrong number of anyons fails`` () =
        ...
        (BraidGroup.applyBraid braid anyons channel AnyonSpecies.AnyonType.Ising)
        |> Result.map (fun _ -> failwith "Should have rejected mismatched anyon count")
        |> Result.defaultWith (fun err -> Assert.Contains("strands but", err.Message))
    ...
    let ``Braiding Fibonacci τ×τ→1 accumulates correct phase`` () =
        ...
        let result =
            applyBraidOrFail braid anyons channel AnyonSpecies.AnyonType.Fibonacci "τ×τ→1"
    ...
    let ``All braid phases lie on unit circle preserving probability`` () =
        ...
        let braid =
            braidFromGensOrFail 3 [ BraidGroup.sigma 0; BraidGroup.sigma 1 ] "Multi-braid"
        ...
        let result =
            applyBraidOrFail braid anyons channel AnyonSpecies.AnyonType.Ising "Test"
    ...
    let ``Ising anyons satisfy Yang-Baxter equation σ_0 σ_1 σ_0 = σ_1 σ_0 σ_1`` () =
        ...
        (BraidGroup.verifyYangBaxter 0 3 anyons channel AnyonSpecies.AnyonType.Ising)
        |> Result.map (fun satisfied -> Assert.True(satisfied, "Ising anyons must satisfy Yang-Baxter"))
    ...
    let ``Fibonacci anyons satisfy Yang-Baxter equation`` () =
        ...
        (BraidGroup.verifyYangBaxter 0 3 anyons channel AnyonSpecies.AnyonType.Fibonacci)
        |> Result.map (fun satisfied -> Assert.True(satisfied, "Fibonacci anyons must satisfy Yang-Baxter"))
        |> Result.defaultWith (fun err -> failwith $"Failed: {err.Message}")
    ...
    let ``Ising anyons satisfy Yang-Baxter for ALL fusion channels`` () =
        ...
        (BraidGroup.verifyYangBaxterAllChannels 0 3 anyons AnyonSpecies.AnyonType.Ising)
        |> Result.map (fun satisfied ->
    ...
    let ``Fibonacci anyons satisfy Yang-Baxter for ALL fusion channels`` () =
        ...
        (BraidGroup.verifyYangBaxterAllChannels 0 3 anyons AnyonSpecies.AnyonType.Fibonacci)
        |> Result.map (fun satisfied ->
    ...
    let ``Display braid shows mathematical notation`` () =
        ...
        let braid =
            braidFromGensOrFail 3 [ BraidGroup.sigma 0; BraidGroup.sigmaInv 1; BraidGroup.sigma 0 ] "Test braid"
