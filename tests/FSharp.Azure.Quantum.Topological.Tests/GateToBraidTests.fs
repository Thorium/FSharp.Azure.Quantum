...
module GateToBraidTests =
    ...
    let private assertExactDecomposition (decomp: GateToBraid.GateDecomposition) =
        Assert.True(
            decomp.ApproximationError < 1e-10,
    ...
    let ``Sequence of T gates compiles to sequence of braids`` () =
        ...
        let gateSeq =
            {
                BraidToGate.GateSequence.Gates =
                    [ CircuitBuilder.Gate.T 0; CircuitBuilder.Gate.T 1; CircuitBuilder.Gate.TDG 0 ]
                NumQubits = 2
                TotalPhase = System.Numerics.Complex(1.0, 0.0)
                Depth = 2
                TCount = 3
        ...
        let compilation =
            unwrapResult (GateToBraid.compileGateSequence gateSeq 1e-10 AnyonSpecies.AnyonType.Ising) "T sequence"
    ...
    let ``Clifford+T gate sequence compiles exactly`` () =
        ...
        let compilation =
            unwrapResult (GateToBraid.compileGateSequence gateSeq 1e-10 AnyonSpecies.AnyonType.Ising) "Clifford+T"
    ...
    let ``Gate sequence with Rz approximation reports total error`` () =
        ...
        let compilation =
            unwrapResult (GateToBraid.compileGateSequence gateSeq 1e-3 AnyonSpecies.AnyonType.Ising) "Rz approx"
    ...
    let ``T gate round-trip preserves gate identity`` () =
        ...
        let decomp =
            unwrapResult (GateToBraid.compileGateToBraid originalGate 2 1e-10) "Round-trip 1"
    ...
    let ``S gate round-trip produces T squared`` () =
        ...
        let decomp =
            unwrapResult (GateToBraid.compileGateToBraid originalGate 3 1e-10) "S decomp"
    ...
    let ``Gate decomposition display shows key information`` () =
        ...
        let decomp =
            unwrapResult (GateToBraid.compileGateToBraid gate 4 1e-10) "Display test"
    ...
    let ``Compilation summary shows statistics`` () =
        ...
        let gateSeq =
            {
                BraidToGate.GateSequence.Gates = [ CircuitBuilder.Gate.T 0; CircuitBuilder.Gate.S 1 ]
                NumQubits = 2
                TotalPhase = System.Numerics.Complex(1.0, 0.0)
                Depth = 1
                TCount = 1
        ...
        let compilation =
            unwrapResult (GateToBraid.compileGateSequence gateSeq 1e-10 AnyonSpecies.AnyonType.Ising) "Summary test"
    ...
    let ``Qubit index is preserved through compilation`` () =
        ...
        for qubitIndex in [ 0; 1; 2; 3 ] do
            ...
            let decomp =
                unwrapResult (GateToBraid.compileGateToBraid gate 5 1e-10) "Qubit index"
    ...
    let ``CZ gate is automatically transpiled in compileGateSequence`` () =
        ...
        let compilation =
            unwrapResult (GateToBraid.compileGateSequence gateSeq 1e-10 AnyonSpecies.AnyonType.Ising) "CZ transpilation"
    ...
    let ``CCX (Toffoli) gate is automatically transpiled in compileGateSequence`` () =
        ...
        let gateSeq =
            {
                BraidToGate.GateSequence.Gates = [ CircuitBuilder.Gate.CCX(0, 1, 2) ]
                NumQubits = 3
                TotalPhase = System.Numerics.Complex(1.0, 0.0)
                Depth = 1
                TCount = 0
    ...
    let ``MCZ gate with 3 controls is automatically transpiled in compileGateSequence`` () =
        ...
        let gateSeq =
            {
                BraidToGate.GateSequence.Gates = [ CircuitBuilder.Gate.MCZ([ 0; 1; 2 ], 3) ]
                NumQubits = 4
                TotalPhase = System.Numerics.Complex(1.0, 0.0)
                Depth = 1
                TCount = 0
    ...
    let ``Fibonacci CZ gate compiles natively via CNOT decomposition`` () =
        ...
        | Error err ->
            ...
            Assert.False(
                err.Message.Contains("transpiled"),
