...
module FusionTreeTests =
    ...
    let ``Topological qubit: Two sigma anyons encode a qubit via fusion outcome`` () =
        ...
        match
            FusionTree.isValid qubitZero AnyonSpecies.AnyonType.Ising,
            FusionTree.isValid qubitOne AnyonSpecies.AnyonType.Ising
        with
        | Ok true, Ok true -> ()
    ...
    let ``Four sigma anyons create 2-dimensional Hilbert space (1 qubit)`` () =
        ...
        match FusionTree.fusionSpaceDimension fourSigmas AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Ising with
        | Ok dimension ->
            // Dimension 2 = 1 qubit
    ...
    let ``Six sigma anyons create 4-dimensional Hilbert space (2 qubits)`` () =
        ...
        match FusionTree.fusionSpaceDimension sixSigmas AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Ising with
        | Ok dimension ->
    ...
    let ``Fibonacci: Four tau anyons create higher-dimensional space`` () =
        ...
        match
            FusionTree.fusionSpaceDimension fourTaus AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Fibonacci
        with
        | Ok dim ->
    ...
    let ``All valid fusion trees can be enumerated`` () =
        ...
        match FusionTree.allTrees fourSigmas AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Ising with
        | Ok trees ->
            ...
            trees |> List.iter (fun tree -> Assert.Equal(4, FusionTree.size tree))
        | Error err -> Assert.Fail($"Expected Ok but got Error: {err.Message}")
    ...
    let ``Enumerated trees match calculated dimension`` () =
        ...
        match
            FusionTree.allTrees fourSigmas AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Ising,
            FusionTree.fusionSpaceDimension fourSigmas AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Ising
        with
        | Ok trees, Ok dimension -> Assert.Equal(dimension, trees.Length)
        | Error err, _
    ...
    let ``allTrees for three sigmas fusing to sigma returns exactly 2 canonical trees`` () =
        ...
        match FusionTree.allTrees threeSigmas AnyonSpecies.Particle.Sigma AnyonSpecies.AnyonType.Ising with
        | Ok trees ->
            Assert.Equal(2, trees.Length)
    ...
    let ``Enumerated Fibonacci trees match calculated dimension`` () =
        ...
        for n in 2..6 do
            ...
            match
                FusionTree.allTrees taus AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Fibonacci,
                FusionTree.fusionSpaceDimension taus AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Fibonacci
            with
            | Ok trees, Ok dimension -> Assert.Equal(dimension, trees.Length)
            | Error err, _
            | _, Error err -> Assert.Fail($"Expected Ok for n={n} but got Error: {err.Message}")
    ...
    let ``No valid trees exist for impossible fusion`` () =
        ...
        match FusionTree.allTrees twoSigmas AnyonSpecies.Particle.Sigma AnyonSpecies.AnyonType.Ising with
        | Ok trees -> Assert.Empty(trees)
        | Error err -> Assert.Fail($"Expected Ok but got Error: {err.Message}")
    ...
    let ``SU2 k=3: Single bit 0 encodes as j=1/2 pair fusing to vacuum`` () =
        ...
        | Ok tree ->
            ...
            leafList
            |> List.iter (fun p -> Assert.Equal(AnyonSpecies.Particle.SpinJ(1, k), p))
    ...
    let ``SU2 k=3: Different bit values produce different trees`` () =
        ...
        match
            FusionTree.fromComputationalBasis [ 0 ] (AnyonSpecies.AnyonType.SU2Level k),
            FusionTree.fromComputationalBasis [ 1 ] (AnyonSpecies.AnyonType.SU2Level k)
        with
        | Ok tree0, Ok tree1 ->
        ...
        | Error e, _
        | _, Error e -> Assert.Fail($"Unexpected error: {e}")
