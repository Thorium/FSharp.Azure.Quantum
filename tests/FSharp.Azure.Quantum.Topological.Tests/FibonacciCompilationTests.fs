...
module FibonacciCompilationTests =
    ...
    let private assertApproxEqual (expected: Complex) (actual: Complex) (msg: string) =
        let diff =
            abs (expected.Real - actual.Real) + abs (expected.Imaginary - actual.Imaginary)
    ...
    let private unwrapResult (result: Result<'T, TopologicalError>) (context: string) : 'T =
        result
        |> Result.defaultWith (fun err -> failwith $"{context}: Unexpected error: {err}")
    ...
    let ``numQubits differs between tau and sigma for same anyon count`` () =
        ...
        let makePair p =
            FusionTree.fuse (FusionTree.leaf p) (FusionTree.leaf p) AnyonSpecies.Particle.Vacuum

        let tauTree =
            FusionTree.fuse
                (makePair AnyonSpecies.Particle.Tau)
                (makePair AnyonSpecies.Particle.Tau)
                AnyonSpecies.Particle.Vacuum

        let sigmaTree =
            FusionTree.fuse
                (makePair AnyonSpecies.Particle.Sigma)
                (makePair AnyonSpecies.Particle.Sigma)
                AnyonSpecies.Particle.Vacuum
    ...
    let ``fibonacciSigma1 has correct eigenvalues`` () =
        ...
        let expectedR1 =
            Complex.Exp(Complex.ImaginaryOne * Complex(4.0 * Math.PI / 5.0, 0.0))

        let expectedRTau =
            Complex.Exp(Complex.ImaginaryOne * Complex(-3.0 * Math.PI / 5.0, 0.0))
    ...
    let ``fibonacciSigma1 and sigma2 do not commute (necessary for universality)`` () =
        ...
        Assert.True(totalDiff > 0.01, $"σ₁ and σ₂ should not commute, but [σ₁,σ₂] has norm {totalDiff}")
    ...
    let ``buildFibonacciBaseSet length 1 has 4 elements`` () =
        ...
        Assert.True(baseSet.Length >= 4, $"Base set length 1 should have at least 4 entries, got {baseSet.Length}")
    ...
    let ``buildFibonacciBaseSet elements are unitary`` () =
        ...
        for (_, matrix) in baseSet do
            ...
            Assert.True(offDiag < 1e-8, $"Base set element not unitary: off-diagonal magnitude {offDiag}")
    ...
    let ``buildFibonacciBaseSet is memoized (same reference on second call)`` () =
        ...
        let set2 = buildFibonacciBaseSet 2
        Assert.True(Object.ReferenceEquals(set1, set2), "Base set should be memoized (same reference)")
    ...
    let ``approximateGateFibonacci can approximate Hadamard gate`` () =
        ...
        Assert.True(error < 0.7, $"Hadamard approximation error {error} exceeds tolerance 0.7")
    ...
    let ``approximateGateFibonacci can approximate X gate`` () =
        ...
        Assert.True(error < 0.7, $"X gate approximation error {error} exceeds tolerance 0.7")
    ...
    let ``approximateGateFibonacci can approximate T gate`` () =
        ...
        Assert.True(error < 0.7, $"T gate approximation error {error} exceeds tolerance 0.7")
    ...
    let ``approximateGateFibonacci can approximate S gate`` () =
        ...
        Assert.True(error < 0.7, $"S gate approximation error {error} exceeds tolerance 0.7")
    ...
    let ``approximateGateFibonacci identity gives low error`` () =
        ...
        Assert.True(error < 0.1, $"Identity approximation error {error} should be very low")
    ...
    let ``compileSingleQubitGateFibonacci compiles T gate`` () =
        ...
        Assert.True(decomp.ApproximationError < 0.7, $"T gate error {decomp.ApproximationError} exceeds tolerance")
    ...
    let ``compileGateToBraidFibonacci dispatches single-qubit gates`` () =
        ...
        for gate in gates do
            ...
            match result with
            | Ok decomp ->
                Assert.True(decomp.BraidSequence.Length >= 0, $"Gate {decomp.GateName} should compile successfully")
            | Error err -> failwith $"Gate should compile for Fibonacci: {err}"
    ...
    let ``compileGateToBraidFibonacci rejects must-transpile gates with descriptive error`` () =
        ...
        match result with
        | Error(TopologicalError.LogicError(name, msg)) -> Assert.Contains("transpiled", msg)
    ...
    let ``compileGateSequence with Fibonacci compiles T gate`` () =
        ...
        let result =
            GateToBraid.compileGateSequence gateSeq 0.5 AnyonSpecies.AnyonType.Fibonacci
    ...
    let ``compileGateSequence with Fibonacci handles multi-gate circuit`` () =
        ...
        Assert.True(compilation.CompiledBraids.Length > 0)
        Assert.True(compilation.CompilationWarnings.Length >= 2, "Should have warnings from H and T compilation notes")
