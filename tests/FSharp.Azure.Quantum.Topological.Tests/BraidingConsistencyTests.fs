...
module BraidingConsistencyTests =
    ...
    let private verifyConsistencyOrFail anyonType =
        (BraidingConsistency.verifyConsistency anyonType)
        |> Result.defaultWith (fun err -> failwith $"Failed to verify consistency for {anyonType}: {err.Message}")
    ...
    let private assertCheckCount (expectedMin: int) (actual: int) (checkType: string) =
        Assert.True(actual >= expectedMin, $"{checkType} checks: expected at least {expectedMin}, got {actual}")
    ...
    let ``Ising hexagon equations tested for braiding-fusion compatibility`` () =
        ...
        let passedCount =
            nonTrivialChecks |> List.filter (fun c -> c.IsSatisfied) |> List.length
    ...
    let ``Ising pentagon equations have negligible deviation`` () =
        ...
        let maxPentagonDeviation =
            summary.PentagonChecks |> List.map (fun c -> c.MaxDeviation) |> List.max
    ...
    let ``SU(2)_2 consistency matches Ising due to isomorphism`` () =
        ...
        let su2PentagonsSatisfied =
            su2Summary.PentagonChecks |> List.forall (fun c -> c.IsSatisfied)

        let isingPentagonsSatisfied =
            isingSummary.PentagonChecks |> List.forall (fun c -> c.IsSatisfied)
    ...
    let ``Hexagon checks skip cases with no valid fusion paths`` () =
        ...
        let trivialChecks =
            summary.HexagonChecks |> List.filter (fun c -> c.Details.Contains "No valid")

        // All trivial checks should be marked as satisfied
        trivialChecks
        |> List.iter (fun check -> Assert.True(check.IsSatisfied, "Trivial hexagon cases should be satisfied"))
    ...
    let ``Hexagon checks include maximum deviation measurement`` () =
        ...
        nonTrivialChecks
        |> List.iter (fun check -> Assert.True(check.MaxDeviation >= 0.0, "Deviation must be non-negative"))
    ...
    let ``Consistency verification succeeds for higher SU(2)_k levels`` () =
        ...
        | Ok summary ->
            ...
            Assert.True(
                failedHexagons.IsEmpty,
                $"SU(2)_5 hexagon equations should all be satisfied ({failedHexagons.Length} failed; e.g. {sample})"
            )
        | Error err -> failwith $"SU(2)_5 consistency check should succeed but got: {err.Message}"
    ...
    let ``Hexagon equations hold for SU(2)_3 with exchange R-symbols`` () =
        ...
        summary.HexagonChecks
        |> List.iter (fun check ->
            Assert.True(check.IsSatisfied, $"SU(2)_3 hexagon check failed: {check.Equation} - {check.Details}"))
    ...
    let ``Consistency check results include equation names for diagnostics`` () =
        ...
        allChecks
        |> List.iter (fun check -> Assert.False(System.String.IsNullOrWhiteSpace(check.Equation)))
