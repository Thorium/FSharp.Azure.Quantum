...
module RMatrixTests =
    ...
    let private computeRMatrixOrFail anyonType =
        ...
        |> Result.defaultWith (fun err -> failwith $"Failed to compute {anyonType} R-matrix: {err.Message}")
    ...
    let ``Ising all R-matrix elements lie on unit circle`` () =
        ...
        data.RSymbols |> Map.iter (fun _ value -> assertOnUnitCircle value)
    ...
    let ``Ising R-matrix passes unitarity validation`` () =
        ...
        (RMatrix.validateRMatrix data)
        |> Result.map (fun validated -> Assert.True(validated.IsValidated, "R-matrix should pass validation"))
    ...
    let ``getRSymbol rejects invalid fusion channels`` () =
        ...
        (RMatrix.getRSymbol data (rIndex sigma sigma tau))
        |> Result.map (fun _ -> failwith "Should have rejected invalid fusion")
        |> Result.defaultWith (fun err -> Assert.Contains("Cannot fuse", err.Message))
    ...
    let ``computeRMatrix accepts general SU(2)_k levels`` () =
        ...
        | Error err -> failwith $"Should have accepted SU(2)_10: {err.Message}"
    ...
    let ``SU(2)_3 R-matrices are unitary`` () =
        ...
        for KeyValue(_, rValue) in data.RSymbols do
            ...
            Assert.True(abs (magnitude - 1.0) < 1e-10, $"R-matrix element should have unit magnitude, got {magnitude}")
    ...
    let ``SU(2)_3 vacuum braiding is trivial`` () =
        ...
        | Some idx ->
            ...
            (RMatrix.getRSymbol data vacuumIndex)
            |> Result.iter (fun rValue ->
                Assert.True(abs (Complex.Abs(rValue) - 1.0) < 1e-10, "Vacuum braiding magnitude should be 1")) // May not exist, that's okay for this test
