...
module FusionRulesTests =
    ...
    let assertFusionEquals
        (expected: AnyonSpecies.Particle list)
        (actualResult: TopologicalResult<FusionRules.Outcome list>)
        =
    ...
    let assertMultiplicityOne (outcomesResult: TopologicalResult<FusionRules.Outcome list>) =
        ...
        | Ok outcomes -> outcomes |> List.iter (fun o -> Assert.Equal(1, o.Multiplicity))
        | Error err -> Assert.Fail($"Expected Ok but got Error: {err.Message}")
    ...
    let ``Ising: Vacuum × Vacuum = Vacuum (identity)`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Vacuum AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Ising
    ...
    let ``Ising: Vacuum × Sigma = Sigma (identity)`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Vacuum AnyonSpecies.Particle.Sigma AnyonSpecies.AnyonType.Ising
    ...
    let ``Ising: Sigma × Vacuum = Sigma (commutativity check)`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Sigma AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Ising
    ...
    let ``Ising: Vacuum × Psi = Psi (identity)`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Vacuum AnyonSpecies.Particle.Psi AnyonSpecies.AnyonType.Ising
    ...
    let ``Ising: Sigma × Sigma = Vacuum + Psi (KEY NON-ABELIAN RULE)`` () =
        ...
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Sigma AnyonSpecies.Particle.Sigma AnyonSpecies.AnyonType.Ising
    ...
    let ``Ising: Sigma × Psi = Sigma`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Sigma AnyonSpecies.Particle.Psi AnyonSpecies.AnyonType.Ising
    ...
    let ``Ising: Psi × Sigma = Sigma (commutativity)`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Psi AnyonSpecies.Particle.Sigma AnyonSpecies.AnyonType.Ising
    ...
    let ``Ising: Psi × Psi = Vacuum (fermion pair annihilates)`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Psi AnyonSpecies.Particle.Psi AnyonSpecies.AnyonType.Ising
    ...
    let ``Fibonacci: Vacuum × Vacuum = Vacuum`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Vacuum AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Fibonacci
    ...
    let ``Fibonacci: Vacuum × Tau = Tau (identity)`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Vacuum AnyonSpecies.Particle.Tau AnyonSpecies.AnyonType.Fibonacci
    ...
    let ``Fibonacci: Tau × Vacuum = Tau (commutativity)`` () =
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Tau AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Fibonacci
    ...
    let ``Fibonacci: Tau × Tau = Vacuum + Tau (FIBONACCI RULE)`` () =
        ...
        let outcomes =
            FusionRules.fuse AnyonSpecies.Particle.Tau AnyonSpecies.Particle.Tau AnyonSpecies.AnyonType.Fibonacci

        assertFusionEquals [ AnyonSpecies.Particle.Vacuum; AnyonSpecies.Particle.Tau ] outcomes
        assertMultiplicityOne outcomes

        outcomes
        |> Result.map (fun list -> Assert.Equal(2, list.Length))
    ...
    let ``Fusion multiplicity: Ising Sigma × Sigma → Sigma is 0`` () =
        ...
        (FusionRules.multiplicity
            ...
            AnyonSpecies.Particle.Sigma
            AnyonSpecies.AnyonType.Ising)
        |> Result.map (fun n -> Assert.Equal(0, n))
        |> Result.defaultWith (fun err -> Assert.Fail($"Expected Ok but got Error: {err.Message}"))
    ...
    let ``Fusion multiplicity is symmetric: N^c_ab = N^c_ba`` () =
        ...
        |> List.iter (fun (a, b) ->
            ...
            |> List.iter (fun c ->
                match
                    FusionRules.multiplicity a b c AnyonSpecies.AnyonType.Ising,
                    FusionRules.multiplicity b a c AnyonSpecies.AnyonType.Ising
                with
                | Ok nab, Ok nba -> Assert.Equal(nab, nba)
                | Error err, _
                | _, Error err -> Assert.Fail($"Expected Ok but got Error: {err.Message}")))
    ...
    let ``Identity axiom: 1 × a = a for all Ising particles`` () =
        ...
        particles
        |> List.iter (fun a ->
            (FusionRules.channels AnyonSpecies.Particle.Vacuum a AnyonSpecies.AnyonType.Ising)
            |> Result.map (fun channels -> Assert.Equal<AnyonSpecies.Particle list>([ a ], channels))
            |> Result.defaultWith (fun err -> Assert.Fail($"Expected Ok but got Error: {err.Message}")))
    ...
    let ``Commutativity: a × b = b × a for all Ising pairs`` () =
        ...
        |> List.iter (fun (a, b) ->
            match
                FusionRules.channels a b AnyonSpecies.AnyonType.Ising,
                FusionRules.channels b a AnyonSpecies.AnyonType.Ising
            with
            | Ok ab, Ok ba ->
    ...
    let ``Fusing Sigma in Fibonacci theory returns ValidationError`` () =
        match
            FusionRules.fuse AnyonSpecies.Particle.Sigma AnyonSpecies.Particle.Vacuum AnyonSpecies.AnyonType.Fibonacci
        with
        | Error(TopologicalError.ValidationError _) -> () // Expected
