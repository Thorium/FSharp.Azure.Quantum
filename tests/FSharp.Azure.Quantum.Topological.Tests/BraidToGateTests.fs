...
module BraidToGateTests =
    ...
    let private braidFromGensOrFail n gens context =
        (BraidGroup.fromGenerators n gens)
        |> Result.defaultWith (fun err -> failwith $"{context}: {err.Message}")

    let private compileOrFail braid anyonType options context =
        (BraidToGate.compileToGates braid anyonType options)
        |> Result.defaultWith (fun err -> failwith $"{context}: {err.Message}")

    let private identityOrFail n context =
        (BraidGroup.identity n)
        |> Result.defaultWith (fun err -> failwith $"{context}: {err.Message}")
    ...
    let ``Ising clockwise braiding compiles to S gate`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Ising compilation"
    ...
    let ``Ising counter-clockwise braiding compiles to S-dagger`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Ising inverse"
    ...
    let ``Multiple Ising braidings compile to sequence of S gates`` () =
        ...
        let braid =
            braidFromGensOrFail 6 [ BraidGroup.sigma 0; BraidGroup.sigma 2; BraidGroup.sigma 0 ] "σ_0 σ_2 σ_0"

        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Multiple braidings"
    ...
    let ``Ising identity braid compiles to empty gate sequence`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Identity"
    ...
    let ``Ising braid on 2(n+1) strands produces n qubit circuit`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Qubit counting"
    ...
    let ``Fibonacci braiding compiles to the exact relative channel phase`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Fibonacci BraidToGate.defaultOptions "Fibonacci compilation"
    ...
    let ``Fibonacci inverse braiding uses negative rotation`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Fibonacci BraidToGate.defaultOptions "Fibonacci inverse"

        match sequence.Gates.[0] with
        | CircuitBuilder.Gate.P(_, angle) -> Assert.True(angle < 0.0)
        | _ -> failwith "Expected P gate"
    ...
    let ``Fibonacci TotalPhase times gate reproduces the exact braid unitary`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Fibonacci BraidToGate.defaultOptions "Fibonacci exactness"
        ...
        | CircuitBuilder.Gate.P(_, angle) ->
            ...
            let diag1 =
                sequence.TotalPhase * Complex.Exp(Complex.ImaginaryOne * Complex(angle, 0.0))
    ...
    let ``Adjacent T and T-dagger gates cancel`` () =
        ...
        let gates =
            [ CircuitBuilder.Gate.T 0; CircuitBuilder.Gate.TDG 0; CircuitBuilder.Gate.H 1 ]
    ...
    let ``Non-adjacent inverse pairs are not cancelled`` () =
        ...
        let gates =
            [ CircuitBuilder.Gate.T 0; CircuitBuilder.Gate.H 1; CircuitBuilder.Gate.TDG 0 ]
    ...
    let ``Braid with inverse compiles to optimized empty sequence`` () =
        ...
        let braid =
            braidFromGensOrFail 4 [ BraidGroup.sigma 0; BraidGroup.sigmaInv 0 ] "σ_0 σ_0^-1"

        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Identity after optimization"
    ...
    let ``Sequential gates on same qubit have depth N`` () =
        ...
        let braid =
            braidFromGensOrFail 4 [ BraidGroup.sigma 0; BraidGroup.sigma 0; BraidGroup.sigma 0 ] "Three sequential"

        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Sequential depth"
    ...
    let ``Parallel gates on different qubits have depth 1`` () =
        ...
        let braid =
            braidFromGensOrFail 6 [ BraidGroup.sigma 0; BraidGroup.sigma 2 ] "Parallel gates"

        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Parallel depth"
    ...
    let ``Parity-pair exchange applies the parity-dependent phase`` () =
        ...
        let braid =
            braidFromGensOrFail 4 [ BraidGroup.sigma 0; BraidGroup.sigma 2 ] "Qubit + parity-pair exchange"

        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Parity pair"

        Assert.Equal<CircuitBuilder.Gate list>([ CircuitBuilder.Gate.S 0; CircuitBuilder.Gate.S 0 ], sequence.Gates)
    ...
    let ``Parity-pair exchange on two qubits computes the joint parity`` () =
        ...
        let braid = braidFromGensOrFail 6 [ BraidGroup.sigma 4 ] "Parity-pair exchange"
    ...
    let ``Gate sequence reports correct T-count`` () =
        ...
        let braid =
            braidFromGensOrFail 6 [ BraidGroup.sigma 0; BraidGroup.sigmaInv 2; BraidGroup.sigma 0 ] "Mixed directions"

        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "T-count test"
    ...
    let ``Empty braid produces zero-depth circuit`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Zero depth"
    ...
    let ``Gate sequence metadata is consistent`` () =
        ...
        let braid =
            braidFromGensOrFail 8 [ BraidGroup.sigma 0; BraidGroup.sigma 2; BraidGroup.sigma 4 ] "Full chain"

        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Metadata test"
    ...
    let ``Gate sequence display includes circuit metadata`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Display test"
    ...
    let ``Statistics display shows gate breakdown`` () =
        ...
        let braid =
            braidFromGensOrFail 6 [ BraidGroup.sigma 0; BraidGroup.sigmaInv 2; BraidGroup.sigma 0 ] "Mixed"

        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Stats test"
    ...
    let ``Custom optimization level is respected`` () =
        ...
        let braid =
            braidFromGensOrFail 4 [ BraidGroup.sigma 0; BraidGroup.sigmaInv 0 ] "Should not optimize"
    ...
    let ``Identity braid has unit total phase`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Identity phase"
    ...
    let ``Single Ising clockwise braid has phase exp(-i*pi/8)`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Ising phase"
    ...
    let ``Single Ising counter-clockwise braid has phase exp(i*pi/8)`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Ising inv phase"
    ...
    let ``Two Ising clockwise braids accumulate phase exp(-i*pi/4)`` () =
        ...
        let braid =
            braidFromGensOrFail 2 [ BraidGroup.sigma 0; BraidGroup.sigma 0 ] "s_0 s_0"
    ...
    let ``Ising clockwise and counter-clockwise cancels phase to unity`` () =
        ...
        let braid =
            braidFromGensOrFail 2 [ BraidGroup.sigma 0; BraidGroup.sigmaInv 0 ] "s_0 s_0^-1"
    ...
    let ``Fibonacci clockwise braid has phase exp(4*pi*i/5)`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Fibonacci BraidToGate.defaultOptions "Fibonacci phase"
    ...
    let ``Fibonacci counter-clockwise braid has conjugate phase`` () =
        ...
        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Fibonacci BraidToGate.defaultOptions "Fibonacci inv phase"
        ...
        let braid =
            braidFromGensOrFail
                ...
                [
                    ...
                    BraidGroup.sigma 2
                    BraidGroup.sigma 4
                    BraidGroup.sigmaInv 0
                    BraidGroup.sigma 2
                ]
                "Long sequence"

        let sequence =
            compileOrFail braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions "Phase magnitude"
    ...
    let ``commutationCancellation does not cancel through non-commuting gate`` () =
        ...
        let gates =
            [ CircuitBuilder.Gate.T 0; CircuitBuilder.Gate.H 0; CircuitBuilder.Gate.TDG 0 ]
    ...
    let ``templateMatching replaces H Z H with X`` () =
        let gates =
            [ CircuitBuilder.Gate.H 0; CircuitBuilder.Gate.Z 0; CircuitBuilder.Gate.H 0 ]
    ...
    let ``templateMatching replaces H X H with Z`` () =
        let gates =
            [ CircuitBuilder.Gate.H 0; CircuitBuilder.Gate.X 0; CircuitBuilder.Gate.H 0 ]
    ...
    let ``optimizeAggressive reduces T T T T to Z via multi-pass`` () =
        ...
        Assert.True(result.Length <= 2, $"Expected at most 2 gates after aggressive optimization, got {result.Length}")
    ...
    let ``optimizeAggressive never increases gate count`` () =
        ...
        let gates =
            [
                ...
                CircuitBuilder.Gate.H 1
                CircuitBuilder.Gate.S 2
                CircuitBuilder.Gate.SDG 2
                CircuitBuilder.Gate.CNOT(0, 1)
            ]

        let result = BraidToGate.optimizeAggressive gates
        Assert.True(result.Length <= gates.Length, $"Optimized {result.Length} should be <= original {gates.Length}")
    ...
    let ``Braid runs on a gate backend (executeOnGateBackend)`` () =
        ...
        match
            BraidToGate.executeOnGateBackend backend braid AnyonSpecies.AnyonType.Ising BraidToGate.defaultOptions
        with
