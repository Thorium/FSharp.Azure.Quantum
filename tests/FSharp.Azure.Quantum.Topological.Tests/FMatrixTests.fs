...
module FMatrixTests =
    ...
    let private computeFMatrixOrFail anyonType =
        ...
        |> Result.defaultWith (fun err -> failwith $"Failed to compute {anyonType} F-matrix: {err.Message}")
    ...
    let ``Ising basis transformation F[σ,σ,σ,σ;1,1] equals 1/√2 preserving normalization`` () =
        ...
        let value =
            getFSymbolOrFail data (fIndex sigma sigma sigma sigma vacuum vacuum) "F[σ,σ,σ,σ;1,1]"
    ...
    let ``Ising basis transformation F[σ,σ,σ,σ;ψ,ψ] equals -1/√2 for orthogonal path`` () =
        ...
        let value =
            getFSymbolOrFail data (fIndex sigma sigma sigma sigma psi psi) "F[σ,σ,σ,σ;ψ,ψ]"
    ...
    let ``Ising trivial F-symbol F[ψ,σ,σ,ψ;σ,1] equals 1 for unambiguous fusion`` () =
        ...
        let value =
            getFSymbolOrFail data (fIndex psi sigma sigma psi sigma vacuum) "F[ψ,σ,σ,ψ;σ,1]"
    ...
    let ``Fibonacci F[τ,τ,τ,1;τ,τ] equals 1 from unitary normalization of 1×1 block`` () =
        ...
        let value =
            getFSymbolOrFail data (fIndex tau tau tau vacuum tau tau) "F[τ,τ,τ,1;τ,τ]"
    ...
    let ``Fibonacci F[τ,τ,τ,τ;1,τ] equals φ⁻¹/² for vacuum-mediated path`` () =
        ...
        let value =
            getFSymbolOrFail data (fIndex tau tau tau tau vacuum tau) "F[τ,τ,τ,τ;1,τ]"
    ...
    let ``Ising F-matrix is unitary for σ×σ×σ→σ preserving quantum probability`` () =
        ...
        (FMatrix.verifyFMatrixUnitarity data sigma sigma sigma sigma)
        |> Result.map (fun isUnitary -> Assert.True(isUnitary, "F F† = I ensures probability conservation"))
        |> Result.defaultWith (fun err -> failwith $"Unitarity check failed: {err.Message}")
    ...
    let ``Computing SU(2)_10 returns valid F-matrix with non-trivial symbols`` () =
        ...
        (FMatrix.computeFMatrix (AnyonSpecies.AnyonType.SU2Level 10))
        |> Result.map (fun data -> Assert.True(data.FSymbols.Count > 0, "SU(2)_10 should have non-trivial F-symbols"))
        |> Result.defaultWith (fun err -> failwith $"Should succeed for SU(2)_10: {err.Message}")
    ...
    let ``getFSymbol returns trivial value (1.0) for valid but unstored F-symbols`` () =
        ...
        (FMatrix.getFSymbol emptyData (fIndex psi sigma sigma psi sigma vacuum))
        |> Result.map (fun value -> assertRealValue 1.0 value)
        |> Result.defaultWith (fun _ -> failwith "Should succeed with trivial value for valid fusion")
    ...
    let ``SU(2)_3 F-matrix for integer-spin sector reproduces Fibonacci F-symbols`` () =
        ...
        let f_11_00 =
            getFSymbolOrFail data (fIndex j1 j1 j1 j1 j0 j0) "F[j1,j1,j1,j1;j0,j0]"
    ...
    let ``SU(2)_3 F-matrix passes pentagon equation validation`` () =
        ...
        |> Result.defaultWith (fun err -> failwith $"SU(2)_3 pentagon validation failed: {err.Message}")
    ...
    let ``SU(2)_4 F-matrix passes pentagon equation validation`` () =
        ...
        |> Result.defaultWith (fun err -> failwith $"SU(2)_4 pentagon validation failed: {err.Message}")
    ...
    let ``SU(2)_5 F-matrix passes pentagon equation validation`` () =
        ...
        |> Result.defaultWith (fun err -> failwith $"SU(2)_5 pentagon validation failed: {err.Message}")
    ...
    let ``SU(2)_k F-matrix has correct vacuum F-symbols for all k`` () =
        ...
        for k in [ 3; 4; 5; 6 ] do
            ...
            let value =
                getFSymbolOrFail data (fIndex j0 j1 j0 j1 j1 j1) $"F[j0,j1,j0,j1;j1,j1] for k={k}"
