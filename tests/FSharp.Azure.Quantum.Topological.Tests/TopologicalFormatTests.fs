...
module TopologicalFormatTests =
    ...
    let ``Parser should parse anyon types correctly`` () =
        ...
        | Ok(AnyonSpecies.AnyonType.Ising), Ok(AnyonSpecies.AnyonType.Fibonacci), Ok(AnyonSpecies.AnyonType.SU2Level 3) ->
            ()
    ...
    let ``Parser should reject invalid anyon types`` () =
        ...
        invalid
        |> Result.iter (fun _ -> failwith "Should have rejected invalid anyon type")
...
"""
        ...
        |> Result.map (fun _ -> failwith "Should have required ANYON declaration")
        |> Result.defaultWith (fun msg -> Assert.Contains("ANYON", msg))
...
"""
        ...
        (Parser.parseProgram program)
        |> Result.map (fun _ -> failwith "Should have rejected negative index")
        |> Result.defaultWith (fun msg -> Assert.Contains("index", msg.ToLowerInvariant()))
        ...
        let program =
            {
                ...
                Operations = [ Comment "# Test program"; Initialize 4; Braid 0; Measure 1 ]
            }
        ...
        let original =
            {
                ...
                Operations = [ Initialize 2; Braid 0; Braid 1; Measure 0 ]
            }
    ...
    let ``File I/O: Write and read should preserve program`` () =
        ...
        try
            let original =
                {
                    AnyonType = AnyonSpecies.AnyonType.Ising
                    Operations = [ Initialize 6; Braid 0; Braid 2; Braid 4; Measure 1; Measure 3 ]
                }
    ...
    let ``Serializer should handle non-existent directory gracefully`` () =
        ...
        | Error msg -> Assert.Contains("Failed to write", msg)
    ...
    let ``executeProgramAsync should produce same result as sync executeProgram`` () : Task =
        task {
            ...
            let program =
                {
                    AnyonType = AnyonSpecies.AnyonType.Ising
                    Operations = [ Initialize 4; Braid 0; Braid 2 ]
                }
            ...
            | Error syncErr, _ -> failwith $"Sync execution failed: {syncErr}"
            | _, Error asyncErr -> failwith $"Async execution failed: {asyncErr}"
        }
    ...
    let ``executeProgramAsync should fail without INIT operation`` () : Task =
        task {
            ...
            | Ok _ -> failwith "Should have failed without INIT operation"
        }
    ...
    let ``executeProgramAsync should respect cancellation`` () : Task =
        task {
            ...
            let program =
                {
                    AnyonType = AnyonSpecies.AnyonType.Ising
                    Operations = [ Initialize 4; Braid 0; Braid 1; Braid 2 ]
                }
        ...
        task {
            ...
            try
                ...
                | Ok() ->
                    ...
                    | Error err -> failwith $"Async file execution failed: {err}"
            finally
                if File.Exists(tempFile) then
                    File.Delete(tempFile)
        }
    ...
    let ``executeFileAsync should fail with non-existent file`` () : Task =
        task {
            ...
            | Ok _ -> failwith "Should have failed with non-existent file"
        }
        ...
        task {
            ...
            try
                let program =
                    {
                        AnyonType = AnyonSpecies.AnyonType.Ising
                        Operations = [ Initialize 6; Braid 0; Braid 2; Braid 4 ]
                    }

                // Write asynchronously
                let! writeResult =
                    Serializer.serializeToFileAsync program tempFile CancellationToken.None
                ...
                | Error err -> failwith $"Async roundtrip failed: {err}"
            finally
                if File.Exists(tempFile) then
                    File.Delete(tempFile)
        }
