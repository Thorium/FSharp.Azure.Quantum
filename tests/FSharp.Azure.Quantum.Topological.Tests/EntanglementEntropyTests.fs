...
module EntanglementEntropyTests =
    ...
    let ``Ising topological entropy equals log(2)`` () =
        ...
        | Error err -> Assert.Fail($"Should succeed, got error: {err.Message}")
    ...
    let ``SU(2)_2 equals Ising`` () =
        ...
        | Ok g1, Ok g2 -> Assert.InRange(abs (g1 - g2), 0.0, 1e-10)
    ...
    let ``Topological entropy is always non-negative`` () =
        ...
        for anyonType in anyonTypes do
            (EntanglementEntropy.topologicalEntropy anyonType)
            |> Result.iter (fun gamma -> Assert.True(gamma >= 0.0, $"{anyonType} entropy should be non-negative")) // Skip unimplemented types
    ...
    let ``Eigenvalues must be non-negative`` () =
        ...
        // Assert
        match result with
        | Ok _ -> Assert.Fail("Should reject negative eigenvalues")
        | Error(TopologicalError.ValidationError(field, reason)) -> Assert.Contains("non-negative", reason)
        | Error err -> Assert.Fail($"Wrong error type: {err.Message}")
    ...
    let ``Eigenvalues must sum to 1`` () =
        ...
        // Assert
        match result with
        | Ok _ -> Assert.Fail("Should reject unnormalized eigenvalues")
        | Error(TopologicalError.ValidationError(field, reason)) -> Assert.Contains("sum to 1", reason)
        | Error err -> Assert.Fail($"Wrong error type: {err.Message}")
    ...
    let ``Verify Kitaev-Preskill against theoretical value`` () =
        ...
        // Assert
        result
        |> Result.map (fun isConsistent -> Assert.True(isConsistent, "Kitaev-Preskill should match theoretical value"))
        |> Result.defaultWith (fun err -> Assert.Fail($"Should succeed, got error: {err.Message}"))
    ...
    let ``Sphere (g=0) has GSD=1`` () =
        ...
        // Assert
        result
        |> Result.map (fun gsd -> Assert.Equal(1, gsd))
        |> Result.defaultWith (fun err -> Assert.Fail($"Should succeed, got error: {err.Message}"))
    ...
    let ``Negative genus is invalid`` () =
        ...
        // Assert
        match result with
        | Ok _ -> Assert.Fail("Should reject negative genus")
        | Error(TopologicalError.ValidationError(field, reason)) -> Assert.Contains("non-negative", reason)
        | Error err -> Assert.Fail($"Wrong error type: {err.Message}")
    ...
    let ``partialTraceB of product state returns subsystem A density matrix`` () =
        ...
        let amps = [ Complex.One; Complex.Zero; Complex.Zero; Complex.Zero ]
        let rho = EntanglementEntropy.densityMatrix amps
    ...
    let ``partialTraceB of Bell state returns maximally mixed state`` () =
        ...
        // ρ_A = Tr_B(|Φ+⟩⟨Φ+|) = I/2
        let s = 1.0 / sqrt 2.0
        let amps = [ Complex(s, 0.0); Complex.Zero; Complex.Zero; Complex(s, 0.0) ]
        let rho = EntanglementEntropy.densityMatrix amps
    ...
    let ``eigenvaluesHermitian of identity matrix returns all ones`` () =
        ...
        let identity =
            Array2D.init n n (fun i j -> if i = j then Complex.One else Complex.Zero)
    ...
    let ``eigenvaluesHermitian of empty matrix returns empty`` () =
        ...
        (EntanglementEntropy.eigenvaluesHermitian matrix 100 1e-12)
        |> Result.map (fun eigenvals -> Assert.Empty(eigenvals))
        |> Result.defaultWith (fun err -> Assert.Fail($"Expected Ok, got: {err.Message}"))

    [<Fact>]
    let ``eigenvaluesHermitian of maximally mixed 2x2 returns equal eigenvalues`` () =
        // I/2 = [[0.5, 0], [0, 0.5]]
        let matrix =
            Array2D.init 2 2 (fun i j -> if i = j then Complex(0.5, 0.0) else Complex.Zero)
    ...
    let ``vonNeumannEntropyFromDensityMatrix of complex pure state is zero`` () =
        ...
        (EntanglementEntropy.vonNeumannEntropyFromDensityMatrix rho)
        |> Result.map (fun result -> Assert.Equal(0.0, result.Entropy, 6))
        |> Result.defaultWith (fun err -> Assert.Fail($"Expected Ok, got: {err.Message}"))
    ...
    let ``entanglementEntropy of complex-phase product state is zero`` () =
        ...
        let s = 1.0 / sqrt 2.0
        let amps = [ Complex(s, 0.0); Complex.Zero; Complex(0.0, s); Complex.Zero ]

        (EntanglementEntropy.entanglementEntropy amps 2 2)
        |> Result.map (fun result -> Assert.Equal(0.0, result.Entropy, 6))
        |> Result.defaultWith (fun err -> Assert.Fail($"Expected Ok, got: {err.Message}"))
    ...
    let ``vonNeumannEntropyFromDensityMatrix of pure state returns zero`` () =
        ...
        let rho =
            Array2D.init 2 2 (fun i j -> if i = 0 && j = 0 then Complex.One else Complex.Zero)

        (EntanglementEntropy.vonNeumannEntropyFromDensityMatrix rho)
        |> Result.map (fun result -> Assert.Equal(0.0, result.Entropy, 8))
        |> Result.defaultWith (fun err -> Assert.Fail($"Expected Ok, got: {err.Message}"))
    ...
    let ``entanglementEntropy of partially entangled state is between 0 and log(2)`` () =
        ...
        let amps = [ Complex(c, 0.0); Complex.Zero; Complex.Zero; Complex(s, 0.0) ]
