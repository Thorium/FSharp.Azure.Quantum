...
module NoiseModelsTests =
    ...
    let ``realisticTopological has lower gate errors than superconducting`` () =
        let sc =
            NoiseModels.realisticSuperconducting ()
            |> Result.defaultWith (fun _ -> failwith "sc")

        let topo =
            NoiseModels.realisticTopological ()
            |> Result.defaultWith (fun _ -> failwith "topo")
    ...
    let ``applyDecoherence on empty superposition returns unchanged`` () =
        ...
        let decoherence =
            (NoiseModels.createDecoherenceParameters 50.0 30.0)
    ...
    let ``applyDepolarizingNoise with error produces uniform amplitudes`` () =
        ...
        let sup =
            makeTwoTermSuperposition (Complex(0.9, 0.0)) (Complex(sqrt (1.0 - 0.81), 0.0))
        ...
        for (amp, _) in result.Terms do
            Assert.True(
                abs (Complex.Abs amp - expectedAmp) < 1e-10,
    ...
    let ``applyMeasurementError with zero rate never flips`` () =
        ...
        for _ in 1..100 do
            let result =
                NoiseModels.applyMeasurementError 0.0 AnyonSpecies.Particle.Vacuum random
    ...
    let ``applyMeasurementError with rate 1 always flips`` () =
        ...
        let result =
            NoiseModels.applyMeasurementError 1.0 AnyonSpecies.Particle.Vacuum random
    ...
    let ``applyMeasurementError flips Sigma to Psi`` () =
        ...
        let result =
            NoiseModels.applyMeasurementError 1.0 AnyonSpecies.Particle.Sigma random
    ...
    let ``checkQuasiparticlePoisoning with zero rate never triggers`` () =
        let poisoning =
            (NoiseModels.createQuasiparticlePoisoningParameters 0.0 0.050)
            |> Result.defaultWith (fun _ -> failwith "")
    ...
    let ``checkQuasiparticlePoisoning probability increases with time`` () =
        ...
        let poisoning =
            (NoiseModels.createQuasiparticlePoisoningParameters 1_000_000.0 0.050)
            |> Result.defaultWith (fun _ -> failwith "")
    ...
    let ``checkQuasiparticlePoisoning with very short time rarely triggers`` () =
        let poisoning =
            (NoiseModels.createQuasiparticlePoisoningParameters 100.0 0.050)
            |> Result.defaultWith (fun _ -> failwith "")
    ...
    let ``effectiveErrorRate increases with operation time`` () =
        ...
        Assert.True(rate10 > rate1, $"Error rate at t=10 ({rate10}) should exceed rate at t=1 ({rate1})")
    ...
    let ``effectiveErrorRate combines decoherence and gate errors`` () =
        ...
        Assert.True(rate > gateError, $"Combined rate ({rate}) should exceed gate error alone ({gateError})")
    ...
    let ``displayNoiseModel for topological model shows poisoning`` () =
        let model =
            NoiseModels.realisticTopological () |> Result.defaultWith (fun _ -> failwith "")
