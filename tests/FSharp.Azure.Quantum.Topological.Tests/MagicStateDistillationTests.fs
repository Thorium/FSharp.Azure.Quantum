...
module MagicStateDistillationTests =
    ...
    let ``Should reject negative error rate`` () =
        (MagicStateDistillation.prepareNoisyMagicState -0.1 AnyonSpecies.AnyonType.Ising)
        |> Result.map (fun _ -> failwith "Should have rejected negative error rate")
        |> Result.defaultWith (fun err -> Assert.Contains("Error rate must be in [0, 1]", err.Message))

    [<Fact>]
    let ``Should reject error rate greater than 1`` () =
        (MagicStateDistillation.prepareNoisyMagicState 1.5 AnyonSpecies.AnyonType.Ising)
        |> Result.map (fun _ -> failwith "Should have rejected error rate > 1")
        |> Result.defaultWith (fun err -> Assert.Contains("Error rate must be in [0, 1]", err.Message))

    [<Fact>]
    let ``Should reject non-Ising anyons for magic states`` () =
        (MagicStateDistillation.prepareNoisyMagicState 0.01 AnyonSpecies.AnyonType.Fibonacci)
        |> Result.map (fun _ -> failwith "Should have rejected Fibonacci anyons")
    ...
    let ``Should reject wrong number of input states`` () =
        ...
        (MagicStateDistillation.distill15to1 (Random()) inputStates)
        |> Result.map (fun _ -> failwith "Should have rejected 10 states")
        |> Result.defaultWith (fun err -> Assert.Contains("requires exactly 15 input states", err.Message))
    ...
    let ``Should reject insufficient states for iterative distillation`` () =
        ...
        (MagicStateDistillation.distillIterative (Random()) 2 inputStates)
        |> Result.map (fun _ -> failwith "Should have rejected insufficient states")
        |> Result.defaultWith (fun err -> Assert.Contains("Need", err.Message))

    [<Fact>]
    let ``Should reject too many rounds`` () =
        let inputStates = [] // Doesn't matter, will fail before using them

        (MagicStateDistillation.distillIterative (Random()) 10 inputStates)
        |> Result.map (fun _ -> failwith "Should have rejected 10 rounds")
        |> Result.defaultWith (fun err -> Assert.Contains("More than 5 rounds is impractical", err.Message))
    ...
    let ``Should reject zero rounds`` () =
        ...
        (MagicStateDistillation.distillIterative (Random()) 0 inputStates)
        |> Result.map (fun _ -> failwith "Should have rejected 0 rounds")
    ...
    let ``Should apply T-gate with high-fidelity magic state`` () =
        ...
        | Ok magicState ->
            ...
            match MagicStateDistillation.applyTGate (Random()) dataQubit magicState with
    ...
    let ``Should reject low-fidelity magic state for T-gate`` () =
        ...
        | Ok lowFidelityState ->
            ...
            (MagicStateDistillation.applyTGate (Random()) dataQubit lowFidelityState)
            |> Result.map (fun _ -> failwith "Should have rejected low-fidelity magic state")
            |> Result.defaultWith (fun err -> Assert.Contains("fidelity too low", err.Message))
    ...
    let ``Should reject non-Ising qubit for T-gate`` () =
        ...
        |> Result.map (fun magicState ->
            (MagicStateDistillation.applyTGate (Random()) fibQubit magicState)
    ...
    let ``Should estimate resources for target fidelity`` () =
        ...
        Assert.True(
            estimate.OutputFidelity >= estimate.TargetFidelity
            || estimate.DistillationRounds >= 5
        ) // Or maxed out
    ...
    let ``Should display distillation result`` () =
        let inputStates =
            ...
            |> List.map (fun _ ->
                (MagicStateDistillation.prepareNoisyMagicState 0.01 AnyonSpecies.AnyonType.Ising)
                |> Result.defaultWith (fun _ -> failwith "Failed"))
    ...
    let ``Integration: Complete universal quantum computation with Ising anyons`` () =
        ...
        let random = Random(42) // Deterministic for testing
