...
module VisualizationTests =
    ...
    type MockBackend() =
        interface IQuantumBackend with
            ...
            member this.ExecuteToStateAsync
                (circuit: FSharp.Azure.Quantum.Core.CircuitAbstraction.ICircuit)
    ...
    let ignoreResult (task: Task<Result<TopologicalBuilder.BuilderContext, QuantumError>>) = task
    ...
    let ``Circuit visualization produces valid Mermaid diagram`` () =
        task {
            ...
            | Error err -> Assert.Fail($"Program execution failed: %s{err.Message}")
        }
        :> Task
