...
module ToricCodeTests =
    ...
    let ``createLattice with zero width fails`` () =
        ...
        | Error(TopologicalError.ValidationError(field, reason)) -> Assert.Contains("positive", reason)
    ...
    let ``buildMatchingGraph with 2 excitations returns 1 edge`` () =
        ...
        let exc =
            [ { ToricCode.X = 0; ToricCode.Y = 0 }; { ToricCode.X = 2; ToricCode.Y = 0 } ]
    ...
    let ``buildMatchingGraph uses toric distance`` () =
        ...
        let exc =
            [ { ToricCode.X = 0; ToricCode.Y = 0 }; { ToricCode.X = 5; ToricCode.Y = 0 } ]
    ...
    let ``greedyMatching with empty excitations returns empty list`` () =
        ...
        (ToricCode.greedyMatching lattice [])
        |> Result.map (fun pairs -> Assert.Empty(pairs))
    ...
    let ``greedyMatching with odd excitations returns error`` () =
        ...
        | Error(TopologicalError.ValidationError(_, reason)) -> Assert.Contains("even", reason)
    ...
    let ``greedyMatching pairs 2 excitations correctly`` () =
        ...
        let exc =
            [ { ToricCode.X = 1; ToricCode.Y = 1 }; { ToricCode.X = 3; ToricCode.Y = 1 } ]

        match ToricCode.greedyMatching lattice exc with
        | Ok pairs ->
            Assert.Equal(1, pairs.Length)
            let (p1, p2) = pairs.[0]
            Assert.True((p1.X = 1 && p2.X = 3) || (p1.X = 3 && p2.X = 1), "Should pair the two excitations together")
    ...
    let ``correctionPath segments carry their actual edge types`` () =
        ...
        let horizontalCount =
            path |> List.filter (fun e -> e.Type = ToricCode.Horizontal) |> List.length

        let verticalCount =
            path |> List.filter (fun e -> e.Type = ToricCode.Vertical) |> List.length
        ...
        let dualHorizontal =
            dualPath |> List.filter (fun e -> e.Type = ToricCode.Horizontal) |> List.length

        let dualVertical =
            dualPath |> List.filter (fun e -> e.Type = ToricCode.Vertical) |> List.length
    ...
    let ``decodeVertexSyndrome after Z error produces matching`` () =
        ...
        if excitations.Length > 0 then
            Assert.True(excitations.Length % 2 = 0, $"Expected even number of excitations, got {excitations.Length}")
    ...
    let ``decodePlaquetteSyndrome after Z error produces matching`` () =
        ...
        if excitations.Length > 0 then
            ...
            (ToricCode.decodePlaquetteSyndrome lattice syndrome)
            |> Result.map (fun result -> Assert.True(result.MatchedPairs.Length > 0))
            |> Result.defaultWith (fun err -> Assert.Fail($"Expected Ok, got: {err.Message}"))
    ...
    let ``applyCorrections with empty corrections is identity`` () =
        ...
        corrected.Qubits
        |> Map.iter (fun edge qubit -> Assert.Equal(Map.find edge state.Qubits, qubit))
    ...
    let ``decodeSyndrome produces correction for single Z error`` () =
        ...
        | Ok(correctedState, vertexResult, plaquetteResult) ->
            ...
            let totalPairs =
                vertexResult.MatchedPairs.Length + plaquetteResult.MatchedPairs.Length
            ...
            if preElectric.Length > 0 then
                Assert.True(vertexResult.MatchedPairs.Length > 0, "Vertex decoder should pair e-particle excitations")
                Assert.True(vertexResult.Corrections.Length > 0, "Vertex decoder should produce correction chains")
        | Error err -> Assert.Fail($"Expected Ok, got: {err.Message}")
