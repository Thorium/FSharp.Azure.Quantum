...
module SU2kCompilationTests =
    ...
    let private unwrapResult (result: Result<'T, TopologicalError>) (context: string) : 'T =
        result
        |> Result.defaultWith (fun err -> failwith $"Expected Ok but got Error in {context}: {err.Message}")
    ...
    let ``computeSU2kSigmaMatrices returns Ok for SU(2)_3`` () =
        ...
        | Error err -> failwith $"Expected Ok but got Error: {err.Message}"
    ...
    let ``SU(2)_3 sigma1 is diagonal`` () =
        ...
        Assert.True(sigma1.B.Magnitude < 1e-10, $"sigma1.B should be zero, got {sigma1.B}")
        Assert.True(sigma1.C.Magnitude < 1e-10, $"sigma1.C should be zero, got {sigma1.C}")
    ...
    let ``SU(2)_3 sigma matrices are unitary`` () =
        ...
        let checkUnitary (name: string) (m: SolovayKitaev.SU2Matrix) =
            ...
            Assert.True(dist < 1e-10, $"{name} is not unitary: distance from identity = {dist}")
    ...
    let ``SU(2)_3 sigma matrices differ from Fibonacci sigma matrices`` () =
        ...
        Assert.True(dist1 > 0.01, $"SU(2)_3 sigma1 should differ from Fibonacci sigma1, but distance = {dist1}")

        let dist2 = SolovayKitaev.operatorDistance su2k_sigma2 fib_sigma2
        Assert.True(dist2 > 0.01, $"SU(2)_3 sigma2 should differ from Fibonacci sigma2, but distance = {dist2}")
    ...
    let ``SU(2)_3 sigma1 diagonal elements have correct phases from CFT`` () =
        ...
        Assert.True(diffA < 1e-10, $"sigma1.A = {sigma1.A}, expected {expectedR0} (diff = {diffA})")
        Assert.True(diffD < 1e-10, $"sigma1.D = {sigma1.D}, expected {expectedR1} (diff = {diffD})")
    ...
    let ``SU(2)_3 integer-spin sector reproduces Fibonacci R-symbols`` () =
        ...
        let r0 =
            unwrapResult
                (RMatrix.getRSymbol
                    rData
                    {
                        RMatrix.RMatrixIndex.A = spin1
                        RMatrix.RMatrixIndex.B = spin1
                        RMatrix.RMatrixIndex.C = spin0
                    })
                "R[1,1;0]"
        ...
        Assert.True((r0 - fibVacuum).Magnitude < 1e-10, $"R[1,1;0] = {r0}, expected Fibonacci R[τ,τ;1] = {fibVacuum}")
        Assert.True((r1 - fibTau).Magnitude < 1e-10, $"R[1,1;1] = {r1}, expected Fibonacci R[τ,τ;τ] = {fibTau}")
    ...
    let ``approximateGateSU2k approximates identity for SU(2)_3`` () =
        ...
        | Error err -> failwith $"Identity approximation failed: {err.Message}"
    ...
    let ``approximateGateSU2k approximates Hadamard for SU(2)_3`` () =
        ...
        match result with
        | Ok(braidOps, error) -> Assert.True(error < 0.7, $"Hadamard approximation error should be < 0.7, got {error}")
        | Error err -> failwith $"Hadamard approximation failed: {err.Message}"
    ...
    let ``approximateGateSU2k approximates T gate for SU(2)_3`` () =
        ...
        match result with
        | Ok(braidOps, error) -> Assert.True(error < 0.7, $"T gate approximation error should be < 0.7, got {error}")
        | Error err -> failwith $"T gate approximation failed: {err.Message}"
    ...
    let ``approximateGateSU2k approximates X gate for SU(2)_3`` () =
        ...
        match result with
        | Ok(braidOps, error) -> Assert.True(error < 0.7, $"X gate approximation error should be < 0.7, got {error}")
        | Error err -> failwith $"X gate approximation failed: {err.Message}"
    ...
    let ``approximateGateSU2k returns different braids than Fibonacci`` () =
        ...
        | Error err -> failwith $"SU(2)_3 approximation failed: {err.Message}"
    ...
    let ``compileGateSequence succeeds for SU(2)_3 with T gate`` () =
        ...
        let result =
            GateToBraid.compileGateSequence gateSeq 0.5 (AnyonSpecies.AnyonType.SU2Level 3)
        ...
        | Error err -> failwith $"SU(2)_3 T gate compilation should succeed but got: {err.Message}"
    ...
    let ``compileGateSequence succeeds for SU(2)_3 with H gate`` () =
        ...
        | Error err -> failwith $"SU(2)_3 H gate compilation should succeed but got: {err.Message}"
    ...
    let ``compileGateSequence succeeds for SU(2)_3 multi-gate circuit`` () =
        ...
        | Ok compilation ->
            ...
            Assert.True(
                compilation.CompilationWarnings.Length >= 2,
                "Should have warnings from H and T compilation notes"
            )
        | Error err -> failwith $"SU(2)_3 multi-gate compilation should succeed but got: {err.Message}"
    ...
    let ``compileGateSequence handles empty circuit for SU(2)_3`` () =
        ...
        | Error err -> failwith $"Empty circuit should succeed but got: {err.Message}"
    ...
    let ``compileGateSequence handles CNOT for SU(2)_3`` () =
        ...
        | Ok compilation ->
            ...
            Assert.Equal(AnyonSpecies.AnyonType.SU2Level 3, compilation.AnyonType)
        | Error err -> failwith $"SU(2)_3 CNOT compilation should succeed but got: {err.Message}"
    ...
    let ``compileGateSequence handles CZ for SU(2)_3`` () =
        ...
        result
        |> Result.map (fun compilation ->
            Assert.True(compilation.CompiledBraids.Length > 0, "CZ should produce braids"))
        |> Result.defaultWith (fun err -> failwith $"SU(2)_3 CZ compilation should succeed but got: {err.Message}")
    ...
    let ``compileGateSequence handles SWAP for SU(2)_3`` () =
        ...
        |> Result.defaultWith (fun err -> failwith $"SU(2)_3 SWAP compilation should succeed but got: {err.Message}")
    ...
    let ``compileGateSequence for SU(2)_4 includes non-universality warning`` () =
        ...
        let result =
            GateToBraid.compileGateSequence gateSeq 0.5 (AnyonSpecies.AnyonType.SU2Level 4)
    ...
    let ``compileGateSequence for SU(2)_5 succeeds without non-universality warning`` () =
        ...
        let result =
            GateToBraid.compileGateSequence gateSeq 0.5 (AnyonSpecies.AnyonType.SU2Level 5)
        ...
        | Ok compilation ->
            ...
            let hasNonUniversalWarning =
                compilation.CompilationWarnings
                |> List.exists (fun w ->
                    w.Contains("not universal", StringComparison.OrdinalIgnoreCase)
                    || w.Contains("non-universal", StringComparison.OrdinalIgnoreCase))

            Assert.False(
                hasNonUniversalWarning,
                $"SU(2)_5 should NOT warn about non-universality. Warnings: {compilation.CompilationWarnings}"
            )
        | Error err -> failwith $"SU(2)_5 H gate compilation should succeed but got: {err.Message}"
    ...
    let ``BraidingOperators element succeeds for SU(2)_3 particles`` () =
        ...
        let result =
            BraidingOperators.element halfSpin halfSpin vacuum (AnyonSpecies.AnyonType.SU2Level 3)

        match result with
        | Ok rValue ->
            // R[1/2,1/2;0] should be a unit-magnitude complex number
            Assert.True(
                abs (rValue.Magnitude - 1.0) < 1e-10,
                $"R-matrix element should have unit magnitude, got {rValue.Magnitude}"
            )
        | Error err -> failwith $"BraidingOperators.element should succeed for SU(2)_3 but got: {err.Message}"
    ...
    let ``BraidingOperators element for SU(2)_3 matches RMatrix module`` () =
        ...
        let braidResult =
            BraidingOperators.element halfSpin halfSpin spin1 (AnyonSpecies.AnyonType.SU2Level 3)

        // R[1/2,1/2;1] via RMatrix
        let rMatrixResult =
            RMatrix.computeRMatrix (AnyonSpecies.AnyonType.SU2Level 3)
            |> Result.bind (fun data ->
                let idx: RMatrix.RMatrixIndex =
                    {
        ...
        | Ok braidVal, Ok rMatVal ->
            ...
            Assert.True(diff < 1e-10, $"BraidingOperators ({braidVal}) should match RMatrix ({rMatVal}), diff = {diff}")
    ...
    let ``compileGateSequence handles RZ gate for SU(2)_3`` () =
        ...
        result
        |> Result.map (fun compilation -> Assert.True(compilation.CompiledBraids.Length > 0))
        |> Result.defaultWith (fun err -> failwith $"SU(2)_3 RZ compilation should succeed but got: {err.Message}")
