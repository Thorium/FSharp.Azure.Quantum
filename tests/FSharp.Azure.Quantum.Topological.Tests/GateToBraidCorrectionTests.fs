...
module GateToBraidCorrectionTests =
    ...
    let ``Z gate compilation has no special warnings`` () =
        ...
        | Ok decomp ->
            Assert.True(
                decomp.DecompositionNotes.IsNone,
    ...
    let ``T gate errors when compiled to braid (not exact for Ising anyons)`` () =
        ...
        (GateToBraid.tGateToBraid 0 2)
        |> Result.map (fun _ -> failwith "T gate should NOT compile to exact braid for Ising anyons")
        |> Result.defaultWith (fun err -> Assert.Contains("not exact", err.Message))

    [<Fact>]
    let ``T† gate errors when compiled to braid (not exact for Ising anyons)`` () =
        (GateToBraid.tDaggerGateToBraid 0 2)
        |> Result.map (fun _ -> failwith "T† gate should NOT compile to exact braid for Ising anyons")
        |> Result.defaultWith (fun err -> Assert.Contains("not exact", err.Message))
    ...
    let ``Rz zero angle produces empty braid`` () =
        (GateToBraid.rzGateToBraid 0 0.0 2 1e-10)
        |> Result.map (fun braid -> Assert.Empty(braid.Generators))
        |> Result.defaultWith (fun err -> failwith $"Unexpected error: {err.Message}")
    ...
    let ``Hadamard braid compilation returns explicit error (off-diagonal, unreachable)`` () =
        ...
        (GateToBraid.compileGateToBraid hGate 2 1e-10)
        |> Result.map (fun _ -> failwith "H braid compilation should fail explicitly for Ising anyons")
        |> Result.defaultWith (fun err -> Assert.Contains("diagonal", err.Message))
    ...
    let ``CNOT braid compilation returns explicit error (requires off-diagonal H)`` () =
        ...
        (GateToBraid.compileGateToBraid cnotGate 2 1e-3)
        |> Result.map (fun _ -> failwith "CNOT braid compilation should fail explicitly for Ising anyons")
        |> Result.defaultWith (fun err -> Assert.False(String.IsNullOrWhiteSpace err.Message))
