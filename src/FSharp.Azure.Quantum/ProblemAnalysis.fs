    ...
    let estimateQuantumAdvantage (input: 'T) : Result<QuantumAdvantage, string> =
        ...
        | Ok problemInfo ->
            ...
            let (recommendation, reasoning) =
                if n < 10 then
                    ("classical", $"Small problem (n={n}). Classical algorithms are faster due to quantum overhead.")
                elif speedup < 2.0 then
                    ("classical", $"Quantum speedup ({speedup:F2}x) does not justify quantum hardware costs.")
                elif speedup < 10.0 then
                    ("quantum", $"Moderate quantum advantage ({speedup:F2}x speedup). Consider quantum if available.")
                else
                    ("quantum",
