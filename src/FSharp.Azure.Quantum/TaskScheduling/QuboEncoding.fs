...
module QuboEncoding =
    ...
    let private durationToSlots (slotMinutes: float) (d: TimeSpan) : int =
        if slotMinutes <= 0.0 then
            1
        else
            max 1 (int (ceil (d.TotalMinutes / slotMinutes)))
    ...
    let private computePenaltyWeights
        ...
        let maxDuration =
            tasks
            |> List.map (fun t -> float (durationToSlots slotMinutes t.Duration))
            |> List.max
    ...
    let private buildOneHotTerms
        ...
        |> List.fold
            (fun acc quboMap -> quboMap |> Map.fold (fun acc2 key value -> addOrUpdate key value acc2) acc)
            Map.empty
                ...
                | Some predTask ->
                    ...
                    |> List.collect (fun t_pred ->
                        ...
                        |> List.choose (fun t_succ ->
                            match
                                Map.tryFind (predId, t_pred) varMapping, Map.tryFind (succId, t_succ) varMapping
                            with
                            | Some predVarIdx, Some succVarIdx ->
                                let (i, j) =
                                    if predVarIdx < succVarIdx then
                                        (predVarIdx, succVarIdx)
                                    else
                                        (succVarIdx, predVarIdx)
    ...
    let decodeBitstring (bitstring: int[]) (reverseMapping: Map<int, string * int>) : Map<string, float> =
        ...
        |> Array.choose (fun (i, bit) ->
            if bit = 1 then
                ...
                | None ->
                    ...
                    failwith
                        $"decodeBitstring: measurement bit {i} is set but has no QUBO variable mapping (bitstring length {bitstring.Length}, mapping size {reverseMapping.Count})"
            else
                None)
        |> Array.groupBy fst
    ...
    let decodeBitstringWithRepair (bitstring: int[]) (reverseMapping: Map<int, string * int>) : Map<string, float> =
        ...
        |> Array.choose (fun (i, bit) ->
            if bit = 1 then
                ...
                | None ->
                    failwith
                        $"decodeBitstringWithRepair: measurement bit {i} is set but has no QUBO variable mapping (bitstring length {bitstring.Length}, mapping size {reverseMapping.Count})"
            else
                None)
        |> Array.groupBy fst
        |> Array.map (fun (taskId, starts) -> (taskId, starts |> Array.map snd |> Array.min))
        |> Map.ofArray
    ...
    let toQubo
        ...
        if numTasks = 0 then
            ...
            let objectiveTerms =
                ...
                | MinimizeMakespan
                | MinimizeCost
                | MaximizeResourceUtilization -> buildObjectiveTerms problem.Tasks varMapping timeHorizon slotMinutes
                | MinimizeLateness -> buildLatenessTerms problem.Tasks varMapping timeHorizon slotMinutes

            let oneHotTerms =
                buildOneHotTerms problem.Tasks varMapping timeHorizon penaltyOneHot

            let dependencyTerms =
                buildDependencyTerms
                    problem.Tasks
