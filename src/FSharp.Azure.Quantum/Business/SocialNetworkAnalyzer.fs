...
module SocialNetworkAnalyzer =
    ...
    let private createPersonIndex (people: PersonId list) : Map<PersonId, int> =
        people |> List.mapi (fun i person -> (person, i)) |> Map.ofList
    ...
    let private calculateStrength (members: PersonId list) (connections: Connection list) : float =
        if members.Length < 2 then
            ...
            let internalConnections =
                connections
                |> List.filter (fun conn -> memberSet.Contains(conn.Person1) && memberSet.Contains(conn.Person2))
                |> List.length
            ...
            if maxConnections = 0 then
                ...
                float internalConnections / float maxConnections

    /// Find communities using quantum clique detection
    let private findCommunitiesQuantum
        (backend: IQuantumBackend)
        (problem: SocialNetworkProblem)
        : QuantumResult<Community list> =
        match problem.MinCommunitySize with
        | None -> Error(QuantumError.ValidationError("MinCommunitySize", "must be specified"))
        | Some size when size < 2 -> Error(QuantumError.ValidationError("MinCommunitySize", "must be at least 2"))
        | Some cliqueSize ->
    ...
    let private findLargestCommunityGrover
        (backend: IQuantumBackend)
        (problem: SocialNetworkProblem)
        : QuantumResult<Community list> =
        ...
        let rec trySize size =
            if size < 2 then
                ...
                let groverProblem =
                    { problem with
                        MinCommunitySize = Some size
                    }

                match findCommunitiesQuantum backend groverProblem with
                | Ok communities when not communities.IsEmpty -> Ok communities // Found a clique of this size
                | Ok _ -> trySize (size - 1) // Try smaller size
                | Error(QuantumError.ValidationError("CliqueSize", _)) -> trySize (size - 1) // CliqueSize > vertices, try smaller
                | Error err -> Error err // Propagate other errors
    ...
    type SocialNetworkBuilder() =
        ...
        member _.Person(problem: SocialNetworkProblem, personId: PersonId) : SocialNetworkProblem =
            { problem with
                People = personId :: problem.People
            }
        ...
        member _.Connection
            (problem: SocialNetworkProblem, person1: PersonId, person2: PersonId)
            : SocialNetworkProblem =
            let conn = { Person1 = person1; Person2 = person2 }

            { problem with
                Connections = conn :: problem.Connections
            }
        ...
        member _.Connections
            (problem: SocialNetworkProblem, connections: (PersonId * PersonId) list)
            : SocialNetworkProblem =
            let newConns =
                connections |> List.map (fun (p1, p2) -> { Person1 = p1; Person2 = p2 })

            { problem with
                Connections = newConns @ problem.Connections
            }
        ...
        member _.FindCommunities(problem: SocialNetworkProblem, size: int) : SocialNetworkProblem =
            { problem with
                MinCommunitySize = Some size
                Mode = Some(FindCommunities size)
            }
