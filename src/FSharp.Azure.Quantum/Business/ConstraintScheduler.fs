...
module ConstraintScheduler =
    ...
    let private createTaskIndex (tasks: TaskId list) : Map<TaskId, int> =
        tasks |> List.mapi (fun i task -> (task, i)) |> Map.ofList
    ...
    let private toMaxSat (problem: SchedulingProblem) : MaxSatConfig =
        ...
        let constraintClauses =
            ...
            |> List.collect (fun constraint' ->
                ...
                | Precedence _ -> [] // Would need temporal logic encoding, ignoring for now
            )
    ...
    let private toWeightedColoring (problem: SchedulingProblem) : WeightedColoringConfig =
        ...
        let colorCosts = problem.Resources |> List.map (fun res -> res.Cost) |> Array.ofList
    ...
    let private optimizeQuantumSat
    ...
    let private optimizeQuantumColoring
        (backend: IQuantumBackend)
        (problem: SchedulingProblem)
        : QuantumResult<Schedule option> =
            ...
            |> List.collect (fun constraint' ->
                ...
                | RequiresResource(task, resource) ->
                    ...
                    | Some t, Some r ->
                        ...
                        [
                            ({
                                Literals = [ ({ Variable = varId; IsNegated = false }: QuantumSatSolver.Literal) ]
                                Weight = 1.0
                            }
                            : QuantumSatSolver.Clause)
                        ]
    ...
    type ConstraintSchedulerBuilder() =
        ...
        member _.Resource(problem: SchedulingProblem, resourceId: ResourceId, cost: float) : SchedulingProblem =
            ...
            { problem with
                Resources = res :: problem.Resources
            }
        ...
        member _.ResourceWithCapacity
            (problem: SchedulingProblem, resourceId: ResourceId, cost: float, capacity: int)
            : SchedulingProblem =
            let res =
                {
                    Id = resourceId
        ...
        member _.Conflict(problem: SchedulingProblem, task1: TaskId, task2: TaskId) : SchedulingProblem =
            ...
            { problem with
                HardConstraints = constraint' :: problem.HardConstraints
            }
        ...
        member _.Prefer
            (problem: SchedulingProblem, task: TaskId, resource: ResourceId, weight: float)
            : SchedulingProblem =
            let constraint' = PreferResource(task, resource, weight)

            { problem with
                SoftConstraints = constraint' :: problem.SoftConstraints
            }
        ...
        member _.Shots(problem: SchedulingProblem, shots: int) : SchedulingProblem = { problem with Shots = shots }
