...
module AnomalyDetector =
    ...
    let private validate (problem: DetectionProblem) : QuantumResult<unit> =
        if problem.NormalData.Length = 0 then
            ...
            let allSameLength =
                problem.NormalData |> Array.forall (fun x -> x.Length = numFeatures)
    ...
    let private sensitivityToThreshold (_sensitivity: Sensitivity) : float = 0.5
    ...
    let train (problem: DetectionProblem) : QuantumResult<Detector> =
        ...
        |> Result.bind (fun () ->
            ...
            |> Result.map (fun kernelMatrix ->
                ...
                let rowMeans =
                    Array.init n (fun i -> (Array.init n (fun j -> kernelMatrix.[i, j]) |> Array.sum) / nf)

                let kernelMean = Array.sum rowMeans / nf

                // Centroid distance of every training point
                let trainDistances =
                    Array.init n (fun i -> sqrt (max 0.0 (kernelMatrix.[i, i] - 2.0 * rowMeans.[i] + kernelMean)))
                ...
                let distanceScale =
                    ...
                    let std =
                        sqrt (trainDistances |> Array.averageBy (fun d -> (d - mean) * (d - mean)))
    ...
    /// Save detector to file
    let save (path: string) (detector: Detector) : QuantumResult<unit> = saveDetector detector path

    /// Load detector from file
    let load (path: string) : QuantumResult<Detector> = loadDetector path
    ...
    type AnomalyDetectionBuilder() =
        ...
        member _.Combine(p1: DetectionProblem, p2: DetectionProblem) =
            { p2 with
                NormalData =
                    if p2.NormalData.Length = 0 then
                        p1.NormalData
                    else
                        p2.NormalData
            }
        ...
        member _.Backend(problem: DetectionProblem, backend: IQuantumBackend) = { problem with Backend = Some backend }

        /// <summary>Set the number of measurement shots.</summary>
        /// <param name="shots">Number of circuit measurements</param>
        [<CustomOperation("shots")>]
        member _.Shots(problem: DetectionProblem, shots: int) = { problem with Shots = shots }
        ...
        member _.Verbose(problem: DetectionProblem, verbose: bool) = { problem with Verbose = verbose }
        ...
        member _.SaveModelTo(problem: DetectionProblem, path: string) = { problem with SavePath = Some path }
        ...
        member _.Note(problem: DetectionProblem, note: string) = { problem with Note = Some note }
