...
module internal ProviderDataLoader =
    ...
    let tryGetProviderForPath (path: string) =
        ...
        | ".sdf"
        | ".mol" -> Some(SdfFileDatasetProvider(path) :> IMoleculeDatasetProvider)
    ...
    let private calculateFormula (atoms: MolecularData.Atom array) =
        ...
        |> Array.sortBy (fun (elem, _) ->
            ...
            | "H" -> "1H"
            | _ -> "2" + elem)
        |> Array.map (fun (elem, count) -> if count = 1 then elem else $"{elem}{count}")
        |> String.concat ""
...
module internal ScreeningScoring =
    ...
    let identifierAt (identifiers: string array) (i: int) =
        if i < identifiers.Length && not (System.String.IsNullOrWhiteSpace identifiers.[i]) then
            identifiers.[i]
        else
            $"Mol_{i}"
...
type QuantumDrugDiscoveryBuilder() =
    ...
    member private this.TrainVQCClassifier
        ...
        let initialParams =
            Array.init numParams (fun _ -> rng.NextDouble() * 2.0 * System.Math.PI)
        ...
        let items =
            ...
            |> Array.mapi (fun i feat ->
...

                {
                    ...
                    DrugDiscoverySolvers.DiverseSelection.Item.Cost = 1.0
                }) // Equal cost per molecule
            |> Array.toList
    ...
    member _.LoadCandidatesFromFile(state: DrugDiscoveryConfiguration, path: string) =
        { state with
            CandidatesPath = Some path
            CandidateSource = Some(FilePath path)
        }
    ...
    member _.LoadCandidatesFromProvider(state: DrugDiscoveryConfiguration, provider: IMoleculeDatasetProvider) =
        { state with
            CandidateSource = Some(Provider provider)
        }

    /// Load candidate molecules from an async dataset provider
    [<CustomOperation("load_candidates_from_provider_async")>]
    member _.LoadCandidatesFromProviderAsync
        (state: DrugDiscoveryConfiguration, provider: IMoleculeDatasetProviderAsync)
        =
        { state with
            CandidateSource = Some(ProviderAsync provider)
        }

    /// Select the screening strategy/method
    [<CustomOperation("use_method")>]
    member _.UseMethod(state: DrugDiscoveryConfiguration, method: ScreeningMethod) = { state with Method = method }
    ...
    member _.VQCLayers(state: DrugDiscoveryConfiguration, layers: int) = { state with VQCLayers = layers }
    ...
    member _.DiversityWeight(state: DrugDiscoveryConfiguration, weight: float) = { state with DiversityWeight = weight }
