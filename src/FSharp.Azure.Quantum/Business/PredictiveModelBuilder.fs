...
module PredictiveModel =
    ...
    type InternalModel =
        ...
        | MultiClassVQC of
            multiClassResult: VQC.MultiClassTrainingResult *
        ...
        if problem.TrainFeatures.Length = 0 then
            ...
            Error(
                QuantumError.ValidationError(
                    "Input",
                    $"Feature count ({problem.TrainFeatures.Length}) must match target count ({problem.TrainTargets.Length})"
                )
            )
    ...
    let saveAsync (path: string) (model: Model) (cancellationToken: CancellationToken) : Task<QuantumResult<unit>> =
        task {
            ...
            | RegressionVQC(result, featureMap, varForm, numQubits) ->
                ...
                return!
                    ModelSerialization.saveVQCRegressionTrainingResultAsync
                        path
                        result
                        numQubits
                        fmType
            ...
            | MultiClassVQC(multiClassResult, featureMap, varForm, numQubits) ->
                ...
                return!
                    ModelSerialization.saveVQCMultiClassTrainingResultAsync
                        path
                        multiClassResult
                        numQubits
                        fmType
            ...
            | HHLRegressor hhlResult ->
                return!
                    HHLModelSerialization.saveHHLRegressionResultAsync
                        path
                        hhlResult
                        model.Metadata.Note
                        cancellationToken
            ...
            | SVMMultiClass multiClassModel ->
                return!
                    SVMModelSerialization.saveMultiClassSVMModelAsync
                        path
                        multiClassModel
                        model.Metadata.Note
                        cancellationToken
        ...
        async {
            ...
            let transferResult =
                ...
                | Ok(parameters, _) when isNull parameters || parameters.Length = 0 ->
                    Error(
                        QuantumError.ValidationError("Input", "Not a VQC regression model: Parameters missing or empty")
                    )
                | other -> other
            ...
            | Error _ ->
                ...
                let multiClassVqcResult =
                    ...
                    | Ok m when
                        isNull m.Classifiers
                        || m.Classifiers.Length = 0
                        || isNull m.ClassLabels
                        || (m.Classifiers
                            |> Array.exists (fun c -> isNull c.Parameters || c.Parameters.Length = 0))
                        ->
                        Error(
                            QuantumError.ValidationError(
                                "Input",
                                "Not a multi-class VQC model: Classifiers missing or empty"
                            )
                        )
                    | other -> other
                ...
                | Ok multiClassModel ->
                    ...
                    return
                        Ok
                            {
                                InternalModel =
                                    MultiClassVQC(multiClassResult, featureMap, varForm, multiClassModel.NumQubits)
                                Metadata =
                                    {
                    ...
                    let hhlResult =
                        ...
                        | Ok r when isNull r.Weights || r.Weights.Length = 0 ->
                            Error(
                                QuantumError.ValidationError(
                                    "Input",
                                    "Not an HHL regression model: Weights missing or empty"
                                )
                            )
                        | other -> other
                    ...
                    | Error _ ->
                        ...
                        let svmResult =
                            ...
                            | Ok m when isNull m.TrainData || m.TrainData.Length = 0 ->
                                Error(
                                    QuantumError.ValidationError(
                                        "Input",
                                        "Not an SVM model: TrainData missing or empty"
                                    )
                                )
                            | other -> other
                        ...
                        | Error _ ->
                            ...
                            | Ok multiClassModel ->
                                let numFeatures =
                                    if
                                        multiClassModel.BinaryModels.Length > 0
                                        && multiClassModel.BinaryModels.[0].TrainData.Length > 0
                                    then
                                        multiClassModel.BinaryModels.[0].TrainData.[0].Length
                                    else
                                        0

                                let numSamples =
                                    if multiClassModel.BinaryModels.Length > 0 then
                                        multiClassModel.BinaryModels.[0].TrainData.Length
                                    else
                                        0
                            ...
                            | Error e ->
                                return
                                    Error(
                                        QuantumError.ValidationError(
                                            "Input",
                                            $"Failed to load model as VQC, multi-class VQC, HHL, or SVM: {e}"
                                        )
                                    )
        }
    ...
    type PredictiveModelBuilder() =
            ...
            { p2 with
                TrainFeatures =
                    if p2.TrainFeatures.Length = 0 then
                        ...
                        p2.TrainFeatures
                TrainTargets =
                    if p2.TrainTargets.Length = 0 then
                        p1.TrainTargets
                    else
                        p2.TrainTargets
            }
        ...
        member _.Architecture(problem: PredictionProblem, arch: Architecture) = { problem with Architecture = arch }
        ...
        member _.LearningRate(problem: PredictionProblem, lr: float) = { problem with LearningRate = lr }

        /// <summary>Set the maximum number of training epochs.</summary>
        /// <param name="epochs">Maximum epochs</param>
        [<CustomOperation("maxEpochs")>]
        member _.MaxEpochs(problem: PredictionProblem, epochs: int) = { problem with MaxEpochs = epochs }
        ...
        member _.Backend(problem: PredictionProblem, backend: IQuantumBackend) = { problem with Backend = Some backend }

        /// <summary>Set the number of measurement shots.</summary>
        /// <param name="shots">Number of circuit measurements</param>
        [<CustomOperation("shots")>]
        member _.Shots(problem: PredictionProblem, shots: int) = { problem with Shots = shots }
        ...
        member _.Verbose(problem: PredictionProblem, verbose: bool) = { problem with Verbose = verbose }
        ...
        member _.SaveModelTo(problem: PredictionProblem, path: string) = { problem with SavePath = Some path }
        ...
        member _.Note(problem: PredictionProblem, note: string) = { problem with Note = Some note }
