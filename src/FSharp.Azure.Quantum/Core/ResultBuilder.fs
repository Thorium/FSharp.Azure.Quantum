...
module ResultBuilder =
    ...
    type ResultBuilder() =
        ...
        member inline _.Bind(x: Result<'T, 'E>, f: 'T -> Result<'U, 'E>) : Result<'U, 'E> = Result.bind f x
        ...
        member inline _.Delay(f: unit -> Result<'T, 'E>) : unit -> Result<'T, 'E> = f

        /// Run delayed computation
        member inline _.Run(f: unit -> Result<'T, 'E>) : Result<'T, 'E> = f ()
...
module Result =
    ...
    let map (f: 'T -> 'U) (result: Result<'T, 'E>) : Result<'U, 'E> = result |> Result.map (fun x -> f x)
    ...
    let bind (f: 'T -> Result<'U, 'E>) (result: Result<'T, 'E>) : Result<'U, 'E> = result |> Result.bind (fun x -> f x)
    ...
    let unsafeGet (result: Result<'T, 'E>) : 'T =
        result
        |> Result.defaultWith (fun e -> raise (InvalidOperationException($"Result.unsafeGet called on Error: {e}")))
    ...
    let traverse (f: 'T -> Result<'U, 'E>) (items: 'T list) : Result<'U list, 'E> = items |> List.map f |> sequence
    ...
    let traverseArray (f: 'T -> Result<'U, 'E>) (items: 'T array) : Result<'U array, 'E> =
        items |> Array.map f |> sequenceArray
