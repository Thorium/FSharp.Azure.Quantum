...
module CircuitAbstraction =
    ...
    type CircuitWrapper(circuit: CircuitBuilder.Circuit) =
        interface ICircuit with
            ...
            member _.Description =
                $"Circuit with %d{circuit.QubitCount} qubits and %d{circuit.Gates.Length} gates"
    ...
    type QaoaCircuitWrapper(circuit: QaoaCircuit) =
        interface ICircuit with
            ...
            member _.Description =
                $"QAOA circuit with %d{circuit.NumQubits} qubits, %d{circuit.Layers.Length} layers"
    ...
    module CircuitAdapter =
        ...
        let private circuitBuilderGateToQaoaGate (gate: CircuitBuilder.Gate) : Result<QuantumGate list, string> =
            ...
            | CircuitBuilder.RYY(q1, q2, angle) ->
                ...
                Ok
                    [
                        QuantumGate.RX(q1, -Math.PI / 2.0)
                        QuantumGate.RX(q2, -Math.PI / 2.0)
                        QuantumGate.CNOT(q1, q2)
                        QuantumGate.RZ(q2, angle)
                        QuantumGate.CNOT(q1, q2)
                        QuantumGate.RX(q1, Math.PI / 2.0)
            ...
            | CircuitBuilder.MCZ _ ->
                ...
                Error
                    "MCZ (multi-controlled Z) gate cannot be converted to QAOA gates. Use LocalBackend for Grover's algorithm."
            | CircuitBuilder.Measure _ ->
                // Measurements are handled separately by the backend
                // Don't include in gate sequence
                Ok []
            | CircuitBuilder.Reset _ -> Error "Reset gate cannot be converted to QAOA gates (not unitary)"
            | CircuitBuilder.Conditional _ ->
                Error
                    "Conditional gates cannot be converted to QAOA gates (QAOA circuits have no classical control flow)"
        ...
        let circuitToQaoaCircuit (circuit: CircuitBuilder.Circuit) : Result<QaoaCircuit, QuantumError> =
            ...
            let gateResults = circuit.Gates |> List.rev |> List.map circuitBuilderGateToQaoaGate
        ...
        let qaoaCircuitToCircuit (qaoa: QaoaCircuit) : CircuitBuilder.Circuit =
            ...
            let layerGates =
                qaoa.Layers
                |> Array.collect (fun layer -> Array.append layer.CostGates layer.MixerGates)
                |> Array.toList
                |> List.collect qaoaGateToCircuitBuilderGates
        ...
        let toIonQCircuit (circuit: ICircuit) : Result<IonQBackend.IonQCircuit, QuantumError> =
            ...
            | None ->
                Error(
                    QuantumError.OperationError(
                        "Circuit extraction",
                        "Cannot extract CircuitBuilder.Circuit from ICircuit wrapper"
                    )
                )
            | Some builderCircuit ->
                // Convert all gates
        ...
        let tryGetQaoaCircuit (circuit: ICircuit) : QaoaCircuit option =
            ...
            | :? CircuitWrapper as wrapper ->
                (circuitToQaoaCircuit wrapper.Circuit)
                |> Result.map (fun qaoa -> Some qaoa)
    ...
    let wrapQaoaCircuit (circuit: QaoaCircuit) : ICircuit = QaoaCircuitWrapper(circuit) :> ICircuit
