...
module QaoaParameterOptimizer =
    ...
    let private optimizeSingleRun
        ...
        (tol: float)
        (progressReporter: Progress.IProgressReporter option)
        : OptimizationResult * (float[] * float) list =

        // Flatten parameters
        let flatParams =
            initialParams |> Array.collect (fun (gamma, beta) -> [| gamma; beta |])

        // Create bounds arrays
        let (gamma_min, gamma_max, beta_min, beta_max) = bounds

        let lowerBounds =
            Array.init flatParams.Length (fun i -> if i % 2 = 0 then gamma_min else beta_min)

        let upperBounds =
            Array.init flatParams.Length (fun i -> if i % 2 = 0 then gamma_max else beta_max)
        ...
        let trackedObjective (parameters: float[]) =
            ...
            progressReporter
            |> Option.iter (fun r -> r.Report(Progress.IterationUpdate(iterationCount.Value, maxIter, Some value)))
        ...
        let result =
            Optimizer.minimizeWithBounds trackedObjective flatParams lowerBounds upperBounds tol maxIter
    ...
    let private optimizeMultiStart
        ...
        let results =
            ...
            |> List.map (fun i ->
                ...
                | Some _ ->
                    let initialEnergy =
                        objectiveFunc (initialParams |> Array.collect (fun (g, b) -> [| g; b |]))
                ...
                let (result, history) =
                    optimizeSingleRun objectiveFunc initialParams bounds maxIter tol progressReporter

                logInfo
                    logger
                    $"  Start {i}/{numStarts}: Final energy = {result.FinalObjectiveValue:F4}, Converged = {result.Converged}"

                (result, history))

        // Select best result
        let (bestResult, bestHistory) =
            results |> List.minBy (fun (res, _) -> res.FinalObjectiveValue)
    ...
    let private optimizeLayerByLayer
        ...
        let (finalParams, allHistory) =
            [ 0 .. p - 1 ]
            |> List.fold
                (fun (accParams, accHistory) layerIdx ->
                    logInfo logger $"  Optimizing layer {layerIdx + 1}/{p}..."

                    // Initialize this layer
                    let layerParams =
                        initializeParameters 1 strategy (seed |> Option.map (fun s -> s + layerIdx + 1))
                    ...
                    let layerObjective (paramArray: float[]) =
                        ...
                        | _ ->
                            failwith
                                $"Layer objective expects exactly 2 parameters (gamma, beta), calling layerObjective with paramArray: {paramArray}"
                    ...
                    |> Option.iter (fun r ->
                        r.Report(
                            Progress.PhaseChanged(
                                "QAOA Layer-by-Layer",
                                Some $"Optimizing layer {layerIdx + 1}/{p}..."
                            )
                        ))
                    ...
                    let (result, history) =
                        optimizeSingleRun layerObjective initialLayerParams bounds (maxIter / p) tol progressReporter
                    ...
                    | [| gamma; beta |] ->
                        ...
                        logInfo
                            logger
                            $"    Layer {layerIdx + 1} optimized: gamma = {gamma:F4}, beta = {beta:F4}, Energy = {energy:F4}"
    ...
    let private optimizeAdaptive
        ...
        let (result1, history1) =
            optimizeSingleRun objectiveFunc initialParams bounds (maxIter / 2) tol progressReporter
        ...
        if result1.Converged then
            ...
            let (result2, history2) =
                optimizeMultiStart objectiveFunc p strategy seed bounds 3 (maxIter / 2) tol progressReporter logger
    ...
    let optimizeQaoaParameters
        ...
        |> Option.iter (fun r ->
            r.Report(
                Progress.PhaseChanged("QAOA Optimization", Some $"Starting {config.OptStrategy} with p={p} layers...")
            ))
        ...
        let objectiveFunc =
            createObjectiveFunction problemHam mixerHam backend config.NumShots config.Logger
        ...
        |> Option.iter (fun r ->
            r.Report(
                Progress.PhaseChanged(
                    "QAOA Complete",
                    Some $"Optimized to energy {result.FinalObjectiveValue:F6} ({statusMsg})"
                )
            ))
