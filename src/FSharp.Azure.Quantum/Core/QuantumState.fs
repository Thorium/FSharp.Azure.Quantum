...
type QuantumStateError =
    ...
    member this.Message =
        match this with
        | ConversionError(src, tgt, reason) -> $"Cannot convert {src} to {tgt}: {reason}"
        | UnsupportedOperation(op, stateType) -> $"Operation '{op}' not supported for {stateType} states"
        | InvalidState reason -> $"Invalid quantum state: {reason}"
        | NotImplemented feature -> $"Feature not yet implemented: {feature}"
...
module QuantumState =
    ...
    let private objToSeq (obj: obj) : obj seq =
        match obj with
        | :? System.Collections.IEnumerable as enumerable -> Seq.cast<obj> enumerable
        | _ -> Seq.empty
    ...
    let numQubits (state: QuantumState) : int =
        ...
        | QuantumState.DensityMatrix(_, n) -> n
        ...
        | QuantumState.IsingSamples(problem, _) ->
            ...
            let linearCoeffs =
                problemType.GetProperty("LinearCoeffs").GetValue problem :?> Map<int, float>
    ...
    /// ```
    let fromStateVector (sv: StateVector.StateVector) : QuantumState = QuantumState.StateVector sv
    ...
    let dimension (state: QuantumState) : int =
        ...
        if n > 30 then
            invalidOp
                $"State-space dimension 2^{n} does not fit in Int32; wide states (n > 30) have no materialisable dimension — consume the state via measure/probability or its histogram entries instead."
