...
module CircuitValidator =
    ...
    let validateGateSet (constraints: BackendConstraints) (circuit: CircuitStats) : Result<unit, ValidationError list> =
        ...
        | gates ->
            gates
            |> List.map (fun gate -> UnsupportedGate(gate, constraints.Name, constraints.SupportedGates))
            |> Error
        ...
        if constraints.HasAllToAllConnectivity then
            ...
            let invalidConnections =
                ...
                |> List.filter (fun (q1, q2) ->
                    ...
                    not (
                        constraints.ConnectedPairs.Contains(q1, q2)
            ...
            | connections ->
                connections
                |> List.map (fun (q1, q2) -> ConnectivityViolation(q1, q2, constraints.Name))
                |> Error
    ...
    let validateCircuit (constraints: BackendConstraints) (circuit: CircuitStats) : Result<unit, ValidationError list> =
        let results =
            [
                ...
                validateQubitCount constraints circuit |> Result.mapError (fun err -> [ err ])

                // Run gate set validation (multiple errors possible)
                validateGateSet constraints circuit

                // Run depth validation (single error)
                validateCircuitDepth constraints circuit |> Result.mapError (fun err -> [ err ])
    ...
    let formatValidationError (error: ValidationError) : string =
        ...
        | QubitCountExceeded(requested, limit, backend) ->
            sprintf
                "Circuit requires %d qubits but %s supports maximum %d qubits. Please reduce circuit size or choose a different backend."
                requested
                backend
                limit
        ...
        | CircuitDepthExceeded(depth, limit, backend) ->
            sprintf
                "Circuit depth of %d gates exceeds %s recommended limit of %d. Consider circuit optimization or decomposition."
                depth
                backend
                limit
        ...
        | ConnectivityViolation(q1, q2, backend) ->
            sprintf
                "Two-qubit gate between qubits %d and %d violates %s connectivity constraints. These qubits are not directly connected."
                q1
                q2
                backend

        | InvalidParameter message -> $"Invalid parameter: %s{message}"

    /// Format multiple validation errors into a summary message
    let formatValidationErrors (errors: ValidationError list) : string =
        let header =
            $"Circuit validation failed with %d{errors.Length} validation error(s):\n"

        let messages =
            errors
            |> List.mapi (fun i err -> sprintf "%d. %s" (i + 1) (formatValidationError err))
            |> String.concat "\n"
    ...
    module KnownTargets =
        ...
        let getConstraints (targetString: string) : BackendConstraints option =
            ...
            | "local"
            | "local.simulator"
            | "qaoa.local" -> Some(BackendConstraints.localSimulator ())

            // IonQ Targets
            | "ionq.simulator" -> Some(BackendConstraints.ionqSimulator ())
            | "ionq.qpu"
            | "ionq.qpu.aria-1"
            | "ionq.qpu.aria-2" -> Some(BackendConstraints.ionqHardware ())
