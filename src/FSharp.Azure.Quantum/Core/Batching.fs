...
module Batching =
        ...
        member _.Add(item: 'T) : 'T list option =
            ...
            lock lockObj (fun () ->
                ...
                if queue.Count >= config.MaxBatchSize then
                    ...
                    if batch.IsEmpty then None else Some batch
                else
                    None // Keep accumulating
            )
    ...
    module BatchMetrics =
        ...
        let averageBatchSize (metrics: BatchMetrics) : float =
            if metrics.BatchCount = 0 then
                0.0
            else
                float metrics.TotalCircuits / float metrics.BatchCount
        ...
        let getEfficiency (maxBatchSize: int) (metrics: BatchMetrics) : float =
            if metrics.BatchCount = 0 || maxBatchSize <= 0 then
                0.0
            else
    ...
    type BatchMetricsAccumulator() =
        ...
        member _.RecordBatch(batchSize: int, executionTimeMs: float) =
            lock lockObj (fun () -> metrics <- BatchMetrics.recordBatch batchSize executionTimeMs metrics)
        ...
        member _.GetSnapshot() : BatchMetrics = lock lockObj (fun () -> metrics)

        /// Total number of circuits processed (thread-safe)
        ///
        /// Note: Acquires lock on each access. For bulk property access, use GetSnapshot().
        member this.TotalCircuits = this.GetSnapshot().TotalCircuits

        /// Total number of batches submitted (thread-safe)
        ///
        /// Note: Acquires lock on each access. For bulk property access, use GetSnapshot().
        member this.BatchCount = this.GetSnapshot().BatchCount

        /// Total execution time across all batches (ms) (thread-safe)
        ///
        /// Note: Acquires lock on each access. For bulk property access, use GetSnapshot().
        member this.TotalExecutionTimeMs = this.GetSnapshot().TotalExecutionTimeMs

        /// Average batch size (thread-safe)
        ///
        /// Note: Acquires lock on each access. For bulk property access, use GetSnapshot().
        member this.AverageBatchSize = this.GetSnapshot() |> BatchMetrics.averageBatchSize
