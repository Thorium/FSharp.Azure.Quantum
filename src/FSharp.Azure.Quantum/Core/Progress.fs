...
module Progress =
    ...
    type ConsoleProgressReporter(?verbose: bool, ?cancellationToken: CancellationToken) =
        ...
        interface IProgressReporter with
            member _.Report(event: ProgressEvent) =
                if verbose then
                    match event with
                    | TrialStarted(id, total, modelType) -> printfn $"[Trial {id}/{total}] Starting: {modelType}"

                    | TrialCompleted(id, score, elapsed) ->
                        printfn $"[Trial {id}] ✓ Score: {score * 100.0:F2}%% ({elapsed:F1}s)"

                    | TrialFailed(id, error) -> printfn $"[Trial {id}] ✗ Failed: {error}"
    ...
    type EventProgressReporter() =
        ...
        member _.SetCancellationToken(token: CancellationToken) = cancellationToken <- Some token

        interface IProgressReporter with
            member _.Report(event: ProgressEvent) = progressEvent.Trigger event
    ...
    let createEventReporter () : EventProgressReporter = EventProgressReporter()
    ...
    let checkCancellation
        (reporter: IProgressReporter option)
