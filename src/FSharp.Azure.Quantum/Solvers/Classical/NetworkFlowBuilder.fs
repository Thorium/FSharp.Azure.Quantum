...
module NetworkFlow =
    ...
    type NetworkFlowProblem =
        { Nodes: Node list; Routes: Route list }
    ...
    let private toQuantumProblem (problem: NetworkFlowProblem) : QuantumNetworkFlowSolver.NetworkFlowProblem =
        ...
        // Build capacity map
        let capacities = problem.Nodes |> List.map (fun n -> n.Id, n.Capacity) |> Map.ofList
    ...
    let createRoute (from: string) (to_: string) (cost: float) : Route = { From = from; To = to_; Cost = cost }
