...
module MaxCut =
    ...
    let createProblem (vertices: string list) (edges: (string * string * float) list) : MaxCutProblem =
        let edgeList =
            edges
            |> List.map (fun (source, target, weight) -> GraphOptimization.edge source target weight)
    ...
    let starGraph (center: string) (spokes: string list) (weight: float) : MaxCutProblem =
        ...
        let edges = spokes |> List.map (fun spoke -> (center, spoke, weight))
    ...
    let solveWithAdaptQaoa
        ...
        try
            ...
            |> Result.bind (fun qubo ->
                FSharp.Azure.Quantum.Algorithms.AdaptQaoa.solveQubo
                    actualBackend
                    qubo.NumVariables
                    qubo.Q
                    FSharp.Azure.Quantum.Algorithms.AdaptQaoa.defaultConfig
        ...
        with ex ->
            Error(QuantumError.OperationError("MaxCut solveWithAdaptQaoa failed: ", $"Failed: {ex.Message}"))
