...
module QuantumArithmeticOps =
    ...
    let private validate (operation: ArithmeticOperation) : Result<unit, QuantumError> =
        ...
        // Check qubits
            ...
            // Check modulus for modular operations
                ...
                | ModularAdd
                | ModularMultiply
                | ModularExponentiate -> operation.Modulus.IsNone
                | Add
                | Multiply -> false
            then
            ...
            // Check modulus is larger than operands
                ...
                match operation.Operation with
                | ModularAdd
                | ModularMultiply when operation.Modulus.IsSome ->
                    operation.OperandA >= operation.Modulus.Value
                    || operation.OperandB >= operation.Modulus.Value
                | _ -> false
            then
                Error(
                    QuantumError.ValidationError(
                        "Operands",
                        "operands must be smaller than modulus for modular arithmetic"
                    )
                )
            ...
            // Check operand A fits in qubit count
                ...
                Error(
                    QuantumError.ValidationError(
                        "OperandA",
                        $"operandA ({operation.OperandA}) requires more than {operation.Qubits} qubits"
                    )
                )

            // Check operand B fits in qubit count (except for exponentiation where B is exponent)
            elif
                operation.Operation <> ModularExponentiate
                && operation.OperandB >= (1 <<< operation.Qubits)
            then
                Error(
                    QuantumError.ValidationError(
                        "OperandB",
                        $"operandB ({operation.OperandB}) requires more than {operation.Qubits} qubits"
                    )
                )
    ...
    type QuantumArithmeticBuilder() =
        ...
        member _.Qubits(operation: ArithmeticOperation, n: int) : ArithmeticOperation = { operation with Qubits = n }
        ...
        member _.Backend
            (operation: ArithmeticOperation, backend: BackendAbstraction.IQuantumBackend)
            : ArithmeticOperation =
            { operation with
            ...
            if bitIndex >= List.length registerQubits then
                ...
                if bit = 1 then
                    (backend.ApplyOperation
                        (QuantumOperation.Gate(CircuitBuilder.X registerQubits.[bitIndex]))
                        currentState)
                    |> Result.bind (fun nextState -> applyBits nextState (bitIndex + 1))
                else
    ...
    let private extractRegisterValue (registerQubits: int list) (state: QuantumState) (shots: int) : int =
        ...
        measurements
        |> Array.map (fun bitstring -> registerQubits |> List.mapi (fun pos q -> bitstring.[q] <<< pos) |> List.sum)
        |> Array.countBy id
        |> Array.maxBy snd
        |> fst
                                        ...
                                        result {
                                            ...
                                            return!
                                                shiftAndAdd
                                                    addResult.State
                                                    (bitIndex + 1)
                                                    (totalOps + addResult.OperationCount)
                                        }
                                    else
    ...
    let estimateResources (qubits: int) (operationType: OperationType) : string =
        let gateEstimate =
            ...
            | Add
            | ModularAdd -> qubits * qubits
            | Multiply
            | ModularMultiply -> qubits * qubits * 2
        ...
        let depthEstimate =
            ...
            | ModularAdd -> qubits * 2
            | Multiply
            | ModularMultiply -> qubits * 3
            | ModularExponentiate -> qubits * 10

        sprintf
            """Quantum Arithmetic Resource Estimate:
  Operation: %A
  Qubits Required: %d
  ...
  Feasibility: %s"""
    ...
    let describeResult (result: ArithmeticResult) : string =
        let modularText =
            if result.IsModular then
                "Yes (modular arithmetic)"
            else
                "No"

        sprintf
            """=== Quantum Arithmetic Result ===
Result Value: %d
Operation: %A
