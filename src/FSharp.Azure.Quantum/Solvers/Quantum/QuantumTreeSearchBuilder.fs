...
module QuantumTreeSearch =
    ...
    let validate (problem: TreeSearchProblem<'T>) : Result<unit, QuantumError> =
        if problem.InitialState.IsNone then
            ...
            Error(
                QuantumError.ValidationError("MaxDepth", "exceeds 8 (would require too many qubits for NISQ devices)")
            )
            ...
            if qubitsNeeded > 16 then
                Error(
                    QuantumError.ValidationError(
                        "TreeSearchSize",
                        $"requires {qubitsNeeded} qubits (depth={problem.MaxDepth}, branching={problem.BranchingFactor}). Max: 16"
                    )
                )
            else
                Ok()
    ...
    type QuantumTreeSearchBuilder<'T>() =
        ...
        member _.For(sequence: seq<'U>, body: 'U -> TreeSearchProblem<'T>) : TreeSearchProblem<'T> =
            ...
            sequence |> Seq.map body |> Seq.fold (fun _ current -> current) zero // Keep last state
        ...
        member _.Backend
            (problem: TreeSearchProblem<'T>, backend: BackendAbstraction.IQuantumBackend)
            : TreeSearchProblem<'T> =
        ...
        member _.SolutionThreshold(problem: TreeSearchProblem<'T>, threshold: float) : TreeSearchProblem<'T> =
            { problem with
                SolutionThreshold = Some threshold
            }
        ...
        member _.SuccessThreshold(problem: TreeSearchProblem<'T>, threshold: float) : TreeSearchProblem<'T> =
            { problem with
                SuccessThreshold = Some threshold
            }
        ...
        member _.LimitSearchSpace(problem: TreeSearchProblem<'T>, enable: bool) : TreeSearchProblem<'T> =
            ...
            if enable then
                let recommendedLimit =
                    GroverSearch.TreeSearch.recommendMaxPaths problem.MaxDepth problem.BranchingFactor

                { problem with
                    MaxPaths = recommendedLimit
                }
            else
        ...
        member _.OnProgress
            (problem: TreeSearchProblem<'T>, reporter: Progress.IProgressReporter)
            : TreeSearchProblem<'T> =
            { problem with
    ...
    let solve (problem: TreeSearchProblem<'T>) : Result<TreeSearchSolution, QuantumError> =
        ...
        try
            ...
            | Ok() ->
                ...
                let initialState =
                    ...
                    | None ->
                        failwith
                            $"InitialState must be set (should have been caught by validation), calling solve with problem: {problem}"
                ...
                |> Option.iter (fun r ->
                    let searchSpace =
                        GroverSearch.TreeSearch.estimateSearchSpaceSize problem.MaxDepth problem.BranchingFactor

                    r.Report(
                        Progress.PhaseChanged(
                            "Quantum Tree Search",
                            Some $"Searching {searchSpace} paths (depth={problem.MaxDepth})..."
                        )
                    ))
                ...
                | Error err ->
                    Error(
                        QuantumError.OperationError("QuantumTreeSearch", $"Quantum tree search failed: {err.Message}")
                    )
                | Ok treeResult ->
                    ...
                    |> Option.iter (fun r ->
                        r.Report(
                            Progress.PhaseChanged(
                                "Search Complete",
                                Some $"Found best move with score {treeResult.Score:F3}"
                            )
                        ))
        ...
        with ex ->
            Error(QuantumError.OperationError("TreeSearchSolver", $"Tree search solve failed: {ex.Message}"))
    ...
    let estimateResources (maxDepth: int) (branchingFactor: int) : string =
        ...
        let searchSpace =
            GroverSearch.TreeSearch.estimateSearchSpaceSize maxDepth branchingFactor

        sprintf
            """Tree Search Resource Estimate:
  Max Depth: %d
  Branching Factor: %d
  Qubits Required: %d
