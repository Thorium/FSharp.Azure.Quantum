    ...
    let toQubo (problem: PortfolioProblem) : Result<GraphOptimization.QuboMatrix, QuantumError> =
        try
            ...
            if numAssets = 0 then
                ...
                let penaltyWeight =
                    let maxReturn =
                        problem.Assets |> List.map (fun a -> abs a.ExpectedReturn) |> List.max
                ...
                let allTerms = objectiveTerms @ budgetConstraintTerms @ diversificationTerms
            ...
            try
                ...
                | Ok quboMatrix ->
                    ...
                    let handleMeasurements (measurements: int array array) =
                        ...
                        let portfolioResults = measurements |> Array.choose (decodeSolution problem)

                        if portfolioResults.Length = 0 then
                            Error(
                                QuantumError.OperationError(
                                    "DecodeSolution",
                                    "No valid portfolio solutions found in quantum measurements"
                                )
                            )
                        else
