...
module QuantumPhaseEstimator =
    ...
    type PhaseEstimatorBuilder() =
        ...
        member _.Backend
            (problem: PhaseEstimatorProblem, b: BackendAbstraction.IQuantumBackend)
            : PhaseEstimatorProblem =
    ...
    let estimate (problem: PhaseEstimatorProblem) : Result<PhaseEstimatorResult, QuantumError> =
        try
            ...
            | Ok() ->
                ...
                let eigenVector = problem.EigenVector |> Option.map QuantumState.fromStateVector
                ...
                | Ok result ->
                    ...
                    let unitaryName =
                        ...
                        | ModularExponentiation(baseNum, modulus) ->
                            $"Modular Exponentiation (a=%d{baseNum}, N=%d{modulus})"
                    ...
                    let builderResult =
                        {
                            ...
                            Success = true
                            Message =
                                $"Phase estimation successful: φ ≈ %.6f{phase} (eigenvalue λ = e^(2πi×%.6f{phase}))"
                        }

                    Ok builderResult

        with ex ->
            Error(QuantumError.OperationError("Phase estimation", ex.Message))
    ...
    let estimateTGate
        (p: int)
        (backend: BackendAbstraction.IQuantumBackend option)
        : Result<PhaseEstimatorProblem, QuantumError> =
        phaseEstimator {
            unitary TGate
            precision p
        }
    ...
    let estimateResources (p: int) (targetQubits: int) : string =
        ...
        sprintf
            """Quantum Phase Estimation Resource Estimate:
  ...
  Quantum Advantage: Polynomial O(n²) gate complexity"""
    ...
    let describeResult (result: PhaseEstimatorResult) : string =
        let eigenvalueStr =
            sprintf
                "λ = %.6f + %.6fi (magnitude: %.6f, angle: %.6f rad)"
