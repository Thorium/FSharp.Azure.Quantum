...
module QuantumLinearSystemSolver =
    ...
    let private validate (problem: LinearSystemProblem) : Result<unit, QuantumError> =
        ...
        // Check matrix is power of 2
            ...
            Error(
                QuantumError.ValidationError("MatrixDimension", $"must be power of 2, got {problem.Matrix.Dimension}")
            )
        ...
        // Check vector matches matrix
            ...
            Error(
                QuantumError.ValidationError(
                    "VectorDimension",
                    $"({problem.InputVector.Dimension}) must match matrix ({problem.Matrix.Dimension})"
                )
            )
    ...
    type LinearSystemSolverBuilder() =
        ...
        member _.Matrix(problem: LinearSystemProblem, elements: float list list) : LinearSystemProblem =
            ...
            (createHermitianMatrix array2D)
            |> Result.map (fun matrix -> { problem with Matrix = matrix })
        ...
        member _.DiagonalMatrix(problem: LinearSystemProblem, eigenvalues: float list) : LinearSystemProblem =
            (createDiagonalMatrix (List.toArray eigenvalues))
            |> Result.map (fun matrix -> { problem with Matrix = matrix })
            |> Result.defaultWith (fun err -> failwith err.Message)
        ...
        member _.Vector(problem: LinearSystemProblem, components: float list) : LinearSystemProblem =
            let complexComponents =
                components |> List.map (fun x -> Complex(x, 0.0)) |> List.toArray

            (createQuantumVector complexComponents)
            |> Result.map (fun vector -> { problem with InputVector = vector })
            |> Result.defaultWith (fun err -> failwith err.Message)
        ...
        member _.InversionMethod
            (problem: LinearSystemProblem, method: EigenvalueInversionMethod)
            : LinearSystemProblem =
            { problem with
        ...
        member _.Backend
            (problem: LinearSystemProblem, backend: BackendAbstraction.IQuantumBackend)
            : LinearSystemProblem =
        ...
        result {
            ...
            let conditionNumber =
                if hhlResult.EstimatedEigenvalues.Length > 0 then
                    let nonZero =
                        hhlResult.EstimatedEigenvalues |> Array.filter (fun x -> abs x > 1e-10)
    ...
    let solveDiagonal (eigenvalues: float list) (inputVector: float list) : Result<LinearSystemSolution, QuantumError> =
