...
module QuantumPeriodFinder =
    ...
    type PeriodFinderBuilder() =
        ...
        member _.Number(problem: PeriodFinderProblem, n: int) : PeriodFinderProblem = { problem with Number = n }

        /// Set random base for modular exponentiation
        [<CustomOperation("chosenBase")>]
        member _.ChosenBase(problem: PeriodFinderProblem, a: int) : PeriodFinderProblem = { problem with Base = Some a }

        /// Set QPE precision qubits
        [<CustomOperation("precision")>]
        member _.Precision(problem: PeriodFinderProblem, p: int) : PeriodFinderProblem = { problem with Precision = p }
        ...
        member _.Backend
            (problem: PeriodFinderProblem, backend: BackendAbstraction.IQuantumBackend)
            : PeriodFinderProblem =
    ...
    let breakRSA (rsaModulus: int) : Result<PeriodFinderProblem, QuantumError> =
        let recommendedPrecision = 2 * int (ceil (log (float rsaModulus) / log 2.0)) + 3
    ...
    let estimateResources (number: int) (precision: int) : string =
        ...
        sprintf
            """Quantum Period Finding Resource Estimate:
  Number to Factor: %d
  Precision Qubits: %d
