...
module MermaidRenderer =
    ...
    module Sequence =
        ...
        let private gateToLines gate =
            ...
            | CircuitGate g ->
                match g with
                // Single-qubit gates
                | CircuitBuilder.X qubit -> [ $"    Note over q{qubit}: X"; $"    q{qubit}->>q{qubit}: Apply X" ]
                | CircuitBuilder.Y qubit -> [ $"    Note over q{qubit}: Y"; $"    q{qubit}->>q{qubit}: Apply Y" ]
                | CircuitBuilder.Z qubit -> [ $"    Note over q{qubit}: Z"; $"    q{qubit}->>q{qubit}: Apply Z" ]
                | CircuitBuilder.H qubit -> [ $"    Note over q{qubit}: H"; $"    q{qubit}->>q{qubit}: Apply H" ]
                | CircuitBuilder.S qubit -> [ $"    Note over q{qubit}: S"; $"    q{qubit}->>q{qubit}: Apply S" ]
                | CircuitBuilder.SDG qubit -> [ $"    Note over q{qubit}: S†"; $"    q{qubit}->>q{qubit}: Apply S†" ]
                | CircuitBuilder.T qubit -> [ $"    Note over q{qubit}: T"; $"    q{qubit}->>q{qubit}: Apply T" ]
                | CircuitBuilder.TDG qubit -> [ $"    Note over q{qubit}: T†"; $"    q{qubit}->>q{qubit}: Apply T†" ]
                ...
                | CircuitBuilder.SWAP(q1, q2) ->
                    [ $"    Note over q{q1},q{q2}: SWAP"; $"    q{q1}<<->>q{q2}: Exchange" ]
                ...
                | CircuitBuilder.CCX(ctrl1, ctrl2, targ) ->
                    ...
                    | _ ->
                        failwith
                            $"Internal error: CCX should have exactly 3 qubits, calling gateToLines with gate: {gate}"
                | CircuitBuilder.MCZ(ctrls, targ) ->
                    ...
                    | [] ->
                        failwith
                            $"Internal error: MCZ should have at least 1 qubit, calling gateToLines with gate: {gate}"
                    | [ single ] ->
                        [
    ...
    module Flowchart =
        ...
        let private processGate (currentId, qubitStates: QubitState, lines) gate =
            ...
            | CircuitGate g ->
                ...
                | CircuitBuilder.RX(qubit, _)
                | CircuitBuilder.RY(qubit, _)
                | CircuitBuilder.RZ(qubit, _)
                | CircuitBuilder.P(qubit, _)
                | CircuitBuilder.U3(qubit, _, _, _)
                | CircuitBuilder.Conditional(qubit, _) ->
                    ...
                    let newStates =
                        qubitStates |> List.mapi (fun i s -> if i = qubit then nextStateId else s)

                    (nextId nextStateId, newStates, lines @ newLines)

                | CircuitBuilder.CNOT(ctrl, targ)
                | CircuitBuilder.CZ(ctrl, targ)
                | CircuitBuilder.CP(ctrl, targ, _)
                | CircuitBuilder.CRX(ctrl, targ, _)
                | CircuitBuilder.CRY(ctrl, targ, _)
                | CircuitBuilder.CRZ(ctrl, targ, _)
                | CircuitBuilder.RXX(ctrl, targ, _)
                | CircuitBuilder.RYY(ctrl, targ, _)
                ...
                | CircuitBuilder.MCZ(controls, target) ->
                    ...
                    let gateLabel =
                        match ctrlCount with
                        | 1 -> "CZ"
                        | 2 -> "CCZ"
                        | _ -> $"C{ctrlCount}Z"

                    let newLines =
                        [ $"    n{gId}[{gateLabel}]" ] @ inputLines @ [ $"    n{gId} --> n{ngId}" ]
                    ...
                    let newStates =
                        qubitStates
                        |> List.mapi (fun i s -> if Set.contains i mczQubits then nextGateId else s)
                ...
                | CircuitBuilder.Measure qubit ->
                    ...
                    let newLines = [ $"    n{mIdVal}[📊 Measure]"; $"    n{prevId} --> n{mIdVal}" ]
                ...
                | CircuitBuilder.Reset qubit ->
                    ...
                    let newLines = [ $"    n{rIdVal}[🔄 Reset |0⟩]"; $"    n{prevId} --> n{rIdVal}" ]
        ...
        let render numQubits gates =
            ...
            let (_, _, allLines) =
                gates |> List.fold processGate (startId, startStates, initLines)

            [ "```mermaid"; "flowchart LR"; yield! allLines; "```" ] |> String.concat "\n"
    ...
    module Graph =
        ...
        let private nodeToLines (node: Node) =
            ...
            | Some color -> [ nodeLine; $"    style {node.Name} fill:#{color}" ]
            | None -> [ nodeLine ]
        ...
        let nodeWithColorAndLabel name color label =
            {
                Name = name
                Color = Some color
                Label = Some label
        ...
        let weightedEdge from' to' weight =
            {
                From = from'
                To = to'
                Label = None
