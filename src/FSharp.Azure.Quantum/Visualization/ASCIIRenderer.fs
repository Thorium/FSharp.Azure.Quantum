...
module ASCIIRenderer =
    ...
    let private gateBox (label: string) : GateBox =
...

        {
            Top =
                BoxChars.topLeft
                + String.replicate width BoxChars.horizontal
                + BoxChars.topRight
    ...
    let private syncWires (wires: Wire list) : Wire list =
        ...
        |> List.map (fun wire ->
            ...
            if diff > 0 then
                wire @ List.replicate diff BoxChars.horizontal
            else
                wire)
        ...
        | CircuitGate g ->
            ...
            | CircuitBuilder.Reset qubit ->
                ...
                |> List.mapi (fun i wire ->
                    if i = qubit then
                        ...
                        wire
                        @ [ String.replicate box.Width BoxChars.horizontal ]
                        @ List.replicate 3 BoxChars.horizontal)
            ...
            | CircuitBuilder.CCX(ctrl1, ctrl2, targ) ->
                ...
                let (minQ, maxQ) =
                    ...
                    | _ ->
                        failwith
                            $"Internal error: CCX should have exactly 3 qubits, calling addGate with numQubits: {numQubits}, wires: {wires}, gate: {gate}"
            ...
            | CircuitBuilder.MCZ(ctrls, targ) ->
                ...
                let (minQ, maxQ) =
                    ...
                    | [] ->
                        failwith
                            $"Internal error: MCZ should have at least 1 qubit, calling addGate with numQubits: {numQubits}, wires: {wires}, gate: {gate}"
            ...
            | CircuitBuilder.Barrier qubits ->
                wires'
                |> List.mapi (fun i wire ->
                    if List.contains i qubits then
                        wire @ [ BoxChars.barrier; BoxChars.barrier; BoxChars.barrier ]
                    else
                        wire @ List.replicate 3 BoxChars.horizontal)
    ...
    let render (numQubits: int) (gates: VisualizationGate list) : string =
        ...
        finalWires |> List.map (fun wire -> String.concat "" wire) |> String.concat "\n"
