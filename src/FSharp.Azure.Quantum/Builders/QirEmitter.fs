...
module QirEmitter =
    ...
    let private qubitRef (q: int) =
        $"%%Qubit* inttoptr (i64 %d{q} to %%Qubit*)"

    let private resultRef (r: int) =
        $"%%Result* inttoptr (i64 %d{r} to %%Result*)"
    ...
    let private dbl (x: float) =
        sprintf "0x%016X" (BitConverter.DoubleToInt64Bits x)
    ...
    let private lowerGate (nextResult: unit -> int) (gate: Gate) : Result<string list * string list, string> =
        ...
        let rot declKey intrinsic (q: int) (theta: float) =
            Ok(
                [
                    sprintf "  call void @__quantum__qis__%s(double %s, %s)" intrinsic (dbl theta) (qubitRef q)
                ],
                [ declKey ]
            )
        // two-qubit
        let two declKey intrinsic a b =
            Ok(
                [
                    sprintf "  call void @__quantum__qis__%s(%s, %s)" intrinsic (qubitRef a) (qubitRef b)
                ],
                [ declKey ]
            )
        ...
        | Measure q ->
            ...
            Ok(
                [
                    sprintf "  call void @__quantum__qis__mz__body(%s, %s)" (qubitRef q) (resultRef r)
        ...
        | other ->
            Error(
                sprintf
                    "QIR base profile has no direct intrinsic for gate '%s'; run GateTranspiler first"
                    (getGateName other)
            )
    ...
    let emit (circuit: Circuit) : Result<string, string> =
        ...
        | Ok(bodyLines, usedDeclKeys) ->
            ...
            let declLines =
                usedDeclKeys |> Set.toList |> List.choose (fun k -> Map.tryFind k declarations)
            ...
            line (
                sprintf
                    "attributes #0 = { \"entry_point\" \"qir_profiles\"=\"base_profile\" \"required_num_qubits\"=\"%d\" \"required_num_results\"=\"%d\" \"output_labeling_schema\" }"
                    numQubits
                    resultCount
            )
