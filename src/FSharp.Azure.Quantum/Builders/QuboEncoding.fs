...
type Variable = { Name: string; VarType: VariableType }
...
module VariableEncoding =
    ...
    let qubitCount (encoding: VariableEncoding) : int =
        ...
        | OneHot n ->
            if n < 1 then
                invalidArg (nameof encoding) "OneHot requires at least 1 option"

            n
        | DomainWall n ->
            if n < 2 then
                invalidArg (nameof encoding) "DomainWall requires at least 2 levels"
        ...
        | BoundedInteger(min, max) ->
            ...
            if range <= 1 then
                ...
                int (System.Math.Ceiling(System.Math.Log(float range, 2.0)))
    ...
    let encode (encoding: VariableEncoding) (value: int) : int list =
        ...
        | BoundedInteger(min, max) ->
            ...
            List.init numQubits (fun i -> if (normalizedValue &&& (1 <<< i)) <> 0 then 1 else 0)
    ...
    let decode (encoding: VariableEncoding) (bits: int list) : int =
        ...
        | OneHot _ ->
            ...
            bits |> List.tryFindIndex ((=) 1) |> Option.defaultValue 0

        | DomainWall _ ->
            // Count number of 1s and add 1
            bits |> List.sumBy id |> (+) 1

        | BoundedInteger(min, _) ->
            // Convert binary (LSB first) to integer
            bits |> List.indexed |> List.sumBy (fun (i, bit) -> bit * (1 <<< i)) |> (+) min
...
type QuboMatrix =
    ...
    member this.GetCoefficient(i: int, j: int) : float = this.Coefficients.[i, j]

type VariableAssignment = { Name: string; Value: int }
...
module QuboEncoding =
    ...
    let private qubitNamesFor (varName: string) (varType: VariableType) : string list =
        ...
        | CategoricalVar categories -> categories |> List.map (fun cat -> $"%s{varName}_%s{cat}")
    ...
    let encodeVariables (variables: Variable list) : QuboMatrix =
        ...
        let allQubitNames =
            variables |> List.collect (fun v -> qubitNamesFor v.Name v.VarType)
    ...
    let encodeVariablesWithCustomConstraints
        ...
        let applyInequalityConstraint indices maxVal =
            ...
            |> List.iter (fun i -> qubo.Coefficients.[i, i] <- qubo.Coefficients.[i, i] + penaltyWeight)

        // Apply each constraint functionally
        constraints
        |> List.iter (fun constr ->
            match constr with
            | EqualityConstraint(indices, target) -> applyEqualityConstraint indices target
            | InequalityConstraint(indices, maxVal) -> applyInequalityConstraint indices maxVal)
...
module ProblemTransformer =
    ...
    let private customTransformations =
        System.Collections.Generic.Dictionary<string, CustomTransformation>()
    ...
    let applyTransformation (problemName: string) (problemData: obj) : QuboMatrix =
        ...
        | false, _ -> failwith $"Problem '%s{problemName}' not registered. Call registerProblem first."
    ...
    let recommendStrategy (problemType: string) (problemSize: int) : EncodingStrategy =
        ...
        | "portfolio"
        | "portfolio-optimization" -> EncodingStrategy.CorrelationBased // Risk modeling via covariance
