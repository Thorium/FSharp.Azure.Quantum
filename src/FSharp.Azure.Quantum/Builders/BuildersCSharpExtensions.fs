...
module BuildersCSharpExtensions =
    ...
    [<Extension>]
    let NodesFromEnumerable
        (builder: GraphOptimization.GraphOptimizationBuilder<'TNode, 'TEdge>)
        (nodes: IEnumerable<GraphOptimization.Node<'TNode>>)
        =
    ...
    let NodesFromArray
        (builder: GraphOptimization.GraphOptimizationBuilder<'TNode, 'TEdge>)
        (nodes: GraphOptimization.Node<'TNode>[])
        =
    ...
    [<Extension>]
    let EdgesFromEnumerable
        (builder: GraphOptimization.GraphOptimizationBuilder<'TNode, 'TEdge>)
        (edges: IEnumerable<GraphOptimization.Edge<'TEdge>>)
        =
    ...
    let EdgesFromArray
        (builder: GraphOptimization.GraphOptimizationBuilder<'TNode, 'TEdge>)
        (edges: GraphOptimization.Edge<'TEdge>[])
        =
    ...
    [<Extension>]
    let ToEnumerable (fsharpList: 'T list) : IEnumerable<'T> = fsharpList :> IEnumerable<'T>

    /// <summary>Convert F# list to List&lt;T&gt; (C# List).</summary>
    [<Extension>]
    let ToCSharpList (fsharpList: 'T list) : ResizeArray<'T> = ResizeArray<'T>(fsharpList)

    /// <summary>Convert F# list to T[] array.</summary>
    [<Extension>]
    let ToArrayExt (fsharpList: 'T list) : 'T[] = List.toArray fsharpList

    /// <summary>Get count of F# list (C# helper - avoids ListModule.Length).</summary>
    [<Extension>]
    let Count (fsharpList: 'T list) : int = List.length fsharpList
    ...
    [<Extension>]
    let MapResult (result: Result<'T, 'E>) (mapper: Func<'T, 'U>) : Result<'U, 'E> = Result.map mapper.Invoke result
    ...
    [<Extension>]
    let IsOk (result: Result<'T, 'E>) : bool = result |> Result.isOk

    /// <summary>Check if Result is Error (C# helper).</summary>
    [<Extension>]
    let IsError (result: Result<'T, 'E>) : bool = result |> Result.isError
    ...
    let GetOkValue (result: Result<'T, 'E>) : 'T =
        result
        |> Result.defaultWith (fun _ -> invalidOp "Cannot get Ok value from Error result")
    ...
    let GetErrorValue (result: Result<'T, 'E>) : 'E =
        result
        |> Result.map (fun _ -> invalidOp "Cannot get Error value from Ok result")
        |> Result.defaultWith id
    ...
    let ValueTuplesToFSharpList (valueTuples: struct (string * float)[]) : (string * float) list =
        valueTuples |> Array.map (fun struct (a, b) -> (a, b)) |> Array.toList
    ...
    let FSharpListToValueTuples (fsharpTuples: (string * float) list) : struct (string * float)[] =
        fsharpTuples |> List.map (fun (a, b) -> struct (a, b)) |> List.toArray
...
type CSharpBuilders private () =
    ...
    static member Node<'T when 'T: equality>(id: string, value: 'T) = GraphOptimization.node id value

    /// <summary>Create a graph node with properties.</summary>
    static member Node<'T when 'T: equality>
        (id: string, value: 'T, [<ParamArray>] properties: struct (string * obj)[])
        =
        let fsharpProps =
            properties |> Array.map (fun struct (k, v) -> (k, v)) |> Array.toList
    ...
    static member MaxCutProblem(vertices: string[], edges: struct (string * string * float)[]) =
        let edgeList =
            edges |> Array.map (fun struct (s, t, w) -> (s, t, w)) |> Array.toList
    ...
    static member KnapsackProblem(items: struct (string * float * float)[], capacity: float) =
        let itemList =
            items |> Array.map (fun struct (id, w, v) -> (id, w, v)) |> Array.toList
    ...
    static member TspProblem(cities: struct (string * float * float)[]) =
        let cityList =
            cities |> Array.map (fun struct (name, x, y) -> (name, x, y)) |> Array.toList
    ...
    static member QuantumTreeSearch<'T>
        (initialState: 'T, evaluator: Func<'T, float>, moveGenerator: Func<'T, IEnumerable<'T>>)
        =
        QuantumTreeSearch.simple initialState evaluator.Invoke (fun state -> moveGenerator.Invoke(state) |> Seq.toList)
    ...
    static member GameAISearch<'T>
        (board: 'T, depth: int, branching: int, evaluator: Func<'T, float>, legalMoves: Func<'T, IEnumerable<'T>>)
        =
        QuantumTreeSearch.forGameAI board depth branching evaluator.Invoke (fun state ->
            legalMoves.Invoke(state) |> Seq.toList)
    ...
    static member DecisionProblem<'T>
        (
            initialDecision: 'T,
            steps: int,
            optionsPerStep: int,
            scorer: Func<'T, float>,
            nextOptions: Func<'T, IEnumerable<'T>>
        ) =
        QuantumTreeSearch.forDecisionProblem initialDecision steps optionsPerStep scorer.Invoke (fun state ->
            nextOptions.Invoke(state) |> Seq.toList)
    ...
    static member QuantumConstraintSolver<'T>
        (searchSpaceSize: int, domain: 'T[], singleConstraint: Func<IDictionary<int, 'T>, bool>)
        =
        let domainList = Array.toList domain

        let constraintFunc (m: Map<int, 'T>) =
            singleConstraint.Invoke(m :> IDictionary<int, 'T>)
    ...
    static member SudokuStyleConstraints<'T>
        (gridSize: int, domain: 'T[], [<ParamArray>] constraints: Func<IDictionary<int, 'T>, bool>[])
        =
        let domainList = Array.toList domain

        let constraintList =
            constraints
            |> Array.map (fun f -> fun (m: Map<int, 'T>) -> f.Invoke(m :> IDictionary<int, 'T>))
            |> Array.toList
    ...
    /// <param name="domain">Array of valid values</param>
    /// <param name="constraints">Array of constraint predicates</param>
    static member NQueensConstraints<'T>
        (boardSize: int, domain: 'T[], [<ParamArray>] constraints: Func<IDictionary<int, 'T>, bool>[])
        =
        let domainList = Array.toList domain
    ...
    static member QpeExactnessExact() : Algorithms.QPE.Exactness = Algorithms.QPE.Exactness.Exact
    ...
    /// <param name="precision">Number of precision qubits (optional, defaults to 8)</param>
    /// <returns>Period finder problem to solve</returns>
    static member FactorIntegerWithBase
        (number: int, baseValue: int, [<Optional; DefaultParameterValue(8)>] precision: int)
        =
    ...
    static member FactorIntegerWithBase
        (number: int, baseValue: int, precision: int, exactness: Algorithms.QPE.Exactness)
        =
        QuantumPeriodFinder.factorIntegerWithBase number baseValue precision
        |> Result.map (fun problem -> { problem with Exactness = exactness })
    ...
    /// <returns>Period finder problem to solve</returns>
    static member BreakRSA(rsaModulus: int) = QuantumPeriodFinder.breakRSA rsaModulus
    ...
    static member ExecutePeriodFinder
        (problem: QuantumPeriodFinder.PeriodFinderProblem, exactness: Algorithms.QPE.Exactness)
        =
    ...
    static member ExecutePhaseEstimator
        (problem: QuantumPhaseEstimator.PhaseEstimatorProblem, exactness: Algorithms.QPE.Exactness)
        =
    ...
    static member CoverageOption(id: string, coveredElements: int[], cost: float) : CoverageOptimizer.CoverageOption =
        {
            Id = id
            CoveredElements = Array.toList coveredElements
            Cost = cost
        }
    ...
    static member PairingCompatibility
        (participant1: string, participant2: string, weight: float)
        : ResourcePairing.Compatibility =
        {
    ...
    static member PackingItem(id: string, size: float) : PackingOptimizer.PackingItem = { Id = id; Size = size }
...
module QuantumBackendCSharpExtensions =
    ...
    [<Extension>]
    let GetName (backend: IQuantumBackend) : string = backend.Name
