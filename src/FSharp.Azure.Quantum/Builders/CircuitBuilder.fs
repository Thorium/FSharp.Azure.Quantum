...
module CircuitBuilder =
    ...
    let empty qubitCount : Circuit = { QubitCount = qubitCount; Gates = [] }

    /// Gets the number of gates in a circuit
    let gateCount (circuit: Circuit) : int = circuit.Gates.Length

    /// Gets the number of qubits in a circuit
    let qubitCount (circuit: Circuit) : int = circuit.QubitCount
    ...
    let addGates (gates: Gate list) (circuit: Circuit) : Circuit =
        { circuit with
            Gates = (List.rev gates) @ circuit.Gates
        }

    /// Adds a measurement operation to the end of a circuit
    let addMeasurement (qubit: int) (circuit: Circuit) : Circuit = addGate (Measure qubit) circuit
    ...
    let compose (circuit1: Circuit) (circuit2: Circuit) : Circuit =
        if circuit1.QubitCount <> circuit2.QubitCount then
            failwith
                $"Cannot compose circuits with different qubit counts, calling compose with circuit1: {circuit1}, circuit2: {circuit2}"
    ...
    let getGates (circuit: Circuit) : Gate list = List.rev circuit.Gates
    ...
    let optimize (circuit: Circuit) : Circuit =
        let rec optimizeGates (gates: Gate list) : Gate list =
            ...
            | H q1 :: H q2 :: rest when q1 = q2 -> optimizeGates rest
            // Remove consecutive X gates on same qubit
            | X q1 :: X q2 :: rest when q1 = q2 -> optimizeGates rest
            // Remove S and SDG pairs on same qubit
            | S q1 :: SDG q2 :: rest when q1 = q2 -> optimizeGates rest
            | SDG q1 :: S q2 :: rest when q1 = q2 -> optimizeGates rest
            // Remove T and TDG pairs on same qubit
            | T q1 :: TDG q2 :: rest when q1 = q2 -> optimizeGates rest
            | TDG q1 :: T q2 :: rest when q1 = q2 -> optimizeGates rest
            // Remove consecutive SWAP gates on same qubits
            | SWAP(q1a, q1b) :: SWAP(q2a, q2b) :: rest when (q1a = q2a && q1b = q2b) || (q1a = q2b && q1b = q2a) ->
                optimizeGates rest
            // Fuse consecutive RX gates on same qubit
            | RX(q1, angle1) :: RX(q2, angle2) :: rest when q1 = q2 -> optimizeGates (RX(q1, angle1 + angle2) :: rest)
            // Fuse consecutive RY gates on same qubit
            | RY(q1, angle1) :: RY(q2, angle2) :: rest when q1 = q2 -> optimizeGates (RY(q1, angle1 + angle2) :: rest)
            // Fuse consecutive RZ gates on same qubit
            | RZ(q1, angle1) :: RZ(q2, angle2) :: rest when q1 = q2 -> optimizeGates (RZ(q1, angle1 + angle2) :: rest)
            // Keep gate and continue
            | gate :: rest -> gate :: optimizeGates rest
    ...
    let validate (circuit: Circuit) : Validation.ValidationResult =
        ...
        let rec validateGate (gate: Gate) : string list =
            ...
            | TDG q
            | Measure q
            | Reset q -> validateQubit q |> Option.toList
            | RX(q, _)
            | RY(q, _)
            | RZ(q, _)
            | P(q, _)
            | U3(q, _, _, _) -> validateQubit q |> Option.toList
            | CNOT(control, target)
            | CZ(control, target)
            | CP(control, target, _)
            | CRX(control, target, _)
            | CRY(control, target, _)
            | CRZ(control, target, _) ->
                let errors = []
        ...
        let allErrors = circuit.Gates |> List.rev |> List.collect validateGate
    ...
    type CircuitBuilderCE() =
        ...
        member _.Yield(_) : Circuit = { QubitCount = 0; Gates = [] }

        /// <summary>Yield an existing circuit (enables yield! syntax)</summary>
        member _.YieldFrom(circuit: Circuit) : Circuit = circuit
        ...
        member _.Zero() : Circuit = { QubitCount = 0; Gates = [] }
        ...
        member _.Qubits(circuit: Circuit, count: int) : Circuit = { circuit with QubitCount = count }
        ...
        member _.P(circuit: Circuit, qubit: int, angle: float) : Circuit =
            { circuit with
                Gates = (P(qubit, angle)) :: circuit.Gates
            }
        ...
        member _.P(circuit: Circuit, qubitAndAngle: int * float) : Circuit =
            { circuit with
                Gates = (P qubitAndAngle) :: circuit.Gates
            }
        ...
        member _.RX(circuit: Circuit, qubit: int, angle: float) : Circuit =
            { circuit with
                Gates = (RX(qubit, angle)) :: circuit.Gates
            }
        ...
        member _.RX(circuit: Circuit, qubitAndangle: int * float) : Circuit =
            { circuit with
                Gates = (RX qubitAndangle) :: circuit.Gates
            }
        ...
        member _.RY(circuit: Circuit, qubit: int, angle: float) : Circuit =
            { circuit with
                Gates = (RY(qubit, angle)) :: circuit.Gates
            }
        ...
        member _.RY(circuit: Circuit, qubitAndAngle: int * float) : Circuit =
            { circuit with
                Gates = (RY qubitAndAngle) :: circuit.Gates
            }
        ...
        member _.RZ(circuit: Circuit, qubit: int, angle: float) : Circuit =
            { circuit with
                Gates = (RZ(qubit, angle)) :: circuit.Gates
            }
        ...
        member _.RZ(circuit: Circuit, qubitAndAngle: int * float) : Circuit =
            { circuit with
                Gates = (RZ qubitAndAngle) :: circuit.Gates
            }
        ...
        member _.CNOT(circuit: Circuit, control: int, target: int) : Circuit =
            { circuit with
                Gates = (CNOT(control, target)) :: circuit.Gates
            }
        ...
        member _.CNOT(circuit: Circuit, controlAndTarget: int * int) : Circuit =
            { circuit with
                Gates = (CNOT controlAndTarget) :: circuit.Gates
            }
        ...
        member _.CZ(circuit: Circuit, control: int, target: int) : Circuit =
            { circuit with
                Gates = (CZ(control, target)) :: circuit.Gates
            }
        ...
        member _.CZ(circuit: Circuit, controlAndTarget: int * int) : Circuit =
            { circuit with
                Gates = (CZ controlAndTarget) :: circuit.Gates
            }
        ...
        member _.CP(circuit: Circuit, control: int, target: int, angle: float) : Circuit =
            { circuit with
                Gates = (CP(control, target, angle)) :: circuit.Gates
            }
        ...
        member _.CP(circuit: Circuit, controlTargetAngle: int * int * float) : Circuit =
            { circuit with
                Gates = (CP controlTargetAngle) :: circuit.Gates
            }
        ...
        member _.SWAP(circuit: Circuit, qubit1: int, qubit2: int) : Circuit =
            { circuit with
                Gates = (SWAP(qubit1, qubit2)) :: circuit.Gates
            }
        ...
        member _.SWAP(circuit: Circuit, qubit1and2: int * int) : Circuit =
            { circuit with
                Gates = (SWAP qubit1and2) :: circuit.Gates
            }
        ...
        member _.RXX(circuit: Circuit, qubit1: int, qubit2: int, angle: float) : Circuit =
            { circuit with
                Gates = (RXX(qubit1, qubit2, angle)) :: circuit.Gates
            }
        ...
        member _.RXX(circuit: Circuit, qubitsAndAngle: int * int * float) : Circuit =
            { circuit with
                Gates = (RXX qubitsAndAngle) :: circuit.Gates
            }
        ...
        member _.RYY(circuit: Circuit, qubit1: int, qubit2: int, angle: float) : Circuit =
            { circuit with
                Gates = (RYY(qubit1, qubit2, angle)) :: circuit.Gates
            }
        ...
        member _.RYY(circuit: Circuit, qubitsAndAngle: int * int * float) : Circuit =
            { circuit with
                Gates = (RYY qubitsAndAngle) :: circuit.Gates
            }
        ...
        member _.RZZ(circuit: Circuit, qubit1: int, qubit2: int, angle: float) : Circuit =
            { circuit with
                Gates = (RZZ(qubit1, qubit2, angle)) :: circuit.Gates
            }
        ...
        member _.RZZ(circuit: Circuit, qubitsAndAngle: int * int * float) : Circuit =
            { circuit with
                Gates = (RZZ qubitsAndAngle) :: circuit.Gates
            }
        ...
        member _.CCX(circuit: Circuit, control1: int, control2: int, target: int) : Circuit =
            { circuit with
                Gates = (CCX(control1, control2, target)) :: circuit.Gates
            }
        ...
        member _.CCX(circuit: Circuit, control1control2target: int * int * int) : Circuit =
            { circuit with
                Gates = (CCX control1control2target) :: circuit.Gates
            }
        ...
        member _.Measure(circuit: Circuit, qubit: int) : Circuit =
            { circuit with
                Gates = (Measure qubit) :: circuit.Gates
            }
        ...
        member _.Reset(circuit: Circuit, qubit: int) : Circuit =
            { circuit with
                Gates = (Reset qubit) :: circuit.Gates
            }
        ...
        member _.Barrier(circuit: Circuit, qubits: int list) : Circuit =
            { circuit with
                Gates = (Barrier qubits) :: circuit.Gates
            }
    ...
    let singleGate (gate: Gate) : Circuit = { QubitCount = 0; Gates = [ gate ] }
    ...
    let (|+) (circuit: Circuit) (gate: Gate) : Circuit = addGate gate circuit

    /// Apply circuit transformation
    /// Example: circuit |>> optimize |>> validate
    let (|>>) (circuit: Circuit) (transform: Circuit -> Circuit) : Circuit = transform circuit
    ...
    let mapGates (f: Gate -> Gate) (circuit: Circuit) : Circuit =
        { circuit with
            Gates = List.map f circuit.Gates
        }
    ...
    let filterGates (predicate: Gate -> bool) (circuit: Circuit) : Circuit =
        { circuit with
            Gates = List.filter predicate circuit.Gates
        }
    ...
    let reverse (circuit: Circuit) : Circuit =
        ...
        { circuit with
            Gates = circuit.Gates |> List.map inverseGate |> List.rev
        }
    ...
    let bellState (q1: int) (q2: int) : Circuit =
        empty (max q1 q2 + 1) |> addGate (H q1) |> addGate (CNOT(q1, q2))
    ...
    let inverseQFT (qubits: int list) : Circuit = qft qubits |> reverse
    ...
    let depth (circuit: Circuit) : int =
        ...
        let rec buildLayers
            (remainingGates: Gate list)
            (currentLayer: Gate list)
            (occupiedQubits: Set<int>)
            (currentDepth: int)
            : int =
            match remainingGates with
            | [] ->
                // Finished all gates - return final depth
                if List.isEmpty currentLayer then
                    currentDepth
                else
                    currentDepth + 1
    ...
    let optimizeWithCommutation (circuit: Circuit) : Circuit =
        ...
        { circuit with
            Gates = optimizeLoop circuit.Gates 10
        }
    ...
    let removeIdentities (circuit: Circuit) : Circuit =
        let isIdentity (gate: Gate) : bool =
            ...
            | RX(_, angle)
            | RY(_, angle)
            | RZ(_, angle) when abs angle < 1e-10 -> true
            | P(_, angle) when abs angle < 1e-10 -> true
            | CP(_, _, angle) when abs angle < 1e-10 -> true
            | _ -> false

        { circuit with
            Gates = circuit.Gates |> List.filter (not << isIdentity)
        }
