...
module QaoaSimulator =
    ...
    let applyCostLayer
        ...
        if costCoefficients.Length <> numQubits then
            failwith
                $"Cost coefficients array length ({costCoefficients.Length}) must match number of qubits ({numQubits})"

        // Apply Rz(2*gamma*cᵢ) to each qubit based on its cost coefficient
        costCoefficients
        |> Array.indexed
        |> Array.fold
            (fun currentState (i, coeff) ->
    ...
    let applyQaoaLayer
        ...
        state |> applyCostLayer gamma costCoefficients |> applyMixerLayer beta
    ...
    let runQaoaCircuit
        ...
        |> Array.fold (fun state (gamma, beta) -> applyQaoaLayer gamma beta costCoefficients state) initialState
    ...
    let computeCostExpectation (costCoefficients: float[]) (state: StateVector.StateVector) : float =

        let numQubits = StateVector.numQubits state

        if costCoefficients.Length <> numQubits then
            failwith
                $"Cost coefficients array length must match number of qubits, calling computeCostExpectation with costCoefficients: {costCoefficients}, state: {state}"
    ...
    let simulate (circuit: QaoaCircuit) (numShots: int) : Result<QaoaResult, string> =
            ...
            if circuit.NumQubits < 1 || circuit.NumQubits > 16 then
                ...
                let bitstringCounts =
                    ...
                    |> Seq.map (fun (basisIndex, count) ->
                        let bitstring =
                            Convert.ToString((basisIndex: int), 2).PadLeft(circuit.NumQubits, '0')
        ...
        with ex ->
            Error $"QAOA simulation failed: {ex.Message}"
