        ...
        try
            if not (File.Exists filePath) then
                ...
                logInfo
                    ...
                    (sprintf
                        "Features: %d"
                        (if model.TrainData.Length > 0 then
                             model.TrainData.[0].Length
                         else
                             0))
                ...
                if not (File.Exists filePath) then
                    ...
                    let binaryModelsResult =
                        ...
                        |> Array.fold
                            (fun state item ->
                                Result.bind
                                    (fun models -> Result.map (fun model -> Array.append models [| model |]) item)
                                    state)
                            (Ok [||])
