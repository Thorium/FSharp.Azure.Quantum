...
module DataPreprocessing =
    ...
    let applyMinMaxNormalization
        ...
        |> Array.map (fun sample ->
            ...
            |> Array.mapi (fun i value ->
                let range = maxValues.[i] - minValues.[i]
                if range = 0.0 then 0.5 else (value - minValues.[i]) / range))
    ...
    let normalizeToRange
        ...
        let scaledData =
            normalized01
            |> Array.map (fun sample -> sample |> Array.map (fun v -> minVal + v * (maxVal - minVal)))
    ...
    let stratifiedTrainTestSplit
        ...
        if data.Length = 0 || labels.Length = 0 then
            ...
            let classSamples =
                ...
                |> Array.groupBy fst
                |> Array.map (fun (classLabel, samples) -> (classLabel, samples |> Array.map snd))
