...
module VariationalForms =
        ...
        if parameters.Length <> expectedParams then
            Error(
                QuantumError.ValidationError(
                    "Input",
                    $"RealAmplitudes requires {expectedParams} parameters for {numQubits} qubits and depth {depth}, got {parameters.Length}"
                )
            )
        else
            let (finalCircuit, _) =
                    ...
                    (fun (circ, paramIdx) layer ->
                        ...
                        let circWithCz =
                            if layer < depth - 1 || depth = 1 then
                                [ 0 .. numQubits - 2 ]
                                |> List.fold (fun c qubit -> addGate (CZ(qubit, qubit + 1)) c) circWithRy
                            else
                                circWithRy
    ...
    let twoLocal
        ...
        if parameters.Length <> expectedParams then
            Error(
                QuantumError.ValidationError(
                    "Input",
                    $"TwoLocal requires {expectedParams} parameters for {numQubits} qubits and depth {depth}, got {parameters.Length}"
                )
            )
        else
            // Parse rotation type
                    ...
                    (fun (circ, paramIdx) layer ->
                        ...
                        let circWithEnt =
                            if layer < depth - 1 || depth = 1 then
                                [ 0 .. numQubits - 2 ]
                                |> List.fold
                                    (fun c qubit -> addGate (entanglementGate qubit (qubit + 1)) c)
                                    circWithRot
                            else
                                circWithRot
    ...
    let efficientSU2 (depth: int) (parameters: float array) (numQubits: int) : QuantumResult<Circuit> =
        ...
        if parameters.Length <> expectedParams then
            Error(
                QuantumError.ValidationError(
                    "Input",
                    $"EfficientSU2 requires {expectedParams} parameters (2 per qubit) for {numQubits} qubits and depth {depth}, got {parameters.Length}"
                )
            )
        else
    ...
    let randomParameters (ansatz: VariationalForm) (numQubits: int) (seed: int option) : float array =
    ...
    let composeWithFeatureMap (featureMapCircuit: Circuit) (variationalCircuit: Circuit) : QuantumResult<Circuit> =

        if featureMapCircuit.QubitCount <> variationalCircuit.QubitCount then
            Error(
                QuantumError.ValidationError(
                    "Input",
                    $"Qubit count mismatch: feature map has {featureMapCircuit.QubitCount} qubits, variational form has {variationalCircuit.QubitCount} qubits"
                )
            )
        else
            try
