...
module FeatureMap =
    ...
    let zzFeatureMap (depth: int) (features: FeatureVector) : Circuit =
        ...
        |> List.fold
            (fun circ layer ->
                ...
                let circWithH =
                    [ 0 .. numQubits - 1 ] |> List.fold (fun c i -> addGate (H i) c) circ
                ...
                |> List.fold
                    (fun c i ->
                        ...
                        c |> addGate (CNOT(i, j)) |> addGate (RZ(j, angle)) |> addGate (CNOT(i, j)))
                    circWithRz)
            (empty numQubits)
    ...
    let pauliFeatureMap (pauliStrings: string list) (depth: int) (features: FeatureVector) : Circuit =
        ...
        let applyPauliRotation circ pauliIdx (pauliStr: string) =
            ...
            | "ZZ" when numQubits >= 2 -> circ |> addGate (CNOT(0, 1)) |> addGate (RZ(1, angle)) |> addGate (CNOT(0, 1))
            | "XX" when numQubits >= 2 ->
                circ
                |> addGate (H 0)
                |> addGate (H 1)
        ...
        |> List.fold
            (fun circ layer ->
                ...
                pauliStrings
                |> List.indexed
                |> List.fold (fun c (pauliIdx, pauliStr) -> applyPauliRotation c pauliIdx pauliStr) circWithH)
            (empty numQubits)
    ...
    let amplitudeEncoding (features: FeatureVector) : Circuit =
        ...
        let computeAngles (amps: float array) : float array =
            ...
            Array.init halfLen (fun i ->
                ...
                if norm < 1e-15 then 0.0 else 2.0 * Math.Atan2(b, a))
        ...
        let trailingZeros n =
            let rec go d pos =
                if d <= 1 then pos else go (d >>> 1) (pos + 1)
    ...
    let buildFeatureMap (featureMapType: FeatureMapType) (features: FeatureVector) : QuantumResult<Circuit> =
        try
            let circuit =
                ...
                | AngleEncoding -> angleEncoding features

                | ZZFeatureMap depth -> zzFeatureMap depth features

                | PauliFeatureMap(paulis, depth) -> pauliFeatureMap paulis depth features

                | AmplitudeEncoding -> amplitudeEncoding features
