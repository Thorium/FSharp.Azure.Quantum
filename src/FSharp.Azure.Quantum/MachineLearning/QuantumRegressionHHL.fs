...
module QuantumRegressionHHL =
    ...
    let private computeGramMatrix (X: float array array) : float[,] =
        ...
        Array2D.init nFeatures nFeatures (fun i j -> [ 0 .. nSamples - 1 ] |> List.sumBy (fun k -> X[k][i] * X[k][j]))
    ...
    let private computeMomentVector (X: float array array) (y: float array) : float array =
        ...
        Array.init nFeatures (fun i -> [ 0 .. nSamples - 1 ] |> List.sumBy (fun k -> X[k][i] * y[k]))
    ...
    let private calculateMSE (yTrue: float array) (yPred: float array) : float =
        Array.zip yTrue yPred |> Array.averageBy (fun (yt, yp) -> (yt - yp) ** 2.0)
        ...
        if config.TrainX.Length = 0 then
            ...
            Error(
                QuantumError.ValidationError(
                    "Input",
                    $"Sample count mismatch: X has {config.TrainX.Length} samples, y has {config.TrainY.Length}"
                )
            )
        ...
        // Dimension checks for quantum hardware
            ...
            if totalQubits > 20 then
                ...
                Error(
                    QuantumError.ValidationError(
                        "Input",
                        $"Matrix dimension {paddedDim}×{paddedDim} exceeds maximum (1024×1024)"
                    )
                )
            else
                Ok()
                ...
                | Ok hermitianMatrix ->
                    ...
                    | Ok quantumVector ->
                        ...
                        | Ok hhlResult ->
                            ...
                            let numerator =
                                Array.zip config.TrainY predictions_unscaled
                                |> Array.sumBy (fun (yi, pi) -> yi * pi)

                            let denominator = predictions_unscaled |> Array.sumBy (fun pi -> pi * pi)

                            let scaleFactor = if denominator > 1e-10 then numerator / denominator else 1.0
                            ...
                            let predictions =
                                X
                                |> Array.map (fun row -> Array.zip row weights |> Array.sumBy (fun (xi, wi) -> xi * wi))
