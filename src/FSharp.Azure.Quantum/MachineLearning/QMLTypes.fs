            ...
            if features.Length <> expected then
                Error(
                    QuantumError.ValidationError(
                        "Input",
                        $"AmplitudeEncoding requires {expected} features for {numQubits} qubits, got {features.Length}"
                    )
                )
            else
                Ok()
        | AngleEncoding ->
            if features.Length <> numQubits then
                Error(
                    QuantumError.ValidationError(
                        "Input",
                        $"AngleEncoding requires {numQubits} features for {numQubits} qubits, got {features.Length}"
                    )
                )
            else
                Ok()
