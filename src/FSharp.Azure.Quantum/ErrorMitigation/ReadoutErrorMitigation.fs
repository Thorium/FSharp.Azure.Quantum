...
module ReadoutErrorMitigation =
    ...
    let withConfidenceLevel (level: float) (config: REMConfig) : REMConfig = { config with ConfidenceLevel = level }

    /// Override clip negative flag (fluent API).
    let withClipNegative (clip: bool) (config: REMConfig) : REMConfig = { config with ClipNegative = clip }
    ...
    let private bitstringToInt (bitstring: string) : int = Convert.ToInt32(bitstring, 2)
        ...
        try
            ...
            if abs det < 1e-10 then
                ...
                if conditionNumber > 1000.0 then
                    ...
                    logWarning
                        logger
                        ($"[WARN] High condition number (%.1f{conditionNumber}) - inversion may be unstable")
                ...
                let result =
                    Array2D.init inverse.RowCount inverse.ColumnCount (fun i j -> inverse.[i, j])

                Ok result
        with ex ->
            Error $"Matrix inversion failed: %s{ex.Message}"
    ...
    let correctReadoutErrors
        ...
        let dimensionMismatch =
            measured
            |> Map.exists (fun bitstring _ -> bitstring.Length <> calibration.Qubits)

        if dimensionMismatch then
            Error
                $"Calibration is for %d{calibration.Qubits} qubits but the measured histogram contains bitstrings of a different width. Supply a calibration matching the measured qubit count."
        else
            ...
            | Ok inverse ->
                try
                    ...
                    let totalShots = measured |> Map.toList |> List.sumBy snd |> float
                ...
                with ex ->
                    Error $"Histogram correction error: %s{ex.Message}"
    ...
    let validateCalibrationMatrix (calibration: CalibrationMatrix) : Result<unit, string> =
        try
            ...
            if rowCount <> expectedDim || colCount <> expectedDim then
                Error(
                    sprintf
                        "Expected %dx%d matrix for %d qubits, got %dx%d"
                        expectedDim
                        expectedDim
                        calibration.Qubits
                        rowCount
                ...
                let sumErrors =
                    [|
                        for j in 0 .. colCount - 1 do
                            let colSum =
                                [| for i in 0 .. rowCount - 1 -> calibration.Matrix.[i, j] |] |> Array.sum
        ...
        with ex ->
            Error $"Validation error: %s{ex.Message}"
