...
module ZeroNoiseExtrapolation =
        ...
        if numPairsToInsert = 0 || originalGateCount = 0 then
            ...
            let rec insertPairs gateList pairsLeft counter acc =
                ...
                | gate :: rest, pairs ->
                    ...
                    let shouldInsert = pairs > 0 && counter >= insertEveryN && rest.Length > 0
    ...
    let fitPolynomial (degree: int) (measurements: (float * float) list) : float list =
        if measurements.Length < degree + 1 then
            failwith (
                sprintf
                    "Need at least %d measurements for degree-%d polynomial (got %d)"
                    (degree + 1)
                    degree
                    measurements.Length
            )
        ...
        let coefficients =
            MathNet.Numerics.Fit.Polynomial(noiseLevels, expectations, degree)
    ...
    let withMinSamples (samples: int) (config: ZNEConfig) : ZNEConfig = { config with MinSamples = samples }
            ...
            if ssTot = 0.0 then
                ...
                let ssRes =
                    ...
                    |> List.sumBy (fun (x, y) ->
                        ...
                        let yPred =
                            coefficients |> List.mapi (fun i coef -> coef * (x ** float i)) |> List.sum
