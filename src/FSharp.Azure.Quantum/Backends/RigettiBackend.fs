...
module RigettiBackend =
    ...
    let serializeGate (gate: QuilGate) : string =
        match gate with
        | SingleQubit(gateName, qubit) -> $"%s{gateName} %d{qubit}"

        | SingleQubitRotation(gateName, angle, qubit) -> sprintf "%s(%s) %d" gateName (angle.ToString "R") qubit

        | TwoQubit(gateName, control, target) -> $"%s{gateName} %d{control} %d{target}"

        | Measure(qubit, memoryRef) -> $"MEASURE %d{qubit} %s{memoryRef}"

        | DeclareMemory(name, typ, size) -> $"DECLARE %s{name} %s{typ}[%d{size}]"
    ...
    let isValidGate (graph: ConnectivityGraph) (gate: QuilGate) : bool =
        ...
        | TwoQubit(_, control, target) ->
            ...
            graph.Edges.Contains(control, target) || graph.Edges.Contains(target, control)
    ...
    let validateProgram (graph: ConnectivityGraph) (program: QuilProgram) : QuantumResult<unit> =
        ...
        let invalidGate = allGates |> List.tryFind (isValidGate graph >> not)
        ...
        | Some gate ->
            ...
            Error(
                QuantumError.ValidationError(
                    "Connectivity",
                    $"Gate '%s{gateText}' violates connectivity constraints. Qubits are not directly connected in the hardware topology."
                )
            )
    ...
    let createJobSubmission (program: QuilProgram) (shots: int) (target: string) (name: string option) : JobSubmission =
    ...
    let mapRigettiError (errorCode: string) (errorMessage: string) : QuantumError =
        ...
        | "InvalidProgram" -> QuantumError.ValidationError("circuit", $"Malformed Quil program: %s{errorMessage}")
        | "TopologyError" -> QuantumError.ValidationError("circuit", $"Connectivity violation: %s{errorMessage}")
        | "TooManyQubits" -> QuantumError.ValidationError("circuit", $"Qubit limit exceeded: %s{errorMessage}")
        | "QuotaExceeded" -> QuantumError.AzureError(AzureQuantumError.QuotaExceeded "quantum-credits")
        | _ ->
            QuantumError.AzureError(AzureQuantumError.UnknownError(0, $"Rigetti error %s{errorCode}: %s{errorMessage}"))
                                        ...
                                        Error(
                                            QuantumError.AzureError(
                                                AzureQuantumError.UnknownError(
                                                    0,
                                                    $"Failed to parse Rigetti results: %s{ex.Message}"
                                                )
                                            )
                                        )
