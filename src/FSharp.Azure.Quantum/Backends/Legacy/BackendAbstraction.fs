...
module BackendAbstraction =
    ...
    type IQuantumBackend =
        /// Set the cancellation token for subsequent operations
        /// 
        /// Parameters:
                ...
                try
                    ...
                    if numShots <= 0 then
                        ...
                        | :? CircuitWrapper as wrapper ->
                            ...
                            // RNG for measurements (fixed seed per shot for reproducibility)
                            let rng = Random()
                            
                            // Helper: Apply a single gate to state (with measurement support)
                            let applyGate state gate =
                                ...
                                | CircuitBuilder.Measure q ->
                                    // Perform realistic mid-circuit measurement with state collapse
