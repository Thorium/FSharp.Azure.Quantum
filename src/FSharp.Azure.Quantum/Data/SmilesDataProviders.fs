...
module SmilesDataProviders =
        ...
        let topology: MoleculeTopology =
            {
                ...
                Bonds = mol.Bonds |> Array.map (fun b -> (b.Atom1, b.Atom2, Some(float b.Order)))
                Charge = mol.Atoms |> Array.sumBy (fun a -> a.Charge) |> Some
                Multiplicity = Some 1 // Assume singlet for SMILES
                Metadata = [ "smiles", mol.Smiles; "formula", mol.Formula ] |> Map.ofList
            }
                        ...
                        if matching.Length > 0 then
                            ...
                            Error(
                                QuantumError.ValidationError(
                                    "MoleculeNotFound",
                                    $"No molecule with SMILES '{name}' found"
                                )
                            ))
                | ByPath _ ->
                    Error(
                        QuantumError.ValidationError(
                            "UnsupportedQuery",
                            "CsvSmilesDatasetProvider does not support path queries"
                        )
                    )
                | ByCategory _ ->
                    Error(
                        QuantumError.ValidationError(
                            "UnsupportedQuery",
                            "CsvSmilesDatasetProvider does not support category queries"
                        )
                    )
            ...
            member _.Load(query: DatasetQuery) =
                ...
                | ByPath _ ->
                    Error(
                        QuantumError.ValidationError(
                            "UnsupportedQuery",
                            "SmilesListDatasetProvider does not support path queries"
                        )
                    )
                | ByCategory _ ->
                    Error(
                        QuantumError.ValidationError(
                            "UnsupportedQuery",
                            "SmilesListDatasetProvider does not support category queries"
                        )
                    )
    ...
    type CompositeDatasetProvider(providers: IMoleculeDatasetProvider list) =
        ...
        interface IMoleculeDatasetProvider with
            member _.Describe() =
                let names = providers |> List.map (fun p -> p.Describe()) |> String.concat "; "
                $"Composite: [{names}]"
            ...
            member _.ListNames() =
                providers |> List.collect (fun p -> p.ListNames()) |> List.distinct
    ...
    let parseSmiles (smiles: string) : QuantumResult<MoleculeInstance> =
        MolecularData.parseSmiles smiles |> Result.map fromSmilesMolecule
    ...
    let parseSmilesMany (smilesList: string list) : MoleculeInstance list =
        smilesList
        |> List.choose (fun s -> (parseSmiles s) |> Result.map (fun mol -> Some mol) |> Result.defaultValue None)
