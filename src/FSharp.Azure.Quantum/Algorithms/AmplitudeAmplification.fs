...
module AmplitudeAmplification =
    ...
    let applyAmplificationIteration
        (oracle: CompiledOracle)
        (reflection: ReflectionOperator)
    ...
    module Unified =
        ...
        type AmplitudeAmplificationPlan =
            ...
            | ExecuteViaGroverIntents of
                prepOps: QuantumOperation list *
                oracleOp: QuantumOperation *
                diffusionOp: QuantumOperation *
                iterations: int *
                exactness: Exactness
        ...
        let private validateIntent
            (intent: AmplitudeAmplificationIntent)
            : Result<AmplitudeAmplificationIntent, QuantumError> =
            if intent.NumQubits <= 0 then
                ...
                Error(
                    QuantumError.ValidationError(
                        "Oracle.NumQubits",
                        $"must equal NumQubits ({intent.NumQubits}), got {intent.Oracle.NumQubits}"
                    )
                )
            elif intent.StatePreparation.QubitCount <> intent.NumQubits then
                Error(
                    QuantumError.ValidationError(
                        "StatePreparation.QubitCount",
                        $"must equal NumQubits ({intent.NumQubits}), got {intent.StatePreparation.QubitCount}"
                    )
                )
            else
        ...
        let private buildLoweredOps
            ...
            let prepOps =
                intent.StatePreparation.Gates |> List.rev |> List.map QuantumOperation.Gate
            ...
            result {
                ...
                let groverPlan =
                    if isUniformSuperpositionCircuit intent.StatePreparation then
                        let prepare =
                            QuantumOperation.Algorithm(AlgorithmOperation.GroverPrepare intent.NumQubits)
                        ...
                        let diffusion =
                            QuantumOperation.Algorithm(AlgorithmOperation.GroverDiffusion intent.NumQubits)

                        if
                            backend.SupportsOperation prepare
                            && backend.SupportsOperation oracle
                            && backend.SupportsOperation diffusion
                        then
                            let prepOps = lowerUniformSuperpositionPrepOps intent.NumQubits

                            Some(
                                AmplitudeAmplificationPlan.ExecuteViaGroverIntents(
                                    prepOps,
                                    oracle,
                                    diffusion,
                                    intent.Iterations,
                ...
                | None ->
                    ...
                    if (prepOps @ iterationOps) |> List.forall backend.SupportsOperation then
                        return
                            AmplitudeAmplificationPlan.ExecuteViaOps(
                                prepOps,
                                iterationOps,
                                intent.Iterations,
                                intent.Exactness
        ...
        let private executePlan
                ...
                result {
                    ...
                    let! stateAfterIterations =
                        ...
                        |> List.fold
                            (fun s _ ->
                                s
                                |> Result.bind (backend.ApplyOperation oracleOp)
                                |> Result.bind (backend.ApplyOperation diffusionOp))
                            (Ok prepared)
            ...
            | AmplitudeAmplificationPlan.ExecuteViaOps(prepOps, iterationOps, iterations, _) ->
                result {
                    ...
                    let applyIteration current =
                        UnifiedBackend.applySequence backend iterationOps current
    ...
    let wStatePreparation (numQubits: int) : StatePreparation =
        fun (state: StateVector.StateVector) ->
            if numQubits <> 3 then
                failwith
                    $"W-state preparation only implemented for 3 qubits, calling wStatePreparation with numQubits: {numQubits}"
    ...
    let partialUniformPreparation (numStates: int) (numQubits: int) : StatePreparation =
        fun (state: StateVector.StateVector) ->
            ...
            if numStates > dimension then
                failwith
                    $"Cannot prepare uniform superposition over {numStates} states in {dimension}-dimensional space"
