...
module Oracle =
        ...
        if target < 0 then
            ...
            Error(
                QuantumError.ValidationError(
                    "Target",
                    $"{target} exceeds search space size {searchSpaceSize} for {numQubits} qubits"
                )
            )
        else
    ...
    let forValues (solutions: int list) (numQubits: int) : QuantumResult<CompiledOracle> =
        ...
        if solutions.IsEmpty then
            ...
            let invalidSolutions =
                solutions |> List.filter (fun s -> s < 0 || s >= searchSpaceSize)

            if not invalidSolutions.IsEmpty then
                Error(
                    QuantumError.ValidationError(
                        "Solutions",
                        $"{invalidSolutions} are outside valid range [0, {searchSpaceSize - 1}] for {numQubits} qubits"
                    )
                )
            else
    ...
    let andSpec (spec1: OracleSpec) (spec2: OracleSpec) : OracleSpec = And(spec1, spec2)

    /// Combine two oracle specs with OR logic
    let orSpec (spec1: OracleSpec) (spec2: OracleSpec) : OracleSpec = Or(spec1, spec2)

    /// Negate oracle spec (mark non-solutions)
    let notSpec (spec: OracleSpec) : OracleSpec = Not spec
    ...
    let andOracle (oracle1: CompiledOracle) (oracle2: CompiledOracle) : QuantumResult<CompiledOracle> =
        if oracle1.NumQubits <> oracle2.NumQubits then
            Error(
                QuantumError.ValidationError(
                    "NumQubits",
                    $"cannot combine oracles with different qubit counts ({oracle1.NumQubits} vs {oracle2.NumQubits})"
                )
            )
        else
    ...
    let listSolutions (oracle: CompiledOracle) : int list =
        ...
        [ 0 .. searchSpaceSize - 1 ] |> List.filter (fun i -> isSolution oracle.Spec i)
    ...
    module Examples =
        ...
        let searchForValue (target: int) : QuantumResult<CompiledOracle> = forValue target 4

        /// Example: Search for multiple specific values
        let searchForMultiple (solutions: int list) : QuantumResult<CompiledOracle> = forValues solutions 4

        /// Example: Search for even numbers in 4-qubit space
        let searchEven: QuantumResult<CompiledOracle> = even 4

        /// Example: Search for numbers divisible by 3
        let searchDivisibleBy3: QuantumResult<CompiledOracle> = divisibleBy 3 4

        /// Example: Search for numbers in range 5-10
        let searchRange5to10: QuantumResult<CompiledOracle> = inRange 5 10 4
    ...
    let var (varIndex: int) : SatLiteral = literal varIndex false

    /// Helper: Create a negative literal (NOT variable)
    let notVar (varIndex: int) : SatLiteral = literal varIndex true

    /// Helper: Create a clause from literals
    let clause (literals: SatLiteral list) : SatClause = { Literals = literals }
    ...
    let private evaluateClause (assignment: int) (clause: SatClause) : bool =
        clause.Literals |> List.exists (evaluateLiteral assignment)
    ...
    let private evaluateFormula (formula: SatFormula) (assignment: int) : bool =
        formula.Clauses |> List.forall (evaluateClause assignment)
    ...
    let private validateFormula (formula: SatFormula) : QuantumResult<unit> =
        if formula.NumVariables < 1 then
            ...
            let invalidLiterals =
                ...
                |> List.filter (fun lit -> lit.VariableIndex < 0 || lit.VariableIndex >= formula.NumVariables)

            if not invalidLiterals.IsEmpty then
                let badIndices =
                    invalidLiterals |> List.map (fun lit -> lit.VariableIndex) |> List.distinct

                Error(
                    QuantumError.ValidationError(
                        "Literals",
                        $"variable indices {badIndices} are out of range [0, {formula.NumVariables - 1}]"
                    )
                )
            else
                Ok()
    ...
    let private countSatisfiedClauses (assignment: int) (formula: SatFormula) : int =
        formula.Clauses |> List.filter (evaluateClause assignment) |> List.length
        ...
        | Ok() ->
            if config.MinClausesSatisfied < 1 then
                Error(
                    QuantumError.ValidationError(
                        "MinClausesSatisfied",
                        $"must be at least 1, got {config.MinClausesSatisfied}"
                    )
                )
    ...
    let private isValidColoring (config: GraphColoringConfig) (assignment: int) : bool =
        ...
        |> List.forall (fun (u, v) ->
            ...
            colorU < config.NumColors && colorV < config.NumColors && colorU <> colorV)
    ...
    let private validateGraphColoring (config: GraphColoringConfig) : QuantumResult<unit> =
        if config.Graph.NumVertices < 1 then
            ...
            Error(
                QuantumError.ValidationError(
                    "NumVertices",
                    $"too large ({config.Graph.NumVertices}), maximum is 10 for Grover"
                )
            )
            ...
            let invalidEdges =
                config.Graph.Edges
                |> List.filter (fun (u, v) ->
                    u < 0 || u >= config.Graph.NumVertices || v < 0 || v >= config.Graph.NumVertices)

            if not invalidEdges.IsEmpty then
                Error(
                    QuantumError.ValidationError(
                        "Edges",
                        $"edges {invalidEdges} reference invalid vertices (must be 0 to {config.Graph.NumVertices - 1})"
                    )
                )
            else
                Ok()
    ...
    let private validateCliqueConfig (config: CliqueConfig) : QuantumResult<unit> =
        if config.Graph.NumVertices < 1 then
            ...
            Error(
                QuantumError.ValidationError(
                    "CliqueSize",
                    $"cannot be larger than number of vertices ({config.Graph.NumVertices})"
                )
            )
        else
            Ok()
