...
module QuboExtraction =
    ...
    let fromProblemHamiltonian (hamiltonian: ProblemHamiltonian) : Map<(int * int), float> =
        ...
        let qubits = Seq.append linear.Keys zzSum.Keys |> Seq.distinct

        let diagonal =
            qubits
            |> Seq.choose (fun i ->
                let ci =
                    match linear.TryGetValue i with
                    | true, v -> v
                    | _ -> 0.0

                let si =
                    match zzSum.TryGetValue i with
                    | true, v -> v
                    | _ -> 0.0
        ...
        Seq.append diagonal offDiagonalEntries |> Map.ofSeq
    ...
    let extractFromICircuit (circuit: ICircuit) : Result<Map<(int * int), float>, string> =
        match circuit with
        | :? QaoaCircuitWrapper as wrapper -> extractFromQaoaCircuit wrapper.QaoaCircuit
        | _ -> Error "Circuit is not a QAOA circuit (D-Wave backend only supports QAOA)"
