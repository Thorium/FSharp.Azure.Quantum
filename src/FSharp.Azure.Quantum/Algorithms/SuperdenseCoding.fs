...
module SuperdenseCoding =
    ...
    type private SuperdensePlan =
        | ExecuteViaOps of
            ...
            encodeOps: QuantumOperation list *
            decodeOps: QuantumOperation list
    ...
    let private buildEncodeOps (intent: SuperdenseIntent) : QuantumOperation list =
        ...
        | (1, 1) ->
            [
                QuantumOperation.Gate(Z intent.AliceQubit)
                QuantumOperation.Gate(X intent.AliceQubit)
            ]
        ...
        | QuantumStateType.Annealing ->
            Error(
                QuantumError.OperationError(
                    "SuperdenseCoding",
                    $"Backend '{backend.Name}' does not support superdense coding (native state type: {backend.NativeStateType})"
                )
            )
        | _ ->
            ...
            if requiredOps |> List.forall backend.SupportsOperation then
                ...
                Error(
                    QuantumError.OperationError(
                        "SuperdenseCoding",
                        $"Backend '{backend.Name}' does not support required operations for superdense coding"
                    )
                )
    ...
    let send (backend: IQuantumBackend) (message: ClassicalMessage) : Result<SuperdenseResult, QuantumError> =
    ...
    let send00 (backend: IQuantumBackend) : Result<SuperdenseResult, QuantumError> = send backend { Bit1 = 0; Bit2 = 0 }

    /// Send message 01 (X encoding)
    let send01 (backend: IQuantumBackend) : Result<SuperdenseResult, QuantumError> = send backend { Bit1 = 0; Bit2 = 1 }

    /// Send message 10 (Z encoding)
    let send10 (backend: IQuantumBackend) : Result<SuperdenseResult, QuantumError> = send backend { Bit1 = 1; Bit2 = 0 }

    /// Send message 11 (ZX encoding)
    let send11 (backend: IQuantumBackend) : Result<SuperdenseResult, QuantumError> = send backend { Bit1 = 1; Bit2 = 1 }
