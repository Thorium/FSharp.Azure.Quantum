...
module TrotterSuzuki =
    ...
    let private getSinglePauliMatrix (pauli: char) : Complex[,] =
        match pauli with
        | 'I' -> array2D [ [ Complex(1.0, 0.0); Complex.Zero ]; [ Complex.Zero; Complex(1.0, 0.0) ] ]
        | 'X' -> array2D [ [ Complex.Zero; Complex(1.0, 0.0) ]; [ Complex(1.0, 0.0); Complex.Zero ] ]
        | 'Y' -> array2D [ [ Complex.Zero; Complex(0.0, -1.0) ]; [ Complex(0.0, 1.0); Complex.Zero ] ]
        | 'Z' -> array2D [ [ Complex(1.0, 0.0); Complex.Zero ]; [ Complex.Zero; Complex(-1.0, 0.0) ] ]
        | _ -> array2D [ [ Complex(1.0, 0.0); Complex.Zero ]; [ Complex.Zero; Complex(1.0, 0.0) ] ]
                            ...
                            |> List.fold
                                (fun acc i ->
                                    let rowSum =
                                        [ 0 .. n - 1 ]
                                        |> List.fold
                                            (fun sum j -> sum + matrix[i, j] * pauliMatrix[j, i])
                                            Complex.Zero
            ...
            [
                for mask in 0 .. n - 1 do
                    ...
                    if abs coefficient > 1e-10 then
                        let pauliOps =
                            Array.init numQubits (fun q -> if (mask >>> q) &&& 1 = 1 then 'Z' else 'I')
    ...
    let synthesizePauliEvolution (pauliString: PauliString) (time: float) (qubits: int[]) (circuit: Circuit) : Circuit =

        if qubits.Length <> pauliString.Operators.Length then
            failwith
                $"Qubit count mismatch, calling synthesizePauliEvolution with pauliString: {pauliString}, time: {time}, qubits: {qubits}, circuit: {circuit}"
    ...
    let private applyFirstOrderTrotterStep
        ...
        hamiltonian.Terms
        |> List.fold (fun circ term -> synthesizePauliEvolution term deltaT qubits circ) circuit
    ...
    let private applyFirstOrderControlledTrotterStep
        ...
        hamiltonian.Terms
        |> List.fold (fun circ term -> synthesizeControlledPauliEvolution controlQubit term deltaT qubits circ) circuit
    ...
    let private applySecondOrderControlledTrotterStep
        ...
        let circ1 =
            applyFirstOrderControlledTrotterStep controlQubit hamiltonian halfDeltaT qubits circuit
    ...
    let defaultConfig = { NumSteps = 10; Time = 1.0; Order = 1 }
    ...
    let estimateTrotterSteps (hamiltonianNorm: float) (time: float) (tolerance: float) (order: int) : int =
        if order = 1 then
            ...
            int (
                ceil (
                    (hamiltonianNorm * hamiltonianNorm * hamiltonianNorm * time * time * time)
                    / (12.0 * tolerance)
                    |> sqrt
                )
            )
