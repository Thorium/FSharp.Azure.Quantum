...
module QuantumDistributions =
    ...
    type private SamplePlan = | GenerateUniformViaQrng
                ...
                | QuantumStateType.Annealing ->
                    Error(
                        QuantumError.OperationError(
                            "QuantumDistributions",
                            $"Backend '{backend.Name}' does not support distribution sampling (native state type: {backend.NativeStateType})"
                        )
                    )
                | _ ->
                    ...
                    if backend.SupportsOperation needs then
                        ...
                        Error(
                            QuantumError.OperationError(
                                "QuantumDistributions",
                                $"Backend '{backend.Name}' does not support required operations for distribution sampling"
                            )
                        )
            ...
            | SamplePlan.GenerateUniformViaQrng ->
                ...
                | Ok qrng ->
                    ...
                    | None ->
                        return
                            Error(
                                QuantumError.BackendError("QuantumDistributions", "Failed to generate uniform random")
                            )
                    | Some bits ->
                        ...
                        |> Option.iter (fun r ->
                            r.Report(
                                Progress.PhaseChanged(
                                    "Sampling Complete",
                                    Some $"Generated {samples.Length} quantum samples"
                                )
                            ))
                    | Error _ -> ()
    ...
    let computeStatistics (samples: SampleResult[]) : SampleStatistics =
        if samples.Length = 0 then
            failwith
                $"Cannot compute statistics on empty sample array, calling computeStatistics with samples: {samples}"
    ...
    let expectedStdDev (dist: Distribution) : float option =
        ...
        | Exponential lambda -> Some(1.0 / lambda)
        | Uniform(minVal, maxVal) -> Some(sqrt ((maxVal - minVal) ** 2.0 / 12.0))
        | Custom _ -> None // Unknown for custom distributions
