    ...
    let mapPasqalError (errorCode: string) (errorMessage: string) : QuantumError =
        ...
        | _ ->
            QuantumError.AzureError(
                AzureQuantumError.UnknownError(0, $"Pasqal error: %s{errorCode} - %s{errorMessage}")
            )
                                        ...
                                        Error(
                                            QuantumError.AzureError(
                                                AzureQuantumError.UnknownError(
                                                    0,
                                                    $"Failed to parse Pasqal results: %s{ex.Message}"
                                                )
                                            )
                                        )
                    | JobStatus.Failed(errorCode, errorMessage) -> return Error(mapPasqalError errorCode errorMessage)
                    | JobStatus.Cancelled ->
