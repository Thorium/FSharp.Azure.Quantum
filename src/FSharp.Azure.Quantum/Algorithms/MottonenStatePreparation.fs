...
module MottonenStatePreparation =
    ...
    let private bitParity (x: int) : int =
        let rec go acc v =
            if v = 0 then acc else go (acc ^^^ (v &&& 1)) (v >>> 1)
        ...
        if m = 0 then
            ...
            let transformed =
                Array.init count (fun j ->
                    ...
                    let signedSum =
                        angles
                        |> Array.mapi (fun c angle -> if bitParity (c &&& gj) = 0 then angle else -angle)
                        |> Array.sum
            ...
            ||> List.fold (fun circ j ->
                ...
                let ctrlBit =
                    if j = count - 1 then
                        ...
                        grayCodeDiffBit (grayCode j) (grayCode (j + 1))
    ...
    let normalizeState (amplitudes: Complex[]) : StateVector =
        ...
        let normalized = amplitudes |> Array.map (fun a -> a / Complex(norm, 0.0))
    ...
    let private calculatePhaseAngles (state: StateVector) : float[] =
        state.Amplitudes
        |> Array.map (fun amp -> if amp.Magnitude < 1e-10 then 0.0 else amp.Phase // arg(αᵢ)
        )
    ...
    let private applyPhaseGates (phases: float[]) (qubits: int[]) (circuit: Circuit) : Circuit =
        ...
        if (1 <<< n) <> dim then
            failwith
                $"Dimension mismatch, calling applyPhaseGates with phases: {phases}, qubits: {qubits}, circuit: {circuit}"
        ...
        |> List.fold
            (fun circ k ->
                ...
                circ |> applyMultiplexedRotation RZ levelAngles qubits[k] controlQubits)
            circuit
    ...
    let prepareState (state: StateVector) (qubits: int[]) (circuit: Circuit) : Circuit =
    ...
    let prepareStateFromAmplitudes (amplitudes: Complex[]) (qubits: int[]) (circuit: Circuit) : Circuit =

        amplitudes |> normalizeState |> fun state -> prepareState state qubits circuit

    /// Prepare state from real amplitude array (convenience function)
    let prepareStateFromRealAmplitudes (amplitudes: float[]) (qubits: int[]) (circuit: Circuit) : Circuit =
