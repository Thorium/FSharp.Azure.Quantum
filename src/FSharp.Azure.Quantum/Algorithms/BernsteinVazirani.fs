...
module BernsteinVazirani =
    ...
    type private BernsteinVaziraniIntent = { NumQubits: int; Oracle: Oracle }
    ...
    let private plan
        ...
        (intent: BernsteinVaziraniIntent)
        : Result<BernsteinVaziraniPlan, QuantumError> =
        match backend.NativeStateType with
        | QuantumStateType.Annealing ->
            Error(
                QuantumError.OperationError(
                    "BernsteinVazirani",
                    $"Backend '{backend.Name}' does not support Bernstein-Vazirani (native state type: {backend.NativeStateType})"
                )
            )
        | _ ->
            ...
            if hadamards |> List.forall backend.SupportsOperation then
                ...
                Error(
                    QuantumError.OperationError(
                        "BernsteinVazirani",
                        $"Backend '{backend.Name}' does not support required operations for Bernstein-Vazirani"
                    )
                )
