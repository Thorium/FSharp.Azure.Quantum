...
module QRNG =
    ...
    type private QrngPlan = | ExecuteViaCircuit
        ...
        | QuantumStateType.Annealing ->
            Error(
                QuantumError.OperationError(
                    "QRNG",
                    $"Backend '{backend.Name}' does not support QRNG (native state type: {backend.NativeStateType})"
                )
            )
        | _ ->
            ...
            if backend.SupportsOperation needs then
                ...
                Error(
                    QuantumError.OperationError(
                        "QRNG",
                        $"Backend '{backend.Name}' does not support required operations for QRNG"
                    )
                )
    ...
    let generateBits (numBits: int) (seed: int option) : QRNGResult =
        ...
        let bits =
            ...
            | Some s ->
                ...
                |> List.map (fun _ ->
                    ...
                    let measurementOutcome =
                        Measurement.measureComputationalBasis rng superpositionState
            ...
            | None ->
                ...
                Array.init numBits (fun i -> (randomBytes.[i / 8] >>> (i % 8)) &&& 1uy = 1uy)
    ...
    let generate (numBits: int) : QRNGResult = generateBits numBits None
    ...
    let generateInt (maxValue: int) : int =
        ...
        let numBits =
            if maxValue <= 1 then
                ...
                int (ceil (log (float maxValue) / log 2.0))
        ...
        let rec generateUntilValid () =
            ...
            | None -> failwith $"Failed to convert to integer, calling generateInt with maxValue: {maxValue}"
    ...
    let generateFloat () : float =
        ...
        match result.AsInteger with
        | Some value -> float value / float (1UL <<< numBits)
        | None -> failwith "Failed to generate random float"
    ...
    let generateWithBackend (numBits: int) (backend: IQuantumBackend) : Async<QuantumResult<QRNGResult>> =
        ...
        async {
            if numBits <= 0 then
                ...
                return
                    Error(QuantumError.ValidationError("NumBits", "too large for single backend execution (max 1000)"))
            else
                try
    ...
    let testRandomness (bits: bool[]) : RandomnessTest =
        ...
        let runCount =
            bits |> Array.pairwise |> Array.filter (fun (a, b) -> a <> b) |> Array.length
