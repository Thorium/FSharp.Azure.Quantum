    ...
    let toAhsProgram (program: RydbergProgram) : string =
        ...
        (use writer = new Utf8JsonWriter(stream, JsonWriterOptions(Indented = true))
         ...
         let writePhysicalField (name: string) (values: ResizeArray<float>) (valueScale: float) =
             ...
             for v in values do
                 writer.WriteNumberValue(v * valueScale)

             writer.WriteEndArray()
             writer.WriteStartArray "times"

             for tt in times do
                 writer.WriteNumberValue(tt * usToSeconds)
         ...
         writer.WriteStartArray "times"
         writer.WriteNumberValue 0.0
         writer.WriteNumberValue(totalTime * usToSeconds)
         writer.WriteEndArray()
         writer.WriteEndObject()
         writer.WriteString("pattern", "uniform")
         writer.WriteEndObject()
         ...
         writer.WriteStartArray "localDetuning"
         writer.WriteEndArray()
