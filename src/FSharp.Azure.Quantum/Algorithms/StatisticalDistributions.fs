...
module StatisticalDistributions =
    ...
    let normalPDF (x: float) : float = invSqrt2pi * exp (-0.5 * x * x)
    ...
    let normalCDF (x: float) : float = 0.5 * (1.0 + erf (x / sqrt2))
    ...
    ///
    /// Φ⁻¹(p; μ, σ²) = μ + σ * Φ⁻¹(p)
    let normalQuantileGeneral (mean: float) (stdDev: float) (p: float) : float = mean + stdDev * (normalQuantile p)
    ...
    let logNormalQuantile (mu: float) (sigma: float) (p: float) : float = exp (mu + sigma * (normalQuantile p))
