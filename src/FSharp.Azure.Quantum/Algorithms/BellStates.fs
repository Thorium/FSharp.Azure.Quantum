...
module BellStates =
    ...
    type private BellPlan = ExecuteViaOps of ops: QuantumOperation list
        ...
        | QuantumStateType.Annealing ->
            Error(
                QuantumError.OperationError(
                    "BellStates",
                    $"Backend '{backend.Name}' does not support Bell state algorithms (native state type: {backend.NativeStateType})"
                )
            )
        | _ ->
            let ops =
            ...
            if ops |> List.forall backend.SupportsOperation then
                ...
                Error(
                    QuantumError.OperationError(
                        "BellStates",
                        $"Backend '{backend.Name}' does not support required operations for Bell states"
                    )
                )
    ...
    /// Returns:
    ///   Measured Bell state
    let measureBellBasis (state: QuantumState) (backend: IQuantumBackend) : Result<BellMeasurement, QuantumError> =
    ...
    let verifyEntanglement
        (bellState: BellStateResult)
