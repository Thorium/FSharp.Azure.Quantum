...
module Grover =
    ...
    let rec private applyOracle
        ...
        | Oracle.OracleSpec.Solutions solutionList ->
            ...
            let applyTargetOracle targetValue currentState =
                ...
                let singleTargetOracle =
                    { oracle with
                        Spec = Oracle.OracleSpec.SingleTarget targetValue
                    }
            ...
            solutionList
            |> List.fold (fun stateResult target -> stateResult |> Result.bind (applyTargetOracle target)) (Ok state)
        ...
        | Oracle.OracleSpec.Predicate pred ->
            ...
            if numQubits > 42 then
                ...
                let solutions = [ 0 .. searchSpaceSize - 1 ] |> List.filter pred
                ...
                if List.isEmpty solutions then
                    ...
                    let solutionsOracle =
                        { oracle with
                            Spec = Oracle.OracleSpec.Solutions solutions
                        }
        ...
        | Oracle.OracleSpec.Or(spec1, spec2) ->
            ...
            if numQubits > 42 then
                ...
                let solutions =
                    [ 0 .. searchSpaceSize - 1 ]
                    |> List.filter (fun i -> Oracle.isSolution spec1 i || Oracle.isSolution spec2 i)
        ...
        | Oracle.OracleSpec.Not spec ->
            ...
            if numQubits > 42 then
                ...
                let solutions =
                    [ 0 .. searchSpaceSize - 1 ] |> List.filter (Oracle.isSolution spec >> not)
    ...
    /// 4. Apply X^⊗n (undo)
    /// 5. Apply H^⊗n
    let private applyDiffusion (backend: IQuantumBackend) (state: QuantumState) : Result<QuantumState, QuantumError> =
    ...
    let private applyGroverIteration
        ...
        state |> applyOracle backend oracle |> Result.bind (applyDiffusion backend)
    ...
    let private extractSolutions (distribution: Map<int, int>) (totalShots: int) (threshold: float) : int list =
    ...
    type GroverPlan =
        ...
        | ExecuteViaOps of
            prepareOps: QuantumOperation list *
            iterationOps: QuantumOperation list *
            iterations: int *
            exactness: Exactness
    ...
    let private enumerateSolutions (oracle: Oracle.CompiledOracle) : Result<int list, QuantumError> =
        ...
        let solutions =
            [ 0 .. searchSpaceSize - 1 ] |> List.filter (Oracle.isSolution oracle.Spec)
    ...
    let private lowerOracleOps (oracle: Oracle.CompiledOracle) : Result<QuantumOperation list, QuantumError> =
        result {
            ...
            return
                solutions
                |> List.collect (fun t -> lowerSingleTargetOracleOps t oracle.NumQubits)
        }
    ...
    let plan (backend: IQuantumBackend) (intent: GroverSearchIntent) : Result<GroverPlan, QuantumError> =
        ...
        | QuantumStateType.Annealing ->
            Error(
                QuantumError.OperationError(
                    "Grover",
                    $"Backend '{backend.Name}' does not support Grover search (native state type: {backend.NativeStateType})"
                )
            )
        | _ ->
            ...
            if
                backend.SupportsOperation ops.Prepare
                && backend.SupportsOperation ops.Oracle
                && backend.SupportsOperation ops.Diffusion
            then
    ...
    let private executePlan
        ...
        let incremental =
            result {
                ...
                | ExecuteViaOps(prepareOps, iterationOps, iterations, _exactness) ->
                    ...
                    let step s =
                        backend.ApplyOperation (QuantumOperation.Sequence iterationOps) s
        ...
        | Error e when UnifiedBackend.isIncrementalUnsupported e ->
            ...
            let iterations =
                match plan with
                | ExecuteNatively(_, k, _)
                | ExecuteViaOps(_, _, k, _) -> k
        ...
        if rounds < 1 then
            ...
            try
                ...
                let results =
                    [ 1..rounds ]
                    |> List.choose (fun _ ->
                        (search oracle backend config)
                        |> Result.map (fun res -> Some res)
                ...
                if List.isEmpty results then
                    ...
                    // Aggregate results
                    let allSolutions =
                        results |> List.collect (fun r -> r.Solutions) |> List.distinct |> List.sort

                    let avgSuccessProb = results |> List.averageBy (fun r -> r.SuccessProbability)

                    let avgIterations = results |> List.averageBy (fun r -> float r.Iterations) |> int
            ...
            with ex ->
                Error(QuantumError.OperationError("Multi-round search", $"failed: {ex.Message}"))
    ...
    let formatResult (result: GroverResult) : string =
        ...
        let statsStr =
            sprintf
                "Iterations: %d | Success probability: %.2f%% | Time: %.2f ms"
                result.Iterations
