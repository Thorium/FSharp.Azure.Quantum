...
module HHL =
            ...
            if eigenvalueQubits < 0 then
                Error(
                    QuantumError.ValidationError(
                        "totalQubits",
                        $"totalQubits too small for vector dimension: {totalQubits}"
                    )
                )
            else
                ...
                let norm =
                    sqrt (amplitudes |> Array.sumBy (fun a -> (a * Complex.Conjugate(a)).Real))
    ...
    /// </remarks>
    let private postSelectAncilla (ancillaQubit: int) (state: QuantumState) : Result<QuantumState, QuantumError> =
        ...
        | QuantumState.StateVector stateVec ->
            ...
            // Renormalize
            let norm = newAmplitudes |> Array.sumBy (fun c -> c.Magnitude * c.Magnitude) |> sqrt
        ...
        | QuantumState.FusionSuperposition fs ->
            ...
            let newAmplitudes =
                Array.init dimension (fun i ->
                    let ancillaIs1 = (i &&& ancillaMask) <> 0
                    if ancillaIs1 then amplitudeVec.[i] else Complex.Zero)
    ...
    let private extractEigenvalues (eigenvalueQubits: int) (state: QuantumState) : float[] =
    ...
    let private validateIntent (intent: HhlExecutionIntent) : Result<float[] * float * float * float, QuantumError> =
        if intent.Matrix.Dimension <> intent.InputVector.Dimension then
            Error(
                QuantumError.ValidationError(
                    "Dimensions",
                    $"Matrix ({intent.Matrix.Dimension}) and vector ({intent.InputVector.Dimension}) dimensions must match"
                )
            )
        else
    ...
    let private inversionRotationAngle
        (method: EigenvalueInversionMethod)
        (eigenvalue: float)
        (minEigenvalue: float)
    ...
    let private buildLoweringOps
        ...
        if intent.Matrix.IsDiagonal then
            ...
            let solutionQubits =
                [
                    intent.EigenvalueQubits .. intent.EigenvalueQubits + intent.SolutionQubits - 1
                ]
            ...
            let angleResults =
                Array.init solutionDim (fun i ->
                    let lambda =
                        if i < spectrumEigenvalues.Length then
                            spectrumEigenvalues[i]
                        else
                            spectrumEigenvalues[0]
                        ...
                        if abs theta < 1e-12 then
                            ...
                            let xFlips =
                                ...
                                |> List.choose (fun bitIdx ->
                                    if ((i >>> bitIdx) &&& 1) = 0 then
                                        Some(CircuitBuilder.X solutionQubitArray[bitIdx])
                                    else
                                        None)

                            xFlips @ multiControlledRyGates solutionQubits ancillaQubit theta @ xFlips)
            ...
            result {
                ...
                let buildControlledEvolutionOps (controlQubit: int) (time: float) : QuantumOperation list =
                    let config: TrotterSuzuki.TrotterConfig =
                        {
                ...
                let angleResults =
                    Array.init eigenRegisterSize (fun k ->
                        if k = 0 then
                            ...
                            let signedK =
                                if canonicalK >= eigenRegisterSize / 2 then
                                    canonicalK - eigenRegisterSize
                                else
                                    canonicalK
                            ...
                            let safeMethod =
                                match intent.InversionMethod with
                                | EigenvalueInversionMethod.ExactRotation c ->
                                    EigenvalueInversionMethod.ExactRotation(min c (abs lambdaEst))
                                | EigenvalueInversionMethod.LinearApproximation c ->
                                    EigenvalueInversionMethod.LinearApproximation(min c (abs lambdaEst))
                ...
                | None ->
                    ...
                    let multiplexedRyGates =
                        ...
                        |> List.collect (fun k ->
                            ...
                            if abs theta < 1e-12 then
                                ...
                                xFlips @ multiControlledRyGates eigenQubits ancillaQubit theta @ xFlips)
                    ...
                    let hadamardsUncompute =
                        eigenQubits |> List.map (CircuitBuilder.H >> QuantumOperation.Gate)
                    ...
                    let gateCountEstimate =
                        ...
                        let uncomputeOps =
                            forwardQft.Length + controlledUncompute.Length + hadamardsUncompute.Length
    ...
    let plan
        (backend: IQuantumBackend)
        (intent: HhlExecutionIntent)
        : Result<HhlPlan * float[] * int * float, QuantumError> =
        result {
                ...
                return!
                    Error(
                        QuantumError.OperationError(
                            "HHL",
                            $"Backend '{backend.Name}' does not support HHL (native state type: {backend.NativeStateType})"
                        )
                    )
            | _ ->
                ...
                | _ ->
                    ...
                    if intent.Matrix.IsDiagonal && backend.SupportsOperation op then
                        return
                            (HhlPlan.ExecuteNatively(toCoreIntent intent, exactness),
                             spectrumEigenvalues,
                             ancillaQubit,
                             conditionNumber)
                    else
                        let! (ops, _gateCount, _ancillaQubit) =
                        ...
                        if transpiledOps |> List.forall backend.SupportsOperation then
                            return
                                (HhlPlan.ExecuteViaOps(transpiledOps, exactness),
                                 spectrumEigenvalues,
                                 ancillaQubit,
                                 conditionNumber)
                        else
                            return!
                                Error(
                                    QuantumError.OperationError(
                                        "HHL",
                                        $"Backend '{backend.Name}' does not support required operations for HHL"
                                    )
                                )
        }
    ...
    /// <param name="backend">Quantum backend</param>
    /// <returns>HHL result or error</returns>
    let solveIdentity (inputVector: Complex[]) (backend: IQuantumBackend) : Result<HHLResult, QuantumError> =
