...
module DeutschJozsa =
    ...
    type private DeutschJozsaIntent = { NumQubits: int; Oracle: Oracle }
        ...
        | QuantumStateType.Annealing ->
            Error(
                QuantumError.OperationError(
                    "DeutschJozsa",
                    $"Backend '{backend.Name}' does not support Deutsch-Jozsa (native state type: {backend.NativeStateType})"
                )
            )
        | _ ->
            ...
            if hadamards |> List.forall backend.SupportsOperation then
                ...
                Error(
                    QuantumError.OperationError(
                        "DeutschJozsa",
                        $"Backend '{backend.Name}' does not support required operations for Deutsch-Jozsa"
                    )
                )
