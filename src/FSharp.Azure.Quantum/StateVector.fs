...
module StateVector =
    ...
    let init (numQubits: int) : StateVector =
        if numQubits < 0 || numQubits > 10 then
            failwith $"Number of qubits must be between 0 and 10, got {numQubits}"
    ...
    let create (amplitudes: Complex[]) : StateVector =
        ...
        if numQubits > 10 then
            failwith $"State vector supports maximum 10 qubits (1024 dimensions), got {numQubits} qubits ({n} dimensions)"
