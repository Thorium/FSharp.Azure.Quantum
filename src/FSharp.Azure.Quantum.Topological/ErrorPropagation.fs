...
module ErrorPropagation =
    ...
    let getGateError (gate: BasicGate) : float * string =
        ...
        | T
        | TDagger -> (0.0, "Exact (single Majorana braiding)")
        | S
        | SDagger -> (0.0, "Exact (double Majorana braiding)")
    ...
    let trackErrors (gates: GateSequence) (model: ErrorModel) : ErrorAccumulation =
        ...
        let maxError =
            if List.isEmpty errorValues then
                0.0
            else
                List.max errorValues
    ...
    let displayAccumulation (accumulation: ErrorAccumulation) : string =
        ...
        let topErrors =
            ...
            |> List.truncate 5
            |> List.map (fun ge -> $"    Gate {ge.Position}: {ge.Gate} (ε = {ge.Error:E6}) - {ge.Source}")
            |> String.concat "\n"
...
Budget Utilization: {assessment.BudgetUtilization:F1}%%"""
    ...
    let generateReport (gates: GateSequence) (budget: ErrorBudget) : string =
        ...
        let suggestionsText =
            suggestions |> List.map (fun s -> $"  {s}") |> String.concat "\n"
