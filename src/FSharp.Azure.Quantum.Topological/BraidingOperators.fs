...
module BraidingOperators =
    ...
    let element
        ...
        | Ok true ->
            ...
            | AnyonSpecies.AnyonType.Ising, AnyonSpecies.Particle.Vacuum, x, _ when c = x -> Ok Complex.One
            | AnyonSpecies.AnyonType.Ising, x, AnyonSpecies.Particle.Vacuum, _ when c = x -> Ok Complex.One

            // σ × σ → 1: R^1_σσ = e^(-iπ/8) (Kitaev convention)
            | AnyonSpecies.AnyonType.Ising,
              AnyonSpecies.Particle.Sigma,
              AnyonSpecies.Particle.Sigma,
              AnyonSpecies.Particle.Vacuum -> Ok(expI (-π / 8.0))

            // σ × σ → ψ: R^ψ_σσ = e^(3iπ/8)
            | AnyonSpecies.AnyonType.Ising,
              AnyonSpecies.Particle.Sigma,
              AnyonSpecies.Particle.Sigma,
              AnyonSpecies.Particle.Psi -> Ok(expI (3.0 * π / 8.0))
            ...
            | AnyonSpecies.AnyonType.Ising,
              AnyonSpecies.Particle.Psi,
              AnyonSpecies.Particle.Sigma,
              AnyonSpecies.Particle.Sigma -> Ok(expI (-π / 2.0))

            // ψ × ψ → 1: R^1_ψψ = -1 (fermion statistics!)
            | AnyonSpecies.AnyonType.Ising,
              AnyonSpecies.Particle.Psi,
              AnyonSpecies.Particle.Psi,
              AnyonSpecies.Particle.Vacuum -> Ok(Complex(-1.0, 0.0))
            ...
            // Vacuum braiding
            | AnyonSpecies.AnyonType.Fibonacci, AnyonSpecies.Particle.Vacuum, x, _ when c = x -> Ok Complex.One
            | AnyonSpecies.AnyonType.Fibonacci, x, AnyonSpecies.Particle.Vacuum, _ when c = x -> Ok Complex.One

            // τ × τ → 1: R^1_ττ = e^(4πi/5)
            | AnyonSpecies.AnyonType.Fibonacci,
              AnyonSpecies.Particle.Tau,
              AnyonSpecies.Particle.Tau,
              AnyonSpecies.Particle.Vacuum -> Ok(expI (4.0 * π / 5.0))

            // τ × τ → τ: R^τ_ττ = e^(-3πi/5)
            | AnyonSpecies.AnyonType.Fibonacci,
              AnyonSpecies.Particle.Tau,
              AnyonSpecies.Particle.Tau,
              AnyonSpecies.Particle.Tau -> Ok(expI (-3.0 * π / 5.0))
    ...
    let matrix
        ...
        | Ok fusionChannels ->
            ...
            let rElementResults =
                fusionChannels |> List.map (fun channel -> element a b channel anyonType)
            ...
            | None ->
                ...
                Ok(Array2D.init n n (fun i j -> if i = j then rElements.[i] else Complex.Zero))
    ...
    let matrixElement
        ...
        match
            FusionRules.isPossible a b e anyonType,
            FusionRules.isPossible e c d anyonType,
            FusionRules.isPossible b c f anyonType,
            FusionRules.isPossible a f d anyonType
        with
        | Error err, _, _, _
        | _, Error err, _, _
        | _, _, Error err, _
    ...
    let fusionBasisChange
        ...
        | Ok channelsAB, Ok channelsBC ->
            ...
            let validEResults =
                channelsAB
                |> List.map (fun e -> FusionRules.isPossible e c d anyonType |> Result.map (fun isPoss -> (e, isPoss)))

            let validFResults =
                channelsBC
                |> List.map (fun f -> FusionRules.isPossible a f d anyonType |> Result.map (fun isPoss -> (f, isPoss)))

            // Check for errors in validation
            match List.tryFind Result.isError validEResults, List.tryFind Result.isError validFResults with
            | Some(Error err), _
            | _, Some(Error err) -> Error err
            | _ ->
