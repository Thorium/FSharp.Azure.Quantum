...
module BraidGroup =
    ...
    let sigma (i: int) : BraidGenerator = { Index = i; IsClockwise = true }

    /// Create an inverse generator σ_i^{-1} (counter-clockwise)
    let sigmaInv (i: int) : BraidGenerator = { Index = i; IsClockwise = false }
    ...
    let fromGenerators (n: int) (generators: BraidGenerator list) : TopologicalResult<BraidWord> =
        ...
        let invalidGen =
            generators |> List.tryFind (fun g -> g.Index < 0 || g.Index >= n - 1)

        match invalidGen with
        | Some g ->
            TopologicalResult.validationError
                "generatorIndex"
                $"Generator index {g.Index} out of range for {n} strands (valid: 0 to {n - 2})"
        | None ->
            Ok
                {
                    StrandCount = n
    ...
    let compose (first: BraidWord) (second: BraidWord) : TopologicalResult<BraidWord> =
        if first.StrandCount <> second.StrandCount then
            TopologicalResult.validationError
                "strandCount"
                $"Cannot compose braids with different strand counts: {first.StrandCount} ≠ {second.StrandCount}"
        else
            Ok
                {
    ...
    let doCommute (g1: BraidGenerator) (g2: BraidGenerator) : bool = abs (g1.Index - g2.Index) >= 2
    ...
    let simplify (braid: BraidWord) : BraidWord =
        ...
        { braid with
            Generators = removeAdjacentInverses braid.Generators
        }
    ...
    let private applyGeneratorWithRData
        ...
        if i < 0 || i >= anyons.Length - 1 then
            ...
            | Ok rValue ->
                ...
                let phase =
                    if generator.IsClockwise then
                        rValue
                    else
                        Complex.Conjugate rValue
    ...
    let applyGenerator
        ...
        (RMatrix.computeRMatrix anyonType)
        |> Result.bind (fun rData -> applyGeneratorWithRData generator anyons fusionChannel rData)
    ...
    let applyBraid
        ...
        if anyons.Length <> braid.StrandCount then
            TopologicalResult.logicError
                "operation"
                $"Braid has {braid.StrandCount} strands but {anyons.Length} anyons provided"
        else
    ...
    let exchange (i: int) (n: int) : TopologicalResult<BraidWord> = fromGenerators n [ sigma i ]

    /// Full exchange (swap) of strands i and i+1: σ_i
    /// Note: In anyon physics, a single crossing already performs the exchange
    let swap (i: int) (n: int) : TopologicalResult<BraidWord> = exchange i n
    ...
    let verifyYangBaxter
        ...
        if i < 0 || i >= n - 2 then
            ...
            | Ok leftBraid, Ok rightBraid ->
                match
                    applyBraid leftBraid anyons fusionChannel anyonType,
                    applyBraid rightBraid anyons fusionChannel anyonType
                with
    ...
    let private formatGenerator (g: BraidGenerator) : string =
        if g.IsClockwise then $"σ_{g.Index}" else $"σ_{g.Index}⁻¹"
    ...
    let displayBraid (braid: BraidWord) : string =
        if braid.Generators.IsEmpty then
            ...
            let word = braid.Generators |> List.map formatGenerator |> String.concat " "
    ...
    let displayBraidResult (result: BraidApplicationResult) : string =
        ...
        let angle =
            System.Math.Atan2(result.Phase.Imaginary, result.Phase.Real) * 180.0
            / System.Math.PI
