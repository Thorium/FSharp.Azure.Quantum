...
module ModularData =
    ...
    [<TailCall>]
    let rec computeSMatrix (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<Complex[,]> =
            ...
            let s =
                Array2D.init 3 3 (fun i j ->
                    ...
                    let value =
                        ...
                        | 2, 2 -> 1.0
                        | _ ->
                            invalidOp $"Array2D.init contract violated: index ({i},{j}) out of bounds for 3x3 matrix"
            ...
            let s =
                Array2D.init 2 2 (fun i j ->
                    ...
                    let value =
                        ...
                        | 1, 1 -> -1.0
                        | _ ->
                            invalidOp $"Array2D.init contract violated: index ({i},{j}) out of bounds for 2x2 matrix"
        ...
        | AnyonSpecies.SU2Level k ->
            ...
            | Ok particleList ->
                ...
                let spinValue =
                    ...
                    | other ->
                        invalidOp
                            $"SU(2)_k S-matrix: particle {other} is not valid for SU(2) theory (expected Vacuum or SpinJ)"

                // Get list of j values
                let jValues = particleList |> List.map spinValue |> List.toArray
    ...
    /// Returns Error if anyon type not supported.
    [<TailCall>]
    let rec computeTMatrix (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<Complex[,]> =
        ...
        | AnyonSpecies.SU2Level k ->
            ...
            | Ok particleList ->
                ...
                let spinValue =
                    ...
                    | other ->
                        invalidOp
                            $"SU(2)_k T-matrix: particle {other} is not valid for SU(2) theory (expected Vacuum or SpinJ)"
    ...
    let centralCharge (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<float> =
    ...
    let computeModularData (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<ModularData> =
    ...
    /// S₀ₐ = dₐ/D
    let totalQuantumDimension (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<float> =
