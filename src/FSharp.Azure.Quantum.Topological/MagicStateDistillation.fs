...
module MagicStateDistillation =
    ...
    let prepareNoisyMagicState (errorRate: float) (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<MagicState> =
    ...
    let distill15to1 (random: Random) (inputStates: MagicState list) : TopologicalResult<DistillationResult> =
        ...
        | states when states.Length <> 15 ->
            TopologicalResult.validationError
                "inputStates"
                $"15-to-1 protocol requires exactly 15 input states, got {states.Length}"
        | states when
            not (
                states
                |> List.forall (fun s -> s.QubitState.AnyonType = AnyonSpecies.AnyonType.Ising)
            )
            ->
            TopologicalResult.validationError "field" "All magic states must be Ising anyons"
        | firstState :: _ ->
            // Calculate average input fidelity
            let avgInputFidelity = inputStates |> List.averageBy (fun s -> s.Fidelity)
    ...
    let distillIterative
        ...
        | r, _ when r < 1 -> TopologicalResult.validationError "field" "Must perform at least 1 distillation round"
        | r, _ when r > 5 ->
    ...
    let estimateResources (targetFidelity: float) (noisyStateFidelity: float) : ResourceEstimate =
