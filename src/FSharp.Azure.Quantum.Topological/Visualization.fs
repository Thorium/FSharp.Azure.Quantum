...
module VisualizationExtensions =
    ...
    type FusionTree.State with
        ...
        member this.ToMermaid() : string =
            ...
            sb.AppendLine(sprintf "        info2[\"Anyons: %d\"]" (FusionTree.size this.Tree))
            |> ignore

            sb.AppendLine(sprintf "        info3[\"Depth: %d\"]" (FusionTree.depth this.Tree))
            |> ignore

            sb.AppendLine(
                sprintf "        info4[\"Total Charge: %A\"]" (FusionTree.totalCharge this.Tree this.AnyonType)
            )
            |> ignore
        ...
        member this.ToASCII() : string =
            ...
            sb.AppendLine(sprintf "Total Charge: %A" (FusionTree.totalCharge this.Tree this.AnyonType))
            |> ignore
            ...
            let rec renderTree (tree: FusionTree.Tree) (indent: string) (isLast: bool) =
                ...
                match tree with
                | FusionTree.Leaf particle -> sb.AppendLine($"%s{prefix}Leaf: %A{particle}") |> ignore
                ...
                |> List.fold
                    (fun anyonCount op ->
                        ...
                        | TopologicalBuilder.Init(anyonType, count) ->
                            sb.AppendLine(
                                sprintf
                                    "    Note over %s,%s: Initialize %d %A Anyons"
                                    (getAnyonName 0)
                                    (getAnyonName (count - 1))
                                    count
                                    anyonType
                            )
                            |> ignore
