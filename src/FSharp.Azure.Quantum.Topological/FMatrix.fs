...
module FMatrix =
    ...
    let private qNumber (n: int) (k: int) : float =
        ...
        if abs sinDenom < 1e-15 then
            0.0
        else
            sin (float n * Math.PI / denom) / sinDenom
    ...
    let private isValidTriad (a_d: int) (b_d: int) (c_d: int) (k: int) : bool =
        let sum = a_d + b_d + c_d
        sum % 2 = 0 && c_d >= abs (a_d - b_d) && c_d <= a_d + b_d && sum / 2 <= k
    ...
    ///
    /// Returns (log|Δ²|, sign).
    let private triangleCoefficientLogSigned (a_d: int) (b_d: int) (c_d: int) (k: int) : float * float =
    ...
    let private quantum6jSymbol
        ...
        if not (isValidTriad j1_d j2_d j3_d k) then
            0.0
        elif not (isValidTriad j1_d j5_d j6_d k) then
            0.0
        elif not (isValidTriad j4_d j2_d j6_d k) then
            0.0
        elif not (isValidTriad j4_d j5_d j3_d k) then
            0.0
        else
            ...
            if sgnDeltaProduct = 0.0 || logDeltaProduct = -infinity then
                0.0
            else
                ...
                if zMin > zMax then
                    ...
                    let prefactorSign = if sgnDeltaProduct >= 0.0 then 1.0 else -1.0
    ...
    let private computeSU2kFSymbols (k: int) : Map<FSymbolIndex, Complex> =
        ...
        let particles =
            (AnyonSpecies.particles (AnyonSpecies.AnyonType.SU2Level k))
            |> Result.defaultValue []

        // For each particle, get fusion channels
        let fusionChannels (p1: AnyonSpecies.Particle) (p2: AnyonSpecies.Particle) =
            (FusionRules.channels p1 p2 (AnyonSpecies.AnyonType.SU2Level k))
            |> Result.defaultValue []
                ...
                for c in particles do
                    ...
                    for e in eChannels do
                        ...
                        for f in fChannels do
                            ...
                            let commonD = Set.intersect (Set.ofList dFromEC) (Set.ofList dFromAF)
    ...
    let computeFMatrix (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<FMatrixData> =
    ...
    let private canFuseTo
        ...
        FusionRules.fuse a b anyonType |> Result.map (toParticles >> List.contains c)
    ...
    let getFSymbol (data: FMatrixData) (index: FSymbolIndex) : TopologicalResult<Complex> =
    ...
    let private tryGetFLocal (data: FMatrixData) (a, b, c, d, e, f) : Complex option =
        (getFSymbol
            data
            {
                A = a
                            ...
                            for h in fusionChannelsLocal b l anyonType do
                                ...
                                | Some lhs1, Some lhs2 ->
                                    ...
                                    let rhs =
                                        ...
                                        |> List.choose (fun k ->
                                            match
                                                tryGetFLocal data (a, b, c, g, f, k),
                                                tryGetFLocal data (a, k, d, e, g, h),
                                                tryGetFLocal data (b, c, d, h, k, l)
                                            with
    ...
    let verifyFMatrixUnitarity
        ...
        let getEChannels () =
            FusionRules.fuse a b data.AnyonType |> Result.map toParticles

        // Get all valid intermediate states f (from b × c)
        let getFChannels () =
            FusionRules.fuse b c data.AnyonType |> Result.map toParticles
            ...
            |> Result.bind (fun eChannels ->
                ...
                |> Result.bind (fun fChannels ->
                    ...
                    let collectFSymbols () =
                        ...
                        |> List.mapi (fun i e ->
                            ...
                            |> List.mapi (fun j f ->
                                ...
                                getFSymbol data index |> Result.map (fun value -> (i, j, value))))
                        |> List.concat
                        |> List.fold
                            (fun acc result ->
                                Result.bind (fun list -> Result.map (fun item -> item :: list) result) acc)
                            (Ok [])
                    ...
                    |> Result.map (fun values ->
                        ...
                        values |> List.iter (fun (i, j, value) -> matrix.[i, j] <- value)
                        matrix)))
        ...
        let checkUnitarity (matrix: Complex[,]) =
            ...
            let computeProduct i j =
                [ 0 .. cols - 1 ]
                |> List.fold (fun sum k -> sum + matrix.[i, k] * Complex.Conjugate(matrix.[j, k])) Complex.Zero
        ...
        buildFMatrix () |> Result.map checkUnitarity
    ...
    let private getAllParticles (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<AnyonSpecies.Particle list> =
    ...
    let validateFMatrix (data: FMatrixData) : TopologicalResult<FMatrixData> =
                            ...
                            for b in particles do
                                for c in particles do
                                    for d in particles do
                                        for e in particles -> verifyPentagonEquation data a b c d e
                    ]
                    ...
                    if allPassed then
                        ...
                        |> Result.bind (fun unitary ->
                            if unitary then
                                ...
                                Error(TopologicalError.Other $"F-matrix unitarity check failed for {data.AnyonType}"))
                    else
                        Error(TopologicalError.Other $"Pentagon equation violated for {data.AnyonType}"))
    ...
    let displayFSymbol (index: FSymbolIndex) (value: Complex) : string =
        sprintf
            "F[%s,%s,%s,%s;%s,%s] = %s"
            (formatParticle index.A)
            (formatParticle index.B)
            (formatParticle index.C)
            (formatParticle index.D)
            (formatParticle index.E)
            (formatParticle index.F)
            (formatComplex value)
