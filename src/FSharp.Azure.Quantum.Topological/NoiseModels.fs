...
module NoiseModels =
    ...
    let createDecoherenceParameters (t1: float) (t2: float) : TopologicalResult<DecoherenceParameters> =
        if t1 <= 0.0 then
            ...
            TopologicalResult.validationError
                "t2Time"
                $"T2 ({t2}μs) cannot exceed 2*T1 ({2.0 * t1}μs) - quantum mechanics constraint"
        else
            Ok
                {
                    T1 = t1
    ...
    let createGateErrorParameters
        ...
        if singleQubitErrorRate < 0.0 || singleQubitErrorRate > 1.0 then
            TopologicalResult.validationError
                ...
                $"Single-qubit error rate must be in [0, 1], got {singleQubitErrorRate}"
        elif twoQubitErrorRate < 0.0 || twoQubitErrorRate > 1.0 then
            TopologicalResult.validationError
                "twoQubitErrorRate"
                $"Two-qubit error rate must be in [0, 1], got {twoQubitErrorRate}"
    ...
    let createMeasurementErrorParameters
        ...
        if readoutErrorRate < 0.0 || readoutErrorRate > 1.0 then
            TopologicalResult.validationError
                "readoutErrorRate"
                $"Readout error rate must be in [0, 1], got {readoutErrorRate}"
    ...
    let realisticTopological () : TopologicalResult<NoiseModel> = realisticTopologicalMajorana2 ()
    ...
    let applyDecoherence
        ...
        | (groundAmp, groundState) :: excitedTerms ->
            ...
            let groundPhase =
                if Complex.Abs groundAmp > 1e-15 then
                    groundAmp / Complex(Complex.Abs groundAmp, 0.0)
                else
                    Complex.One
            ...
            { superposition with
                Terms = (newGroundAmp, groundState) :: dampedExcited
            }
    ...
    let checkQuasiparticlePoisoning
        ...
        let poisoningProbability =
            1.0 - exp (-parameters.PoisoningRate * operationTime / 1_000_000.0)
    ...
    let noisyMeasure
        ...
        | Some measurementErrors -> applyMeasurementError measurementErrors.ReadoutErrorRate outcome random
        | None -> outcome
