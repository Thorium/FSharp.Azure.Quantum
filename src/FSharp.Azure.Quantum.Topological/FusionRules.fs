...
module FusionRules =
    ...
    let fuse
        ...
        | Ok true, Ok true ->
            ...
            | AnyonSpecies.AnyonType.Ising, x, AnyonSpecies.Particle.Vacuum -> Ok [ { Result = x; Multiplicity = 1 } ]
            ...
            | AnyonSpecies.AnyonType.Ising, AnyonSpecies.Particle.Sigma, AnyonSpecies.Particle.Sigma ->
                Ok
                    [
                        {
                            Result = AnyonSpecies.Particle.Vacuum
                            Multiplicity = 1
                        }
                        {
                            Result = AnyonSpecies.Particle.Psi
                            Multiplicity = 1
                        }
                    ]
            ...
            | AnyonSpecies.AnyonType.Ising, AnyonSpecies.Particle.Psi, AnyonSpecies.Particle.Sigma ->
                Ok
                    [
                        {
                            Result = AnyonSpecies.Particle.Sigma
                            Multiplicity = 1
                        }
                    ]
            ...
            | AnyonSpecies.AnyonType.Fibonacci, AnyonSpecies.Particle.Tau, AnyonSpecies.Particle.Tau ->
                Ok
                    [
                        ...
                        {
                            Result = AnyonSpecies.Particle.Tau
                            Multiplicity = 1
                        }
                    ]
            ...
            | AnyonSpecies.AnyonType.Ising, _, AnyonSpecies.Particle.SpinJ _ ->
                TopologicalResult.validationError
                    "particle"
                    "SpinJ particles not valid for Ising anyon type (use SU2Level instead)"
            ...
            | AnyonSpecies.AnyonType.Fibonacci, _, AnyonSpecies.Particle.SpinJ _ ->
                TopologicalResult.validationError
                    "particle"
                    "SpinJ particles not valid for Fibonacci anyon type (use SU2Level instead)"
            ...
            | AnyonSpecies.AnyonType.SU2Level k,
              AnyonSpecies.Particle.SpinJ(j1_doubled, _),
              AnyonSpecies.Particle.SpinJ(j2_doubled, _) ->
                ...
                if j_max < j_min then
                    ...
                    let fusion_results =
                        ...
                        |> List.map (fun j3 ->
                            {
                                Result = AnyonSpecies.Particle.SpinJ(int (j3 * 2.0), k)
                                Multiplicity = 1
                            })
            ...
            | AnyonSpecies.AnyonType.SU2Level k, AnyonSpecies.Particle.SpinJ(j, _), AnyonSpecies.Particle.Vacuum ->
                Ok
                    [
                        {
                            Result = AnyonSpecies.Particle.SpinJ(j, k)
                            Multiplicity = 1
                        }
                    ]
            ...
            | AnyonSpecies.AnyonType.SU2Level _, _, _ ->
                TopologicalResult.validationError
                    "particle"
                    "Mixing Ising/Fibonacci particles with SU(2)_k SpinJ particles is invalid"
    ...
    let isPossible
        ...
        multiplicity a b c anyonType |> Result.map (fun m -> m > 0)
                                ...
                                if not antiParticleCheck then
                                    Error(
                                        TopologicalError.Other
                                            "Anti-particle axiom violated: a × ā does not contain 1 for some a"
                                    )
                                else
                                    Ok()
