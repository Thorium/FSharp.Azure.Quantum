...
module EntanglementEntropy =
    ...
    let topologicalEntropy (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<float> =
        AnyonSpecies.totalQuantumDimension anyonType |> Result.map log
    ...
    let topologicalEntropyLog2 (anyonType: AnyonSpecies.AnyonType) : TopologicalResult<float> =
        topologicalEntropy anyonType |> Result.map (fun gamma -> gamma / log 2.0)
    ...
    let levinWen (regions: LevinWenRegions) : float * float =
        ...
        let alpha = if abs deltaP > 1e-10 then deltaS / deltaP else 0.0 // Degenerate case
    ...
    let vonNeumannEntropyFromEigenvalues (eigenvalues: float list) : TopologicalResult<VonNeumannEntropyCalculation> =

        // Validate eigenvalues
        if eigenvalues |> List.exists (fun lambda -> lambda < -1e-10) then
            TopologicalResult.validationError
                "eigenvalues"
                "Eigenvalues must be non-negative (density matrix must be positive semidefinite)"
    ...
    let partialTraceB (rho: Complex[,]) (dimA: int) (dimB: int) : TopologicalResult<Complex[,]> =

        let totalDim = Array2D.length1 rho

        if totalDim <> dimA * dimB then
            TopologicalResult.validationError "dimensions" $"Total dimension {totalDim} ≠ dimA ({dimA}) × dimB ({dimB})"
    ...
    let partialTraceA (rho: Complex[,]) (dimA: int) (dimB: int) : TopologicalResult<Complex[,]> =
    ...
    /// instead of {1, 0}.)
    let private jacobiEigenvaluesRealSymmetric (a: float[,]) (maxIterations: int) (tolerance: float) : float list =
    ...
    let eigenvaluesHermitian
        ...
        if n <> Array2D.length2 matrix then
            ...
            for i in 0 .. n - 1 do
                for j in 0 .. n - 1 do
                    if abs matrix.[i, j].Imaginary > 1e-14 then
                        hasImaginary <- true

            if not hasImaginary then
                // Symmetrize defensively against tiny Hermiticity violations.
                let a =
                    Array2D.init n n (fun i j -> 0.5 * (matrix.[i, j].Real + matrix.[j, i].Real))
    ...
    let vonNeumannEntropyFromDensityMatrix (rho: Complex[,]) : TopologicalResult<VonNeumannEntropyCalculation> =
    ...
    let groundStateDegeneracy (anyonType: AnyonSpecies.AnyonType) (genus: int) : TopologicalResult<int> =
        ...
        if genus < 0 then
            ...
            |> Result.bind (fun data -> ModularData.groundStateDegeneracy data genus)
    ...
    let groundStateDegeneracyEntropy (anyonType: AnyonSpecies.AnyonType) (genus: int) : TopologicalResult<float> =

        groundStateDegeneracy anyonType genus |> Result.map (float >> log)
    ...
    let compare (anyonType1: AnyonSpecies.AnyonType) (anyonType2: AnyonSpecies.AnyonType) : TopologicalResult<string> =
