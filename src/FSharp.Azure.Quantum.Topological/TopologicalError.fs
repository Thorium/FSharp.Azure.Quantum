...
type TopologicalError =
    ...
    member this.Message =
        ...
        | LogicError(operation, reason) -> $"Logic error in '{operation}': {reason}"

        | BackendError(backend, reason) -> $"Backend '{backend}' error: {reason}"

        | ComputationError(operation, context) -> $"Computation '{operation}' failed: {context}"
...
module TopologicalResult =
    ...
    let ofStringResultWithContext (context: string) (result: Result<'T, string>) : TopologicalResult<'T> =
        result
        |> Result.mapError (fun err -> TopologicalError.Other $"{context}: {err}")

    /// Convert TopologicalError to string (extracts Message)
    /// Temporary helper for backward compatibility
    let toString (error: TopologicalError) : string = error.Message

    /// Convert TopologicalResult to string-based Result
    /// Temporary helper for backward compatibility
    let toStringResult (result: TopologicalResult<'T>) : Result<'T, string> = result |> Result.mapError toString
...
module TopologicalResultBuilder =
    ...
    type TopologicalResultBuilder() =
        ...
        member _.Return(value: 'T) : TopologicalResult<'T> = Ok value

        /// Wraps a value in a successful TopologicalResult
        member _.ReturnFrom(result: TopologicalResult<'T>) : TopologicalResult<'T> = result
        ...
        /// Delays computation
        member _.Delay(f: unit -> TopologicalResult<'T>) : unit -> TopologicalResult<'T> = f

        /// Runs delayed computation
        member _.Run(f: unit -> TopologicalResult<'T>) : TopologicalResult<'T> = f ()

        /// Combines two TopologicalResults sequentially
        member _.Combine
            (result1: TopologicalResult<unit>, result2: unit -> TopologicalResult<'T>)
            : TopologicalResult<'T> =
            Result.bind (fun () -> result2 ()) result1

        /// Zero value (unit result)
        member _.Zero() : TopologicalResult<unit> = Ok()

        /// Try-with for exception handling
        member _.TryWith
            (body: unit -> TopologicalResult<'T>, handler: exn -> TopologicalResult<'T>)
            : TopologicalResult<'T> =
            try
                body ()
            with ex ->
        ...
        member this.Using
            (resource: 'T :> System.IDisposable, binder: 'T -> TopologicalResult<'U>)
            : TopologicalResult<'U> =
            this.TryFinally(
                (fun () -> binder resource),
                (fun () ->
