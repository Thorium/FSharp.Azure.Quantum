...
module BraidToGate =
    ...
    let cliffordPlusT = Set.ofList [ "H"; "CNOT"; "T"; "S"; "X"; "Y"; "Z" ]
    ...
    let rec getAffectedQubits (gate: CircuitBuilder.Gate) : int list =
        match gate with
        | CircuitBuilder.Gate.H q
        | CircuitBuilder.Gate.X q
        | CircuitBuilder.Gate.Y q
        | CircuitBuilder.Gate.Z q
        | CircuitBuilder.Gate.P(q, _)
        | CircuitBuilder.Gate.T q
        | CircuitBuilder.Gate.TDG q
        | CircuitBuilder.Gate.S q
        | CircuitBuilder.Gate.SDG q
        | CircuitBuilder.Gate.RZ(q, _)
        | CircuitBuilder.Gate.U3(q, _, _, _)
        | CircuitBuilder.Gate.RX(q, _)
        | CircuitBuilder.Gate.RY(q, _)
        | CircuitBuilder.Gate.Measure q
        | CircuitBuilder.Gate.Reset q -> [ q ]
        | CircuitBuilder.Gate.CNOT(c, t)
        | CircuitBuilder.Gate.CZ(c, t)
        | CircuitBuilder.Gate.SWAP(c, t)
        | CircuitBuilder.Gate.CP(c, t, _)
        | CircuitBuilder.Gate.CRX(c, t, _)
        | CircuitBuilder.Gate.CRY(c, t, _)
        | CircuitBuilder.Gate.CRZ(c, t, _)
        | CircuitBuilder.Gate.RXX(c, t, _)
        | CircuitBuilder.Gate.RYY(c, t, _)
    ...
    let isClifford (gate: CircuitBuilder.Gate) : bool =
        ...
        | CircuitBuilder.Gate.SDG _ -> true
        | _ -> false
    ...
    let countTGates (gates: CircuitBuilder.Gate list) : int =
        ...
        |> List.filter (fun g ->
            ...
            | CircuitBuilder.Gate.TDG _ -> true
            | _ -> false)
        |> List.length
    ...
    let private su2HalfSpinRSymbols (k: int) : Complex * Complex =
        ...
        | Ok rData ->
            let getR c =
                ...
                |> Result.defaultWith (fun e -> failwith $"R-symbol lookup failed for SU(2)_%d{k}: %A{e}")
    ...
    let accumulateBraidingPhase
        ...
        : Complex =
        generators
        |> List.fold (fun (acc: Complex) gen -> acc * braidingPhase anyonType gen.IsClockwise) Complex.One
    ...
    let isingBraidingToGates (generatorIndex: int) (isClockwise: bool) (strandCount: int) : CircuitBuilder.Gate list =
        ...
        if generatorIndex % 2 <> 0 then
            failwith
                $"Braid generator σ_{generatorIndex} is a cross-pair exchange (odd leaf index) and cannot be mapped to a single-qubit gate in the Ising σ-pair encoding"
            ...
            if numQubits = 0 then
                ...
                let phaseGate =
                    if isClockwise then
                        CircuitBuilder.Gate.S last
                    else
                        CircuitBuilder.Gate.SDG last
    ...
    let fibonacciBraidingToGates
        (generatorIndex: int)
        (isClockwise: bool)
        (tolerance: float)
        : CircuitBuilder.Gate list =
        ...
        if generatorIndex % 2 <> 0 then
            failwith
                $"Fibonacci braid generator σ_{generatorIndex} (cross-pair σ₂ exchange) mixes fusion channels via the F-matrix and cannot be mapped to a phase gate"
    ...
    let optimizeBasic (gates: CircuitBuilder.Gate list) : CircuitBuilder.Gate list =
        gates |> cancelInverses |> mergeRotations |> cancelInverses // Run again after merging
    ...
    let templateMatching (gates: CircuitBuilder.Gate list) : CircuitBuilder.Gate list =
        let rec loop acc remaining =
            ...
            | CircuitBuilder.Gate.SDG q1 :: CircuitBuilder.Gate.SDG q2 :: rest when q1 = q2 ->
                loop acc (CircuitBuilder.Gate.Z q1 :: rest)
            // H Z H → X
            | CircuitBuilder.Gate.H q1 :: CircuitBuilder.Gate.Z q2 :: CircuitBuilder.Gate.H q3 :: rest when
                q1 = q2 && q2 = q3
                ->
                loop acc (CircuitBuilder.Gate.X q1 :: rest)
            // H X H → Z
            | CircuitBuilder.Gate.H q1 :: CircuitBuilder.Gate.X q2 :: CircuitBuilder.Gate.H q3 :: rest when
                q1 = q2 && q2 = q3
                ->
                loop acc (CircuitBuilder.Gate.Z q1 :: rest)
    ...
    let compileGenerator
        ...
        match anyonType with
        | AnyonSpecies.AnyonType.Ising -> isingBraidingToGates gen.Index gen.IsClockwise strandCount
        ...
        | AnyonSpecies.AnyonType.SU2Level k ->
            ...
            if gen.Index % 2 <> 0 then
                failwith
                    $"Braid generator σ_{gen.Index} (cross-pair exchange) cannot be mapped to a single-qubit gate for {anyonType}"
    ...
    let compileToGates
        ...
        (anyonType: AnyonSpecies.AnyonType)
        (options: CompilationOptions)
        : Result<GateSequence, TopologicalError> =

        try
            // Compile each generator to gates
            let allGates =
                braid.Generators
                |> List.collect (fun gen -> compileGenerator gen anyonType braid.StrandCount options)
            ...
            let numQubits =
                ...
                | AnyonSpecies.AnyonType.Fibonacci
                | AnyonSpecies.AnyonType.SU2Level _ -> max 1 ((braid.StrandCount - 1) / 2)
...
Non-Clifford gates: {nonCliffordCount}"""
    ...
    let toCircuit (sequence: GateSequence) : CircuitBuilder.Circuit =
        ...
        CircuitBuilder.empty qubits |> CircuitBuilder.addGates sequence.Gates
    ...
    let compileToCircuit
        ...
        compileToGates braid anyonType options |> Result.map toCircuit
    ...
    let executeOnGateBackend
        ...
        |> Result.bind (fun circuit ->
            (backend.ExecuteToState(Core.CircuitAbstraction.wrapCircuit circuit))
            |> Result.mapError (fun qerr -> TopologicalError.BackendError("gate-backend", qerr.Message)))
