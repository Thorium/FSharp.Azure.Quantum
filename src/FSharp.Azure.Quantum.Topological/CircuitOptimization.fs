...
module CircuitOptimization =
    ...
    let rec cancelInverses (gates: GateSequence) : GateSequence =
        ...
        | [ g ] -> [ g ]
        | T :: TDagger :: rest
        | TDagger :: T :: rest -> cancelInverses rest
        | S :: SDagger :: rest
        | SDagger :: S :: rest -> cancelInverses rest
    ...
    let calculateDepth (gates: GateSequence) : int = gates.Length

    /// Apply basic optimizations
    let optimizeBasic (gates: GateSequence) : GateSequence =
        gates |> cancelInverses |> mergeAdjacentGates |> cancelInverses // Run again after merging
    ...
    let displayStats (stats: OptimizationStats) : string =
        let gateReduction =
            if stats.OriginalGateCount > 0 then
                100.0 * float (stats.OriginalGateCount - stats.OptimizedGateCount)
                / float stats.OriginalGateCount
            else
                0.0

        let tReduction =
            if stats.OriginalTCount > 0 then
                100.0 * float (stats.OriginalTCount - stats.OptimizedTCount)
                / float stats.OriginalTCount
            else
                0.0

        let depthReduction =
            if stats.OriginalDepth > 0 then
                100.0 * float (stats.OriginalDepth - stats.OptimizedDepth)
                / float stats.OriginalDepth
            else
                0.0
