...
module SurfaceCode =
    ...
    let initializePlanarGroundState (lattice: PlanarLattice) : PlanarCodeState =
        ...
        let qubits = edges |> List.map (fun e -> (e, Plus)) |> Map.ofList
    ...
    let getPlanarVertexEdges (lattice: PlanarLattice) (vertex: Coords) : PlanarEdge list =
        ...
        [
            ...
            if vertex.X > 0 then
                {
                    Position = { X = vertex.X - 1; Y = vertex.Y }
                    EdgeType = PVertical
                }
            ...
            if vertex.Y > 0 then
                {
                    Position = { X = vertex.X; Y = vertex.Y - 1 }
                    EdgeType = PHorizontal
                }
            // Above vertical edge
    ...
    let getPlanarPlaquetteEdges (lattice: PlanarLattice) (plaquette: Coords) : PlanarEdge list =
        [
            {
                Position = { X = plaquette.X; Y = plaquette.Y }
                EdgeType = PHorizontal
            }
            {
                Position = { X = plaquette.X; Y = plaquette.Y + 1 }
                EdgeType = PHorizontal
            }
            {
                Position = { X = plaquette.X; Y = plaquette.Y }
            ...
            {
                Position = { X = plaquette.X + 1; Y = plaquette.Y }
                EdgeType = PVertical
            }
        ]
    ...
    let planarDistance (p1: Coords) (p2: Coords) : int = abs (p1.X - p2.X) + abs (p1.Y - p2.Y)
    ...
    let decodePlanarSyndrome
        ...
        if defects.IsEmpty then
            ...
            let boundaryEdges =
                defects
                |> List.map (fun d -> (d, boundaryNode, distanceToBoundary lattice d isXDefects))
            ...
            let rec matchLoop remaining (matched: Set<Coords>) pairs boundaryCount =
                ...
                | [] -> (pairs, boundaryCount)
                | (a, _, _, isBoundary) :: rest when matched.Contains a -> matchLoop rest matched pairs boundaryCount
                | (_, b, _, false) :: rest when matched.Contains b -> matchLoop rest matched pairs boundaryCount
                | (a, b, _, isBoundary) :: rest ->
                    let newMatched =
                        if isBoundary then
                            matched |> Set.add a
                        else
                            matched |> Set.add a |> Set.add b

                    let newBoundaryCount = if isBoundary then boundaryCount + 1 else boundaryCount
            ...
            let totalWeight =
                ...
                |> List.sumBy (fun (a, b) ->
                    if b = boundaryNode then
                        distanceToBoundary lattice a isXDefects
                    else
                        planarDistance a b)
                ...
                |> List.collect (fun (a, b) ->
                    let target =
                        if b = boundaryNode then
                            ...
                            if isXDefects then
                                let ty =
                                    if a.Y <= (lattice.Distance - 2) / 2 then
                                        -1
                                    else
                                        lattice.Distance - 1
                                ...
                                let tx =
                                    if a.X <= (lattice.Distance - 2) / 2 then
                                        -1
                                    else
                                        lattice.Distance - 1
                            ...
                            edges <-
                                {
                                    Position =
                                        {
                                            X = cx |> max 0 |> min (lattice.Distance - 1)
                                            Y = edgeY
                                        }
                                    EdgeType = PHorizontal
                                }
    ...
    let decodePlanarCode
        ...
        |> Result.bind (fun xResult ->
            ...
            |> Result.map (fun zResult ->
                // Apply X corrections
                let afterX =
                    xResult.Corrections |> List.fold (fun s e -> applyPlanarZError e s) state
                // Apply Z corrections
                let afterXZ =
                    zResult.Corrections |> List.fold (fun s e -> applyPlanarXError s e) afterX
    ...
    let colorCodePhysicalQubits (lattice: ColorCodeLattice) : int = lattice.QubitPositions.Length
    ...
    let initializeColorCodeGroundState (lattice: ColorCodeLattice) : ColorCodeState =
        let qubits =
            lattice.QubitPositions |> List.map (fun pos -> (pos, Plus)) |> Map.ofList
    ...
    let decodeColorCodeSyndrome
        ...
        if defects.IsEmpty then
            ...
            let totalWeight = pairs |> List.sumBy (fun (a, b) -> planarDistance a b)
    ...
    let decodeColorCode
        ...
        |> Result.bind (fun xResult ->
            ...
            |> Result.map (fun zResult ->
                // Apply corrections
                let afterX =
                    xResult.Corrections |> List.fold (fun s pos -> applyColorCodeZError s pos) state
    ...
    let isValid3Coloring (lattice: ColorCodeLattice) : bool =
        let facesByVertex =
            lattice.Faces
            |> List.collect (fun face -> face.Vertices |> List.map (fun v -> (v, face)))
            |> List.groupBy fst
