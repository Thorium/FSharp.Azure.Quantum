...
module TopologicalOperations =
    ...
    let normalize (superposition: Superposition) : Superposition =
        ...
        let normSquared =
            combined.Terms |> List.sumBy (fun (amp, _) -> (Complex.Abs amp) ** 2.0)
    ...
    let rec private collectNodesAtDepth
        (targetDepth: int)
        (tree: FusionTree.Tree)
        : (Branch list * FusionTree.Tree) list =
    ...
    let rec private replaceAtPath
        (path: Branch list)
        (replacement: FusionTree.Tree)
        (tree: FusionTree.Tree)
        : FusionTree.Tree =
    ...
    let fMove (direction: FMoveDirection) (nodeDepth: int) (state: FusionTree.State) : Superposition =
        ...
        if targets.IsEmpty then
            ...
            targets
            |> List.fold
                (fun (superpos: Superposition) (path, subtree) ->
                    let expanded =
                        superpos.Terms
    ...
    let rec private tryFindFusedLeafPairChannel
        (targetLeftIndex: int)
        (tree: FusionTree.Tree)
        : AnyonSpecies.Particle option =
    ...
    let braidAdjacentAnyonsDirected
        ...
        if leftIndex < 0 || leftIndex >= anyons.Length - 1 then
            ...
            topologicalResult {
                ...
                let braidWithinTriple
                    ...
                    topologicalResult {
                        ...
                        let braidedInRightBasis =
                            ...
                            |> List.choose (fun (fAmp, rightAssocSubtree) ->
                                ...
                                | FusionTree.Fusion(_, FusionTree.Fusion(_, _, f), _) ->
                                    (BraidingOperators.element b c f state.AnyonType)
                                    |> Result.map (fun rPhase ->
                                        Some(fAmp * conjugateIfInverse isClockwise rPhase, rightAssocSubtree))
                        ...
                        let! backTerms =
                            braidedInRightBasis
                            |> List.fold
                                (fun accResult (amp, rightAssocTree) ->
                                    topologicalResult {
                                        let! acc = accResult
                                        ...
                                        let expanded =
                                            invTerms
                                            |> List.map (fun (invAmp, leftAssocTree2) ->
                                                (amp * invAmp, leftAssocTree2))
                ...
                // ((a×b→e)×c→d)
                | FusionTree.Fusion(FusionTree.Fusion(FusionTree.Leaf a, FusionTree.Leaf b, e), FusionTree.Leaf c, d) when
                    leftIndex = 0
                    ->
                    ...
                    return
                        normalize
                            {
                                Terms = [ (conjugateIfInverse isClockwise phase, state) ]
                                AnyonType = state.AnyonType
                            }
                ...
                | FusionTree.Fusion(FusionTree.Fusion(FusionTree.Leaf a, FusionTree.Leaf b, e), FusionTree.Leaf c, d) when
                    ...
                    let mixedStates =
                        mixed |> List.map (fun (amp, t) -> (amp, FusionTree.create t state.AnyonType))
                ...
                // (a×(b×c→f)→d)
                | FusionTree.Fusion(FusionTree.Leaf a, FusionTree.Fusion(FusionTree.Leaf b, FusionTree.Leaf c, f), d) when
                    leftIndex = 1
                    ->
                ...
                | FusionTree.Fusion(FusionTree.Leaf a, FusionTree.Fusion(FusionTree.Leaf b, FusionTree.Leaf c, f), d) when
                    ...
                    let braidedLeft =
                        ...
                        |> List.choose (fun (amp, leftAssocTree) ->
                            ...
                            | FusionTree.Fusion(FusionTree.Fusion(FusionTree.Leaf a2, FusionTree.Leaf b2, e),
                                                FusionTree.Leaf c2,
                                                d2) ->
                                (BraidingOperators.element a2 b2 e state.AnyonType)
                                |> Result.map (fun rPhase ->
                                    Some(amp * conjugateIfInverse isClockwise rPhase, leftAssocTree))
                                |> Result.defaultValue None
                            | _ -> None)
                    ...
                    let! backTerms =
                        braidedLeft
                        |> List.fold
                            (fun accResult (amp, leftAssocTree) ->
                                topologicalResult {
                                    let! acc = accResult
                                    ...
                                    let expanded =
                                        invTerms
                                        |> List.map (fun (invAmp, rightAssocTree) -> (amp * invAmp, rightAssocTree))
                    ...
                    let mixedStates =
                        backTerms
                        |> List.map (fun (amp, t) -> (amp, FusionTree.create t state.AnyonType))
    ...
    let measureFusion (leftIndex: int) (state: FusionTree.State) : TopologicalResult<(float * OperationResult) list> =
        ...
        if leftIndex < 0 || leftIndex >= anyons.Length - 1 then
            TopologicalResult.validationError
                "leftIndex"
                $"Invalid measurement index {leftIndex} for {anyons.Length} anyons"
        else
            let anyon1 = anyons.[leftIndex]
            let anyon2 = anyons.[leftIndex + 1]
                ...
                if outcomes.IsEmpty then
                    ...
                    if outcomeProbs.IsEmpty then
                        ...
                        let! results =
                            outcomeProbs
                            |> List.fold
                                (fun resultsResult (outcome, probability) ->
                                    topologicalResult {
                                                ...
                                                |> List.fold
                                                    (fun treeResult p ->
                                                        topologicalResult {
                                                            ...
                                                            let intermediate =
                                                                FusionTree.totalCharge tree state.AnyonType

                                                            let! channels =
                                                                FusionRules.channels intermediate p state.AnyonType

                                                            if channels.IsEmpty then
                                                                return!
                                                                    TopologicalResult.logicError
                                                                        "fusion"
                                                                        $"Cannot fuse {intermediate} and {p}"
                                                            else
                                                                ...
                                                                | None ->
                                                                    return!
                                                                        TopologicalResult.logicError
                                                                            "fusion"
                                                                            "Internal error: channels empty after non-empty check"
                                                                | Some firstChannel ->
                                                                    return
                                                                        FusionTree.fuse
                                                                            tree
    ...
    let private qubitChannels (anyonType: AnyonSpecies.AnyonType) : AnyonSpecies.Particle * AnyonSpecies.Particle =
        ...
        | AnyonSpecies.AnyonType.SU2Level k -> AnyonSpecies.Particle.SpinJ(0, k), AnyonSpecies.Particle.SpinJ(2, k)
        | AnyonSpecies.AnyonType.Ising -> AnyonSpecies.Particle.Vacuum, AnyonSpecies.Particle.Psi
    ...
    let private replaceQubitChannel
        ...
        let rec collectPairs
            (t: FusionTree.Tree)
            : (AnyonSpecies.Particle * AnyonSpecies.Particle * AnyonSpecies.Particle) list option =
            match t with
            | FusionTree.Fusion(FusionTree.Leaf a, FusionTree.Leaf b, ch) -> Some [ (a, b, ch) ]
        ...
        | Some pairs ->
            ...
            let isIsingParityEncoding =
                pairs.Length >= 2
                && pairs
                   |> List.forall (fun (a, b, _) -> a = AnyonSpecies.Particle.Sigma && b = AnyonSpecies.Particle.Sigma)
                && (match anyonType with
                    | AnyonSpecies.AnyonType.Ising -> true
                    | AnyonSpecies.AnyonType.Fibonacci
                    | AnyonSpecies.AnyonType.SU2Level _ -> false)
            ...
            if qubitIndex < 0 || qubitIndex >= qubitPairCount then
                None
            else
                ...
                let withNewChannel =
                    pairs
                    |> List.mapi (fun i (a, b, ch) -> if i = qubitIndex then (a, b, newChannel) else (a, b, ch))
                ...
                let updatedPairs =
                    if isIsingParityEncoding then
                        let qubitChs =
                            withNewChannel
                            |> List.take (pairs.Length - 1)
                            |> List.map (fun (_, _, ch) -> ch)

                        let psiCount =
                            qubitChs |> List.filter ((=) AnyonSpecies.Particle.Psi) |> List.length

                        let parityChannel =
                            if psiCount % 2 = 0 then
    ...
    let private getQubitChannel (qubitIndex: int) (tree: FusionTree.Tree) : AnyonSpecies.Particle option =
        let rec findAtPairIndex
            (idx: int)
            (currentPairIdx: int)
            (t: FusionTree.Tree)
            : (int * AnyonSpecies.Particle option) =
            match t with
            | FusionTree.Fusion(FusionTree.Leaf _, FusionTree.Leaf _, ch) ->
                if currentPairIdx = idx then
                    (currentPairIdx + 1, Some ch)
                else
                    (currentPairIdx + 1, None)
            | FusionTree.Fusion(left, right, _) ->
    ...
    let cnot (controlIndex: int) (targetIndex: int) (superposition: Superposition) : TopologicalResult<Superposition> =
        ...
        if controlIndex < 0 || controlIndex >= numQubits then
            ...
            TopologicalResult.validationError "targetIndex" "Control and target qubits must be different"
        else
    ...
    let isNormalized (superposition: Superposition) : bool =
        let totalProb = superposition.Terms |> List.sumBy (fun (amp, _) -> probability amp)

        abs (totalProb - 1.0) < 1e-10

    /// Get the dimension of the Hilbert space
    let dimension (superposition: Superposition) : int = superposition.Terms.Length
    ...
    let measureAll (superposition: Superposition) (shots: int) : int[][] =
        ...
        let probabilities = normalized.Terms |> List.map (fun (amp, _) -> probability amp)

        let cumulativeProbs = probabilities |> List.scan (+) 0.0 |> List.tail // Remove initial 0.0
    ...
    let probabilityOfBitstring (bitstring: int[]) (superposition: Superposition) : float =
        ...
        |> List.sumBy (fun (amp, state) ->
            ...
            if bitsArray.Length = bitstring.Length && Array.forall2 (=) bitsArray bitstring then
    ...
    let getBasisStates (superposition: Superposition) : FusionTree.Tree list =
        superposition.Terms |> List.map (fun (_, state) -> state.Tree)
    ...
    let getAmplitudes (superposition: Superposition) : Complex[] =
        superposition.Terms |> List.map fst |> Array.ofList
    ...
    type SuperpositionWrapper(superposition: Superposition) =
        ...
        interface ITopologicalSuperposition with
            ...
            member _.MeasureAll shots = measureAll superposition shots

            member _.Probability bitstring =
                probabilityOfBitstring bitstring superposition

            member _.IsNormalized = isNormalized superposition
