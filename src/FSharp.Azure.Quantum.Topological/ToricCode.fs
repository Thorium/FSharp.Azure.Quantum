...
module ToricCode =
    ...
    /// Edge identifier
    type Edge = { Position: Coords; Type: EdgeType }
    ...
    let getVertexEdges (lattice: Lattice) (vertex: Coords) : Edge list =
        ...
        [
            {
                Position = wrapCoords lattice { X = v.X - 1; Y = v.Y }
                Type = Horizontal
            }
            { Position = v; Type = Horizontal }
            {
                Position = wrapCoords lattice { X = v.X; Y = v.Y - 1 }
                Type = Vertical
            }
    ...
    let getPlaquetteEdges (lattice: Lattice) (plaquette: Coords) : Edge list =
        ...
        [
            ...
            {
                Position = wrapCoords lattice { X = p.X; Y = p.Y + 1 }
                Type = Horizontal
            }
            { Position = p; Type = Vertical }
            {
                Position = wrapCoords lattice { X = p.X + 1; Y = p.Y }
                Type = Vertical
            }
        ]
    ...
    let initializeGroundState (lattice: Lattice) : ToricCodeState =
        ...
        let qubits = edges |> List.map (fun edge -> (edge, Plus)) |> Map.ofList
    ...
    let physicalQubits (lattice: Lattice) : int = 2 * lattice.Width * lattice.Height

    /// Code distance
    ///
    /// Minimum weight of a logical operator = min(Width, Height)
    let codeDistance (lattice: Lattice) : int = min lattice.Width lattice.Height
    ...
    let greedyMatching (lattice: Lattice) (excitations: Coords list) : TopologicalResult<(Coords * Coords) list> =
    ...
    let correctionPath (lattice: Lattice) (p1: Coords) (p2: Coords) (kind: SyndromeKind) : Edge list =

        let wrapX x =
            ((x % lattice.Width) + lattice.Width) % lattice.Width

        let wrapY y =
            ((y % lattice.Height) + lattice.Height) % lattice.Height
        ...
        let dxShortest =
            ...
            elif dx > 0 then dx - lattice.Width
            else dx + lattice.Width

        let dyShortest =
            if abs dy <= lattice.Height - abs dy then dy
            elif dy > 0 then dy - lattice.Height
            else dy + lattice.Height
                ...
                [
                    for i in 0 .. abs dxShortest - 1 do
                        ...
                        | VertexSyndrome ->
                            ...
                            {
                                Position = { X = wrapX edgeX; Y = wrapY p1.Y }
                                Type = Horizontal
                            }
                        | PlaquetteSyndrome ->
    ...
    let decodeVertexSyndrome (lattice: Lattice) (syndrome: Syndrome) : TopologicalResult<DecoderResult> =
        ...
        |> Result.map (fun pairs ->
            ...
            let totalWeight = pairs |> List.sumBy (fun (p1, p2) -> toricDistance lattice p1 p2)
    ...
    /// m-particle pairs.
    let decodePlaquetteSyndrome (lattice: Lattice) (syndrome: Syndrome) : TopologicalResult<DecoderResult> =
    ...
    let applyCorrections
        ...
        corrections
        |> List.fold
            (fun currentState chain -> chain.Edges |> List.fold (fun s edge -> applyFn s edge) currentState)
            state
    ...
    let decodeSyndrome (state: ToricCodeState) : TopologicalResult<ToricCodeState * DecoderResult * DecoderResult> =
