    ...
    [
        {
            ...
            Description = "Which example: sudoku|queens|scheduling|all"
            Default = Some "all"
        }
        {
...
let puzzle4x4 = [| 1; 2; 0; 0; 3; 0; 1; 2; 2; 0; 4; 3; 0; 3; 0; 1 |]
    ...
    [
        {
            Id = 0
            Name = "Alice"
            Skills = [ "Machine A"; "Machine B" ]
            AvailableShifts = [ 0; 1; 2 ]
        }
        {
            Id = 1
            Name = "Bob"
        ...
        }
        {
            Id = 2
            Name = "Charlie"
            Skills = [ "Machine B"; "Machine C" ]
            AvailableShifts = [ 0; 2; 4 ]
        }
        {
            Id = 3
            Name = "Diana"
            Skills = [ "Machine C" ]
            AvailableShifts = [ 1; 3; 4 ]
        }
        {
            Id = 4
            Name = "Eve"
            Skills = [ "Machine A"; "Machine C" ]
            AvailableShifts = [ 0; 3; 4 ]
        }
    ]
    ...
    [
        {
            Id = 0
            Name = "Morning A"
            RequiredSkill = "Machine A"
        }
        {
            Id = 1
            Name = "Morning B"
            RequiredSkill = "Machine B"
        }
        {
            Id = 2
            Name = "Afternoon A"
...
let checkSudoku4x4 (assignment: Map<int, int>) =
    if assignment.Count < emptyCells.Length then
        false
    else
...
let checkQueens (assignment: Map<int, int>) =
    if assignment.Count < 4 then
        false
    else
        let positions =
            [ 0..3 ]
            |> List.choose (fun row -> Map.tryFind row assignment |> Option.map (fun col -> (row, col)))

        if positions.Length < 4 then
            false
        else
            let uniqueCols = positions |> List.map snd |> List.distinct |> List.length = 4

            let noDiagonalAttacks =
                positions
                |> List.forall (fun (r1, c1) ->
                    positions
                    |> List.forall (fun (r2, c2) -> r1 = r2 || (abs (r1 - r2) <> abs (c1 - c2))))

            uniqueCols && noDiagonalAttacks

let checkSchedule (assignment: Map<int, int>) =
    if assignment.Count < 5 then
        false
    else
        let allShiftsAssigned =
            [ 0..4 ] |> List.forall (fun shift -> Map.containsKey shift assignment)

        if not allShiftsAssigned then
            false
        else
    ...
    | Ok solution ->
        if not quiet then
            ...
            for shift in shifts do
                ...
                let worker = workers.[workerId]
                printfn "    %s -> %s (skill: %s)" shift.Name worker.Name shift.RequiredSkill
            ...
            for worker in workers do
                let assignedShifts =
                    shifts
                    |> List.filter (fun shift -> solution.Assignment.[shift.Id] = worker.Id)
