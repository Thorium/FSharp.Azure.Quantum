...
module Board =
    ...
    let isValidPosition (board: Board) (pos: Position) : bool =
        pos.Row >= 0
        && pos.Row < board.Config.Size
        && pos.Col >= 0
        && pos.Col < board.Config.Size
    ...
    let isEmpty (board: Board) (pos: Position) : bool = getCell board pos = Empty
    ...
    let checkWinAtPosition (board: Board) (pos: Position) (player: Cell) : bool =
        if getCell board pos <> player then
            ...
            directions
            |> List.exists (fun dir -> countInDirection board pos player dir >= board.Config.WinLength)
