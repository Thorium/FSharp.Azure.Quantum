...
module LocalHybrid =
    ...
    let explainStrategy (metrics: MoveMetrics) : string =
        ...
        | Classical reason ->
            sprintf
                """Strategy: Classical Heuristic Search
Reason: %s

Approach: Evaluated %d positions using pattern-based scoring
          Selected move with highest tactical value"""
                reason
        ...
        | Quantum reason ->
            ...
            sprintf
                """Strategy: Quantum-Enhanced Search
Reason: %s
...
Problem size does not warrant quantum overhead."""
    ...
    let updateThresholds (adaptive: AdaptiveHybrid) (metrics: MoveMetrics) (measuredSpeedup: float) : unit =
        ...
        | Classical _ -> adaptive.PerformanceHistory <- (false, 1.0) :: adaptive.PerformanceHistory
