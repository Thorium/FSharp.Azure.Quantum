...
module Classical =
    ...
    let private countConsecutiveDetail (board: Board) (pos: Position) (player: Cell) (dir: Direction) : int * int =
        ...
        let rec countDirection pos delta =
            let nextPos =
                {
                    Row = pos.Row + fst delta
                    Col = pos.Col + snd delta
                }
    ...
    let private isOpenEnded (board: Board) (pos: Position) (dir: Direction) (player: Cell) : bool =
        ...
        let frontPos =
            {
                Row = pos.Row + rowDelta * (forward + 1)
                Col = pos.Col + colDelta * (forward + 1)
            }
        // Back end: one step past the last piece in backward direction
        let backPos =
            {
                Row = pos.Row - rowDelta * (backward + 1)
                Col = pos.Col - colDelta * (backward + 1)
            }
    ...
    let evaluatePosition (board: Board) (pos: Position) (player: Cell) : float =
        if not (Board.isEmpty board pos) then
            ...
            | Ok newBoard ->
                ...
                let threatScore =
                    directionThreats |> List.sumBy (fun (_, threat, _, _) -> float (int threat))
                ...
                let significantThreats =
                    directionThreats
                    |> List.filter (fun (_, threat, _, _) -> threat >= ThreatLevel.OpenTwo) // OpenTwo or better
                    |> List.length

                let openThreeOrBetter =
                    directionThreats
                    |> List.filter (fun (_, threat, _, _) -> threat >= ThreatLevel.OpenThree) // OpenThree or better
                    |> List.length
                ...
                let centerBonus =
                    if List.isEmpty board.MoveHistory then
                        ...
                        let distFromCenter = abs (pos.Row - centerRow) + abs (pos.Col - centerCol)
    ...
    let evaluateMoves (board: Board) (player: Cell) : (Position * float) list =
        ...
        |> List.map (fun pos ->
            ...
            let offenseMultiplier = if player = White && moveNumber <= 50 then 1.1 else 1.5
    ...
    let selectBestMove (board: Board) : Position option =
        ...
        | None ->
            ...
            | topMoves ->
                ...
                let candidates =
                    topMoves |> List.filter (fun (_, score) -> abs (score - bestScore) < epsilon)
    ...
    let getTopCandidates (board: Board) (n: int) : Position list =
        ...
        scored |> List.truncate n |> List.map fst
