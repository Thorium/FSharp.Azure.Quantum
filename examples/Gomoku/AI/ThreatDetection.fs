...
module ThreatDetection =
    ...
    let private countInDirectionWithGaps
        ...
        (dir: Direction)
        (maxGaps: int)
        : int * int = // (total pieces, total gaps)
        ...
        let rec countDir offset gapsSoFar piecesSoFar =
            if gapsSoFar > maxGaps then
                ...
                if not (Board.isValidPosition board checkPos) then
                    ...
                    | c when c = player -> countDir (offset + 1) gapsSoFar (piecesSoFar + 1)
                    | Cell.Empty when gapsSoFar < maxGaps -> countDir (offset + 1) (gapsSoFar + 1) piecesSoFar
        ...
        | Ok newBoard ->
            ...
            |> List.exists (fun dir ->
                ...
                if total >= 4 then
                    ...
                    let fwdOpen =
                        Board.isValidPosition newBoard fwdEnd
                        && Board.getCell newBoard fwdEnd = Cell.Empty

                    let bwdOpen =
                        Board.isValidPosition newBoard bwdEnd
                        && Board.getCell newBoard bwdEnd = Cell.Empty
    ...
    let getImmediateThreat (board: Board) : Position option =
        ...
        let relevantMoves =
            if legalMoves.Length > 100 && board.MoveHistory.Length > 4 then
                ...
                |> List.filter (fun pos ->
                    occupied
                    |> List.exists (fun occ -> abs (pos.Row - occ.Row) <= 2 && abs (pos.Col - occ.Col) <= 2))
            else
                legalMoves
        ...
        | [] ->
            ...
            let opponentWins =
                relevantMoves |> List.filter (fun pos -> isWinningPosition board pos opponent)
            ...
            | [] ->
                ...
                let ourFours =
                    relevantMoves |> List.filter (fun pos -> isOpenFour board pos currentPlayer)
                ...
                | [] ->
                    ...
                    let opponentFours =
                        relevantMoves |> List.filter (fun pos -> isOpenFour board pos opponent)
    ...
    let getAllThreats (board: Board) : (Position * float) list =
        ...
        |> List.choose (fun pos ->
            ...
            let score =
                if isWinningPosition board pos currentPlayer then
                    Some(pos, 100000.0) // WIN!
                elif isWinningPosition board pos opponent then
                    Some(pos, 90000.0) // MUST BLOCK!
                elif isOpenFour board pos currentPlayer then
                    Some(pos, 50000.0) // Create four
                elif isOpenFour board pos opponent then
                    Some(pos, 40000.0) // Block four
                elif isDangerousThree board pos opponent then
                    Some(pos, 10000.0) // Block three
                elif isDangerousThree board pos currentPlayer then
                    Some(pos, 8000.0) // Create three
                else
                    None
