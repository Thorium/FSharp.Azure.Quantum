...
module LocalQuantum =
    ...
    let private decodePosition (bits: bool list) : int =
        bits |> List.mapi (fun i bit -> if bit then (1 <<< i) else 0) |> List.sum

    /// Calculate number of qubits needed to represent N items
    let private calculateQubits (n: int) : int =
        if n <= 1 then 1 else int (ceil (log (float n) / log 2.0))
    ...
    let private groverSearch
        (scoredPositions: (Position * float) list)
        (backend: BackendAbstraction.IQuantumBackend)
        : QuantumResult<Position option> =
        let n = scoredPositions.Length

        if n = 0 then
            Ok None
        elif n = 1 then
            Ok(Some(fst scoredPositions.[0]))
        else
            try
                ...
                let indexToPosition =
                    scoredPositions |> List.mapi (fun i (pos, _) -> (i, pos)) |> Map.ofList
                ...
                | Ok result when not result.Solutions.IsEmpty ->
                    ...
                    let validSolutions =
                        result.Solutions |> List.filter (fun idx -> idx < scoredPositions.Length)
                    ...
                    if validSolutions.IsEmpty then
                        ...
                        let bestIdx = validSolutions |> List.maxBy (fun idx -> snd scoredPositions.[idx])
            ...
            with ex ->
                Error(QuantumError.OperationError("Grover search", $"Exception: {ex.Message}"))
    ...
    let selectBestMove
        (board: Board)
        (backend: BackendAbstraction.IQuantumBackend)
        (candidates: Position list option)
        : Position option * int =
