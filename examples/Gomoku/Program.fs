...
module Program =
    ...
    let playTurn (board: Board) (isAI: bool) (aiPlayer: AIPlayer option) : (Board option * bool) =
        if isAI then
            let pos =
                match aiPlayer with
                | Some ClassicalAI ->
                ...
                | Some LocalQuantumAI ->
                    ...
                    let (move, iterations) = LocalQuantum.selectBestMove board None
            ...
            match pos with
            | Some p -> (Board.makeMove board p, false)
            | None -> (None, false)
        else
            // Human player
            ...
            | InputHandler.Move pos -> (Board.makeMove board pos, false)
    ...
    let rec gameLoop (board: Board) (aiPlayer: AIPlayer) (playerIsBlack: bool) : GameResult option =
        ...
        | Board.InProgress ->
            ...
            match playTurn board isAITurn (Some aiPlayer) with
    ...
    let rec aiVsAiLoop (board: Board) (ai1: AIPlayer) (ai2: AIPlayer) (metrics: LocalHybrid.MoveMetrics list) : GameResult =
        // Display current state (less verbose for benchmark)
        ...
        | Board.InProgress ->
            ...
            match playTurn board true (Some currentAI) with
            | (Some newBoard, _) -> 
                ...
                aiVsAiLoop newBoard ai1 ai2 newMetrics
            | (None, _) ->
                { Winner = None; TotalMoves = board.MoveHistory.Length; AIMetrics = metrics; FinalBoard = board }
    ...
    let runBenchmark() : unit =
        ...
        let result = aiVsAiLoop board ClassicalAI LocalQuantumAI []
        startTime.Stop()
        ...
        try
            ...
            | [| "--ai-vs-ai"; ai1Name; ai2Name |] ->
                ...
                | Some ai1, Some ai2 ->
                    ...
                    let startTime = System.Diagnostics.Stopwatch.StartNew()
                    let result = aiVsAiLoop board ai1 ai2 []
                    startTime.Stop()
                    ...
                    | None ->
                        ConsoleRenderer.displayGameStatus Board.Draw
                        AnsiConsole.WriteLine()
                        AnsiConsole.MarkupLine("[yellow]Game ended in a draw![/]")
                ...
                | _ ->
                    ...
                    AnsiConsole.WriteLine()
                    AnsiConsole.MarkupLine("[cyan]Usage:[/] Gomoku --ai-vs-ai <ai1> <ai2>")
            ...
            | [| "--help" |] | [| "-h" |] ->
                ...
                AnsiConsole.MarkupLine("  [yellow]Gomoku --ai-vs-ai <ai1> <ai2>[/]  - Run automated AI vs AI match")
                ...
                AnsiConsole.MarkupLine("  Gomoku --ai-vs-ai classical quantum")
                AnsiConsole.MarkupLine("  Gomoku --ai-vs-ai quantum classical")
