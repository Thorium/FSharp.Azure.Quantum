...
let doErrorCorrection =
    System.String.Equals((Cli.getOr "error-correction" "true" args), "true", System.StringComparison.OrdinalIgnoreCase)
...
let result =
    runCompleteQKD keyLength quantumBackend doErrorCorrection securityParam seed

match result with
| Error err -> pr "[FAIL] QKD pipeline failed: %A" err
    ...
    |> Option.iter (fun path ->
        let payload =
            {|
                ...
                errorsDetected =
                    qkd.ErrorCorrection
                    |> Option.map (fun ec -> ec.ErrorsDetected)
                    |> Option.defaultValue 0
                errorsCorrected =
                    qkd.ErrorCorrection
                    |> Option.map (fun ec -> ec.ErrorsCorrected)
                    |> Option.defaultValue 0
