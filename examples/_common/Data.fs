...
module Data =
    ...
    let readAllBytes (path: string) = File.ReadAllBytes path
    ...
    let fileSha256Hex (path: string) = readAllBytes path |> sha256Hex
    ...
    let private splitCsvLine (line: string) : string array =
        ...
        finalState.FieldsRev |> List.rev |> List.map (fun x -> x.Trim()) |> List.toArray
    ...
    let readCsvWithHeader (path: string) : CsvRow list = readCsvWithHeaderWithErrors path |> fst
            ...
            [
                for elem in doc.RootElement.EnumerateArray() do
                    if elem.ValueKind = JsonValueKind.Object then
                        let values =
                            [ for prop in elem.EnumerateObject() -> prop.Name, prop.Value.ToString() ]
                            |> Map.ofList
    ...
    let resolveRelative (scriptDir: string) (path: string) : string =
        if Path.IsPathRooted path then
            ...
            Path.Combine(scriptDir, path) |> Path.GetFullPath
