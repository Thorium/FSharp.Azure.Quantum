...
Cli.exitIfHelp
    "SparseQubo_Example.fsx"
    "Sparse QUBO pipeline and budget-constrained QAOA execution"
    [
        {
            Name = "example"
            Description = "Which example: all, evaluate, gridsearch, optimize, budget"
            Default = Some "all"
        }
        {
            Name = "shots"
...
if runAll || exampleName = "gridsearch" then
    ...
    | Error e -> pr "GridSearch FAILED: %A" e
...
if runAll || exampleName = "optimize" then
    ...
    | Error e -> pr "Optimization FAILED: %A" e
...
if runAll || exampleName = "budget" then
    ...
    let denseQubo =
        ...
        sparseQubo |> Map.iter (fun (i, j) v -> q.[i, j] <- v)
        q
    ...
    pr
        "  Budget: %d total shots, %s time limit"
        budget.MaxTotalShots
    ...
    | Error e -> pr "Budget Execution FAILED: %A" e
