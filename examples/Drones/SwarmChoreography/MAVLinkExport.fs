...
module PythonScript =
    ...
    let generate (swarm: SwarmMission) : string =
        ...
        sb.AppendLine(sprintf "Generated at: %s" (swarm.Metadata.GeneratedAt.ToString("O")))
        |> ignore

        sb.AppendLine(sprintf "Optimization: %s" swarm.Metadata.OptimizationMethod)
        |> ignore
        ...
        sb.AppendLine("    from dronekit import connect, VehicleMode, LocationGlobalRelative")
        |> ignore
        ...
        sb.AppendLine("    print('Warning: dronekit not installed. Install with: pip install dronekit')")
        |> ignore
        ...
        for mission in swarm.Missions do
            sb.AppendLine(
                sprintf
                    "    {'name': '%s', 'connection': '%s', 'sysid': %d},"
                    mission.Drone.Name
                    mission.Drone.ConnectionString
                    mission.Drone.SystemId
            )
            |> ignore
        ...
        | Some home ->
            sb.AppendLine(
                sprintf
                    "HOME_POSITION = (%.8f, %.8f, %.2f)  # (lat, lon, alt)"
                    home.Latitude
                    home.Longitude
                    home.Altitude
            )
            |> ignore
        | None -> sb.AppendLine("HOME_POSITION = None  # Use GPS position at arm time") |> ignore
        ...
        for mission in swarm.Missions do
            ...
            for item in mission.Items do
                sb.AppendLine(
                    sprintf
                        "        {'cmd': %d, 'lat': %.8f, 'lon': %.8f, 'alt': %.2f, 'p1': %.2f, 'p2': %.2f},"
                        (mavCmdId item.Command)
                        item.Latitude
                        item.Longitude
                        item.Altitude
        ...
        sb.AppendLine("def connect_drone(connection_string: str, timeout: int = CONNECTION_TIMEOUT):")
        |> ignore

        sb.AppendLine("    \"\"\"Connect to drone with timeout and return vehicle object.\"\"\"")
        |> ignore
        ...
        sb.AppendLine("    vehicle = connect(connection_string, wait_ready=True, timeout=timeout)")
        |> ignore

        sb.AppendLine("    print(f'  Connected. Firmware: {vehicle.version}')")
        |> ignore
        ...
        sb.AppendLine("    \"\"\"Arms vehicle and flies to target altitude.\"\"\"")
        |> ignore

        sb.AppendLine("    print(f'Arming and taking off to {target_altitude}m...')")
        |> ignore
        ...
        sb.AppendLine("        print('  Waiting for vehicle to become armable...')")
        |> ignore
        ...
        sb.AppendLine("        current_alt = vehicle.location.global_relative_frame.alt")
        |> ignore
        ...
        sb.AppendLine("def goto_position(vehicle, lat: float, lon: float, alt: float):")
        |> ignore
        ...
        sb.AppendLine("    vehicle.simple_goto(target, groundspeed=MAX_GROUNDSPEED)")
        |> ignore

        sb.AppendLine("    print(f'  Going to ({lat:.6f}, {lon:.6f}, {alt:.1f}m)')")
        |> ignore
        ...
        sb.AppendLine(
            "def wait_for_position(vehicle, lat: float, lon: float, radius: float = WAYPOINT_ACCEPTANCE_RADIUS, timeout: float = 60):"
        )
        |> ignore

        sb.AppendLine("    \"\"\"Wait until vehicle reaches position within acceptance radius.\"\"\"")
        |> ignore
        ...
        sb.AppendLine("        a = sin(dphi/2)**2 + cos(phi1)*cos(phi2)*sin(dlambda/2)**2")
        |> ignore
        ...
        sb.AppendLine("        current = vehicle.location.global_relative_frame")
        |> ignore

        sb.AppendLine("        distance = haversine(current.lat, current.lon, lat, lon)")
        |> ignore
        ...
        sb.AppendLine("            print(f'  WARNING: Waypoint timeout (distance: {distance:.1f}m)')")
        |> ignore
        ...
        sb.AppendLine("def execute_drone_mission(drone_config: dict, waypoints: list):")
        |> ignore
        ...
        sb.AppendLine("                print(f'{name}: Waypoint {i+1}/{len(waypoints)}')")
        |> ignore

        sb.AppendLine("                goto_position(vehicle, wp['lat'], wp['lon'], wp['alt'])")
        |> ignore

        sb.AppendLine("                wait_for_position(vehicle, wp['lat'], wp['lon'])")
        |> ignore
        ...
        sb.AppendLine("                print(f'{name}: Loitering for {wp[\"p1\"]}s')")
        |> ignore
        ...
        sb.AppendLine("    \"\"\"Main entry point for swarm choreography.\"\"\"")
        |> ignore

        sb.AppendLine("    if not DRONEKIT_AVAILABLE:") |> ignore

        sb.AppendLine("        print('Error: dronekit is required. Install with:')")
        |> ignore
        ...
        sb.AppendLine(sprintf "    print('MAVLink Swarm Choreography - %d Drones')" swarm.Metadata.NumDrones)
        |> ignore

        sb.AppendLine(sprintf "    print('Optimization: %s')" swarm.Metadata.OptimizationMethod)
        |> ignore
        ...
        sb.AppendLine("    # For single-threaded execution (safer for testing)")
        |> ignore

        sb.AppendLine("    for drone_config in DRONE_CONNECTIONS:") |> ignore

        sb.AppendLine("        waypoints = DRONE_WAYPOINTS.get(drone_config['name'], [])")
        |> ignore

        sb.AppendLine("        if waypoints:") |> ignore

        sb.AppendLine("            execute_drone_mission(drone_config, waypoints)")
        |> ignore
        ...
        sb.AppendLine("    \"\"\"Execute all drone missions in parallel (for synchronized swarm).\"\"\"")
        |> ignore
        ...
        sb.AppendLine("    parser = argparse.ArgumentParser(description='MAVLink Swarm Choreography')")
        |> ignore

        sb.AppendLine("    parser.add_argument('--parallel', action='store_true', help='Run drones in parallel')")
        |> ignore
...
let exportShow
    ...
    let swarm =
        createSwarmMission title homePosition scale transitionSpeed formations numDrones optimizationMethod
