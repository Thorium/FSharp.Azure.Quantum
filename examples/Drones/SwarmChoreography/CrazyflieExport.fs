...
let toJson (show: ShowDefinition) : string =
    ...
    for fi, formation in Array.indexed show.Formations do
        ...
        for wi, wp in Array.indexed formation.Waypoints do
            ...
            sb.AppendLine(
                $"        {{ \"drone_id\": {wp.DroneId}, \"x\": {wp.Position.X:F3}, \"y\": {wp.Position.Y:F3}, \"z\": {wp.Position.Z:F3}, \"duration\": {wp.Duration:F1}{colorStr} }}{wcomma}"
            )
            |> ignore
...
let toPythonScript (show: ShowDefinition) : string =
    ...
    for formation in show.Formations do
        ...
        for wp in formation.Waypoints do
            ...
            sb.AppendLine(
                $"            {wp.DroneId}: {{'x': {wp.Position.X:F3}, 'y': {wp.Position.Y:F3}, 'z': {wp.Position.Z:F3}, 'duration': {wp.Duration:F1}{colorStr}}},"
            )
            |> ignore
...
let fromTransitionResults
            ...
            for i in 0 .. numDrones - 1 do
                {
                    Id = i
                    Uri =
                        if i < defaultDroneUris.Length then
                            defaultDroneUris.[i]
                        else
                            $"radio://0/80/2M/E7E7E7E7{i:X2}"
                    Name = Some $"Drone {i}"
                }
        |]
                        ...
                        for i, pos in Array.indexed startFormation.Positions do
                            {
                                ...
                                Color =
                                    if i < defaultColors.Length then
                                        Some defaultColors.[i]
                                    else
                                        None
    ...
    for t in transitions do
        ...
        let targetFormation = formations |> Array.tryFind (fun f -> f.Name = t.ToFormation)
                        ...
                        [|
                            for a in t.Assignments |> Array.sortBy (fun x -> x.DroneId) do
...

                                {
                                    ...
                                    Color =
                                        if a.DroneId < defaultColors.Length then
                                            Some defaultColors.[a.DroneId]
                                        else
                                            None
                                    Duration = transitionDuration
                                }
...
module CollisionFreeExport =
    ...
    let private colorForDrone (droneId: int) : LedColor option =
        if droneId < defaultColors.Length then
            Some defaultColors.[droneId]
        else
            None
            ...
            |> List.map (fun path ->
                ...
                let delay, targetPos: float * CollisionAvoidance.Position3D =
                    ...
                    | [] ->
                        ...
                        (0.0,
                         {
                             CollisionAvoidance.Position3D.X = 0.0
                             Y = 0.0
                             Z = 0.0
                         })
                    | [ single ] ->
        ...
        let optimizationMethod =
            ...
            |> fun quantumCount ->
                if quantumCount > plans.Length / 2 then
                    "Quantum (QAOA) - Collision-Free"
                elif quantumCount > 0 then
                    "Hybrid - Collision-Free"
                else
                    "Classical - Collision-Free"
    ...
    let toJson (show: CollisionFreeShowDefinition) : string =
        ...
        |> Array.iteri (fun fi formation ->
            ...
            |> Array.iteri (fun wi wp ->
                ...
                sb.AppendLine(
                    $"        {{ \"drone_id\": {wp.DroneId}, \"x\": {wp.Position.X:F3}, \"y\": {wp.Position.Y:F3}, \"z\": {wp.Position.Z:F3}, \"delay\": {wp.DelaySeconds:F2}, \"duration\": {wp.DurationSeconds:F1}{colorStr} }}{wcomma}"
                )
                |> ignore)
    ...
    let toPythonScript (show: CollisionFreeShowDefinition) : string =
        ...
        show.Drones
        |> Array.iter (fun drone -> sb.AppendLine($"    '{drone.Uri}',  # Drone {drone.Id}") |> ignore)
        ...
        |> Array.iter (fun formation ->
            ...
            |> Array.iter (fun wp ->
                ...
                sb.AppendLine(
                    $"            {wp.DroneId}: {{'x': {wp.Position.X:F3}, 'y': {wp.Position.Y:F3}, 'z': {wp.Position.Z:F3}, 'delay': {wp.DelaySeconds:F2}, 'duration': {wp.DurationSeconds:F1}{colorStr}}},"
                )
                |> ignore)
