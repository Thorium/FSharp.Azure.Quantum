...
module Geometry =
    ...
    let positionAtTime (startPos: Position3D) (endPos: Position3D) (t: float) : Position3D = lerp t startPos endPos
...
module CollisionDetection =
    ...
    let checkPairCollision
        ...
        let minDist, timeOfMin =
            Geometry.minPathSeparation constraints.SamplesPerPath (startA, endA) (startB, endB)
    ...
    let checkTimingCollisions
        ...
        let getPathWithTiming droneId =
            let targetIdx =
                assignments |> List.tryFind (fun (d, _) -> d = droneId) |> Option.map snd
                ...
                for d1 in 0 .. n - 1 do
                    for d2 in d1 + 1 .. n - 1 do
                        ...
                        | Some pathA, Some pathB ->
                            let minDist, timeOfMin =
                                Geometry.minOffsetPathSeparation constraints.SamplesPerPath pathA pathB
...
let validateTransition
    ...
    CollisionDetection.detectCollisions constraints currentPositions targetPositions assignments
...
let planTransition
    ...
    let risk =
        validateTransition currentPositions targetFormation.Positions assignments constraints
    ...
    | Safe minSep ->
        ...
        let totalDist =
            assignments
            |> List.sumBy (fun (d, t) -> Geometry.distance currentPositions.[d] targetFormation.Positions.[t])
    ...
    | MultipleCollisions _ ->
        ...
        |> Result.map (fun timings ->
            ...
            let maxDelay = if k <= 1 then 0.0 else maxDelayStep / float (k - 1)
            let moveDuration = 1.0 / (float k + 1.0) // Safe: k >= 1 from constraints

            let paths =
                timings
                |> List.map (fun (droneId, delayStep) ->
                    let targetIdx = assignments |> List.find (fun (d, _) -> d = droneId) |> snd
                    let startTime = if k <= 1 then 0.0 else float delayStep / float k
...

            {
                ...
                Method =
                    if isSafe then
                        "Quantum (QAOA) - Staggered Timing"
                    else
                        "Quantum (QAOA) - Staggered Timing [UNRESOLVED COLLISION]"
            })
...
let ensureSafeTransition
    ...
    planTransition backend shots currentPositions targetFormation assignments PlanningConstraints.defaults
