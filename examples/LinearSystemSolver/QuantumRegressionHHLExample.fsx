...
Cli.exitIfHelp
    ...
    [
        {
            Name = "example"
            Description = "Which example: 1|2|3|all"
            Default = Some "all"
        }
        {
            Name = "eigenvalue-qubits"
            Description = "QPE qubits for HHL"
            Default = Some "4"
        }
        {
            Name = "shots"
            Description = "Simulation shots"
            Default = Some "5000"
        }
        {
            Name = "min-eigenvalue"
            Description = "Min eigenvalue threshold"
            Default = Some "0.01"
        }
        {
            Name = "output"
...
if shouldRun 1 then
    ...
    | Error err -> pr "Training FAILED: %s" err.Message
    | Ok result ->
        pr ""
...
if shouldRun 2 then
    ...
    let trainX = [| [| 1.0; 1.0 |]; [| 2.0; 1.0 |]; [| 1.0; 2.0 |]; [| 3.0; 2.0 |] |]
...
if shouldRun 3 then
    ...
    | Ok result ->
        ...
        let preds =
            QuantumRegressionHHL.predictBatch result.Weights testX result.HasIntercept
        ...
        let avgError = Array.map2 (fun p t -> abs (p - t)) preds testY |> Array.average
        let maxError = Array.map2 (fun p t -> abs (p - t)) preds testY |> Array.max
        pr ""
        pr "Summary:"
