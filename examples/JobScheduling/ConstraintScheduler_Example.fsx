...
Cli.exitIfHelp
    "ConstraintScheduler_Example.fsx"
    "Quantum constraint scheduling and resource allocation"
    [
        {
            Name = "example"
            Description = "Which example: all, simple, workforce, cloud, manufacturing"
            Default = Some "all"
        }
        {
            Name = "shots"
...
let displaySchedule (label: string) (result: Result<ConstraintScheduler.SchedulingResult, QuantumError>) =
    ...
    | Ok schedResult ->
        ...
        | Some schedule ->
            ...
            |> List.iter (fun a -> pr "    %s -> %s ($%.2f)" a.Task a.Resource a.Cost)
            ...
            |> List.iter (fun a ->
                csvRows <-
                    [ label; a.Task; a.Resource; $"%.2f{a.Cost}"; $"%b{schedule.IsFeasible}" ]
                    :: csvRows)
        | None ->
...
if runAll || exampleName = "cloud" then
    ...
    | Ok result ->
        ...
        | Some schedule ->
            // Group by server
            let byServer =
                schedule.Assignments |> List.groupBy (fun a -> a.Resource) |> List.sortBy fst

            pr "  VM Assignment:"

            byServer
            |> List.iter (fun (server, assignments) ->
                pr "    %s:" server
                assignments |> List.iter (fun a -> pr "      - %s ($%.2f)" a.Task a.Cost))
            ...
            |> List.iter (fun a ->
                csvRows <-
                    [ "cloud"; a.Task; a.Resource; $"%.2f{a.Cost}"; $"%b{schedule.IsFeasible}" ]
                    :: csvRows)
        | None ->
...
if runAll || exampleName = "manufacturing" then
    ...
    let manufacturingTasks: ScheduledTask<unit> list =
        [
            ...
            scheduledTask {
                taskId "Welding_Job1"
                duration (hours 1.0)
                requires "WeldingStation_A" 1.0
            }
            scheduledTask {
                taskId "Welding_Job2"
                duration (hours 1.0)
                requires "WeldingStation_B" 1.0
            }
            scheduledTask {
                taskId "Assembly_Job1"
                duration (hours 1.0)
                requires "AssemblyLine_1" 1.0
            }
    ...
    let manufacturingProblem: SchedulingProblem<unit, unit> =
        scheduling {
    ...
    | Ok schedule ->
        ...
        |> List.iter (fun a ->
            ...
            pr "    %-16s %.1fh -> %.1fh  on [%s]" a.TaskId a.StartTime.TotalHours a.EndTime.TotalHours res)
        ...
        let startOf id =
            ordered
            |> List.tryPick (fun a -> if a.TaskId = id then Some a.StartTime else None)

        match startOf "Welding_Job1", startOf "Assembly_Job1", startOf "Painting_Job1" with
        | Some w, Some asm, Some p ->
            pr "  Precedence respected: Painting after Welding (%b) and after Assembly (%b)" (p > w) (p > asm)
        | _ -> ()
