/// Drone Swarm Choreography Mini Example (4 Drones)
/// 
/// A smaller version of the SwarmChoreography example that fits within
/// the LocalBackend's 20-qubit limit (4 drones × 4 positions = 16 qubits).
/// 
/// This allows the quantum solver to actually execute rather than falling
/// back to the classical solver.
/// 
/// QUANTUM ADVANTAGE DEMONSTRATION:
/// - 4 drones = 16 QUBO variables = 16 qubits (fits LocalBackend)
/// - Direct comparison of classical vs quantum solutions
/// - Shows QAOA working on real problem instance
namespace FSharp.Azure.Quantum.Examples.Drone.SwarmChoreographyMini
...
type Metrics = {
    ...
    transitions: {| from_formation: string; to_formation: string; distance: float; method: string |}[]
    elapsed_ms: int64
}
...
module Program =
    ...
    let main argv =
        ...
        if Cli.hasFlag "help" args || Cli.hasFlag "h" args then
            printfn "╔══════════════════════════════════════════════════╗"
            printfn "║  DRONE SWARM CHOREOGRAPHY MINI                   ║"
            printfn "║  4-Drone Quantum-Ready Version                   ║"
            printfn "╠══════════════════════════════════════════════════╣"
            ...
            printfn "║    --method <m>      classical|quantum|both      ║"
            ...
            let outDir = Cli.getOr "out" (Path.Combine("runs", "drone", "swarm-mini")) args
            ...
            printfn "Method: %s | Shots: %d" method shots
            printfn ""
            ...
            for i in 0 .. formations.Length - 2 do
                ...
                let assignments, methodUsed =
                    ...
                    | _ -> // "both" - run both and compare
                        ...
                        printfn "Running QAOA with %d shots..." shots
                        let quantumResult = Solver.solve backend shots distMatrix
                        ...
                        | Ok quantumAssignments ->
                            ...
                            printfn "  │ COMPARISON                          │"
                            printfn "  ├─────────────────────────────────────┤"
                            printfn "  │ Classical: %8.2f m              │" classicalDist
                            printfn "  │ Quantum:   %8.2f m              │" quantumDist
                            if quantumDist < classicalDist then
                                printfn "  │ Winner: QUANTUM! (%.1f%% better)     │" ((classicalDist - quantumDist) / classicalDist * 100.0)
                            elif quantumDist > classicalDist then
                                printfn "  │ Winner: Classical (%.1f%% better)   │" ((quantumDist - classicalDist) / quantumDist * 100.0)
                            else
                                printfn "  │ Result: TIE                        │"
                            printfn "  └─────────────────────────────────────┘"
            ...
            let quantumWins = transitions |> Seq.filter (fun t -> t.Method.Contains("Quantum")) |> Seq.length
            let classicalWins = transitions.Count - quantumWins
            ...
            if method = "both" then
                printfn "╠══════════════════════════════════════════════════╣"
                printfn "║  QUANTUM vs CLASSICAL SCORE                      ║"
                printfn "║  Quantum wins: %d | Classical wins: %d             ║" quantumWins classicalWins
            printfn "╚══════════════════════════════════════════════════╝"
            ...
            let metrics: Metrics = {
                ...
                transitions = 
                    transitions.ToArray()
                    |> Array.map (fun t -> {| from_formation = t.FromFormation; to_formation = t.ToFormation; distance = t.TotalDistance; method = t.Method |})
            ...
            let report = $"""# Drone Swarm Choreography Mini Results
...
| Transition | From | To | Distance | Quantum | Winner |
|------------|------|----|----------:|--------:|--------|
{transitionTable}

## Key Difference from Full Version

This "mini" version uses **4 drones** instead of 8, which means:
- **16 qubits** instead of 64 qubits
- Fits within LocalBackend's 20-qubit limit
- Quantum solver can actually execute
- Direct comparison of quantum vs classical results possible
