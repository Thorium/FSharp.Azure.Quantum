...
module QuantumPlayer =
    ...
    type GameContext =
        {
            ...
            CardsRemaining: int
        } // cards still in deck + all hands combined
    ...
    type PlayEvaluation =
        ...
        } // the selected capture option (None for Place)
    ...
    let private evaluateOptionInContext
        (ctx: GameContext)
        (handCard: Card)
        (option: Rules.CaptureOption)
        ...
        | _ ->
            ...
            let best =
                ...
                | LaistoKasino ->
                    ...
                    evals
                    |> List.sortBy (fun e -> (e.PointValue, float e.CardsCaptured, (if e.IsSweep then 1.0 else 0.0)))
                    |> List.head

            { best with CaptureOptions = options }

    /// Evaluate all cards in hand against the table (static, for human display)
    let evaluateAllPlays
        (backend: BackendAbstraction.IQuantumBackend option)
        (hand: Card list)
        (tableCards: Card list)
        : PlayEvaluation list =
    ...
    let evaluateAllPlaysInContext
        ...
        : PlayEvaluation list =
        hand
        |> List.map (fun c -> evaluatePlayInContext backend ctx variant c tableCards)
    ...
    let chooseBestMisa
        ...
        if not (List.isEmpty nonCaptures) then
            ...
            nonCaptures
            |> List.sortByDescending (fun e -> (scoreFn e.HandCard, float (Cards.tableValue e.HandCard.Rank)))
            |> List.head
        else
