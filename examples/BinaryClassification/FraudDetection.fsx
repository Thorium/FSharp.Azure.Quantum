...
if shouldRun 2 then
    if not quiet then
        printfn "=== Example 2: Production Fraud Detector ===\n"
    ...
    | Ok classifier ->
        ...
        | Ok metrics ->
            ...
            if not quiet then
                ...
                printfn
                    "  Accuracy:  %.2f%%  (majority-class baseline: %.2f%%)"
                ...
                printfn
                    "  F1 Score:  %.2f%%  (harmonic mean of precision/recall -- the headline metric for imbalanced fraud)"
                    (metrics.F1Score * 100.0)
...
if shouldRun 4 then
    if not quiet then
        printfn "=== Example 4: Load and Use Saved Model ===\n"
    ...
    | Error err ->
        ...
        results.Add(
            {|
                ...
                Status = "skipped"
                Details = Map.ofList [ "reason", box "Model file not found (run example 2 first)" ]
            |}
        )
        ...
        | Ok prediction ->
            let recommendation =
                if prediction.IsPositive && prediction.Confidence > 0.8 then
                    "BLOCK"
                elif prediction.IsPositive && prediction.Confidence > 0.5 then
                    "REVIEW"
                else
                    "ALLOW"
