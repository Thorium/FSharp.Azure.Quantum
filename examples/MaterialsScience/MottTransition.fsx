    ...
    [
        {
            ...
            Description = "Comma-separated material short names (Si, Ge, VO2, V2O3, NdNiO3)"
            Default = None
        }
        {
        ...
        {
            Cli.OptionSpec.Name = "hopping"
            Description = "Hubbard hopping parameter t (eV)"
            Default = Some "0.1"
        }
        {
...
let hubbardU (material: MottMaterial) : float =
    ...
    let U_J =
        e_charge * e_charge
        / (4.0 * Math.PI * 8.854e-12 * material.DielectricConstant * a_H)
...
let printTable () =
    ...
    printfn
        "  %-6s %-26s %6s %6s %8s %10s %6s %6s %6s %10s"
        "Key"
        "Name"
        "eps_r"
        "m*/me"
    ...
    for mat in selectedMaterials do
        ...
        printfn
            "  %-6s %-26s %6.1f %6.2f %8.1f %10.2e %6.2f %6.2f %6.2f %10s"
            mat.ShortName
            mat.Name
            mat.DielectricConstant
            mat.EffectiveMass
            a_H
            mat.CriticalDensity
            U
    ...
    printfn "  %s" divV
    printfn "  %-7s %-22s %12s %6s %8s %8s" "R(A)" "Regime" "Energy(Ha)" "Iters" "Time(s)" "Status"
    printfn "  %s" divV
