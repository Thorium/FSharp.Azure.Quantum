    ...
    [
        {
            ...
            Description = "Comma-separated material short names (Si, Ge, GaAs, InP, CdTe, ZnO)"
            Default = None
        }
        {
            Cli.OptionSpec.Name = "input"
            Description = "CSV file with custom material definitions"
            Default = None
        }
        {
            Cli.OptionSpec.Name = "temperature"
            Description = "Temperature in Kelvin for Varshni analysis"
            Default = Some "300"
        }
        {
        ...
        | _ ->
            {
                ...
                EffectiveMassE =
                    get "effective_mass_e"
                    |> fun s ->
                        match Double.TryParse s with
                        | true, v -> v
                        | _ -> 0.1
                EffectiveMassH =
                    get "effective_mass_h"
                    |> fun s ->
                        match Double.TryParse s with
                        | true, v -> v
                        | _ -> 0.3
...
let bandGapVsTemperature (material: Semiconductor) (T_Kelvin: float) : float =
    let E_g0 =
        material.BandGap
        + material.Varshni_alpha * 300.0 * 300.0
          / (300.0 + material.Varshni_beta)
          / 1000.0

    E_g0
    - (material.Varshni_alpha / 1000.0) * T_Kelvin * T_Kelvin
      / (T_Kelvin + material.Varshni_beta)
...

    {
        ...
        Atoms =
            [
                ...
                {
                    Element = "H"
                    Position = (r, 0.0, h_coord)
                }
                {
                    Element = "H"
                    Position = (-r * 0.5, r * 0.866, h_coord)
                }
                {
                    Element = "H"
                    Position = (-r * 0.5, -r * 0.866, h_coord)
...
let printTable () =
    ...
    printfn
        "  %-6s %-24s %8s %8s %6s %6s %8s %10s %8s %6s"
        "Key"
        "Name"
        "Gap(eV)"
        "Type"
    ...
    for semi in selectedMaterials do
        ...
        printfn
            "  %-6s %-24s %8.3f %8s %6.3f %6.3f %8.4f %10.2e %8.1f %6s"
            semi.ShortName
            semi.Name
            semi.BandGap
            semi.BandGapType
    ...
    printfn "  %s" divM
    printfn "  %-16s %4s %10s %10s" "Metal" "Z" "n(m-3)" "E_F(eV)"
    printfn "  %s" divM
