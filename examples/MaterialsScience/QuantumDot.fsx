    ...
    [
        {
            ...
            Description = "Comma-separated material short names (CdSe, InAs, Si, Ge)"
            Default = None
        }
        {
        ...
        {
            Cli.OptionSpec.Name = "size"
            Description = "Dot size in nm for analysis"
            Default = Some "4.0"
        }
        {
...
let effectiveBandGap (size_nm: float) (material: QDMaterial) : float =
    ...
    let E_exciton =
        1.8 * e_charge * e_charge
        / (4.0 * Math.PI * material.DielectricConstant * epsilon_0 * L)
...
let createCdSeDimer () : Molecule =
    {
        ...
        Atoms =
            [
                ...
                {
                    Element = "Se"
                    Position = (cdSeBondLength, 0.0, 0.0)
                }
            ]
        Bonds =
            [
...
let printTable () =
    ...
    for mat in selectedMaterials do
        ...
        printfn
            "  %-6s %-26s %6.3f %6.3f %8.3f %8.3f %10.3f %6.0f %8s"
            mat.ShortName
            mat.Name
            mat.ElectronMass
            mat.HoleMass
