...
Cli.exitIfHelp
    ...
    [
        {
            Cli.OptionSpec.Name = "materials"
            Description = "Comma-separated ferromagnet short names (Fe, Co, Ni, Permalloy)"
            Default = None
        }
        {
            Cli.OptionSpec.Name = "input"
            Description = "CSV file with custom ferromagnet definitions"
            Default = None
        }
        {
            Cli.OptionSpec.Name = "layers"
            Description = "Number of magnetic layers for multilayer calculation"
            Default = Some "3"
        }
        {
...
let private spacerCr =
    {
        Name = "Chromium (Cr)"
        FermiWavevector = 12.0
        Resistivity = 12.9
    }

let private spacerCu =
    {
        Name = "Copper (Cu)"
        FermiWavevector = 13.6
        Resistivity = 1.7
    }
        ...
        | _ ->
            {
                ...
                ShortName =
                    ...
                    if s = "" then
                        ...
                        s
                SpinPolarization =
                    get "spin_polarization"
                    |> fun s ->
                        match Double.TryParse s with
                        | true, v -> v
                        | _ -> 0.3
                ExchangeSplitting =
                    get "exchange_splitting"
                    |> fun s ->
                        match Double.TryParse s with
                        | true, v -> v
                        | _ -> 1.0
                MajorityResistivity =
                    get "majority_resistivity"
                    |> fun s ->
                        match Double.TryParse s with
                        | true, v -> v
                        | _ -> 4.0
                MinorityResistivity =
                    get "minority_resistivity"
                    |> fun s ->
                        match Double.TryParse s with
                        | true, v -> v
                        | _ -> 12.0
                CurieTemperature =
                    get "curie_temperature"
                    |> fun s ->
                        match Double.TryParse s with
                        | true, v -> v
                        | _ -> 800.0
...
/// R_parallel = 2rR / (r + R)
let resistanceParallel (r: float) (bigR: float) : float = 2.0 * r * bigR / (r + bigR)

/// R_antiparallel = (r + R) / 2
let resistanceAntiparallel (r: float) (bigR: float) : float = (r + bigR) / 2.0
...
let rkkyCoupling (spacer: SpacerMaterial) (thickness_nm: float) : float =
    Math.Cos(2.0 * spacer.FermiWavevector * thickness_nm)
...
let createFe2Dimer (multiplicity: int) : Molecule =
    {
        ...
        Atoms =
            [
                ...
                {
                    Element = "Fe"
                    Position = (feFeBondLength, 0.0, 0.0)
                }
            ]
        Bonds =
            [
...
let exchangeRow =
    ...
    | Some eH, Some eL ->
        ...
        if not quiet then
            printfn "  Exchange coupling J = %.1f meV (%s)" J_meV kind
    ...
    | _ -> Map.ofList [ "quantity", "exchange_coupling"; "has_vqe_failure", "true" ]
...
let printTable () =
    ...
    printfn "  %s" divG
    printfn "  %-10s %-22s %6s %6s %8s %8s %8s %8s %6s" "Key" "Name" "r(uO)" "R(uO)" "R/r" "R_P" "R_AP" "GMR(%)" "P"
    printfn "  %s" divG
    ...
    for fm in selectedMaterials do
        ...
        printfn
            "  %-10s %-22s %6.1f %6.1f %8.2f %8.2f %8.2f %8.1f %6.2f"
            fm.ShortName
            fm.Name
            fm.MajorityResistivity
            fm.MinorityResistivity
            alpha
            R_P
            R_AP
            (gmr * 100.0)
    ...
    printfn "  %s" divV
    printfn "  %-18s %-24s %12s %6s %8s %8s" "Molecule" "Label" "Energy(Ha)" "Iters" "Time(s)" "Status"
    printfn "  %s" divV
...
| Some path ->
    let header =
        [
            "material"
            "short_name"
            "majority_resistivity"
            "minority_resistivity"
            "spin_asymmetry"
            "spin_polarization"
            "R_parallel"
            "R_antiparallel"
