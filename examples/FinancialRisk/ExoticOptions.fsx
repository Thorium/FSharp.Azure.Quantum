...
Cli.exitIfHelp
    ...
    [
        {
            Cli.OptionSpec.Name = "options"
            Description = "Comma-separated option keys to include"
            Default = None
        }
        {
            Cli.OptionSpec.Name = "input"
            Description = "CSV file with custom option definitions"
            Default = None
        }
        {
...
let private presetDownInPut =
    BarrierSpec(
        ...
        "Down-and-In Barrier Put",
        {
            ...
            BarrierLevel = max 1.0 (spotPrice * 0.8)
            Rebate = 0.0
            MonitoringPoints = 252
        }
    )
...
let private builtInOptions =
    [
        presetUpOutCall
        presetDownInPut
        presetLookbackFloatCall
        presetLookbackFixedPut
    ]
...
let selectedOptions =
    ...
    | filter ->
        ...
        base'
        |> List.filter (fun s -> filterSet.Contains((specKey s).ToLowerInvariant()))
...
if not quiet then
    ...
    printfn
        "Options: %d  Spot: $%.2f  Strike: $%.2f  Vol: %.1f%%  Expiry: %.1fy"
...
let private generatePricePaths (market: MarketParams) (numPaths: int) (numSteps: int) (seed: int) : float[][] =
    ...
    let drift =
        (market.RiskFreeRate - market.DividendYield - 0.5 * market.Volatility ** 2.0)
        * dt
...
let private barrierBreached (barrierType: BarrierType) (barrier: float) (path: float[]) : bool =
    ...
    | UpAndOut
    | UpAndIn -> path |> Array.exists (fun p -> p >= barrier)
    | DownAndOut
    | DownAndIn -> path |> Array.exists (fun p -> p <= barrier)
...
let private barrierPayoff (option: BarrierOption) (strike: float) (path: float[]) : float =
    ...
    | UpAndOut
    | DownAndOut -> if breached then option.Rebate else intrinsicValue
    | UpAndIn
    | DownAndIn -> if breached then intrinsicValue else option.Rebate
...
let private priceWithQuantumMC
    ...
    async {
        ...
        return
            result
            |> Result.map (fun qmcResult ->
                let avgPayoff = payoffs |> Array.average

                let price =
                    discountFactor * avgPayoff * qmcResult.ExpectationValue * float payoffs.Length
...
let private priceBarrierOption
    (market: MarketParams)
...
let private priceLookbackOption
    (market: MarketParams)
    ...
    async {
        ...
        let! priceUp =
            priceOption
                spec
                { market with
                    Volatility = market.Volatility + bump
                }
                backend
        ...
        if market.TimeToExpiry <= dayBump then
            ...
            let! priceLater =
                priceOption
                    spec
                    { market with
                        TimeToExpiry = market.TimeToExpiry - dayBump
                    }
                    backend
...
if not quiet then
    printfn "Pricing %d exotic options with quantum Monte Carlo..." selectedOptions.Length
...
let optionResults =
    ...
    |> List.map (fun spec ->
        ...
        let name =
            match spec with
            | BarrierSpec(_, n, _) -> n
            | LookbackSpec(_, n, _) -> n

        let optType =
            match spec with
            | BarrierSpec _ -> "Barrier"
        ...
        | Ok pr ->
            ...
            if not quiet then
                printfn
                    "  [OK]   %-35s  Price: $%8.4f  Delta: %7.4f  Vega: %7.4f  Theta: %7.4f"
                    name
                    pr.Price
                    delta
                    vega
...
let sortedResults =
    optionResults
    |> List.sortByDescending (fun r -> if Double.IsNaN r.Price then Double.MinValue else r.Price)
...
let printTable () =
    ...
    printfn
        "  Market: Spot=$%.2f  Strike=$%.2f  Rate=%.1f%%  Vol=%.1f%%  Expiry=%.1fy"
        market.Spot
        market.Strike
        (market.RiskFreeRate * 100.0)
        (market.Volatility * 100.0)
        market.TimeToExpiry
