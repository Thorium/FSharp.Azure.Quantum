...
Cli.exitIfHelp
    ...
    [
        {
            Cli.OptionSpec.Name = "scenarios"
            Description = "Comma-separated scenario keys to include"
            Default = None
        }
        {
            Cli.OptionSpec.Name = "input"
            Description = "CSV file with custom scenario definitions"
            Default = None
        }
        {
...
let private parseCategory (s: string) =
    ...
    | "historical"
    | "historicalcrisis" -> HistoricalCrisis
    | "regulatory"
    | "regulatorymandated" -> RegulatoryMandated
    | "hypothetical"
    | "hypotheticalextreme" -> HypotheticalExtreme
    | "geopolitical"
    | "geopoliticalevent" -> GeopoliticalEvent
    | "dislocation"
    | "marketdislocation" -> MarketDislocation
...
if not quiet then
    ...
    printfn
        "Scenarios: %d  Confidence: %.1f%%  Horizon: %d days  Portfolio: $%s"
...
let portfolioReturns =
    let weights =
        marketData |> List.map (fun (_, _, _, _, w, _, _) -> w) |> List.toArray
...
let private applyStressScenario (scenario: ScenarioInfo) : float * float =
    let stressedValue =
        ...
        |> Array.sumBy (fun pos ->
            ...
            let shock = scenario.AssetShocks |> Map.tryFind shockKey |> Option.defaultValue 0.0
...
if not quiet then
    printfn "Running quantum amplitude estimation for %d scenarios..." selectedScenarios.Length
...
let scenarioResults =
    ...
    |> List.map (fun scenario ->
        ...
        let stressedStatePrep =
            buildStressedStatePrep portfolioReturns scenario quantumQubits
...
let sortedResults =
    scenarioResults |> List.sortByDescending (fun r -> r.ClassicalLoss)
...
let printTable () =
    ...
    for r in sortedResults do
        ...
        printfn
            ...
            (if r.Scenario.Name.Length > 32 then
                 r.Scenario.Name.[..31]
             else
                 r.Scenario.Name)
            catStr
    ...
    |> List.map (fun r ->
        [
            ...
            if Double.IsNaN r.QuantumVaR then
                ...
                $"%.2f{r.QuantumVaR}"
            "quantum_es",
            if Double.IsNaN r.QuantumES then
                ""
            else
                $"%.2f{r.QuantumES}"
            "tail_probability",
