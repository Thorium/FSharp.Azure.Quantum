...
Cli.exitIfHelp
    ...
    [
        ...
        }
        {
            ...
            Description = "CSV file with node definitions (id,type,capacity,cost,demand,revenue)"
            Default = Some "built-in network"
        }
        {
            Cli.OptionSpec.Name = "edges"
            Description = "CSV file with edge definitions (source,target,cost)"
            Default = Some "built-in routes"
        }
        {
...
let private loadNodesFromCsv (path: string) : SupplyChainNode list =
    ...
    |> List.choose (fun row ->
        ...
        | _ ->
            if not quiet then
                eprintfn "  Warning: node row missing 'id' or 'type'"
...
let private loadEdgesFromCsv (path: string) : (string * string * float) list =
    ...
    |> List.choose (fun row ->
        ...
        | Some src, Some tgt, Some costStr ->
            ...
            | _ ->
                if not quiet then
                    eprintfn "  Warning: invalid cost for edge %s->%s" src tgt

                None
        | _ ->
            if not quiet then
                eprintfn "  Warning: edge row missing 'source', 'target', or 'cost'"
...
let supplyChainNodes =
    ...
    | Some path ->
        ...
        if not quiet then
            printfn "Loading nodes from: %s" resolved
...
let transportRoutes =
    ...
    | Some path ->
        ...
        if not quiet then
            printfn "Loading edges from: %s" resolved
...
let supplies =
    nodesByType "Supplier" |> List.map (fun n -> n.Id, n.Capacity) |> Map.ofList

let edges =
    transportRoutes
    |> List.map (fun (src, tgt, cost) ->
        {
            Source = src
            Target = tgt
            Weight = cost
...
if not quiet then
    ...
    printfn
        "  Nodes:        %d (Suppliers: %d, Warehouses: %d, Distributors: %d, Customers: %d)"
...
let hasQuantumFailure, routeMaps =
    ...
    | Ok solution ->
        ...
        if not quiet then
            ...
            stages
            |> List.iter (fun (label, srcPfx, tgtPfx) ->
                let stageRoutes =
                    routes
                    |> List.filter (fun e -> e.Source.StartsWith(srcPfx) && e.Target.StartsWith(tgtPfx))

                if not (List.isEmpty stageRoutes) then
                    printfn "  %s:" label

                    stageRoutes
                    |> List.iter (fun r -> printfn "    %s -> %s  $%.2f/unit" r.Source r.Target r.Weight)
