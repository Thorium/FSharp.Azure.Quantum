...
Cli.exitIfHelp
    "SupplyChain-Small.fsx"
    "Small 2-stage supply chain optimization (4 qubits)"
    [
        {
            Name = "shots"
...
| Ok solution ->
    ...
    |> List.iter (fun edge -> pr "    %s -> %s: $%.2f" edge.Source edge.Target edge.Weight)
    ...
    |> Option.iter (fun path ->
        ...
        let rows =
            solution.SelectedEdges
            |> List.map (fun e -> [ e.Source; e.Target; $"%.2f{e.Weight}" ])
