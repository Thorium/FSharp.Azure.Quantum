...
Cli.exitIfHelp
    ...
    [
        {
            Name = "example"
            Description = "Which example (all|bell|ghz|qft|super|kickback|toffoli|optimize|validate)"
            Default = Some "all"
        }
        {
            Name = "ghz-qubits"
...
if not quiet then
    ...
    jsonResults
    |> List.iter (fun r -> pr "  [OK] %-25s %d qubits, %d gates" r.Label r.Qubits r.Gates)
