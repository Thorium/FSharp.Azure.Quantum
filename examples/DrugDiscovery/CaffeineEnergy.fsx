    ...
    [
        {
            ...
            Description = "CSV file with custom fragment definitions"
            Default = Some "built-in presets"
        }
        {
            Cli.OptionSpec.Name = "fragments"
    ...
    let mol: Molecule =
        {
            ...
            Atoms =
                [
                    ...
                    {
                        Element = "C"
                        Position = (1.07, 0.0, 0.0)
                    } // C-H bond ~1.07 A
                    {
                        Element = "N"
                        Position = (2.22, 0.0, 0.0)
                    }
                ] // C-N triple ~1.15 A
            Bonds =
                [
...
let ranked =
    ...
    |> List.sortBy (fun r ->
        if r.HasVqeFailure then
            (2, infinity)
        elif r.EnergyPerElectron >= 0.0 then
            (1, r.EnergyPerElectron)
        else
            (0, r.EnergyPerElectron))
...
let printTable () =
    ...
    printfn "=================================================================="
    printfn ""
    printfn "  %-4s  %-16s  %-16s  %4s  %12s  %14s  %10s" "#" "Fragment" "Group" "e-" "E (Ha)" "E/e- (Ha)" "Time (s)"
    printfn "  %s" (String('=', 95))
    ...
    |> List.iteri (fun i r ->
        if r.HasVqeFailure then
            ...
            printfn
                "  %-4d  %-16s  %-16s  %4d  %12.6f  %14.6f  %10.1f"
                (i + 1)
                r.Fragment.Name
                r.Fragment.FunctionalGroup
                r.Electrons
    ...
    printfn "  %-4s  %-16s  %s" "#" "Fragment" "Pharmacological Role"
    printfn "  %s" (String('-', 75))

    ranked
    |> List.iteri (fun i r -> printfn "  %-4d  %-16s  %s" (i + 1) r.Fragment.Name r.Fragment.PharmRole)
...
if not quiet then
    ...
    | best :: _ ->
        ...
        printfn
            "  Most stable (per e-):  %s (%s, E/e- = %.6f Ha)"
            best.Fragment.Name
            best.Fragment.FunctionalGroup
            best.EnergyPerElectron
...
let resultMaps =
    ...
    |> List.mapi (fun i r ->
        [
            ...
            "energy_per_electron_ha",
            (if r.HasVqeFailure then
                 "FAILED"
             else
                 $"%.6f{r.EnergyPerElectron}")
            "energy_kcal_mol",
            (if r.HasVqeFailure then
                 "FAILED"
             else
                 sprintf "%.2f" (r.Energy * hartreeToKcalMol))
            "energy_ev",
            (if r.HasVqeFailure then
                 "FAILED"
             else
                 sprintf "%.4f" (r.Energy * hartreeToEv))
