...
Cli.exitIfHelp
    "DrugDiscoverySolvers_Example.fsx"
    "Quantum drug-discovery solvers (IndependentSet, Influence, Diverse)"
    [
        {
            Name = "example"
            Description = "Which example: all, independent, influence, diverse"
            Default = Some "all"
        }
        {
            Name = "shots"
...
if runAll || exampleName = "independent" then
    ...
    | Ok sol ->
        ...
        for n in problem.Nodes do
            ...
            csvRows <-
                [ "IndependentSet"; n.Id; $"%.2f{n.Weight}"; (if sel then "true" else "false") ]
                :: csvRows
    ...
    | Error e -> pr "IndependentSet FAILED: %A" e
...
if runAll || exampleName = "influence" then
    ...
    | Ok sol ->
        ...
        for n in problem.Nodes do
            ...
            csvRows <-
                [ "Influence"; n.Id; $"%.2f{n.Score}"; (if sel then "true" else "false") ]
                :: csvRows
        ...
        pr "  Backend:        %s (%d shots)" sol.BackendName sol.NumShots
    | Error e -> pr "InfluenceMaximization FAILED: %A" e
...
if runAll || exampleName = "diverse" then
    ...
    | Ok sol ->
        ...
        pr "  Backend:          %s (%d shots)" sol.BackendName sol.NumShots
    | Error e -> pr "DiverseSelection FAILED: %A" e
