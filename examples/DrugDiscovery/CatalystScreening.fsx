...
Cli.exitIfHelp
    ...
    [
        ...
        }
        {
            ...
            Description = "Industrial process temperature (K)"
            Default = Some "310"
        }
        {
...
let private ticl4Preset =
    createCatalyst
        "Ti"
        1.78
        "Titanium (TiCl4 model)"
        "TiCl4"
        "Strong"
        "Oxophilic, excellent for carbonyls"
...
let substrateEnergy =
    ...
    | Ok e ->
        if not quiet then
            printfn "  E_substrate = %.6f Ha  (%.1fs)" e substrateTime

        e
    | Error _ ->
        if not quiet then
            printfn "  E_substrate = FAILED  (%.1fs)" substrateTime
...
let printTable () =
    ...
    |> List.iteri (fun i r ->
        if r.HasVqeFailure then
            ...
            printfn
                "  %-4d  %-22s  %-10s  %14.6f  %14.2f  %12.1f  %10.1f"
                (i + 1)
                $"%s{r.Catalyst.Name} (%s{r.Catalyst.Formula})"
                r.Catalyst.LewisAcidity
    ...
    printfn "  %-4s  %-22s  %s" "#" "Catalyst" "Selectivity / Industrial Use"
    printfn "  %s" (String('-', 75))
...
if not quiet then
    ...
    | best :: _ ->
        let totalTime =
            (results |> List.sumBy (fun r -> r.ComputeTimeSeconds)) + substrateTime

        printfn
            "  Best catalyst:  %s (%s, E_bind = %.2f kcal/mol)"
            best.Catalyst.Name
            best.Catalyst.Formula
...
let resultMaps =
    ...
    |> List.mapi (fun i r ->
        [
            ...
            "catalyst_energy_hartree",
            (if r.HasVqeFailure then
                 "FAILED"
             else
                 $"%.6f{r.CatalystEnergy}")
            "substrate_energy_hartree",
            (if r.HasVqeFailure then
                 "FAILED"
             else
                 $"%.6f{r.SubstrateEnergy}")
            ...
            (if r.HasVqeFailure then
                 ...
                 $"%.6f{r.BindingEnergyHartree}")
            "binding_energy_kcal_mol",
            (if r.HasVqeFailure then
                 "FAILED"
             else
                 $"%.2f{r.BindingEnergyKcal}")
            "estimated_barrier_reduction_kcal",
            (if r.HasVqeFailure then
                 "FAILED"
             else
                 $"%.1f{r.EstimatedBarrierReduction}")
            "estimated_barrier_kcal",
            (if r.HasVqeFailure then
                 "FAILED"
             else
                 $"%.1f{r.EstimatedBarrier}")
