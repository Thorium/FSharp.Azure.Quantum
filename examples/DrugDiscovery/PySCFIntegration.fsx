...
Cli.exitIfHelp
    ...
    [
        ...
        {
            Cli.OptionSpec.Name = "molecules"
            Description = "Comma-separated molecule names to include (default: all)"
            Default = Some "all"
        }
        {
            Cli.OptionSpec.Name = "basis"
            Description = "Basis set (sto-3g, 6-31g, cc-pvdz)"
            Default = Some "sto-3g"
        }
        {
            Cli.OptionSpec.Name = "mode"
            Description = "Mode: integrals (compute only) or vqe (full VQE)"
            Default = Some "vqe"
        }
        {
        ...
        }
        {
            ...
            Description = "Convergence tolerance (Hartree)"
            Default = Some "1e-6"
        }
        {
...

    {
        ...
        Atoms =
            [
                ...
                {
                    Element = "H"
                    Position = (0.0, ohLength * sin (angle / 2.0), ohLength * cos (angle / 2.0))
                }
                {
                    Element = "H"
                    Position = (0.0, -ohLength * sin (angle / 2.0), ohLength * cos (angle / 2.0))
...
let private discoverPythonDll () =
    ...
    try
        ...
        if not (String.IsNullOrWhiteSpace output) then
            ...
            if isWindows then
                IO.Directory.GetFiles(pythonDir, "python3*.dll") |> Array.tryHead
            else
...
let private createPySCFProvider (basis: string) : IntegralProvider =
    fun (molecule: Molecule) ->
        try
            ...
            let eriMO4D =
                ao2mo.InvokeMethod("restore", [| toPyInt 1; eriMO; toPyInt numOrbitals |])
...
let computeMolecule (info: MoleculeInfo) : MoleculeResult =
    ...
    | Ok integrals ->
        if not quiet then
            printfn
                "  [%s] Integrals OK: %d orbitals, %d electrons, Enuc=%.4f Ha"
                info.Name
                integrals.NumOrbitals
                integrals.NumElectrons
                integrals.NuclearRepulsion
        ...
        if mode <> "vqe" then
            ...
            | Ok result ->
                let corrEnergy =
                    integrals.ReferenceEnergy |> Option.map (fun hf -> result.Energy - hf)
...
let printTable () =
    ...
    let title =
        if isVqeMode then
            "Molecular Ground-State Energies (PySCF + VQE)"
        else
    ...
    if isVqeMode then
        ...
        for r in ranked do
            if r.HasVqeFailure then
                printfn
                    "  %-4d  %-6s  %5d  %5d  %12s  %12s  %12s  %8.1f"
                    r.Rank
                    r.Info.Name
                    r.Info.Molecule.Atoms.Length
                    r.NumOrbitals
                ...
                printfn
                    ...
                    (r.CorrelationEnergyHartree
                     |> Option.map (sprintf "%.6f")
                     |> Option.defaultValue "N/A")
                    r.ComputeTimeSeconds
    else
        printfn
            "  %-4s  %-6s  %5s  %5s  %5s  %12s  %12s  %8s"
            "#"
            "Name"
            "Atoms"
            "MOs"
...
if not quiet then
    let successCount =
        ranked |> List.filter (fun r -> not r.HasVqeFailure) |> List.length
...
let resultMaps =
    ...
    |> List.map (fun r ->
        [
            ...
            "basis_set", basisSet
            "hf_energy_hartree", (r.HfEnergyHartree |> Option.map (sprintf "%.6f") |> Option.defaultValue "")
            "vqe_energy_hartree", (r.VqeEnergyHartree |> Option.map (sprintf "%.6f") |> Option.defaultValue "")
