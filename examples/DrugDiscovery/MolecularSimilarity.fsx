...
|> Cli.exitIfHelp
    ...
    [
        {
            Name = "actives"
            Description = "SMILES/CSV file with known active compounds"
            Default = Some "built-in 3 kinase inhibitors"
        }
        {
            Name = "library"
            Description = "SMILES/CSV file with candidate library"
            Default = Some "built-in 10 candidates"
        }
        {
            Name = "output"
        ...
        {
            Name = "top"
            Description = "Return top N candidates by similarity"
            Default = Some "5"
        }
        {
            Name = "threshold"
            Description = "Similarity threshold for hit classification"
            Default = Some "0.7"
        }
        {
            Name = "quiet"
...
if not quiet then
    ...
    for (idx, smiles, desc, sim) in classicalResults |> List.take (min topN classicalResults.Length) do
        let shortSmiles =
            if smiles.Length > 40 then
                smiles.Substring(0, 37) + "..."
            else
                smiles
...
if not quiet then
    printfn
        ...
        (if activeFeatures.Length > 0 then
             activeFeatures.[0].Length
         else
             0)
...
if not quiet then
    ...
    for i in 0 .. candidateData.Length - 1 do
        ...
        let movement =
            if quantumRank < classicalRank then
                sprintf "(+%d)" (classicalRank - quantumRank)
            elif quantumRank > classicalRank then
                sprintf "(-%d)" (quantumRank - classicalRank)
            else
                "(=)"
...
if not quiet then
    ...
    let checkLipinski (desc: MolecularData.MolecularDescriptors) =
        let violations =
            [
                if desc.MolecularWeight > 500.0 then
                    "MW > 500"
                if desc.LogP > 5.0 then
                    "LogP > 5"
                if desc.HydrogenBondDonors > 5 then
                    "HBD > 5"
                if desc.HydrogenBondAcceptors > 10 then
                    "HBA > 10"
            ]
    ...
    for (idx, smiles, desc, sim) in quantumResults |> List.take (min 5 quantumResults.Length) do
        ...
        let status =
            if violations.IsEmpty then
                ...
                sprintf "FAIL: %s" (String.concat ", " violations)

        let shortSmiles =
            if smiles.Length > 35 then
                smiles.Substring(0, 32) + "..."
            else
                smiles
...
| Some path ->
    ...
    let rows =
        topResults
        |> List.map (fun (idx, smiles, desc, qSim) ->
            let cSim =
                classicalResults
                |> List.tryFind (fun (i, _, _, _) -> i = idx)
                |> Option.map (fun (_, _, _, sim) -> sim)
            ...
            [
                ...
                $"%.4f{qSim}"
                $"%.4f{cSim}"
                $"%.1f{desc.MolecularWeight}"
                $"%.2f{desc.LogP}"
                $"%.1f{desc.TPSA}"
                string (qSim >= similarityThreshold)
            ])
...
if not usingExternalInput && outputFile.IsNone && csvFile.IsNone then
    ...
    printfn
        "  dotnet fsi MolecularSimilarity.fsx -- --actives actives.csv --library library.smi --threshold 0.8 --csv hits.csv --quiet"
...
if candidateData.IsEmpty then exit 1 else exit 0
