...
Cli.exitIfHelp
    "DruggabilityScoring.fsx"
    "Quantum pharmacophore feature selection via Maximum Weight Independent Set"
    [
        {
            Cli.OptionSpec.Name = "input"
            Description = "CSV file with custom feature definitions"
            Default = Some "built-in EGFR pocket"
        }
        {
            Cli.OptionSpec.Name = "features"
            Description = "Comma-separated feature IDs to include (default: all)"
            Default = Some "all"
        }
        {
            Cli.OptionSpec.Name = "overlap-threshold"
            Description = "Distance threshold for feature overlap (Angstroms)"
            Default = Some "2.5"
        }
        {
...
let private loadFeaturesFromCsv (path: string) : PharmacophoreFeature list =
    ...
    |> List.choose (fun row ->
        ...
        | None ->
            let featureType =
                get "feature_type"
                |> Option.map parseFeatureType
                |> Option.defaultValue Hydrophobic
...
if not quiet then
    ...
    printfn ""
    printfn "  %-8s  %-16s  %16s  %10s" "ID" "Type" "Position" "Importance"
    printfn "  %s" (String('-', 55))

    for f in features do
        let (x, y, z) = f.Position
        printfn "  %-8s  %-16s  (%4.1f,%4.1f,%4.1f)  %10.2f" f.Id (featureTypeName f.Type) x y z f.Importance
...
let toIndependentSetProblem
    (featureList: PharmacophoreFeature list)
    (overlapPairs: (int * int) list)
...
let druggabilityScore, druggabilityRating =
    ...
    | Some solution ->
        let selectedFeatures =
            ranked |> List.filter (fun r -> r.Selected) |> List.map (fun r -> r.Feature)

        let featureTypes = selectedFeatures |> List.map (fun f -> f.Type) |> List.distinct

        let hasHBondAcceptor =
            selectedFeatures |> List.exists (fun f -> f.Type = HBondAcceptor)
...
let printTable () =
    ...
    printfn "=================================================================="
    printfn ""
    printfn "  %-4s  %-8s  %-16s  %16s  %10s  %8s" "#" "ID" "Type" "Position" "Importance" "Selected"
    printfn "  %s" (String('=', 75))
    ...
    for r in ranked do
        ...
        if r.HasVqeFailure then
            printfn
                "  %-4d  %-8s  %-16s  (%4.1f,%4.1f,%4.1f)  %10.2f  %8s"
                r.Rank
                r.Feature.Id
                (featureTypeName r.Feature.Type)
                x
                y
                z
    ...
    | Some solution ->
        ...
        if not selectedFeatures.IsEmpty then
            ...
            printfn
                "    Type diversity:%d distinct types (%s)"
                featureTypes.Length
...
let resultMaps =
    ...
    |> List.map (fun r ->
        ...
        let totalWeight =
            match solutionStats with
            | Some s -> $"%.2f{s.TotalWeight}"
            | None -> "FAILED"

        let isValid =
            match solutionStats with
            | Some s -> string s.IsValid
            | None -> "FAILED"
