...
Cli.exitIfHelp
    "ConfigurationOptimizer.fsx"
    "Quantum pattern matching for configuration search"
    [
        {
            Name = "example"
            Description = "Which example: all, db, ml, feature"
            Default = Some "all"
        }
        {
            Name = "shots"
...
let benchmarkDatabase (config: DbConfig) =
    ...
    let cpuUsage =
        40.0
        + (if config.LogLevel = "Debug" then 25.0 else 0.0)
...
if exampleChoice = "all" || exampleChoice = "db" then
    ...
    | Ok result ->
        ...
        |> List.iteri (fun i index ->
            ...
            pr
                "  #%d - Config %d: Cache=%dMB Pool=%d Timeout=%ds Log=%s => TP=%.0f q/s Lat=%.1fms CPU=%.0f%%"
                (i + 1)
                index
                config.CacheSize
                config.PoolSize
...
if exampleChoice = "all" || exampleChoice = "ml" then
    ...
    | Ok result ->
        ...
        |> List.iteri (fun i index ->
            ...
            pr
                "  #%d - Config %d: LR=%.3f Batch=%d Layers=%d Dropout=%.1f => Acc=%.2f%%"
                (i + 1)
                index
                config.LearningRate
                config.BatchSize
            ...
            csvRows <-
                [
                    "ml"
                    string index
                    $"%.3f{config.LearningRate}"
                    $"%d{config.BatchSize}"
                    $"%d{config.Layers}"
                    $"%.1f{config.DropoutRate}"
                    $"%.2f{accuracy}"
                    ""
                ]
                :: csvRows)
...
if exampleChoice = "all" || exampleChoice = "feature" then
    ...
    let evaluateFeatureSet (featureSet: string list) : (float * int) =
        ...
        let penalty =
            if featureCount > 4 then
                float (featureCount - 4) * 3.0
            else
                0.0
    ...
    | Ok result ->
        ...
        |> List.iteri (fun i index ->
            ...
            pr
                "  #%d - Subset %d: [%s] (%d features, %.2f%% acc)"
                (i + 1)
                index
                (String.concat ", " featureSet)
                count
                accuracy
