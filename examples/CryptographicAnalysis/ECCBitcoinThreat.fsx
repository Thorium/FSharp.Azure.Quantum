...
Cli.exitIfHelp
    "ECCBitcoinThreat.fsx"
    "Quantum ECDLP threat analysis for elliptic curve cryptography (Bitcoin, Ethereum, etc.)."
    [
        {
            Cli.OptionSpec.Name = "input"
            Description = "CSV file with custom curve definitions"
            Default = Some "built-in presets"
        }
        {
            Cli.OptionSpec.Name = "curves"
            Description = "Comma-separated curve names to analyse (default: all)"
            Default = Some "all"
        }
        {
...
let modInverse (a: int) (m: int) : int option =
    ...
    if g <> 1 then None else Some(((x % m) + m) % m)
...
let pointOrder (curve: EllipticCurve) (point: ECPoint) : int =
    let rec loop current n =
        if n > curve.P * curve.P then
            n
        else
            let next = pointAdd curve current point
            if next = Infinity then n else loop next (n + 1)
    ...
    |> List.choose (fun row ->
        ...
        | None ->
            ...
            | Some pStr, Some aStr, Some bStr ->
                ...
                | _ ->
                    if not quiet then
                        eprintfn "  Warning: invalid numeric fields for '%s'" name

                    None
            | _ ->
                if not quiet then
                    eprintfn "  Warning: row '%s' missing 'prime'/'a'/'b' or 'preset'" name
...
let curves: CurvePreset list =
    let allCurves =
        ...
        | Some path ->
            ...
            if not quiet then
                printfn "Loading curves from: %s" resolved
...
let analyseCurve (preset: CurvePreset) : ECDLPResult =
    ...
    | Some(generator, genOrder) ->
        ...
        let qpePeriod, hasFailure =
            ...
            | Error err ->
                if not quiet then
                    eprintfn "  QPE error for %s: %s" preset.Name err.Message
        ...
        if not quiet then
            printfn
                "  %s: G=%s  order=%d  victim_d=%d  Q=%s  recovered=%s  %s"
                preset.Name
                (formatPoint generator)
                genOrder
                victimKey
...

        {
            Curve = preset
            CurveOrder = curveOrder
            GeneratorPoint = formatPoint generator
            GeneratorOrder = genOrder
            VictimPrivateKey = victimKey
            VictimPublicKey = formatPoint victimPub
...
let ranked =
    results |> List.sortBy (fun r -> (not r.AttackSuccess, r.Curve.RealBits))
...
let printTable () =
    ...
    |> List.iteri (fun i r ->
        ...
        printfn
            "  %-4d  %-16s  %-20s  %5d  %5d  %6d  %-6s  %s"
            (i + 1)
            r.Curve.Name
            r.Curve.RealCurve
            r.Curve.Prime
            r.CurveOrder
            r.Curve.LogicalQubits
            safeStr
            resultStr)
...
if not quiet then
    ...
    printfn
        "  Compromised curves:  %d  (%s)"
        compromised.Length
        (compromised
         |> List.map (fun r -> r.Curve.Name)
         |> String.concat ", "
